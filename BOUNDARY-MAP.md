...
| Scalar local readout | LITERAL cup-span adapter / SEAM arithmetic | `KummerCupSpanReadout` needs values only on the span of actual Kummer cups, not all discrete `H²`.  Construction and normalization of the continuous local invariant, cup--Hilbert comparison, and factorization through this span remain open. |
