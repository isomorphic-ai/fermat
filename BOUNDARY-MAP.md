...
| Downstream relation (7a) route | CONCRETE LIFT INSTALLED / CIRCULAR PROCESSING BOUNDARY | `CanonicalConjugatePairNormalizedEndpoint59` removes the lift argument from the normalized coefficient and conditional endpoint.  `canonicalConjugatePairNormalizedWildCoefficient59_eq_zero` and `canonicalConjugatePairProcessesAtLeastSevenA_iff_gauge_eq_zero` preserve the exact warning: one-column reciprocity makes the coefficient zero and the remaining processing premise equivalent to vanishing of the whole class gauge, which already implies (7a).  The endpoint remains a conditional/regression theorem, not a non-circular crossing. |
| Retained auxiliary/full-orbit route | CONCRETE 58-PLACE LEDGER + CONSERVATION RECEIPT / LOCAL-GLOBAL COMPARISON OPEN | `canonicalConjugatePairGlobalLedger827` installs the canonical lift in the globally root-oriented 827 ledger, proves the ledger nonzero, and proves its scalar total zero.  `CanonicalConjugatePairConservation827` routes that exact cancellation through `PlaceLedger`, a zero-spent vacuum `Transfer`, and scheduler L1.  `pairAt_lambdaPlace59_eq_zero_of_canonicalConjugatePair827` removes the lift premise but honestly retains `GlobalReciprocityLaw`, comparison at all 58 orbit places, and silence at every other nonwild place.  No producer for those inputs or independent gauge comparison is constructed. |
...
| `vandiverSevenA_of_normalizedContinuousCanonicalConjugatePair` | **NO-LIFT CONDITIONAL (7a) ENDPOINT / REGRESSION ONLY** | The canonical lift, seating, incidence, boundary nonvanishing, and fiber inhabitation are installed.  `ClassValuedSevenAGaugeSeating`, `WildProcessesAtLeastSevenA`, and one-column reciprocity remain.  `canonicalConjugatePairNormalizedWildCoefficient59_eq_zero` and `canonicalConjugatePairProcessesAtLeastSevenA_iff_gauge_eq_zero` prove the latter two inputs already contain whole-gauge vanishing and hence (7a).  The theorem is formally sound but not a non-circular derivation. |
