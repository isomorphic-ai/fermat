import Fermat.Descent.Irregular.TakagiFurtwangler691
import Fermat.Descent.Irregular.KummerRealFixedField691
...
/-!
# The global Takagi--Furtwängler reflection layer at exponent 691
...
step: construction of an unramified cyclic degree-691 extension of the
maximal real subfield.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiReflection691
...
open Fermat.Irregular.KummerConjugationDescent691
open Fermat.Irregular.KummerRealFixedField691
...
the exact bridge from `KummerExtension691Unramified` to the `IsUnramified`
...
degree-`2`/degree-`691` compositum configuration.

Suppose `K/F` is Galois of degree `2`, `E/F` is Galois of degree `691`,
...
ramification theory for `E/F` also makes that index divide `691`; primality
then forces it to equal `1`. -/
theorem isUnramifiedAtFinitePlaces_of_degreeTwo_degreeSixHundredNinetyOne
    {F K E L : Type}
  ...
  have hcardE : Nat.card Gal(E/F) = 691 := by
    rw [IsGalois.card_aut_eq_finrank, hFE]
  have hdvd : Ideal.ramificationIdx q R.asIdeal ∣ 691 := by
    rw [hcardE] at hfundE
    rw [← heIn_eq]
    exact ⟨
  ...
  have heR : Ideal.ramificationIdx q R.asIdeal = 1 := by
    rcases (Nat.dvd_prime (by norm_num : Nat.Prime 691)).mp hdvd with h | h
    · exact h
    · omega
  exact
...
/-- The actual cyclic degree-691 Kummer extension, once unramified at every
finite prime, contributes `691`-torsion to the full cyclotomic class group.
...
theorem sixHundredNinetyOne_dvd_classNumber_of_kummerExtensionUnramified
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 691 - C (a : K)))
    (hunramified : KummerExtension691Unramified hirr) :
...
/-- Hilbert 94 turns a cyclic unramified degree-691 extension into an
explicit nonprincipal ideal whose 691st power is principal. -/
theorem hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension691
    {F : Type} [Field F] [NumberField F]
    (E : UnramifiedCyclicExtension691 F) :
    HasNonprincipalIdealWithPrincipalPower F 691 := by
  letI := E.fieldL
  letI := E.numberFieldL
  letI := E.algebraFL
...
cyclic extension of degree `691` over the maximal real subfield.

The algebraic fixed-field package comes from
`KummerRealFixedField691`.  Finite-prime unramifiedness descends from the
given unramified Kummer extension by the coprime-degree theorem above:
each ramification index divides both `2` and `691`. -/
noncomputable def realFixedFieldUnramifiedCyclicExtension691
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 691 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness691 a)
    (hunramified : KummerExtension691Unramified hirr) :
    UnramifiedCyclicExtension691 K⁺ := by
  letI : Fact (Irreducible (X ^ 691 - C (a : K))) := ⟨hirr⟩
  let L := KummerExtension691 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K)
      (by norm_num : 691 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  letI : Algebra K⁺ L :=
    algebraKummerOverReal691 (K := K) (a := a)
  letI : IsScalarTower K⁺ K L :=
    scalarTowerKummerOverReal691 (K := K) (a := a)
  letI : FiniteDimensional K⁺ L :=
    finiteDimensionalKummerOverReal691 (K := K) (a := a)
  let E := realFixedField691 hanti
  ...
  letI : FiniteDimensional K⁺ E :=
    FiniteDimensional.of_finrank_pos <| by
      rw [show Module.finrank K⁺ E = 691 by
        exact finrank_realFixedField691 (a := a) hanti]
      norm_num
  letI : FiniteDimensional E L :=
    FiniteDimensional.right K⁺ E L
  letI : NumberField E := NumberField.of_module_finite K⁺ E
  letI : IsGalois K⁺ E :=
    isGalois_realFixedField691 (a := a) hanti
  letI : IsCyclic (E ≃ₐ[K⁺] E) :=
    isCyclic_realFixedFieldAut691 (a := a) hanti
  have hrealUnramified : IsUnramifiedAtFinitePlaces K⁺ E :=
    @isUnramifiedAtFinitePlaces_of_degreeTwo_degreeSixHundredNinetyOne
      K⁺ K E L
      ...
      (finrank_realFixedField691 (a := a) hanti)
      hunramified
  exact UnramifiedCyclicExtension691.ofIsUnramifiedAtFinitePlaces
    (finrank_realFixedField691 (a := a) hanti)
    hrealUnramified

/-- Hilbert 94 applied to the explicit conjugation-fixed field: an
anti-invariant, everywhere-unramified degree-`691` Kummer extension gives
nontrivial `691`-torsion in the maximal-real class group. -/
theorem antiInvariantKummerReflection691
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 691 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness691 a)
    (hunramified : KummerExtension691Unramified hirr) :
    HasNonprincipalIdealWithPrincipalPower K⁺ 691 :=
  hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension691
    (realFixedFieldUnramifiedCyclicExtension691
      hirr hanti hunramified)

set_option maxRecDepth 50000 in
/-- The conjugate-pair instance of Vandiver's Lemma 1 at exponent `691`.
...
for `x * conj(x)^690` supplies the lifted conjugation, so the theorem above
would force `691` to divide the maximal-real class number, contradicting
the checked Sinnott--Kummer computation. -/
theorem conjugatePairIdealRoot_isPrincipal691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ (conjugatePairRadicand691 x))
    (hpow :
      I ^ 691 = Ideal.span {conjugatePairRadicand691 x}) :
    Submodule.IsPrincipal (I : Ideal (𝓞 K)) := by
  by_contra hnonprincipal
  let hirr :
      Irreducible
        (X ^ 691 - C (conjugatePairRadicand691 x : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 691) hpow hnonprincipal
  have hunramified : KummerExtension691Unramified hirr :=
    primaryIdealRootGivesUnramifiedKummer691
      hζ hprimary hpow hnonprincipal
  have htorsion :
      HasNonprincipalIdealWithPrincipalPower K⁺ 691 :=
    antiInvariantKummerReflection691 hirr
      (conjugatePairAntiInvariantWitness691 x)
      hunramified
  have hdvd : 691 ∣ NumberField.classNumber K⁺ :=
    (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := K⁺) (p := 691) (by norm_num)).mp htorsion
...
If `I^691 = (x)`, `J^691 = (conj(x))`, and the normalized product
`x * conj(x)^690` is primary, the explicit real fixed-field reflection
principalizes `I * J^690`. -/
theorem exists_conjugatePairEquationSevenAGenerator691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (I J : Ideal (𝓞 K)) (x y : 𝓞 K)
    (hIpow : I ^ 691 = Ideal.span {x})
    (hJpow : J ^ 691 = Ideal.span {y})
    (hconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K x = y)
    (hprimary : IsKummerPrimary hζ (x * y ^ 690)) :
    ∃ r : 𝓞 K, I * J ^ 690 = Ideal.span {r} := by
  have hradicand :
      conjugatePairRadicand691 x = x * y ^ 690 := by
    simp only [conjugatePairRadicand691, hconj]
  have hpow :
      (I * J ^ 690) ^ 691 =
        Ideal.span {conjugatePairRadicand691 x} := by
    calc
      (I * J ^ 690) ^ 691 =
          I ^ 691 * (J ^ 691) ^ 690 := by
        rw [mul_pow, ← pow_mul, ← pow_mul,
          Nat.mul_comm 690 691]
      _ = Ideal.span {x} * (Ideal.span {y}) ^ 690 := by
        rw [hIpow, hJpow]
      _ = Ideal.span {x * y ^ 690} := by
        rw [Ideal.span_singleton_pow,
          Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {conjugatePairRadicand691 x} := by
        rw [hradicand]
  have hprimary' :
      IsKummerPrimary hζ (conjugatePairRadicand691 x) := by
    rw [hradicand]
    exact hprimary
  obtain ⟨r, hr⟩ :=
    (conjugatePairIdealRoot_isPrincipal691
...
must produce a cyclic unramified degree-691 extension of the maximal real
subfield.

This is deliberately an extension-valued statement, rather than assuming
the desired class-group torsion conclusion. -/
def PrimaryKummerProducesRealUnramifiedExtension691
    (K : Type) [Field K] [NumberField K]
    [IsCyclotomicExtension {691} ℚ K] : Prop :=
  ∀ {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    {a : 𝓞 K},
...
`TakagiFurtwangler691`. -/
theorem primaryUnramifiedKummerReflection691_of_realExtension
    (hreal : PrimaryKummerProducesRealUnramifiedExtension691 K) :
    PrimaryUnramifiedKummerReflection691 K := by
  intro zeta hzeta a hprimary hirr hunramified
  obtain ⟨E⟩ := hreal hzeta hprimary hirr hunramified
  exact
    hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension691 E

end

end Fermat.Irregular.TakagiReflection691
