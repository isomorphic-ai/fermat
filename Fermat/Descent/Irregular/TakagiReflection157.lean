import Fermat.Descent.Irregular.TakagiFurtwangler157
import Fermat.Descent.Irregular.KummerRealFixedField157
...
/-!
# The global Takagi--Furtwängler reflection layer at exponent 157
...
step: construction of an unramified cyclic degree-157 extension of the
maximal real subfield.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiReflection157
...
open Fermat.Irregular.KummerRealFixedField157
...
the exact bridge from `KummerExtension157Unramified` to the `IsUnramified`
...
degree-`2`/degree-`157` compositum configuration.

Suppose `K/F` is Galois of degree `2`, `E/F` is Galois of degree `157`,
...
ramification theory for `E/F` also makes that index divide `157`; primality
then forces it to equal `1`. -/
theorem isUnramifiedAtFinitePlaces_of_degreeTwo_degreeOneHundredFiftySeven
    {F K E L : Type}
  ...
  have hcardE : Nat.card Gal(E/F) = 157 := by
    rw [IsGalois.card_aut_eq_finrank, hFE]
  have hdvd : Ideal.ramificationIdx q R.asIdeal ∣ 157 := by
    rw [hcardE] at hfundE
    rw [← heIn_eq]
    exact ⟨
  ...
  have heR : Ideal.ramificationIdx q R.asIdeal = 1 := by
    rcases (Nat.dvd_prime (by norm_num : Nat.Prime 157)).mp hdvd with h | h
    · exact h
    · omega
  exact
...
/-- The actual cyclic degree-157 Kummer extension, once unramified at every
finite prime, contributes `157`-torsion to the full cyclotomic class group.
...
theorem oneHundredFiftySeven_dvd_classNumber_of_kummerExtensionUnramified
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 157)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 157 - C (a : K)))
    (hunramified : KummerExtension157Unramified hirr) :
...
/-- Hilbert 94 turns a cyclic unramified degree-157 extension into an
explicit nonprincipal ideal whose 157th power is principal. -/
theorem hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension157
    {F : Type} [Field F] [NumberField F]
    (E : UnramifiedCyclicExtension157 F) :
    HasNonprincipalIdealWithPrincipalPower F 157 := by
  letI := E.fieldL
  letI := E.numberFieldL
  letI := E.algebraFL
...
cyclic extension of degree `157` over the maximal real subfield.

The algebraic fixed-field package comes from
`KummerRealFixedField157`.  Finite-prime unramifiedness descends from the
given unramified Kummer extension by the coprime-degree theorem above:
each ramification index divides both `2` and `157`. -/
noncomputable def realFixedFieldUnramifiedCyclicExtension157
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 157 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness157 a)
    (hunramified : KummerExtension157Unramified hirr) :
    UnramifiedCyclicExtension157 K⁺ := by
  letI : Fact (Irreducible (X ^ 157 - C (a : K))) := ⟨hirr⟩
  let L := KummerExtension157 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K)
      (by norm_num : 157 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  letI : Algebra K⁺ L :=
    algebraKummerOverReal157 (K := K) (a := a)
  letI : IsScalarTower K⁺ K L :=
    scalarTowerKummerOverReal157 (K := K) (a := a)
  letI : FiniteDimensional K⁺ L :=
    finiteDimensionalKummerOverReal157 (K := K) (a := a)
  let E := realFixedField157 hanti
  ...
  letI : FiniteDimensional K⁺ E :=
    FiniteDimensional.of_finrank_pos <| by
      rw [show Module.finrank K⁺ E = 157 by
        exact finrank_realFixedField157 (a := a) hanti]
      norm_num
  ...
  letI : IsCyclic (E ≃ₐ[K⁺] E) :=
    isCyclic_realFixedFieldAut157 (a := a) hanti
  have hrealUnramified : IsUnramifiedAtFinitePlaces K⁺ E :=
    @isUnramifiedAtFinitePlaces_of_degreeTwo_degreeOneHundredFiftySeven
      K⁺ K E L
      ...
      hunramified
  exact UnramifiedCyclicExtension157.ofIsUnramifiedAtFinitePlaces
    (finrank_realFixedField157 (a := a) hanti)
    hrealUnramified

/-- Hilbert 94 applied to the explicit conjugation-fixed field: an
anti-invariant, everywhere-unramified degree-`157` Kummer extension gives
nontrivial `157`-torsion in the maximal-real class group. -/
theorem antiInvariantKummerReflection157
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 157 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness157 a)
    (hunramified : KummerExtension157Unramified hirr) :
    HasNonprincipalIdealWithPrincipalPower K⁺ 157 :=
  hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension157
    (realFixedFieldUnramifiedCyclicExtension157
      hirr hanti hunramified)

set_option maxRecDepth 50000 in
/-- The conjugate-pair instance of Vandiver's Lemma 1 at exponent `157`.
...
for `x * conj(x)^156` supplies the lifted conjugation, so the theorem above
would force `157` to divide the maximal-real class number, contradicting
the checked Sinnott--Kummer computation. -/
theorem conjugatePairIdealRoot_isPrincipal157
    {ζ : K} (hζ : IsPrimitiveRoot ζ 157)
    {x : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ (conjugatePairRadicand157 x))
    (hpow :
      I ^ 157 = Ideal.span {conjugatePairRadicand157 x}) :
    Submodule.IsPrincipal (I : Ideal (𝓞 K)) := by
  by_contra hnonprincipal
  let hirr :
      Irreducible
        (X ^ 157 - C (conjugatePairRadicand157 x : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 157) hpow hnonprincipal
  have hunramified : KummerExtension157Unramified hirr :=
    primaryIdealRootGivesUnramifiedKummer157
      hζ hprimary hpow hnonprincipal
  have htorsion :
      HasNonprincipalIdealWithPrincipalPower K⁺ 157 :=
    antiInvariantKummerReflection157 hirr
      (conjugatePairAntiInvariantWitness157 x)
      hunramified
  have hdvd : 157 ∣ NumberField.classNumber K⁺ :=
    (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := K⁺) (p := 157) (by norm_num)).mp htorsion
...
If `I^157 = (x)`, `J^157 = (conj(x))`, and the normalized product
`x * conj(x)^156` is primary, the explicit real fixed-field reflection
principalizes `I * J^156`. -/
theorem exists_conjugatePairEquationSevenAGenerator157
    {ζ : K} (hζ : IsPrimitiveRoot ζ 157)
    (I J : Ideal (𝓞 K)) (x y : 𝓞 K)
    (hIpow : I ^ 157 = Ideal.span {x})
    (hJpow : J ^ 157 = Ideal.span {y})
    (hconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K x = y)
    (hprimary : IsKummerPrimary hζ (x * y ^ 156)) :
    ∃ r : 𝓞 K, I * J ^ 156 = Ideal.span {r} := by
  have hradicand :
      conjugatePairRadicand157 x = x * y ^ 156 := by
    simp only [conjugatePairRadicand157, hconj]
  have hpow :
      (I * J ^ 156) ^ 157 =
        Ideal.span {conjugatePairRadicand157 x} := by
    calc
      (I * J ^ 156) ^ 157 =
          I ^ 157 * (J ^ 157) ^ 156 := by
        rw [mul_pow, ← pow_mul, ← pow_mul,
          Nat.mul_comm 156 157]
      _ = Ideal.span {x} * (Ideal.span {y}) ^ 156 := by
        rw [hIpow, hJpow]
      _ = Ideal.span {x * y ^ 156} := by
        rw [Ideal.span_singleton_pow,
          Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {conjugatePairRadicand157 x} := by
        rw [hradicand]
  have hprimary' :
      IsKummerPrimary hζ (conjugatePairRadicand157 x) := by
    rw [hradicand]
    exact hprimary
  obtain ⟨r, hr⟩ :=
    (conjugatePairIdealRoot_isPrincipal157
...
must produce a cyclic unramified degree-157 extension of the maximal real
subfield.

This is deliberately an extension-valued statement, rather than assuming
the desired class-group torsion conclusion. -/
def PrimaryKummerProducesRealUnramifiedExtension157
    (K : Type) [Field K] [NumberField K]
    [IsCyclotomicExtension {157} ℚ K] : Prop :=
  ∀ {zeta : K} (hzeta : IsPrimitiveRoot zeta 157)
    {a : 𝓞 K},
...
`TakagiFurtwangler157`. -/
theorem primaryUnramifiedKummerReflection157_of_realExtension
    (hreal : PrimaryKummerProducesRealUnramifiedExtension157 K) :
    PrimaryUnramifiedKummerReflection157 K := by
  intro zeta hzeta a hprimary hirr hunramified
  obtain ⟨E⟩ := hreal hzeta hprimary hirr hunramified
  exact
    hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension157 E

end

end Fermat.Irregular.TakagiReflection157
