import Fermat.Descent.Irregular.TakagiFurtwangler491
import Fermat.Descent.Irregular.KummerRealFixedField491
...
/-!
# The global Takagi--Furtwängler reflection layer at exponent 491
...
step: construction of an unramified cyclic degree-491 extension of the
maximal real subfield.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiReflection491
...
open Fermat.Irregular.KummerRealFixedField491
...
the exact bridge from `KummerExtension491Unramified` to the `IsUnramified`
...
degree-`2`/degree-`491` compositum configuration.

Suppose `K/F` is Galois of degree `2`, `E/F` is Galois of degree `491`,
...
ramification theory for `E/F` also makes that index divide `491`; primality
then forces it to equal `1`. -/
theorem isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFourHundredNinetyOne
    {F K E L : Type}
  ...
  have hcardE : Nat.card Gal(E/F) = 491 := by
    rw [IsGalois.card_aut_eq_finrank, hFE]
  have hdvd : Ideal.ramificationIdx q R.asIdeal ∣ 491 := by
    rw [hcardE] at hfundE
    rw [← heIn_eq]
    exact ⟨
  ...
  have heR : Ideal.ramificationIdx q R.asIdeal = 1 := by
    rcases (Nat.dvd_prime (by norm_num : Nat.Prime 491)).mp hdvd with h | h
    · exact h
    · omega
  exact
...
/-- The actual cyclic degree-491 Kummer extension, once unramified at every
finite prime, contributes `491`-torsion to the full cyclotomic class group.
...
theorem fourHundredNinetyOne_dvd_classNumber_of_kummerExtensionUnramified
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 491)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K)))
    (hunramified : KummerExtension491Unramified hirr) :
...
/-- Hilbert 94 turns a cyclic unramified degree-491 extension into an
explicit nonprincipal ideal whose 491th power is principal. -/
theorem hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension491
    {F : Type} [Field F] [NumberField F]
    (E : UnramifiedCyclicExtension491 F) :
    HasNonprincipalIdealWithPrincipalPower F 491 := by
  letI := E.fieldL
  letI := E.numberFieldL
  letI := E.algebraFL
...
cyclic extension of degree `491` over the maximal real subfield.

The algebraic fixed-field package comes from
`KummerRealFixedField491`.  Finite-prime unramifiedness descends from the
given unramified Kummer extension by the coprime-degree theorem above:
each ramification index divides both `2` and `491`. -/
noncomputable def realFixedFieldUnramifiedCyclicExtension491
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness491 a)
    (hunramified : KummerExtension491Unramified hirr) :
    UnramifiedCyclicExtension491 K⁺ := by
  letI : Fact (Irreducible (X ^ 491 - C (a : K))) := ⟨hirr⟩
  let L := KummerExtension491 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K)
      (by norm_num : 491 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  letI : Algebra K⁺ L :=
    algebraKummerOverReal491 (K := K) (a := a)
  letI : IsScalarTower K⁺ K L :=
    scalarTowerKummerOverReal491 (K := K) (a := a)
  letI : FiniteDimensional K⁺ L :=
    finiteDimensionalKummerOverReal491 (K := K) (a := a)
  let E := realFixedField491 hanti
  ...
  letI : FiniteDimensional K⁺ E :=
    FiniteDimensional.of_finrank_pos <| by
      rw [show Module.finrank K⁺ E = 491 by
        exact finrank_realFixedField491 (a := a) hanti]
      norm_num
  ...
  letI : IsCyclic (E ≃ₐ[K⁺] E) :=
    isCyclic_realFixedFieldAut491 (a := a) hanti
  have hrealUnramified : IsUnramifiedAtFinitePlaces K⁺ E :=
    @isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFourHundredNinetyOne
      K⁺ K E L
      ...
      hunramified
  exact UnramifiedCyclicExtension491.ofIsUnramifiedAtFinitePlaces
    (finrank_realFixedField491 (a := a) hanti)
    hrealUnramified

/-- Hilbert 94 applied to the explicit conjugation-fixed field: an
anti-invariant, everywhere-unramified degree-`491` Kummer extension gives
nontrivial `491`-torsion in the maximal-real class group. -/
theorem antiInvariantKummerReflection491
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness491 a)
    (hunramified : KummerExtension491Unramified hirr) :
    HasNonprincipalIdealWithPrincipalPower K⁺ 491 :=
  hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension491
    (realFixedFieldUnramifiedCyclicExtension491
      hirr hanti hunramified)

set_option maxRecDepth 50000 in
/-- The conjugate-pair instance of Vandiver's Lemma 1 at exponent `491`.
...
for `x * conj(x)^490` supplies the lifted conjugation, so the theorem above
would force `491` to divide the maximal-real class number, contradicting
the checked Sinnott--Kummer computation. -/
theorem conjugatePairIdealRoot_isPrincipal491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {x : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ (conjugatePairRadicand491 x))
    (hpow :
      I ^ 491 = Ideal.span {conjugatePairRadicand491 x}) :
    Submodule.IsPrincipal (I : Ideal (𝓞 K)) := by
  by_contra hnonprincipal
  let hirr :
      Irreducible
        (X ^ 491 - C (conjugatePairRadicand491 x : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 491) hpow hnonprincipal
  have hunramified : KummerExtension491Unramified hirr :=
    primaryIdealRootGivesUnramifiedKummer491
      hζ hprimary hpow hnonprincipal
  have htorsion :
      HasNonprincipalIdealWithPrincipalPower K⁺ 491 :=
    antiInvariantKummerReflection491 hirr
      (conjugatePairAntiInvariantWitness491 x)
      hunramified
  have hdvd : 491 ∣ NumberField.classNumber K⁺ :=
    (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := K⁺) (p := 491) (by norm_num)).mp htorsion
...
If `I^491 = (x)`, `J^491 = (conj(x))`, and the normalized product
`x * conj(x)^490` is primary, the explicit real fixed-field reflection
principalizes `I * J^490`. -/
theorem exists_conjugatePairEquationSevenAGenerator491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    (I J : Ideal (𝓞 K)) (x y : 𝓞 K)
    (hIpow : I ^ 491 = Ideal.span {x})
    (hJpow : J ^ 491 = Ideal.span {y})
    (hconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K x = y)
    (hprimary : IsKummerPrimary hζ (x * y ^ 490)) :
    ∃ r : 𝓞 K, I * J ^ 490 = Ideal.span {r} := by
  have hradicand :
      conjugatePairRadicand491 x = x * y ^ 490 := by
    simp only [conjugatePairRadicand491, hconj]
  have hpow :
      (I * J ^ 490) ^ 491 =
        Ideal.span {conjugatePairRadicand491 x} := by
    calc
      (I * J ^ 490) ^ 491 =
          I ^ 491 * (J ^ 491) ^ 490 := by
        rw [mul_pow, ← pow_mul, ← pow_mul,
          Nat.mul_comm 490 491]
      _ = Ideal.span {x} * (Ideal.span {y}) ^ 490 := by
        rw [hIpow, hJpow]
      _ = Ideal.span {x * y ^ 490} := by
        rw [Ideal.span_singleton_pow,
          Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {conjugatePairRadicand491 x} := by
        rw [hradicand]
  have hprimary' :
      IsKummerPrimary hζ (conjugatePairRadicand491 x) := by
    rw [hradicand]
    exact hprimary
  obtain ⟨r, hr⟩ :=
    (conjugatePairIdealRoot_isPrincipal491
...
must produce a cyclic unramified degree-491 extension of the maximal real
subfield.

This is deliberately an extension-valued statement, rather than assuming
the desired class-group torsion conclusion. -/
def PrimaryKummerProducesRealUnramifiedExtension491
    (K : Type) [Field K] [NumberField K]
    [IsCyclotomicExtension {491} ℚ K] : Prop :=
  ∀ {zeta : K} (hzeta : IsPrimitiveRoot zeta 491)
    {a : 𝓞 K},
...
`TakagiFurtwangler491`. -/
theorem primaryUnramifiedKummerReflection491_of_realExtension
    (hreal : PrimaryKummerProducesRealUnramifiedExtension491 K) :
    PrimaryUnramifiedKummerReflection491 K := by
  intro zeta hzeta a hprimary hirr hunramified
  obtain ⟨E⟩ := hreal hzeta hprimary hirr hunramified
  exact
    hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension491 E

end

end Fermat.Irregular.TakagiReflection491
