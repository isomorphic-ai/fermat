import Fermat.Descent.Irregular.TakagiFurtwangler587
import Fermat.Descent.Irregular.KummerRealFixedField587
...
/-!
# The global Takagi--Furtwängler reflection layer at exponent 587
...
step: construction of an unramified cyclic degree-587 extension of the
maximal real subfield.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiReflection587
...
open Fermat.Irregular.KummerRealFixedField587
...
the exact bridge from `KummerExtension587Unramified` to the `IsUnramified`
...
degree-`2`/degree-`587` compositum configuration.

Suppose `K/F` is Galois of degree `2`, `E/F` is Galois of degree `587`,
...
ramification theory for `E/F` also makes that index divide `587`; primality
then forces it to equal `1`. -/
theorem isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFiveHundredEightySeven
    {F K E L : Type}
  ...
  have hcardE : Nat.card Gal(E/F) = 587 := by
    rw [IsGalois.card_aut_eq_finrank, hFE]
  have hdvd : Ideal.ramificationIdx q R.asIdeal ∣ 587 := by
    rw [hcardE] at hfundE
    rw [← heIn_eq]
    exact ⟨
  ...
  have heR : Ideal.ramificationIdx q R.asIdeal = 1 := by
    rcases (Nat.dvd_prime (by norm_num : Nat.Prime 587)).mp hdvd with h | h
    · exact h
    · omega
  exact
...
/-- The actual cyclic degree-587 Kummer extension, once unramified at every
finite prime, contributes `587`-torsion to the full cyclotomic class group.
...
theorem fiveHundredEightySeven_dvd_classNumber_of_kummerExtensionUnramified
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 587)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K)))
    (hunramified : KummerExtension587Unramified hirr) :
...
/-- Hilbert 94 turns a cyclic unramified degree-587 extension into an
explicit nonprincipal ideal whose 587th power is principal. -/
theorem hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension587
    {F : Type} [Field F] [NumberField F]
    (E : UnramifiedCyclicExtension587 F) :
    HasNonprincipalIdealWithPrincipalPower F 587 := by
  letI := E.fieldL
  letI := E.numberFieldL
  letI := E.algebraFL
...
cyclic extension of degree `587` over the maximal real subfield.

The algebraic fixed-field package comes from
`KummerRealFixedField587`.  Finite-prime unramifiedness descends from the
given unramified Kummer extension by the coprime-degree theorem above:
each ramification index divides both `2` and `587`. -/
noncomputable def realFixedFieldUnramifiedCyclicExtension587
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness587 a)
    (hunramified : KummerExtension587Unramified hirr) :
    UnramifiedCyclicExtension587 K⁺ := by
  letI : Fact (Irreducible (X ^ 587 - C (a : K))) := ⟨hirr⟩
  let L := KummerExtension587 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K)
      (by norm_num : 587 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  letI : Algebra K⁺ L :=
    algebraKummerOverReal587 (K := K) (a := a)
  letI : IsScalarTower K⁺ K L :=
    scalarTowerKummerOverReal587 (K := K) (a := a)
  letI : FiniteDimensional K⁺ L :=
    finiteDimensionalKummerOverReal587 (K := K) (a := a)
  let E := realFixedField587 hanti
  ...
  letI : FiniteDimensional K⁺ E :=
    FiniteDimensional.of_finrank_pos <| by
      rw [show Module.finrank K⁺ E = 587 by
        exact finrank_realFixedField587 (a := a) hanti]
      norm_num
  ...
  letI : IsCyclic (E ≃ₐ[K⁺] E) :=
    isCyclic_realFixedFieldAut587 (a := a) hanti
  have hrealUnramified : IsUnramifiedAtFinitePlaces K⁺ E :=
    @isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFiveHundredEightySeven
      K⁺ K E L
      ...
      hunramified
  exact UnramifiedCyclicExtension587.ofIsUnramifiedAtFinitePlaces
    (finrank_realFixedField587 (a := a) hanti)
    hrealUnramified

/-- Hilbert 94 applied to the explicit conjugation-fixed field: an
anti-invariant, everywhere-unramified degree-`587` Kummer extension gives
nontrivial `587`-torsion in the maximal-real class group. -/
theorem antiInvariantKummerReflection587
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness587 a)
    (hunramified : KummerExtension587Unramified hirr) :
    HasNonprincipalIdealWithPrincipalPower K⁺ 587 :=
  hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension587
    (realFixedFieldUnramifiedCyclicExtension587
      hirr hanti hunramified)

set_option maxRecDepth 50000 in
/-- The conjugate-pair instance of Vandiver's Lemma 1 at exponent `587`.
...
for `x * conj(x)^586` supplies the lifted conjugation, so the theorem above
would force `587` to divide the maximal-real class number, contradicting
the checked Sinnott--Kummer computation. -/
theorem conjugatePairIdealRoot_isPrincipal587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {x : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ (conjugatePairRadicand587 x))
    (hpow :
      I ^ 587 = Ideal.span {conjugatePairRadicand587 x}) :
    Submodule.IsPrincipal (I : Ideal (𝓞 K)) := by
  by_contra hnonprincipal
  let hirr :
      Irreducible
        (X ^ 587 - C (conjugatePairRadicand587 x : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 587) hpow hnonprincipal
  have hunramified : KummerExtension587Unramified hirr :=
    primaryIdealRootGivesUnramifiedKummer587
      hζ hprimary hpow hnonprincipal
  have htorsion :
      HasNonprincipalIdealWithPrincipalPower K⁺ 587 :=
    antiInvariantKummerReflection587 hirr
      (conjugatePairAntiInvariantWitness587 x)
      hunramified
  have hdvd : 587 ∣ NumberField.classNumber K⁺ :=
    (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := K⁺) (p := 587) (by norm_num)).mp htorsion
...
If `I^587 = (x)`, `J^587 = (conj(x))`, and the normalized product
`x * conj(x)^586` is primary, the explicit real fixed-field reflection
principalizes `I * J^586`. -/
theorem exists_conjugatePairEquationSevenAGenerator587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    (I J : Ideal (𝓞 K)) (x y : 𝓞 K)
    (hIpow : I ^ 587 = Ideal.span {x})
    (hJpow : J ^ 587 = Ideal.span {y})
    (hconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K x = y)
    (hprimary : IsKummerPrimary hζ (x * y ^ 586)) :
    ∃ r : 𝓞 K, I * J ^ 586 = Ideal.span {r} := by
  have hradicand :
      conjugatePairRadicand587 x = x * y ^ 586 := by
    simp only [conjugatePairRadicand587, hconj]
  have hpow :
      (I * J ^ 586) ^ 587 =
        Ideal.span {conjugatePairRadicand587 x} := by
    calc
      (I * J ^ 586) ^ 587 =
          I ^ 587 * (J ^ 587) ^ 586 := by
        rw [mul_pow, ← pow_mul, ← pow_mul,
          Nat.mul_comm 586 587]
      _ = Ideal.span {x} * (Ideal.span {y}) ^ 586 := by
        rw [hIpow, hJpow]
      _ = Ideal.span {x * y ^ 586} := by
        rw [Ideal.span_singleton_pow,
          Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {conjugatePairRadicand587 x} := by
        rw [hradicand]
  have hprimary' :
      IsKummerPrimary hζ (conjugatePairRadicand587 x) := by
    rw [hradicand]
    exact hprimary
  obtain ⟨r, hr⟩ :=
    (conjugatePairIdealRoot_isPrincipal587
...
must produce a cyclic unramified degree-587 extension of the maximal real
subfield.

This is deliberately an extension-valued statement, rather than assuming
the desired class-group torsion conclusion. -/
def PrimaryKummerProducesRealUnramifiedExtension587
    (K : Type) [Field K] [NumberField K]
    [IsCyclotomicExtension {587} ℚ K] : Prop :=
  ∀ {zeta : K} (hzeta : IsPrimitiveRoot zeta 587)
    {a : 𝓞 K},
...
`TakagiFurtwangler587`. -/
theorem primaryUnramifiedKummerReflection587_of_realExtension
    (hreal : PrimaryKummerProducesRealUnramifiedExtension587 K) :
    PrimaryUnramifiedKummerReflection587 K := by
  intro zeta hzeta a hprimary hirr hunramified
  obtain ⟨E⟩ := hreal hzeta hprimary hirr hunramified
  exact
    hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension587 E

end

end Fermat.Irregular.TakagiReflection587
