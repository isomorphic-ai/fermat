import Fermat.Descent.Irregular.TakagiFurtwangler59
import Fermat.Descent.Irregular.KummerRealFixedField59
import FltRegular.NumberTheory.Hilbert94
import Mathlib.NumberTheory.RamificationInertia.Galois

/-!
# The global Takagi--Furtwängler reflection layer at exponent 59
...
step: construction of an unramified cyclic degree-59 extension of the
maximal real subfield.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiReflection59
...
open Fermat.Irregular.KummerRealFixedField59
...
the exact bridge from `KummerExtension59Unramified` to the `IsUnramified`
...
degree-`2`/degree-`59` compositum configuration.

Suppose `K/F` is Galois of degree `2`, `E/F` is Galois of degree `59`,
...
ramification theory for `E/F` also makes that index divide `59`; primality
then forces it to equal `1`. -/
theorem isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFiftyNine
    {F K E L : Type}
  ...
  have hcardE : Nat.card Gal(E/F) = 59 := by
    rw [IsGalois.card_aut_eq_finrank, hFE]
  have hdvd : Ideal.ramificationIdx q R.asIdeal ∣ 59 := by
    rw [hcardE] at hfundE
    rw [← heIn_eq]
    exact ⟨
  ...
  have heR : Ideal.ramificationIdx q R.asIdeal = 1 := by
    rcases (Nat.dvd_prime (by norm_num : Nat.Prime 59)).mp hdvd with h | h
    · exact h
    · omega
  exact
...
/-- The actual cyclic degree-59 Kummer extension, once unramified at every
finite prime, contributes `59`-torsion to the full cyclotomic class group.
...
theorem fiftyNine_dvd_classNumber_of_kummerExtensionUnramified
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 59 - C (a : K)))
    (hunramified : KummerExtension59Unramified hirr) :
...
/-- Hilbert 94 turns a cyclic unramified degree-59 extension into an
explicit nonprincipal ideal whose 59th power is principal. -/
theorem hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension59
    {F : Type} [Field F] [NumberField F]
    (E : UnramifiedCyclicExtension59 F) :
    HasNonprincipalIdealWithPrincipalPower F 59 := by
  letI := E.fieldL
  letI := E.numberFieldL
  letI := E.algebraFL
...
cyclic extension of degree `59` over the maximal real subfield.

The algebraic fixed-field package comes from
`KummerRealFixedField59`.  Finite-prime unramifiedness descends from the
given unramified Kummer extension by the coprime-degree theorem above:
each ramification index divides both `2` and `59`. -/
noncomputable def realFixedFieldUnramifiedCyclicExtension59
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 59 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness59 a)
    (hunramified : KummerExtension59Unramified hirr) :
    UnramifiedCyclicExtension59 K⁺ := by
  letI : Fact (Irreducible (X ^ 59 - C (a : K))) := ⟨hirr⟩
  let L := KummerExtension59 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K)
      (by norm_num : 59 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  letI : Algebra K⁺ L :=
    algebraKummerOverReal59 (K := K) (a := a)
  letI : IsScalarTower K⁺ K L :=
    scalarTowerKummerOverReal59 (K := K) (a := a)
  letI : FiniteDimensional K⁺ L :=
    finiteDimensionalKummerOverReal59 (K := K) (a := a)
  let E := realFixedField59 hanti
  ...
  letI : FiniteDimensional K⁺ E :=
    FiniteDimensional.of_finrank_pos <| by
      rw [show Module.finrank K⁺ E = 59 by
        exact finrank_realFixedField59 (a := a) hanti]
      norm_num
  ...
  letI : IsCyclic (E ≃ₐ[K⁺] E) :=
    isCyclic_realFixedFieldAut59 (a := a) hanti
  have hrealUnramified : IsUnramifiedAtFinitePlaces K⁺ E :=
    @isUnramifiedAtFinitePlaces_of_degreeTwo_degreeFiftyNine
      K⁺ K E L
      ...
      hunramified
  exact UnramifiedCyclicExtension59.ofIsUnramifiedAtFinitePlaces
    (finrank_realFixedField59 (a := a) hanti)
    hrealUnramified

/-- Hilbert 94 applied to the explicit conjugation-fixed field: an
anti-invariant, everywhere-unramified degree-`59` Kummer extension gives
nontrivial `59`-torsion in the maximal-real class group. -/
theorem antiInvariantKummerReflection59
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 59 - C (a : K)))
    (hanti : ConjugationAntiInvariantWitness59 a)
    (hunramified : KummerExtension59Unramified hirr) :
    HasNonprincipalIdealWithPrincipalPower K⁺ 59 :=
  hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension59
    (realFixedFieldUnramifiedCyclicExtension59
      hirr hanti hunramified)

set_option maxRecDepth 2000 in
/-- The conjugate-pair instance of Vandiver's Lemma 1 at exponent `59`.
...
for `x * conj(x)^58` supplies the lifted conjugation, so the theorem above
would force `59` to divide the maximal-real class number, contradicting
the checked Sinnott--Kummer computation. -/
theorem conjugatePairIdealRoot_isPrincipal59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ (conjugatePairRadicand59 x))
    (hpow :
      I ^ 59 = Ideal.span {conjugatePairRadicand59 x}) :
    Submodule.IsPrincipal (I : Ideal (𝓞 K)) := by
  by_contra hnonprincipal
  let hirr :
      Irreducible
        (X ^ 59 - C (conjugatePairRadicand59 x : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 59) hpow hnonprincipal
  have hunramified : KummerExtension59Unramified hirr :=
    primaryIdealRootGivesUnramifiedKummer59
      hζ hprimary hpow hnonprincipal
  have htorsion :
      HasNonprincipalIdealWithPrincipalPower K⁺ 59 :=
    antiInvariantKummerReflection59 hirr
      (conjugatePairAntiInvariantWitness59 x)
      hunramified
  have hdvd : 59 ∣ NumberField.classNumber K⁺ :=
    (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := K⁺) (p := 59) (by norm_num)).mp htorsion
...
If `I^59 = (x)`, `J^59 = (conj(x))`, and the normalized product
`x * conj(x)^58` is primary, the explicit real fixed-field reflection
principalizes `I * J^58`. -/
theorem exists_conjugatePairEquationSevenAGenerator59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (I J : Ideal (𝓞 K)) (x y : 𝓞 K)
    (hIpow : I ^ 59 = Ideal.span {x})
    (hJpow : J ^ 59 = Ideal.span {y})
    (hconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K x = y)
    (hprimary : IsKummerPrimary hζ (x * y ^ 58)) :
    ∃ r : 𝓞 K, I * J ^ 58 = Ideal.span {r} := by
  have hradicand :
      conjugatePairRadicand59 x = x * y ^ 58 := by
    simp only [conjugatePairRadicand59, hconj]
  have hpow :
      (I * J ^ 58) ^ 59 =
        Ideal.span {conjugatePairRadicand59 x} := by
    calc
      (I * J ^ 58) ^ 59 =
          I ^ 59 * (J ^ 59) ^ 58 := by
        rw [mul_pow, ← pow_mul, ← pow_mul,
          Nat.mul_comm 58 59]
      _ = Ideal.span {x} * (Ideal.span {y}) ^ 58 := by
        rw [hIpow, hJpow]
      _ = Ideal.span {x * y ^ 58} := by
        rw [Ideal.span_singleton_pow,
          Ideal.span_singleton_mul_span_singleton]
      _ = Ideal.span {conjugatePairRadicand59 x} := by
        rw [hradicand]
  have hprimary' :
      IsKummerPrimary hζ (conjugatePairRadicand59 x) := by
    rw [hradicand]
    exact hprimary
  obtain ⟨r, hr⟩ :=
    (conjugatePairIdealRoot_isPrincipal59
...
must produce a cyclic unramified degree-59 extension of the maximal real
subfield.

This is deliberately an extension-valued statement, rather than assuming
the desired class-group torsion conclusion. -/
def PrimaryKummerProducesRealUnramifiedExtension59
    (K : Type) [Field K] [NumberField K]
    [IsCyclotomicExtension {59} ℚ K] : Prop :=
  ∀ {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    {a : 𝓞 K},
...
`TakagiFurtwangler59`. -/
theorem primaryUnramifiedKummerReflection59_of_realExtension
    (hreal : PrimaryKummerProducesRealUnramifiedExtension59 K) :
    PrimaryUnramifiedKummerReflection59 K := by
  intro zeta hzeta a hprimary hirr hunramified
  obtain ⟨E⟩ := hreal hzeta hprimary hirr hunramified
  exact
    hasNonprincipalIdealWithPrincipalPower_of_unramifiedCyclicExtension59 E

end

end Fermat.Irregular.TakagiReflection59
