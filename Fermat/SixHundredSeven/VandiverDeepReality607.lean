...
import Fermat.SixHundredSeven.VandiverRelationNormalization607

/-!
# Deep Vandiver units are real at exponent 607
...
that the depth-1214 local hypothesis itself forces an ambient cyclotomic unit
to be real.
...
namespace Fermat.SixHundredSeven.VandiverDeepReality
...
inverse. -/
theorem unitsComplexConj_zeta607 {zeta : K}
...
theorem ringOfIntegersComplexConj_zeta_sub_one607 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) :
  ...
  have hconjzeta :
    ...
    congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K)
      (unitsComplexConj_zeta607 hzeta)
  rw [hconjzeta]
  have hinv :
...
conjugation. -/
theorem zeta_sub_one_pow_dvd_conj_of_dvd607 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) (n : ℕ) (a : 𝓞 K)
    (h : ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow,
    ringOfIntegersComplexConj_zeta_sub_one607, mul_pow]
...
/-- A square of a 607st root of unity which is `1` modulo
`(zeta - 1)^2` is literally `1`. -/
theorem zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd607 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) (j : ℕ)
  ...
  · have hknotdvd : ¬607 ∣ k := by
      intro h
      obtain ⟨t, ht⟩ := h
      apply hk
      omega
    have hkcoprime : k.Coprime 607 := by
      apply Nat.coprime_comm.mpr
      exact (show Nat.Prime 607 by norm_num).coprime_iff_not_dvd.mpr
        hknotdvd
...
theorem unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj607 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) (u : (𝓞 K)ˣ)
    (hdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
  ...
  have hroot :=
    zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd607 hzeta j hrootdiv
  rw [hroot] at hj
  have hucu : u = cu := by
    calc
      u = (u * cu⁻¹) * cu := by simp
...
/-- The depth-1214 hypothesis forces a unit to be real. -/
theorem isVandiverDeep_unitsComplexConj_eq {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 607) hzeta u) :
    NumberField.IsCMField.unitsComplexConj K u = u := by
  obtain ⟨c, hc⟩ := hdeep
  have hsign : ((hzeta.unit' : 𝓞 K) - 1) ^ 1214 ∣
      (u : 𝓞 K) - (c : 𝓞 K) ^ 607 := by
    have hbase : (hzeta.unit' : 𝓞 K) - 1 ∣
        (1 : 𝓞 K) - hzeta.unit' := by
      refine ⟨-1, ?_⟩
      ring
    exact (pow_dvd_pow_of_dvd hbase 1214).trans hc
  have hconj := zeta_sub_one_pow_dvd_conj_of_dvd607
    hzeta 1214 ((u : 𝓞 K) - (c : 𝓞 K) ^ 607) hsign
  have hconj' : ((hzeta.unit' : 𝓞 K) - 1) ^ 1214 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (u : 𝓞 K) -
        (c : 𝓞 K) ^ 607 := by
    simpa only [map_sub, map_pow, map_intCast] using hconj
  have hdefect1214 : ((hzeta.unit' : 𝓞 K) - 1) ^ 1214 ∣
      (u : 𝓞 K) -
  ...
  have hdefect2 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
    ...
    (pow_dvd_pow ((hzeta.unit' : 𝓞 K) - 1) (by omega)).trans
      hdefect1214
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj607
    hzeta u hdefect2

/-- A deeply congruent ambient unit, packaged in the real-unit subgroup. -/
def deepRealUnit607 {zeta : K} (hzeta : IsPrimitiveRoot zeta 607)
    (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 607) hzeta u) :
...
@[simp]
theorem deepRealUnit607_coe {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 607) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 607) hzeta u) :
...
end Fermat.SixHundredSeven.VandiverDeepReality
