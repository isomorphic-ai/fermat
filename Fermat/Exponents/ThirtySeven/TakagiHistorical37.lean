import Fermat.Descent.Irregular.TakagiReflection37
import Fermat.Exponents.ThirtySeven.VandiverHistorical
...
conjugation. -/
theorem exists_historicalEquationEight_realUnit37_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 37) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal37 hζ s = Ideal.span {ρ} ∧
  ...
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight37_unconditional hζ s hs
  exact ⟨ρ, η, hI, heq,
...
theorem exists_historicalEquationEight_pair_one37_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 37) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρ, η₀, -, heq, hη₀real⟩ :=
    exists_historicalEquationEight_realUnit37_unconditional hζ s hs
  let η : (𝓞 K)ˣ := -η₀
  let ρminus : 𝓞 K :=
...
theorem exists_historicalEquationEight_pair_two37_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 37) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one37_unconditional
      hζtwo stwo hstwo
  have hassoc :
