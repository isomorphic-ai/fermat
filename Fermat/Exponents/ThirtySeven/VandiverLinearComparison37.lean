import Fermat.Exponents.ThirtySeven.VandiverCongruenceSupport37
...
depth-38 wrapper is enough for Lemma 1; equation (10) uses the entire
source exponent. -/
theorem historicalState_omega_add_theta_fullHighDivisibility37
    {ζ : K} (hζ : IsPrimitiveRoot ζ 37) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 1) * 37 + 1) ∣
      s.omega + s.theta := by
...
Writing `Lᵢ = ηᵢ*rᵢ^37`, the two equations imply
...
gives the result. -/
theorem equationEight_one_two_coefficients_close37
    ...
    (heq₁ :
      ω + (hζ.unit' : 𝓞 K) * θ =
        (1 - (hζ.unit' : 𝓞 K)) * η₁ * r₁ ^ 37)
    (heq₂ :
      ω + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * θ =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * η₂ * r₂ ^ 37) :
    ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
      (η₁ : 𝓞 K) * r₁ ^ 37 - (η₂ : 𝓞 K) * r₂ ^ 37 := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let L₁ : 𝓞 K := (η₁ : 𝓞 K) * r₁ ^ 37
  let L₂ : 𝓞 K := (η₂ : 𝓞 K) * r₂ ^ 37
  have hπ0 : π ≠ 0 :=
...
`(2*m - 2)*37`. -/
theorem historicalEquationEight_one_two_coefficients_close37
    {ζ : K} (hζ : IsPrimitiveRoot ζ 37)
  ...
  let D : ℕ := (2 * s.m - 2) * 37
  have hfull :=
    historicalState_omega_add_theta_fullHighDivisibility37 hζ s hs
  have hle : D + 1 ≤ (2 * s.m - 1) * 37 + 1 := by
    dsimp [D]
    omega
  have hhigh :
      ((hζ.unit' : 𝓞 K) - 1) ^ (D + 1) ∣
  ...
  simpa only [D] using
    equationEight_one_two_coefficients_close37
