...
/-!
# Prime-support bookkeeping for Vandiver's historical descent at 691
...
lemma primeIdealFactorSupport691_subset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q) :
    primeIdealFactorSupport691 a ⊆ primeIdealFactorSupport691 b := by
...
lemma primeIdealFactorSupport691_ssubset_of_span_factor_of_witness
  ...
  have hsubset :=
    primeIdealFactorSupport691_subset_of_span_factor hb hfactor
...
`(a)`. -/
lemma primeIdealFactorSupport691_ssubset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q)
    (hQ : Q ≠ ⊤)
  ...
  apply primeIdealFactorSupport691_ssubset_of_span_factor_of_witness
    hb hfactor hPQ
  intro hPa
...
and gives the strict inclusion required by `ConjugationPowerReductionData691`.
-/
theorem historicalEquationEightA_square_support_strict691
    ...
    (hQ : Q ≠ ⊤)
    (hcop :
      IsCoprime (historicalEquationEightAIdeal691 hζ s) Q) :
    primeIdealFactorSupport691 (ρzero ^ 2) ⊂
      primeIdealFactorSupport691 s.xi := by
  rw [primeIdealFactorSupport691_pow ρzero (by norm_num : 2 ≠ 0)]
  apply primeIdealFactorSupport691_ssubset_of_span_factor
    s.xi_ne_zero
