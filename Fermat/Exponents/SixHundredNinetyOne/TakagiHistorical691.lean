import Fermat.Descent.Irregular.TakagiReflection691
import Fermat.Exponents.SixHundredNinetyOne.VandiverHistorical
...
The universal predicate `LemmaOne K 691` is stronger than the instance
...
`TakagiReflection691` therefore proves the required historical equation
...
namespace Fermat.SixHundredNinetyOne.TakagiHistorical691

noncomputable section

open Fermat.Irregular.VandiverHistoricalDescent
open Fermat.Irregular.VandiverLemmaOne
open Fermat.Irregular.TakagiReflection691
open Fermat.SixHundredNinetyOne.VandiverHistorical
...
theorem exists_historicalEquationSevenA691_unconditional
    ...
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal691 hζ s *
        ...
        Ideal.span {r} := by
  exact exists_conjugatePairEquationSevenAGenerator691 hζ
    (historicalZetaFactorIdeal691 hζ s)
...
historical results. -/
theorem exists_historicalEquationEight691_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
  ...
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA691_unconditional hζ s hs
  obtain ⟨t, hsevenD⟩ :=
    exists_historicalEquationSevenD691 hζ s hs
  obtain ⟨hI0, hJ0⟩ :=
    historicalFactorIdeals_ne_zero691 hζ s hs
...
conjugation. -/
theorem exists_historicalEquationEight_realUnit691_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal691 hζ s = Ideal.span {ρ} ∧
  ...
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight691_unconditional hζ s hs
  exact ⟨ρ, η, hI, heq,
...
theorem exists_historicalEquationEight_pair_one691_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρ, η₀, -, heq, hη₀real⟩ :=
    exists_historicalEquationEight_realUnit691_unconditional hζ s hs
  let η : (𝓞 K)ˣ := -η₀
  let ρminus : 𝓞 K :=
  ...
  have hρnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρ := by
    ...
    exact dvd_mul_of_dvd_right
      (dvd_pow (n := 691) hρ (by norm_num)) _
...
theorem exists_historicalEquationEight_pair_two691_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one691_unconditional
      hζtwo stwo hstwo
  have hassoc :
...
end Fermat.SixHundredNinetyOne.TakagiHistorical691
