import Fermat.Exponents.SixHundredNinetyOne.TakagiHistorical691
import Fermat.Exponents.SixHundredNinetyOne.VandiverPostNineGenerator691
import Fermat.Exponents.SixHundredNinetyOne.VandiverGeneratorSupport691

/-!
# Prepared conjugate equation-(8) pairs at exponent 691
...
structure PreparedEquationEightPair691
  ...
  equation_plus :
    s.omega + (t : 𝓞 K) * s.theta =
      (1 - (t : 𝓞 K)) * coefficient * rplus ^ 691
  equation_minus :
    s.omega + (t⁻¹ : (𝓞 K)ˣ) * s.theta =
      (1 - (t⁻¹ : (𝓞 K)ˣ)) * coefficient * rminus ^ 691
  conjugate :
  ...
  mu_not_ramified :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ mu
  close_plus :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 2) * 691) ∣
      rplus - mu ^ 691
  close_minus :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 2) * 691) ∣
      rminus - mu ^ 691

namespace PreparedEquationEightPair691

variable {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
  (s : HistoricalState hζ) (t : (𝓞 K)ˣ)
  (d : PreparedEquationEightPair691 hζ s t)
...
lemma quadraticEquation :
  ...
  calc
    ...
    _ = ((1 - (t : 𝓞 K)) * d.coefficient * d.rplus ^ 691) *
...
lemma quadraticEquation_one
    (d : PreparedEquationEightPair691 hζ s hζ.unit') :
    s.omega ^ 2 +
        equationTenTraceOne691 hζ * (s.omega * s.theta) +
        s.theta ^ 2 =
      kappa hζ *
        ((d.coefficient ^ 2 : (𝓞 K)ˣ) *
          (d.rplus * d.rminus) ^ 691) := by
  simpa only [equationTenTraceOne691, kappa] using
    d.quadraticEquation hζ s hζ.unit'

/-- Quadratic equation (10) for the prepared pair at `2,-2`. -/
lemma quadraticEquation_two
    (d : PreparedEquationEightPair691 hζ s (hζ.unit' ^ 2)) :
    s.omega ^ 2 +
        equationTenTraceTwo691 hζ * (s.omega * s.theta) +
        s.theta ^ 2 =
      kappa hζ *
        ((equationTenTraceTwoUnit691 hζ * d.coefficient ^ 2 :
            (𝓞 K)ˣ) *
          (d.rplus * d.rminus) ^ 691) := by
  have htrace :
      (((hζ.unit' ^ 2 : (𝓞 K)ˣ) : 𝓞 K) +
          (((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) : 𝓞 K)) =
        equationTenTraceTwo691 hζ := by
    simp only [equationTenTraceTwo691, Units.val_pow_eq_pow_val]
    congr 1
  have hden :
    ...
    calc
      ...
      _ = _ := kappa_powTwoPrimitiveRoot691 hζ
...
end PreparedEquationEightPair691

namespace PostEquationNineNormalizedData691

/-- Since both Vandiver's fixed-denominator quotient and its chosen
generator are real, the residual unit in `q = η μ^691` is real as well. -/
lemma generator_unit_real
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (d : PostEquationNineNormalizedData691 hζ)
    (μ : 𝓞 K) (η : (𝓞 K)ˣ)
    (hμreal :
      NumberField.IsCMField.ringOfIntegersComplexConj K μ = μ)
    (hμnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ μ)
    (hq : quotient hζ d = η * μ ^ 691) :
  ...
  change
    NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
      (η : 𝓞 K)
  apply mul_right_cancel₀ (pow_ne_zero 691 hμ0)
  have hqreal := d.quotient_real hζ
  calc
    (NumberField.IsCMField.unitsComplexConj K η : 𝓞 K) * μ ^ 691 =
        NumberField.IsCMField.ringOfIntegersComplexConj K
          ((η : 𝓞 K) * μ ^ 691) := by
...
end PostEquationNineNormalizedData691
...
unit. -/
theorem exists_preparedEquationEightPair691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ)
    (a : ℕ) (ha : a.Coprime 691)
    ...
    (hplus :
      s.omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) * eta * rhoPlus ^ 691)
    (hminus :
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
    ...
    (hzero :
      s.omega + s.theta =
        etaZero * kappa hζ ^ (691 * s.m - 345) * rhoZero ^ 691)
    (hconj :
    ...
    Nonempty
      (PreparedEquationEightPair691 hζ s (hζ.unit' ^ a)) := by
  classical
  obtain ⟨epsilon, hdifference⟩ :=
    exists_equationEight_difference691 hζ a s.m ha s.one_lt_m
      s.omega s.theta rhoPlus rhoMinus rhoZero eta etaZero
      hplus hminus hzero
  obtain ⟨nplus, nminus, hnplus, hnminus, hclose,
      hnconj, hnminusNot⟩ :=
    equationNineA_normalized_conjugate691 hζ s.m s.one_lt_m
      rhoPlus rhoMinus rhoZero epsilon hdifference hconj hminusNot
  have hnpair : IsCoprime nplus nminus := by
    have hpowers : IsCoprime (nplus ^ 691) (nminus ^ 691) := by
      rw [hnplus, hnminus]
      exact hpair.pow_left.pow_right
    exact
      (IsCoprime.pow_left_iff (by norm_num : 0 < 691)).mp
        ((IsCoprime.pow_right_iff (by norm_num : 0 < 691)).mp hpowers)
  let D : ℕ := (2 * s.m - 2) * 691
  let post : PostEquationNineNormalizedData691 hζ :=
    { m := s.m
      one_lt_m := s.one_lt_m
      rplus := nplus
      rminus := nminus
  ...
  let coefficient : (𝓞 K)ˣ := eta * delta ^ 691
  ...
  have hdeltaCancel :
      (delta : 𝓞 K) ^ 691 *
          ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 691 = 1 := by
    rw [← mul_pow, ← Units.val_mul]
    simp
  have hequationPlus :
      s.omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) *
          coefficient * rplus ^ 691 := by
    have hscaled :
        (coefficient : 𝓞 K) * rplus ^ 691 =
          (eta : 𝓞 K) * rhoPlus ^ 691 := by
      dsimp [coefficient, rplus]
      simp only [mul_pow]
      rw [hnplus]
      calc
        ((eta : 𝓞 K) * (delta : 𝓞 K) ^ 691) *
            (((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 691 *
              rhoPlus ^ 691) =
          (eta : 𝓞 K) *
            (((delta : 𝓞 K) ^ 691 *
              ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 691) *
                rhoPlus ^ 691) := by ac_rfl
        _ = (eta : 𝓞 K) * rhoPlus ^ 691 := by
    ...
    calc
      ...
      _ = (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) *
          ((coefficient : 𝓞 K) * rplus ^ 691) := by
        rw [hscaled]
      _ = _ := by rw [mul_assoc]
  have hequationMinus :
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) *
          coefficient * rminus ^ 691 := by
    have hscaled :
        (coefficient : 𝓞 K) * rminus ^ 691 =
          (eta : 𝓞 K) * rhoMinus ^ 691 := by
      ...
      calc
        ((eta : 𝓞 K) * (delta : 𝓞 K) ^ 691) *
          ...
          (eta : 𝓞 K) *
            (((delta : 𝓞 K) ^ 691 *
              ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 691) *
                rhoMinus ^ 691) := by ac_rfl
        _ = (eta : 𝓞 K) * rhoMinus ^ 691 := by
          rw [hdeltaCancel, one_mul]
    calc
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
          (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) *
  ...
  have hrplusNot :
    ...
    have hn : (hζ.unit' : 𝓞 K) - 1 ∣ nplus := by
      ...
      simp
    have hc := zeta_sub_one_pow_dvd_conj_of_dvd691 hζ 1 nplus
...
theorem equationEight_pair_one_generators_coprime691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ)
    (rhoPlus rhoMinus : 𝓞 K) (eta : (𝓞 K)ˣ)
    (hplus :
      s.omega + (hζ.unit' : 𝓞 K) * s.theta =
        (1 - (hζ.unit' : 𝓞 K)) * eta * rhoPlus ^ 691)
    (hminus :
  ...
  have hcMinus :
    ...
    rw [zetaUnit_inv_eq_pow_sixHundredNinety691 hζ]
  ...
  exact coprime_generators_of_distinct_linearEquations691
...
theorem equationEight_pair_two_generators_coprime691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ)
    (rhoPlus rhoMinus : 𝓞 K) (eta : (𝓞 K)ˣ)
    (hplus :
      s.omega + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * eta * rhoPlus ^ 691)
    (hminus :
      s.omega + ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) * s.theta =
  ...
  have hcMinus :
      Associated
        ((1 - ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ)) *
          (eta : 𝓞 K)) pi := by
    rw [zetaUnit_sq_inv_eq_pow_sixHundredEightyNine691 hζ]
    simpa only [pi, Units.val_pow_eq_pow_val] using
...
theorem exists_preparedEquationEightPair_one691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    ...
    (hzero :
      s.omega + s.theta =
        etaZero * kappa hζ ^ (691 * s.m - 345) * rhoZero ^ 691) :
    Nonempty (PreparedEquationEightPair691 hζ s hζ.unit') := by
  obtain ⟨rhoPlus, rhoMinus, eta, hetaReal, hplus, hminus,
      hconj, -, hminusNot⟩ :=
    _root_.Fermat.SixHundredNinetyOne.TakagiHistorical691.exists_historicalEquationEight_pair_one691_unconditional
      hζ s hs
  have hpair :=
    equationEight_pair_one_generators_coprime691
      hζ s rhoPlus rhoMinus eta hplus hminus
  simpa only [pow_one] using
    exists_preparedEquationEightPair691 hζ s 1
...
Takagi--Furtwängler input already discharged. -/
theorem exists_preparedEquationEightPair_two691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s)
    (rhoZero : 𝓞 K) (etaZero : (𝓞 K)ˣ)
    (hzero :
      s.omega + s.theta =
        etaZero * kappa hζ ^ (691 * s.m - 345) * rhoZero ^ 691) :
    Nonempty (PreparedEquationEightPair691 hζ s (hζ.unit' ^ 2)) := by
  obtain ⟨rhoPlus, rhoMinus, eta, hetaReal, hplus, hminus,
      hconj, -, hminusNot⟩ :=
    _root_.Fermat.SixHundredNinetyOne.TakagiHistorical691.exists_historicalEquationEight_pair_two691_unconditional
      hζ s hs
  have hpair :=
    equationEight_pair_two_generators_coprime691
      hζ s rhoPlus rhoMinus eta hplus hminus
  exact exists_preparedEquationEightPair691 hζ s 2
    (by norm_num) rhoPlus rhoMinus rhoZero eta etaZero
    hetaReal
