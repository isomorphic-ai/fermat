import Fermat.Exponents.SixHundredNinetyOne.VandiverCongruenceSupport691
...
depth-692 wrapper is enough for Lemma 1; equation (10) uses the entire
source exponent. -/
theorem historicalState_omega_add_theta_fullHighDivisibility691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 1) * 691 + 1) ∣
      s.omega + s.theta := by
...
Writing `Lᵢ = ηᵢ*rᵢ^691`, the two equations imply
...
gives the result. -/
theorem equationEight_one_two_coefficients_close691
    ...
    (heq₁ :
      ω + (hζ.unit' : 𝓞 K) * θ =
        (1 - (hζ.unit' : 𝓞 K)) * η₁ * r₁ ^ 691)
    (heq₂ :
      ω + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * θ =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * η₂ * r₂ ^ 691) :
    ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
      (η₁ : 𝓞 K) * r₁ ^ 691 - (η₂ : 𝓞 K) * r₂ ^ 691 := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let L₁ : 𝓞 K := (η₁ : 𝓞 K) * r₁ ^ 691
  let L₂ : 𝓞 K := (η₂ : 𝓞 K) * r₂ ^ 691
  have hπ0 : π ≠ 0 :=
...
`(2*m - 2)*691`. -/
theorem historicalEquationEight_one_two_coefficients_close691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
  ...
  let D : ℕ := (2 * s.m - 2) * 691
  have hfull :=
    historicalState_omega_add_theta_fullHighDivisibility691 hζ s hs
  have hle : D + 1 ≤ (2 * s.m - 1) * 691 + 1 := by
    dsimp [D]
    omega
  have hhigh :
      ((hζ.unit' : 𝓞 K) - 1) ^ (D + 1) ∣
  ...
  simpa only [D] using
    equationEight_one_two_coefficients_close691
