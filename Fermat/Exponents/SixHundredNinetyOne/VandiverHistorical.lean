...
import Fermat.Exponents.SixHundredNinetyOne.SinnottKummer
import FltRegular.NumberTheory.Cyclotomic.MoreLemmas

/-!
# Vandiver's historical descent at exponent 691

This file instantiates the source-faithful historical descent with the
concrete real-subfield invariant at exponent `691`. It first proves the
passage from a primitive rational second-case solution to Vandiver's
equation (6), using

`691 ~ (1 - ζ) ^ 690 ~ ((1 - ζ) * (1 - ζ⁻¹)) ^ 345`.
...
inverse. -/
lemma unitsComplexConj_zeta691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691) :
...
explicit unit `-ζ⁻¹`. -/
lemma ringOfIntegersComplexConj_zeta_sub_one691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
  ...
  have hconjζ :
    ...
    congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K) (unitsComplexConj_zeta691 hζ)
  rw [hconjζ]
  have hinv :
...
`ringOfIntegersComplexConj_zeta_sub_one691` keeps the proof entirely at
element level. -/
lemma zeta_sub_one_pow_dvd_conj_of_dvd691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (n : ℕ) (a : 𝓞 K)
    (h : ((hζ.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hζ.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow, ringOfIntegersComplexConj_zeta_sub_one691, mul_pow]
  refine ⟨(((-hζ.unit'⁻¹ : (𝓞 K)ˣ) ^ n : (𝓞 K)ˣ) : 𝓞 K) *
      NumberField.IsCMField.ringOfIntegersComplexConj K b, ?_⟩
  simp only [Units.val_pow_eq_pow_val]
  ring

/-- Every 691st power is fixed by complex conjugation modulo
`(ζ - 1)^691`.  Both powers are compared with the same rational 691st
power; conjugating the first comparison is legitimate by the preceding
uniformizer calculation. -/
lemma zeta_sub_one_pow_sixHundredNinetyOne_dvd_pow_sub_conj_pow691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 691 ∣
      a ^ 691 -
        NumberField.IsCMField.ringOfIntegersComplexConj K a ^ 691 := by
  obtain ⟨c, hc⟩ := exists_int_pow_congruent_mod_primary hζ a
  have hcconj := zeta_sub_one_pow_dvd_conj_of_dvd691 hζ 691
    (a ^ 691 - (c : 𝓞 K) ^ 691) hc
  have hcconj' : ((hζ.unit' : 𝓞 K) - 1) ^ 691 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a ^ 691 -
        (c : 𝓞 K) ^ 691 := by
    simpa only [map_sub, map_pow, map_intCast] using hcconj
  convert dvd_sub hc hcconj' using 1
  ring

/-- A 691st root of unity which is `1` modulo `(ζ - 1)^2` is literally
`1`.  A nontrivial power `ζ^k - 1` is associated to the single
uniformizer `ζ - 1`, so it cannot contain its square. -/
lemma zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (j : ℕ)
  ...
  · have hknotdvd : ¬ 691 ∣ k := by
      intro h
      obtain ⟨t, ht⟩ := h
      apply hk
      omega
    have hkcoprime : k.Coprime 691 := by
      apply Nat.coprime_comm.mpr
      exact (show Nat.Prime 691 by norm_num).coprime_iff_not_dvd.mpr hknotdvd
...
lemma unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (u : (𝓞 K)ˣ)
    (hdiv : ((hζ.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
  ...
  have hroot := zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd691 hζ j hrootdiv
  rw [hroot] at hj
  have hucu : u = cu := by
    calc
      u = (u * cu⁻¹) * cu := by simp
...
completely explicit. -/
def kappaUnit691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691) : (𝓞 K)ˣ :=
  -hζ.unit'⁻¹

omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
/-- The literal unit identity relating Vandiver's `κ` to `(ζ - 1)²`. -/
lemma kappa_eq_kappaUnit691_mul_sq {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    kappa hζ = (kappaUnit691 hζ : 𝓞 K) *
      ((hζ.unit' : 𝓞 K) - 1) ^ 2 := by
  simp only [kappa, kappaUnit691, Units.val_neg, neg_mul, pow_two]
...
omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
/-- Power form of `kappa_eq_kappaUnit691_mul_sq`. -/
lemma kappa_pow_eq_kappaUnit691_pow_mul {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (m : ℕ) :
    kappa hζ ^ m = ((kappaUnit691 hζ ^ m : (𝓞 K)ˣ) : 𝓞 K) *
      ((hζ.unit' : 𝓞 K) - 1) ^ (2 * m) := by
  rw [kappa_eq_kappaUnit691_mul_sq, mul_pow, ← Units.val_pow_eq_pow_val,
    ← pow_mul]
...
def historicalRegularUnit691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ) : (𝓞 K)ˣ :=
  s.eta * kappaUnit691 hζ ^ (s.m * 691)

omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
/-- Every historical state at exponent `691` is literally an input to the
generic factor-ideal construction, with upstream depth parameter
`2 * m - 1`:

`ω^691 + θ^691 = ε * (((ζ - 1)^(2*m) * ξ)^691)`.
...
without alteration. -/
lemma historicalState_regularEquation691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    s.omega ^ 691 + s.theta ^ 691 = historicalRegularUnit691 hζ s *
      (((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 1) + 1) * s.xi) ^ 691 := by
  have hm : 1 ≤ 2 * s.m := by
    have := s.one_lt_m
    omega
  rw [Nat.sub_add_cancel hm]
  rw [s.equation, kappa_pow_eq_kappaUnit691_pow_mul]
  simp only [historicalRegularUnit691, mul_pow, ← Units.val_pow_eq_pow_val,
    Units.val_mul]
  rw [← pow_mul]
  ac_rfl
...
lemma historicalState_theta_not_dvd691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ s.theta := by
  intro htheta
  have hsum : (hζ.unit' : 𝓞 K) - 1 ∣
      s.omega ^ 691 + s.theta ^ 691 :=
    zeta_sub_one_dvd (p := 691) hζ
      (historicalState_regularEquation691 hζ s)
  have homegaPow : (hζ.unit' : 𝓞 K) - 1 ∣ s.omega ^ 691 := by
    simpa using dvd_sub hsum (dvd_pow (n := 691) htheta (by norm_num))
...
/-- For a factorization with real entries at exponent `691`, the unique
...
therefore gives `ζ^i = ζ⁻ⁱ`.  Since `691` is odd, this forces `i = 0`.
...
theorem distinguishedRoot_eq_one_of_real691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x y z : 𝓞 K} {ε : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 691 + y ^ 691 = ε *
      ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
    (hy : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ y)
    (hxreal : NumberField.IsCMField.ringOfIntegersComplexConj K x = x)
    (hyreal : NumberField.IsCMField.ringOfIntegersComplexConj K y = y) :
    zeta_sub_one_dvd_root (by norm_num : 691 ≠ 2) hζ e hy =
      oneNthRoot (K := K) (p := 691) := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let η0 := zeta_sub_one_dvd_root (by norm_num : 691 ≠ 2) hζ e hy
  let q0 : 𝓞 K :=
    div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ e η0
  have hπ0 : π ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hq0 : π ∣ q0 := by
    simpa only [π, q0, η0] using
      (Ideal.Quotient.eq_zero_iff_dvd
        ((hζ.unit' : 𝓞 K) - 1)
        (div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ e
          (zeta_sub_one_dvd_root
            (by norm_num : 691 ≠ 2) hζ e hy))).mp
        (zeta_sub_one_dvd_root_spec
          (by norm_num : 691 ≠ 2) hζ e hy)
  have hetaPow : (η0 : 𝓞 K) ^ 691 = 1 := by
    exact (Polynomial.mem_nthRootsFinset
      (by norm_num : 0 < 691) (1 : 𝓞 K)).mp η0.prop
  obtain ⟨i, hi, heta⟩ :=
  ...
  · have hiPos : 0 < i := Nat.pos_of_ne_zero hi0
    ...
    have hj : j < 691 := by
      dsimp [j]
      omega
    let ηj : Polynomial.nthRootsFinset 691 (1 : 𝓞 K) :=
      ⟨(hζ.unit' : 𝓞 K) ^ j, by
        rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 691)]
        rw [← pow_mul, show j * 691 = 691 * j by omega, pow_mul,
          hζ.unit'_coe.pow_eq_one, one_pow]⟩
    let qj : 𝓞 K :=
      div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ e ηj
    have hzpowU : hζ.unit' ^ 691 = 1 := by
      apply Units.ext
      apply NumberField.RingOfIntegers.ext
      change ζ ^ 691 = 1
      exact hζ.pow_eq_one
...
it carries far more than the `692` local powers needed for Lemma 1. -/
theorem historicalState_omega_add_theta_highDivisibility691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 692 ∣ s.omega + s.theta := by
  let e := historicalState_regularEquation691 hζ s
  let hy := historicalState_theta_not_dvd691 hζ s
  have hroot := distinguishedRoot_eq_one_of_real691 hζ e hy hs.1 hs.2.1
  have hhigh := distinguishedFactor_highDivisibility
    (by norm_num : 691 ≠ 2) hζ e hy
  rw [hroot] at hhigh
  simp only [oneNthRoot, mul_one] at hhigh
  have hle : 692 ≤ (2 * s.m - 1) * 691 + 1 := by
    have hm := s.one_lt_m
    omega
...
`(ω + θ) / (ζ - 1)`. -/
noncomputable def historicalOneFactor691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (oneNthRoot (K := K) (p := 691))

/-- The allocated ideal root of the distinguished factor at `1`. -/
noncomputable def historicalOneFactorIdeal691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
    (oneNthRoot (K := K) (p := 691))
...
noncomputable def historicalEquationEightAIdeal691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  a_eta_zero_dvd_p_pow (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)

/-- Multiplying the fixed-denominator factor at `1` back by `ζ - 1`
recovers `ω + θ`. -/
lemma historicalOneFactor_mul_zeta_sub_one691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    historicalOneFactor691 hζ s * ((hζ.unit' : 𝓞 K) - 1) =
      s.omega + s.theta := by
  simpa only [historicalOneFactor691, oneNthRoot, mul_one] using
    (div_zeta_sub_one_mul_zeta_sub_one
      (by norm_num : 691 ≠ 2) hζ
      (historicalState_regularEquation691 hζ s)
      (oneNthRoot (K := K) (p := 691)))

set_option maxRecDepth 50000 in
/-- The allocated ideal at the root `1` has 691st power generated by its
literal fixed-denominator factor. -/
lemma historicalOneFactorIdeal_pow691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    historicalOneFactorIdeal691 hζ s ^ 691 =
      Ideal.span {historicalOneFactor691 hζ s} := by
  exact (linearFactorQuotient_span_eq_factorRoot_pow
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
    s.coprime_omega_theta
    (oneNthRoot (K := K) (p := 691))).symm

/-- The forced ramified part times the residual equation-(8a) ideal is the
allocated ideal at the root `1`. -/
lemma historicalEquationEightAIdeal_spec691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ^ (2 * s.m - 1) *
        historicalEquationEightAIdeal691 hζ s =
      historicalOneFactorIdeal691 hζ s := by
  have hspec := a_eta_zero_dvd_p_pow_spec
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
  rw [distinguishedRoot_eq_one_of_real691 hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s) hs.1 hs.2.1] at hspec
  simpa only [historicalEquationEightAIdeal691,
    historicalOneFactorIdeal691] using hspec

/-- The real factor `ω + θ` is nonzero in every historical state. -/
lemma historicalState_omega_add_theta_ne_zero691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
  ...
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit691_mul_sq]
    exact mul_ne_zero (kappaUnit691 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have hrhs :
      (s.eta : 𝓞 K) * (kappa hζ ^ s.m * s.xi) ^ 691 ≠ 0 :=
    mul_ne_zero s.eta.isUnit.ne_zero
      (pow_ne_zero 691
        (mul_ne_zero (pow_ne_zero s.m hkappa0) s.xi_ne_zero))
  apply hrhs
  rw [← s.equation, htheta]
  simp only [Odd.neg_pow (by norm_num : Odd 691), add_neg_cancel]

/-- The exact `κ`-power displayed in equation (8a) divides `ω + θ`.
The exponent identity is

`2 * (691*m - 345) = (2*m - 1)*691 + 1`.

Thus this is the element-level counterpart of the forced ramified ideal
factorization above. -/
lemma historicalState_kappaPower_dvd_omega_add_theta691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    kappa hζ ^ (691 * s.m - 345) ∣ s.omega + s.theta := by
  let E : ℕ := 691 * s.m - 345
  let N : ℕ := (2 * s.m - 1) * 691 + 1
  have hexp : 2 * E = N := by
    dsimp [E, N]
    have hm := s.one_lt_m
    omega
  have hroot := distinguishedRoot_eq_one_of_real691 hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s) hs.1 hs.2.1
  have hhigh := distinguishedFactor_highDivisibility
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
  ...
  let u : (𝓞 K)ˣ := kappaUnit691 hζ ^ E
  refine ⟨(u⁻¹ : (𝓞 K)ˣ) * c, ?_⟩
  rw [kappa_pow_eq_kappaUnit691_pow_mul, hexp]
...
noncomputable def historicalEquationEightAQuotient691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) : 𝓞 K :=
  (historicalState_kappaPower_dvd_omega_add_theta691 hζ s hs).choose

/-- Definition equation for the integral quotient in (8a). -/
lemma historicalEquationEightAQuotient_spec691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    s.omega + s.theta =
      kappa hζ ^ (691 * s.m - 345) *
        historicalEquationEightAQuotient691 hζ s hs :=
  (historicalState_kappaPower_dvd_omega_add_theta691 hζ s hs).choose_spec

/-- The quotient occurring in equation (8a) is real. -/
lemma historicalEquationEightAQuotient_real691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (historicalEquationEightAQuotient691 hζ s hs) =
      historicalEquationEightAQuotient691 hζ s hs := by
  let E : ℕ := 691 * s.m - 345
  let q : 𝓞 K := historicalEquationEightAQuotient691 hζ s hs
  have hkappa0 : kappa hζ ^ E ≠ 0 := by
    rw [kappa_pow_eq_kappaUnit691_pow_mul]
    exact mul_ne_zero (kappaUnit691 hζ ^ E).isUnit.ne_zero
      (pow_ne_zero (2 * E) (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have hq := historicalEquationEightAQuotient_spec691 hζ s hs
...
lemma historicalEquationEightAQuotient_ne_zero691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAQuotient691 hζ s hs ≠ 0 := by
  intro hq
  apply historicalState_omega_add_theta_ne_zero691 hζ s
  rw [historicalEquationEightAQuotient_spec691 hζ s hs, hq, mul_zero]

/-- Principal-ideal form of the exact ramified exponent in equation (8a). -/
lemma historical_span_kappaPower691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    Ideal.span ({kappa hζ ^ (691 * s.m - 345)} : Set (𝓞 K)) =
  ...
  have hexp : 2 * E = N := by
    ...
    have hm := s.one_lt_m
    omega
  rw [show 691 * s.m - 345 = E by rfl,
    kappa_pow_eq_kappaUnit691_pow_mul, hexp]
  change Ideal.span
      ({((kappaUnit691 hζ ^ E : (𝓞 K)ˣ) : 𝓞 K) *
        ((hζ.unit' : 𝓞 K) - 1) ^ N} : Set (𝓞 K)) =
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ^ N
  rw [← Ideal.span_singleton_mul_span_singleton]
  rw [Ideal.span_singleton_eq_top.mpr
    (kappaUnit691 hζ ^ E).isUnit, ← Ideal.one_eq_top, one_mul]
  exact (Ideal.span_singleton_pow ((hζ.unit' : 𝓞 K) - 1) N).symm

set_option maxRecDepth 50000 in
/-- The residual equation-(8a) ideal has 691st power generated by the
literal real quotient `(ω + θ) / κ^(691*m-345)`. -/
theorem historicalEquationEightAIdeal_pow691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAIdeal691 hζ s ^ 691 =
      Ideal.span {historicalEquationEightAQuotient691 hζ s hs} := by
  let P : Ideal (𝓞 K) :=
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K))
  let A : Ideal (𝓞 K) := historicalOneFactorIdeal691 hζ s
  let J : Ideal (𝓞 K) := historicalEquationEightAIdeal691 hζ s
  let qone : 𝓞 K := historicalOneFactor691 hζ s
  let qzero : 𝓞 K := historicalEquationEightAQuotient691 hζ s hs
  let E : ℕ := 691 * s.m - 345
  let d : ℕ := 2 * s.m - 1
  let N : ℕ := d * 691 + 1
  have hPJ : P ^ d * J = A := by
    simpa only [P, d, J, A] using
      historicalEquationEightAIdeal_spec691 hζ s hs
  have hA691 : A ^ 691 = Ideal.span {qone} := by
    simpa only [A, qone] using historicalOneFactorIdeal_pow691 hζ s
  have hqone : qone * ((hζ.unit' : 𝓞 K) - 1) =
      s.omega + s.theta := by
    simpa only [qone] using historicalOneFactor_mul_zeta_sub_one691 hζ s
  have hqzero : s.omega + s.theta = kappa hζ ^ E * qzero := by
    simpa only [E, qzero] using
      historicalEquationEightAQuotient_spec691 hζ s hs
  have hspanK : Ideal.span {kappa hζ ^ E} = P ^ N := by
    simpa only [E, P, N, d] using historical_span_kappaPower691 hζ s
  have hramified : P ^ N * J ^ 691 =
      Ideal.span {s.omega + s.theta} := by
    calc
      P ^ N * J ^ 691 = P * (P ^ d * J) ^ 691 := by
        dsimp only [N]
        rw [mul_pow, ← pow_mul]
        rw [pow_succ']
        ring
      _ = P * A ^ 691 := by rw [hPJ]
      _ = P * Ideal.span {qone} := by rw [hA691]
      _ = Ideal.span
      ...
      _ = Ideal.span {s.omega + s.theta} := by
        congr 2
        rw [mul_comm, hqone]
  have hspanEq : Ideal.span {kappa hζ ^ E} * J ^ 691 =
      Ideal.span {kappa hζ ^ E} * Ideal.span {qzero} := by
    calc
      Ideal.span {kappa hζ ^ E} * J ^ 691 =
          P ^ N * J ^ 691 := by rw [hspanK]
...
noncomputable def historicalZetaFactor691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (zetaNthRoot (K := K) (p := 691) hζ)

/-- The fixed-denominator conjugate linear factor at `ζ⁻¹`. -/
noncomputable def historicalInverseZetaFactor691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (inverseZetaNthRoot (K := K) (p := 691) hζ)

/-- The unit-normalized inverse factor.  Multiplication by `-ζ` changes
the fixed denominator `ζ - 1` to its conjugate normalization. -/
noncomputable def normalizedHistoricalInverseZetaFactor691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : 𝓞 K :=
  ((-hζ.unit' : (𝓞 K)ˣ) : 𝓞 K) *
    historicalInverseZetaFactor691 hζ s

/-- The actual normalized factor product to which Vandiver applies
Lemma 1 is Kummer-primary in every real historical state. -/
theorem historicalConjugateFactorProduct_isKummerPrimary691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    IsKummerPrimary hζ
      (historicalZetaFactor691 hζ s *
        normalizedHistoricalInverseZetaFactor691 hζ s ^ 690) := by
  simpa only [historicalZetaFactor691,
    normalizedHistoricalInverseZetaFactor691,
    historicalInverseZetaFactor691] using
    normalizedConjugateLinearFactor_isKummerPrimary
      (by norm_num : 691 ≠ 2) hζ
      (historicalState_regularEquation691 hζ s)
      (historicalState_theta_not_dvd691 hζ s)
      (historicalState_omega_add_theta_highDivisibility691 hζ s hs)

/-- The allocated ideal root of the linear factor at `ζ`. -/
noncomputable def historicalZetaFactorIdeal691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
...
noncomputable def historicalInverseZetaFactorIdeal691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
...
set_option maxRecDepth 50000 in
/-- The allocated ideal at `ζ` has 691st power generated by the literal
historical linear factor. -/
theorem historicalZetaFactorIdeal_pow691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    historicalZetaFactorIdeal691 hζ s ^ 691 =
      Ideal.span {historicalZetaFactor691 hζ s} := by
  exact (linearFactorQuotient_span_eq_factorRoot_pow
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
    s.coprime_omega_theta
    (zetaNthRoot (K := K) (p := 691) hζ)).symm

set_option maxRecDepth 50000 in
/-- The allocated inverse ideal has 691st power generated by the
unit-normalized inverse factor. -/
theorem historicalInverseZetaFactorIdeal_pow691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    historicalInverseZetaFactorIdeal691 hζ s ^ 691 =
      Ideal.span {normalizedHistoricalInverseZetaFactor691 hζ s} := by
  calc
    historicalInverseZetaFactorIdeal691 hζ s ^ 691 =
        Ideal.span {historicalInverseZetaFactor691 hζ s} :=
      (linearFactorQuotient_span_eq_factorRoot_pow
        (by norm_num : 691 ≠ 2) hζ
        (historicalState_regularEquation691 hζ s)
        (historicalState_theta_not_dvd691 hζ s)
        s.coprime_omega_theta
        (inverseZetaNthRoot (K := K) (p := 691) hζ)).symm
    _ = Ideal.span {normalizedHistoricalInverseZetaFactor691 hζ s} :=
...
theorem ringOfIntegersComplexConj_historicalZetaFactor691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (historicalZetaFactor691 hζ s) =
      normalizedHistoricalInverseZetaFactor691 hζ s := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let qplus : 𝓞 K := historicalZetaFactor691 hζ s
  let qminus : 𝓞 K := historicalInverseZetaFactor691 hζ s
  let u : (𝓞 K)ˣ := -hζ.unit'⁻¹
  have hπ0 : π ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hconjζ :
...
uniformizer. -/
theorem historicalZetaFactor_not_ramified691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ historicalZetaFactor691 hζ s := by
  have hlocal := normalizedConjugateLinearFactors
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
    (historicalState_omega_add_theta_highDivisibility691 hζ s hs)
  simpa only [historicalZetaFactor691] using hlocal.1

/-- The normalized historical factor and its conjugate agree modulo
`(ζ - 1)^691`.  This is the local compatibility behind Vandiver's claim
that the unit in equation (8) may be chosen real. -/
theorem historicalZetaFactor_sub_conj_highDivisibility691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 691 ∣
      historicalZetaFactor691 hζ s -
        NumberField.IsCMField.ringOfIntegersComplexConj K
          (historicalZetaFactor691 hζ s) := by
  have hlocal := normalizedConjugateLinearFactors
    (by norm_num : 691 ≠ 2) hζ
    (historicalState_regularEquation691 hζ s)
    (historicalState_theta_not_dvd691 hζ s)
    (historicalState_omega_add_theta_highDivisibility691 hζ s hs)
  rw [ringOfIntegersComplexConj_historicalZetaFactor691 hζ s hs]
  simpa only [historicalZetaFactor691, historicalInverseZetaFactor691,
    normalizedHistoricalInverseZetaFactor691] using hlocal.2.2
...
theorem exists_historicalEquationSevenA691
    (hlemma : LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal691 hζ s *
          historicalInverseZetaFactorIdeal691 hζ s ^ 690 =
        Ideal.span {r} := by
  exact exists_equationSevenA_generator hlemma hζ
    (historicalZetaFactorIdeal691 hζ s)
    (historicalInverseZetaFactorIdeal691 hζ s)
    (historicalZetaFactor691 hζ s)
    (normalizedHistoricalInverseZetaFactor691 hζ s)
    (historicalZetaFactorIdeal_pow691 hζ s)
    (historicalInverseZetaFactorIdeal_pow691 hζ s)
    (by simpa using
      historicalConjugateFactorProduct_isKummerPrimary691 hζ s hs)
...
`RelevantIdealQuotientsPrincipal` at the irregular prime `691`: a genuine
`691`-torsion minus class may remain.  The definitions and theorems below
...
`I / conj(I)`.  Since `I ^ 691` is already principal, once the symmetric
...
/-- The exact class-group consequence of the exponent-`691`
Sinnott--Kummer theorem used by Vandiver in (7b) and again after (9): a
fractional ideal of the maximal real field whose `691`th power is principal
...
theorem realFractionalIdeal_isPrincipal_of_pow691
...
has `691`th power `(a)`, then it has a real generator `ρ`, and `a` differs
from `ρ ^ 691` by a real unit. -/
theorem exists_real_unit_mul_pow_generator691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
...
produced by (7b).  If `J ^ 691 = (a)` in the cyclotomic ring, multiplicativity
of the relative ideal norm gives

`Norm(J) ^ 691 = (intNorm(a))`

in the maximal real ring.  The unconditional result `691 ∤ h⁺` then
...
theorem exists_realGenerator_of_relativeNorm691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (J : Ideal (𝓞 K)) (a : 𝓞 K)
    (hpow : J ^ 691 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
...
If `J ^ 691 = (a)`, relative ideal norm and `691 ∤ h⁺` produce a real
...
theorem exists_equationSevenD_of_idealPower691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (J : Ideal (𝓞 K)) (a : 𝓞 K)
    (hpow : J ^ 691 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
...
exponent `691`.  Principality of `I * J^690` and `I * J` gives principality
of `J^689`; the coprime powers `689` and `691` then principalize `J`, and
division principalizes `I`. -/
theorem fractionalIdeal_isPrincipal_of_vandiverSeven691
    {A L : Type*} [CommRing A] [IsDedekindDomain A]
    [Field L] [Algebra A L] [IsFractionRing A L]
    {I J : FractionalIdeal A⁰ L} (hI0 : I ≠ 0) (hJ0 : J ≠ 0)
    (hJ691 : Submodule.IsPrincipal
      ((J ^ 691 : FractionalIdeal A⁰ L) : Submodule A L))
    (hsevenA : Submodule.IsPrincipal
      ((I * J ^ 690 : FractionalIdeal A⁰ L) : Submodule A L))
...
`J = 𝔦₋ₐ`, then (7a) says `I * J^690` is principal, while (7d)
says `I * J` is principal.  Their quotient makes `J^689` principal.  Since
`J^691` is principal as well, the checked Bézout lemma
`fractionalIdeal_isPrincipal_of_vandiverSeven691` principalizes `I`.
Finally `I^691 = (a)` gives the element equation
`a = η * ρ^691`, which is exactly (8). -/
theorem exists_equationEight_of_sevenASevenD691
    (I J : Ideal (𝓞 K)) (a b r s : 𝓞 K)
    (hI0 : I ≠ 0) (hJ0 : J ≠ 0)
    (hIpow : I ^ 691 = Ideal.span {a})
    (hJpow : J ^ 691 = Ideal.span {b})
    (hsevenA : I * J ^ 690 = Ideal.span {r})
...
The primary element in Lemma 1 is exactly `a * b^690`: its principal ideal
is the 691st power of `I * J^690`.  `exists_equationSevenA_generator` turns
...
Lemma-1 theorem. -/
theorem exists_equationEight_of_lemmaOneSevenD691
    (hlemma : Fermat.Irregular.VandiverLemmaOne.LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    ...
    (hJpow : J ^ 691 = Ideal.span {b})
    (hprimary :
      Fermat.Irregular.VandiverLemmaOne.IsKummerPrimary hζ (a * b ^ 690))
    (hsevenD : I * J = Ideal.span {s}) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      I = Ideal.span {ρ} ∧ a = η * ρ ^ 691 := by
...
is therefore naturally `u * (a * b^690)`.  Since `u` does not change the
principal ideal, Lemma 1 still principalizes `I * J^690`; the rest is the
same checked Bézout calculation as in
`exists_equationEight_of_lemmaOneSevenD691`. -/
theorem exists_equationEight_of_lemmaOneSevenDUnit691
    (hlemma : Fermat.Irregular.VandiverLemmaOne.LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
...
inverse factor `q₋`.  Comparing 691st powers of integral ideals and using
injectivity of the power map then proves that the product of the two
allocated factor ideals is `(ρ)`. -/
theorem exists_historicalEquationSevenD691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal691 hζ s *
          historicalInverseZetaFactorIdeal691 hζ s =
        Ideal.span {r} := by
  let I : Ideal (𝓞 K) := historicalZetaFactorIdeal691 hζ s
  let J : Ideal (𝓞 K) := historicalInverseZetaFactorIdeal691 hζ s
  let a : 𝓞 K := historicalZetaFactor691 hζ s
  let b : 𝓞 K := normalizedHistoricalInverseZetaFactor691 hζ s
  have hIpow : I ^ 691 = Ideal.span {a} := by
    simpa only [I, a] using historicalZetaFactorIdeal_pow691 hζ s
  have hJpow : J ^ 691 = Ideal.span {b} := by
    simpa only [J, b] using historicalInverseZetaFactorIdeal_pow691 hζ s
  have hbconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K a = b := by
    simpa only [a, b] using
      ringOfIntegersComplexConj_historicalZetaFactor691 hζ s hs
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower691 hζ I a hIpow
  let ρK : 𝓞 K := algebraMap (𝓞 K⁺) (𝓞 K) ρ
  let εK : (𝓞 K)ˣ :=
    Units.map (algebraMap (𝓞 K⁺) (𝓞 K)).toMonoidHom ε
  have hnorm' : a * b = (εK : 𝓞 K) * ρK ^ 691 := by
    rw [← hbconj]
    change a * NumberField.IsCMField.ringOfIntegersComplexConj K a =
      algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
        (algebraMap (𝓞 K⁺) (𝓞 K) ρ) ^ 691
    exact hnorm
  have hpow : (I * J) ^ 691 = (Ideal.span {ρK}) ^ 691 := by
    calc
      (I * J) ^ 691 = I ^ 691 * J ^ 691 := by rw [mul_pow]
      _ = Ideal.span {a} * Ideal.span {b} := by rw [hIpow, hJpow]
      _ = Ideal.span {a * b} :=
        Ideal.span_singleton_mul_span_singleton a b
      _ = Ideal.span {(εK : 𝓞 K) * ρK ^ 691} := by rw [hnorm']
      _ = Ideal.span {ρK ^ 691} :=
        Ideal.span_singleton_eq_span_singleton.mpr
          (associated_unit_mul_left _ _ εK.isUnit)
      _ = (Ideal.span {ρK}) ^ 691 :=
        (Ideal.span_singleton_pow ρK 691).symm
  exact ⟨ρK,
    pow_left_injective (M := Ideal (𝓞 K))
      (by norm_num : 691 ≠ 0) hpow⟩
...
theorem historicalFactorIdeals_ne_zero691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalZetaFactorIdeal691 hζ s ≠ 0 ∧
      historicalInverseZetaFactorIdeal691 hζ s ≠ 0 := by
  let a : 𝓞 K := historicalZetaFactor691 hζ s
  let b : 𝓞 K := normalizedHistoricalInverseZetaFactor691 hζ s
  have hprimary :=
    historicalConjugateFactorProduct_isKummerPrimary691 hζ s hs
  have hab0 : a * b ^ 690 ≠ 0 := by
    intro hzero
    apply hprimary.1
    change (hζ.unit' : 𝓞 K) - 1 ∣ a * b ^ 690
    rw [hzero]
    exact dvd_zero _
  have ha0 : a ≠ 0 := left_ne_zero_of_mul hab0
  have hbpow0 : b ^ 690 ≠ 0 := right_ne_zero_of_mul hab0
  have hb0 : b ≠ 0 := by
    intro hzero
    apply hbpow0
    rw [hzero]
  ...
  · intro hzero
    apply hspanA
    rw [← historicalZetaFactorIdeal_pow691 hζ s, hzero]
    norm_num
  · intro hzero
    apply hspanB
    rw [← historicalInverseZetaFactorIdeal_pow691 hζ s, hzero]
    norm_num

set_option maxRecDepth 50000 in
/-- Vandiver's equation (8), constructed from an arbitrary real historical
state at exponent `691`.
...
theorem exists_historicalEquationEight691
    (hlemma : LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal691 hζ s = Ideal.span {ρ} ∧
      historicalZetaFactor691 hζ s = η * ρ ^ 691 := by
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA691 hlemma hζ s hs
  obtain ⟨t, hsevenD⟩ := exists_historicalEquationSevenD691 hζ s hs
  obtain ⟨hI0, hJ0⟩ := historicalFactorIdeals_ne_zero691 hζ s hs
...
The factor at `ζ` agrees with its conjugate modulo `(ζ - 1)^691`, and so
does every 691st power.  After cancelling the generator, which is prime to
`ζ - 1`, the unit's conjugation defect is divisible by `(ζ - 1)^2`.
`unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj691` then removes its possible
cyclotomic torsion component.  This is the compatibility needed to obtain
equation (8) at `a` and `-a` with one common unit. -/
theorem historicalEquationEight_unit_real691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) (ρ : 𝓞 K) (η : (𝓞 K)ˣ)
    (heq : historicalZetaFactor691 hζ s = η * ρ ^ 691) :
    NumberField.IsCMField.unitsComplexConj K η = η := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  have hqnot : ¬ π ∣ historicalZetaFactor691 hζ s := by
    simpa only [π] using historicalZetaFactor_not_ramified691 hζ s hs
  have hρnot : ¬ π ∣ ρ := by
    intro hρ
    apply hqnot
    rw [heq]
    exact dvd_mul_of_dvd_right (dvd_pow (n := 691) hρ (by norm_num)) _
  have hρpowNot : ¬ π ∣ ρ ^ 691 := by
    intro h
    exact hρnot ((hζ.zeta_sub_one_prime'.dvd_pow_iff_dvd
      (by norm_num : 691 ≠ 0)).mp h)
  have hq691 := historicalZetaFactor_sub_conj_highDivisibility691 hζ s hs
  have hq2 : π ^ 2 ∣
      historicalZetaFactor691 hζ s -
        NumberField.IsCMField.ringOfIntegersComplexConj K
          (historicalZetaFactor691 hζ s) := by
    exact (pow_dvd_pow_of_dvd_of_le (dvd_refl π) (by norm_num : 2 ≤ 691)).trans
      (by simpa only [π] using hq691)
  have hρ691 :=
    zeta_sub_one_pow_sixHundredNinetyOne_dvd_pow_sub_conj_pow691 hζ ρ
  have hρ2 : π ^ 2 ∣
      ρ ^ 691 -
        NumberField.IsCMField.ringOfIntegersComplexConj K ρ ^ 691 := by
    exact (pow_dvd_pow_of_dvd_of_le (dvd_refl π) (by norm_num : 2 ≤ 691)).trans
      (by simpa only [π] using hρ691)
  have herror : π ^ 2 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) *
        (ρ ^ 691 -
          NumberField.IsCMField.ringOfIntegersComplexConj K ρ ^ 691) :=
  ...
  have hunit : π ^ 2 ∣
    ...
    obtain ⟨d₁, hd₁⟩ := hd
    obtain ⟨c, hc⟩ := hmul
    have hcancel : d₁ * ρ ^ 691 = π * c := by
      apply mul_left_cancel₀ hπ0
      calc
        π * (d₁ * ρ ^ 691) = d * ρ ^ 691 := by rw [hd₁]; ring
        _ = π ^ 2 * c := hc
        _ = π * (π * c) := by ring
    have hd₁div : π ∣ d₁ := by
      have hπd₁ : π ∣ d₁ * ρ ^ 691 := ⟨c, hcancel⟩
    ...
    rw [hd₁, hd₂]
    ring
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj691 hζ η
...
theorem exists_historicalEquationEight_realUnit691
    (hlemma : LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal691 hζ s = Ideal.span {ρ} ∧
      historicalZetaFactor691 hζ s = η * ρ ^ 691 ∧
      NumberField.IsCMField.unitsComplexConj K η = η := by
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight691 hlemma hζ s hs
  exact ⟨ρ, η, hI, heq,
    historicalEquationEight_unit_real691 hζ s hs ρ η heq⟩
...
exactly the same `-η`. -/
theorem exists_historicalEquationEight_pair_one691
    (hlemma : LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  have hqmul : historicalZetaFactor691 hζ s *
      ((hζ.unit' : 𝓞 K) - 1) =
  ...
  have hminus :
      s.omega + (hζ.unit'⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit'⁻¹ : (𝓞 K)ˣ)) * η * ρminus ^ 691 := by
    have hc := congrArg
  ...
  have hρnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρ := by
    intro hρ
    apply historicalZetaFactor_not_ramified691 hζ s hs
    rw [heq]
    exact dvd_mul_of_dvd_right (dvd_pow (n := 691) hρ (by norm_num)) _
  have hρminusNot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρminus := by
    intro hρminus
    have hc := zeta_sub_one_pow_dvd_conj_of_dvd691 hζ 1 ρminus
...
`ρₐ^691 - ρ₋ₐ^691 = ε * (((ζ-1)^(2*m-1) * ρ₀)^691)`.
...
`2 * (691*m - 345) - 1 = (2*m - 1) * 691`.

No ideal-class hypothesis occurs here. -/
theorem exists_equationEight_difference691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (a m : ℕ) (ha : a.Coprime 691) (hm : 1 < m)
    (omega theta rhoa rhominus rhozero : 𝓞 K)
    (etaa etazero : (𝓞 K)ˣ)
    (hea : omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * theta =
      (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) * etaa * rhoa ^ 691)
    (heminus : omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * theta =
      (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) * etaa * rhominus ^ 691)
    (hezero : omega + theta =
      etazero * kappa hζ ^ (691 * m - 345) * rhozero ^ 691) :
    ∃ ε : (𝓞 K)ˣ,
      rhoa ^ 691 - rhominus ^ 691 =
        ε * (((hζ.unit' : 𝓞 K) - 1) ^ (2 * m - 1) * rhozero) ^ 691 := by
  let t : (𝓞 K)ˣ := hζ.unit' ^ a
  have htprim : IsPrimitiveRoot (t : 𝓞 K) 691 :=
    hζ.unit'_coe.pow_of_coprime a ha
  let hplus : IsUnit ((1 : 𝓞 K) + t) := by
    simpa [add_comm] using
      htprim.geom_sum_isUnit (by norm_num) (by norm_num : Nat.Coprime 2 691)
  ...
  have helim := equationEight_pair_difference691 t etaa omega theta rhoa rhominus
  ...
  let rightUnit : (𝓞 K)ˣ := uplus * etazero * kappaUnit691 hζ ^ E
  have hpi : (hζ.unit' : 𝓞 K) - 1 ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hcancel : (leftUnit : 𝓞 K) * (rhoa ^ 691 - rhominus ^ 691) =
      (rightUnit : 𝓞 K) *
        ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 691 := by
    apply mul_left_cancel₀ hpi
    calc
      ((hζ.unit' : 𝓞 K) - 1) *
          ((leftUnit : 𝓞 K) * (rhoa ^ 691 - rhominus ^ 691)) =
          (1 - (t : 𝓞 K)) * etaa *
            (rhoa ^ 691 - rhominus ^ 691) := by
        dsimp [leftUnit]
        rw [huden]
        ring
      ...
      _ = ((hζ.unit' : 𝓞 K) - 1) *
        ...
        have hezero' : omega + theta =
            etazero * kappa hζ ^ E * rhozero ^ 691 := by
  ...
  refine ⟨ε, ?_⟩
  have hdiff : rhoa ^ 691 - rhominus ^ 691 =
      (ε : 𝓞 K) * ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 691 := by
    calc
      rhoa ^ 691 - rhominus ^ 691 =
          (leftUnit⁻¹ : (𝓞 K)ˣ) *
            ((leftUnit : 𝓞 K) * (rhoa ^ 691 - rhominus ^ 691)) := by
        rw [← mul_assoc, ← Units.val_mul]
        simp
      _ = (leftUnit⁻¹ : (𝓞 K)ˣ) *
          ((rightUnit : 𝓞 K) *
            ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 691) := by rw [hcancel]
      _ = (ε : 𝓞 K) * ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 691 := by
        dsimp [ε]
        ring
  rw [hdiff]
  dsimp [N]
...
def conjugateIdeal691 (I : Ideal (𝓞 K)) : Ideal (𝓞 K) :=
...
cyclotomic ring. -/
def extendRealIdeal691 (I : Ideal (𝓞 K⁺)) : Ideal (𝓞 K) :=
  I.map (algebraMap (𝓞 K⁺) (𝓞 K))

/-- An extended real ideal is fixed by CM complex conjugation. -/
@[simp] theorem conjugateIdeal691_extendRealIdeal691
    (I : Ideal (𝓞 K⁺)) :
    conjugateIdeal691 (extendRealIdeal691 I) = extendRealIdeal691 I := by
  rw [conjugateIdeal691, extendRealIdeal691, Ideal.map_map]
  have hmap :
...
theorem extendRealIdeal691_span (ρ : 𝓞 K⁺) :
    extendRealIdeal691 (Ideal.span {ρ}) =
      Ideal.span {algebraMap (𝓞 K⁺) (𝓞 K) ρ} := by
  simp only [extendRealIdeal691, Ideal.map_span, Set.image_singleton]
...
`𝓞 K⁺ → 𝓞 K`. -/
theorem extendRealIdeal691_injective :
    Function.Injective (extendRealIdeal691 (K := K)) := by
  intro I J hIJ
  have hcomap := congrArg
    (Ideal.comap (algebraMap (𝓞 K⁺) (𝓞 K))) hIJ
  simpa only [extendRealIdeal691,
    Ideal.comap_map_eq_self_of_faithfullyFlat] using hcomap

omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
/-- Extension commutes with ideal powers. -/
theorem extendRealIdeal691_pow (I : Ideal (𝓞 K⁺)) (n : ℕ) :
    extendRealIdeal691 (I ^ n) = extendRealIdeal691 I ^ n := by
  exact Ideal.map_pow (algebraMap (𝓞 K⁺) (𝓞 K)) I n

set_option maxRecDepth 50000 in
/-- The complete plus-class-number bridge needed at each of Vandiver's
real ideals in (7b) and (9).  A real ideal with 691st power `(a)` extends to
...
the extension of `I` and verify the 691st-power equality `hpow`; all class
number and generator extraction work is discharged here. -/
theorem exists_extendedRealGenerator691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
    (hpow : I ^ 691 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      extendRealIdeal691 I =
          Ideal.span {algebraMap (𝓞 K⁺) (𝓞 K) ρ} ∧
      conjugateIdeal691 (extendRealIdeal691 I) = extendRealIdeal691 I ∧
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (algebraMap (𝓞 K⁺) (𝓞 K) ρ) =
        algebraMap (𝓞 K⁺) (𝓞 K) ρ ∧
      algebraMap (𝓞 K⁺) (𝓞 K) a =
        algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
          algebraMap (𝓞 K⁺) (𝓞 K) ρ ^ 691 := by
  obtain ⟨ρ, ε, hI, ha⟩ := exists_real_unit_mul_pow_generator691 hzeta I a hpow
  refine ⟨ρ, ε, ?_, conjugateIdeal691_extendRealIdeal691 I,
    (NumberField.IsCMField.ringOfIntegersComplexConj K).commutes ρ, ?_⟩
  · rw [hI, extendRealIdeal691_span]
  · rw [ha, map_mul, map_pow]

set_option maxRecDepth 50000 in
/-- Version of `exists_extendedRealGenerator691` whose 691st-power identity
...
to the real ideal, where `691 ∤ h⁺` supplies the generator. -/
theorem exists_extendedRealGenerator691_of_pow_eq_span
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
    (hpow : extendRealIdeal691 I ^ 691 =
    ...
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      ...
      algebraMap (𝓞 K⁺) (𝓞 K) a =
        algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
          algebraMap (𝓞 K⁺) (𝓞 K) ρ ^ 691 := by
  apply exists_extendedRealGenerator691 hzeta I a
  apply extendRealIdeal691_injective (K := K)
  rw [extendRealIdeal691_pow, extendRealIdeal691_span]
  exact hpow

/-- Complex conjugation preserves whether an integral ideal is zero. -/
@[simp] lemma conjugateIdeal691_eq_zero_iff (I : Ideal (𝓞 K)) :
    conjugateIdeal691 I = 0 ↔ I = 0 := by
...
denominator. -/
def conjugateIdealQuotient691 (A B : Ideal (𝓞 K)) :
    FractionalIdeal (𝓞 K)⁰ K :=
  (conjugateIdeal691 A : FractionalIdeal (𝓞 K)⁰ K) /
    (conjugateIdeal691 B : FractionalIdeal (𝓞 K)⁰ K)

/-- The actual fractional ideal whose principality is requested by
`RelevantIdealQuotientsPrincipal` at exponent `691`. -/
noncomputable def relevantIdealQuotient691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 691 + y ^ 691 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
    (hy : ¬ hζ.unit'.1 - 1 ∣ y)
    (η : Polynomial.nthRootsFinset 691 (1 : 𝓞 K)) :
    FractionalIdeal (𝓞 K)⁰ K :=
  (root_div_zeta_sub_one_dvd_gcd (K := K) (p := 691)
      (x := x) (y := y) (z := z) (ε := ε₀) (m := m)
        (by norm_num) hζ e hy η : FractionalIdeal (𝓞 K)⁰ K) /
    (a_eta_zero_dvd_p_pow (K := K) (p := 691)
      (x := x) (y := y) (z := z) (ε := ε₀) (m := m)
        (by norm_num) hζ e hy : FractionalIdeal (𝓞 K)⁰ K)

/-- The conjugate of the relevant exponent-`691` quotient. -/
noncomputable def conjugateRelevantIdealQuotient691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 691 + y ^ 691 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
...
def RelevantIdealQuotientPlusComponentsPrincipal691 : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 691 + y ^ 691 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
    ...
    Submodule.IsPrincipal
      (((relevantIdealQuotient691 hζ e hy η *
          conjugateRelevantIdealQuotient691 hζ e hy η) :
        FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K)

/-- Every antisymmetric relevant quotient class is principal.  At `691`,
this is exactly the residual minus-component assertion that cannot follow
from `691 ∤ h⁺` alone. -/
def RelevantIdealQuotientMinusComponentsPrincipal691 : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 691 + y ^ 691 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
...
variable {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
  {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
  (e : x ^ 691 + y ^ 691 =
    ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 691)
  (hy : ¬ hζ.unit'.1 - 1 ∣ y)
...
lemma relevantIdealQuotient691_conjugate_eq_zero_iff
    (η : Polynomial.nthRootsFinset 691 (1 : 𝓞 K)) :
    ((𝔞 η / 𝔞₀ : FractionalIdeal (𝓞 K)⁰ K) = 0) ↔
      conjugateIdealQuotient691 (𝔞 η) 𝔞₀ = 0 := by
  simp only [conjugateIdealQuotient691, div_eq_mul_inv,
    mul_eq_zero, inv_eq_zero, FractionalIdeal.coeIdeal_eq_zero]
  constructor <;> rintro (h | h)
  · exact Or.inl ((conjugateIdeal691_eq_zero_iff _).mpr h)
  · exact Or.inr ((conjugateIdeal691_eq_zero_iff _).mpr h)
  · exact Or.inl ((conjugateIdeal691_eq_zero_iff _).mp h)
  · exact Or.inr ((conjugateIdeal691_eq_zero_iff _).mp h)
...
The proof uses the unconditional fact that the quotient's `691`th power is
principal and the coprimality of `691` and `2`. -/
lemma relevantIdealQuotient691_isPrincipal_iff_minus
    (η : Polynomial.nthRootsFinset 691 (1 : 𝓞 K))
    (hplus : Submodule.IsPrincipal
      ((((𝔞 η / 𝔞₀ : FractionalIdeal (𝓞 K)⁰ K) *
          conjugateIdealQuotient691 (𝔞 η) 𝔞₀) :
        FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K)) :
    Submodule.IsPrincipal
  ...
  let J : FractionalIdeal (𝓞 K)⁰ K :=
    conjugateIdealQuotient691 (𝔞 η) 𝔞₀
  have hzero : I = 0 ↔ J = 0 :=
    relevantIdealQuotient691_conjugate_eq_zero_iff hζ e hy η
  ...
  · have hJ0 : J ≠ 0 := fun h ↦ hI0 (hzero.mpr h)
    apply fractionalIdeal_isPrincipal_iff_minus_of_plus (p := 691)
      (by norm_num) hI0 hJ0
    · exact relevantIdealQuotient_pow_isPrincipal
        (K := K) (p := 691) (by norm_num) hζ e hy η
    · exact hplus
...
theorem relevantIdealQuotientsPrincipal_iff_minus_of_plus691
    (hplus : RelevantIdealQuotientPlusComponentsPrincipal691 (K := K)) :
    RelevantIdealQuotientsPrincipal (K := K) (p := 691) (by norm_num) ↔
      RelevantIdealQuotientMinusComponentsPrincipal691 (K := K) := by
  constructor
  · intro hprincipal ζ hζ x y z ε₀ m e hy η
    have hI := hprincipal hζ e hy η
    have hiff := relevantIdealQuotient691_isPrincipal_iff_minus hζ e hy η
      (hplus hζ e hy η)
    simpa only [relevantIdealQuotient691,
      conjugateRelevantIdealQuotient691] using hiff.mp hI
  · intro hminus ζ hζ x y z ε₀ m e hy η
    have hiff := relevantIdealQuotient691_isPrincipal_iff_minus hζ e hy η
      (hplus hζ e hy η)
    apply hiff.mpr
    simpa only [relevantIdealQuotient691,
      conjugateRelevantIdealQuotient691] using hminus hζ e hy η
...
lemma ringOfIntegersComplexConj_eq_mod_zeta_sub_one691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K) :
    Ideal.Quotient.mk
        (Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)))
...
lemma conjugateIdeal691_span (a : 𝓞 K) :
    conjugateIdeal691 (Ideal.span {a}) =
      Ideal.span {ringOfIntegersComplexConj K a} := by
  simp only [conjugateIdeal691, Ideal.map_span, Set.image_singleton]
  rfl
...
because `2 ∉ (ζ - 1)`. -/
lemma conjugation_eq_zeta_pow_of_stable_principal691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K)
    (ha : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a)
    (hstable : conjugateIdeal691 (Ideal.span {a}) = Ideal.span {a}) :
    ∃ j : ℕ, ringOfIntegersComplexConj K a =
      (hζ.unit' ^ j : (𝓞 K)ˣ) * a := by
  have hassoc : Associated (ringOfIntegersComplexConj K a) a := by
    rw [← Ideal.span_singleton_eq_span_singleton]
    rw [← conjugateIdeal691_span]
    exact hstable
  obtain ⟨u, hu⟩ := hassoc
  let v : (𝓞 K)ˣ := u⁻¹
  have ha0 : a ≠ 0 := by
    ...
    have hvQ : algebraMap (𝓞 K) Q (v : 𝓞 K) = 1 := by
      ...
      calc
        ...
        _ = algebraMap (𝓞 K) Q a :=
          ringOfIntegersComplexConj_eq_mod_zeta_sub_one691 hζ a
...
quotient of a stable principal ideal a pure `691`th root of unity.
...
lemma conjugation_sq_eq_zeta_pow_of_stable_principal691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K)
    (ha0 : a ≠ 0)
    (hstable : conjugateIdeal691 (Ideal.span {a}) = Ideal.span {a}) :
...
conjugation exponent required by `ConjugationPowerReductionData691`. -/
lemma exists_conjugation_power_generator691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (I : Ideal (𝓞 K))
    (hprincipal : Submodule.IsPrincipal
      (I : Submodule (𝓞 K) (𝓞 K)))
    (hprime : ¬ Ideal.span
      ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ∣ I)
    (hstable : conjugateIdeal691 I = I) :
    ∃ (a : 𝓞 K) (j : ℕ),
      I = Ideal.span {a} ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a ∧
      ringOfIntegersComplexConj K a =
  ...
  have ha : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a := by
    ...
    exact ha
  have hstableA : conjugateIdeal691 (Ideal.span {a}) =
      Ideal.span {a} := by
    rw [← haI]
    exact hstable
  obtain ⟨j, hj⟩ := conjugation_eq_zeta_pow_of_stable_principal691
    hζ a ha hstableA
  exact ⟨a, j, haI, ha, hj⟩

end StablePrincipalGenerator

/-- The inverse of 2 modulo 691, used to make a generator real. -/
def realGeneratorHalfExponent691 : ℕ := 346

theorem two_mul_realGeneratorHalfExponent691_mod :
    2 * realGeneratorHalfExponent691 % 691 = 1 := by
  decide

/-- The explicit adjustment of a principal generator by the half-power of
its cyclotomic conjugation quotient. -/
def realAdjustedGenerator691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (a : 𝓞 K) (j : ℕ) : 𝓞 K :=
  (hζ.unit' ^ (realGeneratorHalfExponent691 * j) : (𝓞 K)ˣ) * a

omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
lemma realAdjustedGenerator691_associated {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K) (j : ℕ) :
    Associated (realAdjustedGenerator691 hζ a j) a := by
  let v : (𝓞 K)ˣ := hζ.unit' ^ (realGeneratorHalfExponent691 * j)
  refine ⟨v⁻¹, ?_⟩
...
omit [NumberField K] [IsCyclotomicExtension {691} ℚ K] in
lemma realAdjustedGenerator691_pow_sixHundredNinetyOne {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K) (j : ℕ) :
    realAdjustedGenerator691 hζ a j ^ 691 = a ^ 691 := by
  rw [realAdjustedGenerator691, mul_pow]
  have hzpow : hζ.unit' ^ 691 = 1 := by
    ext
    exact hζ.pow_eq_one
  have hvpow :
      (hζ.unit' ^ (realGeneratorHalfExponent691 * j)) ^ 691 = 1 := by
    rw [← pow_mul]
    rw [show (realGeneratorHalfExponent691 * j) * 691 =
      691 * (realGeneratorHalfExponent691 * j) by omega]
    rw [pow_mul, hzpow, one_pow]
  rw [← Units.val_pow_eq_pow_val, hvpow]
  simp

lemma realAdjustedGenerator691_real
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (a : 𝓞 K) (j : ℕ)
    (ha : NumberField.IsCMField.ringOfIntegersComplexConj K a =
      (hζ.unit' ^ j : (𝓞 K)ˣ) * a) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
      (realAdjustedGenerator691 hζ a j) =
        realAdjustedGenerator691 hζ a j := by
  let k := realGeneratorHalfExponent691 * j
  let v : (𝓞 K)ˣ := hζ.unit' ^ k
  have hzpow : hζ.unit' ^ 691 = 1 := by
    ext
    exact hζ.pow_eq_one
  have hv_sq : v ^ 2 = hζ.unit' ^ j := by
    dsimp [v, k, realGeneratorHalfExponent691]
    rw [← pow_mul]
    have hexp : (346 * j) * 2 = j + 691 * j := by omega
  ...
  have hvconj : NumberField.IsCMField.ringOfIntegersComplexConj K (v : 𝓞 K) =
    ...
    have hvconjU :
      ...
      dsimp [v]
      rw [map_pow, unitsComplexConj_zeta691 hζ, inv_pow]
...
behind Vandiver's equations (7b)--(10a): 346 is the inverse of 2 modulo
691, so

conj (ζ ^ (346*j) * a) = ζ ^ (j-346*j) * a = ζ ^ (346*j) * a.

The resulting generator is associated to the original one, hence generates
the same principal ideal. What remains in
RealPrincipalGeneratorElimination691 is to prove that the conjugation
...
lemma exists_real_associated_generator_of_conj_eq_zeta_pow
  ...
  exact ⟨realAdjustedGenerator691 hζ a j,
    realAdjustedGenerator691_associated hζ a j,
    realAdjustedGenerator691_real hζ a j ha⟩

set_option maxRecDepth 50000 in
/-- The real-generator conclusion used immediately after Vandiver's
equation (9).

Suppose `J ^ 691 = (q)`, the element `q` is fixed by conjugation, and `J` is
...
691st-power map on the unique-factorization monoid of integral ideals shows
that `J²` is principal.  Since `J^691` is principal and
`gcd(2,691)=1`, `J` itself is principal.  Its conjugation stability follows
from the reality of `q`; the explicit `ζ^(346*j)` normalization then
chooses a real generator `μ`.  Finally

`q = η * μ^691`,

which is the element equation written after (9) in the 1929 paper. -/
theorem exists_realEquationNineGenerator691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (J : Ideal (𝓞 K)) (q : 𝓞 K)
    (hprime : ¬ Ideal.span
    ...
    ∃ (μ : 𝓞 K) (η : (𝓞 K)ˣ),
      ...
      q = η * μ ^ 691 := by
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower691 hzeta J q hpow
  let ρK : 𝓞 K := algebraMap (𝓞 K⁺) (𝓞 K) ρ
  let εK : (𝓞 K)ˣ := Units.map
    (algebraMap (𝓞 K⁺) (𝓞 K)).toMonoidHom ε
  have hq_sq : q ^ 2 = (εK : 𝓞 K) * ρK ^ 691 := by
    change q ^ 2 =
      algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
        (algebraMap (𝓞 K⁺) (𝓞 K) ρ) ^ 691
    simpa only [pow_two, hqreal] using hnorm
  have hassoc : Associated (q ^ 2) (ρK ^ 691) := by
    refine ⟨εK⁻¹, ?_⟩
    rw [hq_sq]
    calc
      ((εK : 𝓞 K) * ρK ^ 691) * (εK⁻¹ : (𝓞 K)ˣ) =
          ρK ^ 691 * ((εK : 𝓞 K) * (εK⁻¹ : (𝓞 K)ˣ)) := by
        ac_rfl
      _ = ρK ^ 691 := by rw [← Units.val_mul]; simp
  have hspan : Ideal.span {q ^ 2} = Ideal.span {ρK ^ 691} :=
    Ideal.span_singleton_eq_span_singleton.mpr hassoc
  have hpoweq : (J ^ 2) ^ 691 = (Ideal.span {ρK}) ^ 691 := by
    calc
      (J ^ 2) ^ 691 = (J ^ 691) ^ 2 := by
        rw [← pow_mul, ← pow_mul]
      _ = (Ideal.span {q}) ^ 2 := by rw [hpow]
      _ = Ideal.span {q ^ 2} := Ideal.span_singleton_pow q 2
      _ = Ideal.span {ρK ^ 691} := hspan
      _ = (Ideal.span {ρK}) ^ 691 := (Ideal.span_singleton_pow ρK 691).symm
  have hJ2eq : J ^ 2 = Ideal.span {ρK} :=
    pow_left_injective (M := Ideal (𝓞 K)) (by norm_num : 691 ≠ 0) hpoweq
  have hJ2 : Submodule.IsPrincipal (J ^ 2 : Ideal (𝓞 K)) := by
    rw [hJ2eq]
    infer_instance
  have hJ691 : Submodule.IsPrincipal (J ^ 691 : Ideal (𝓞 K)) := by
    rw [hpow]
    infer_instance
  have hJprincipal : Submodule.IsPrincipal (J : Ideal (𝓞 K)) :=
    ideal_isPrincipal_of_coprime_powers (L := K) (by norm_num) J hJ2 hJ691
  have hstable : conjugateIdeal691 J = J := by
    apply pow_left_injective (M := Ideal (𝓞 K)) (by norm_num : 691 ≠ 0)
    calc
      conjugateIdeal691 J ^ 691 = conjugateIdeal691 (J ^ 691) := by
        exact (Ideal.map_pow
          (NumberField.IsCMField.ringOfIntegersComplexConj K).toRingHom J 691).symm
      _ = conjugateIdeal691 (Ideal.span {q}) := by rw [hpow]
      _ = Ideal.span
          {NumberField.IsCMField.ringOfIntegersComplexConj K q} :=
        conjugateIdeal691_span q
      _ = Ideal.span {q} := by rw [hqreal]
      _ = J ^ 691 := hpow.symm
  obtain ⟨a, j, hJa, -, hconj⟩ := exists_conjugation_power_generator691
  ...
  have hassoc_q : Associated (μ ^ 691) q := by
    rw [← Ideal.span_singleton_eq_span_singleton,
...
If `J ^ 691 = (q)` and `q` is real, relative norm makes `J ^ 2`
principal.  The coprime exponents `2` and `691` then principalize `J`, and
...
theorem ideal_isPrincipal_and_stable_of_real_pow691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
    (J : Ideal (𝓞 K)) (q : 𝓞 K)
    (hqreal : NumberField.IsCMField.ringOfIntegersComplexConj K q = q)
    (hpow : J ^ 691 = Ideal.span {q}) :
    Submodule.IsPrincipal (J : Ideal (𝓞 K)) ∧
      conjugateIdeal691 J = J := by
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower691 hzeta J q hpow
  let ρK : 𝓞 K := algebraMap (𝓞 K⁺) (𝓞 K) ρ
  let εK : (𝓞 K)ˣ := Units.map
...
theorem exists_squaredConjugationGenerator_of_real_pow691
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 691)
  ...
  obtain ⟨hprincipal, hstable⟩ :=
    ideal_isPrincipal_and_stable_of_real_pow691 hzeta J q hqreal hpow
  ...
  have hρ0 : ρ ≠ 0 := by
    ...
    calc
      Ideal.span {q} = J ^ 691 := hpow.symm
      _ = (Ideal.span {ρ}) ^ 691 := by rw [hJρ]
      _ = 0 := by
        rw [hρ]
        norm_num
        rw [Ideal.zero_eq_bot]
  have hstableρ : conjugateIdeal691 (Ideal.span {ρ}) =
      Ideal.span {ρ} := by
    rw [← hJρ]
    exact hstable
  obtain ⟨j, hj⟩ :=
    conjugation_sq_eq_zeta_pow_of_stable_principal691 hzeta ρ hρ0 hstableρ
  have hassoc_q : Associated (ρ ^ 691) q := by
    rw [← Ideal.span_singleton_eq_span_singleton,
...
elimination uses. -/
theorem exists_historicalEquationEightA691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρzero : 𝓞 K) (ηzero : (𝓞 K)ˣ) (jzero : ℕ),
      s.omega + s.theta =
        ηzero * kappa hζ ^ (691 * s.m - 345) * ρzero ^ 691 ∧
      historicalEquationEightAIdeal691 hζ s = Ideal.span {ρzero} ∧
  ...
  obtain ⟨ρ, η, j, hJ, hq, hρconj⟩ :=
    exists_squaredConjugationGenerator_of_real_pow691 hζ J q
      (by simpa only [q] using
        historicalEquationEightAQuotient_ne_zero691 hζ s hs)
      (by simpa only [q] using
        historicalEquationEightAQuotient_real691 hζ s hs)
      (by simpa only [J, q] using
        historicalEquationEightAIdeal_pow691 hζ s hs)
  have hq0 : q ≠ 0 := by
    simpa only [q] using historicalEquationEightAQuotient_ne_zero691 hζ s hs
  have hρ0 : ρ ≠ 0 := by
    intro hρ
    apply hq0
    rw [hq, hρ]
    norm_num
  have hzeta691 : hζ.unit' ^ 691 = 1 := by
    apply Units.ext
    apply NumberField.RingOfIntegers.ext
    change ζ ^ 691 = 1
    exact hζ.pow_eq_one
  have hroot691 : (hζ.unit' ^ j) ^ 691 = 1 := by
    rw [← pow_mul]
    rw [show j * 691 = 691 * j by omega, pow_mul, hzeta691, one_pow]
  have hρ1382real :
      NumberField.IsCMField.ringOfIntegersComplexConj K (ρ ^ 1382) =
        ρ ^ 1382 := by
    have h1382 : ρ ^ 1382 = (ρ ^ 2) ^ 691 := by
      rw [← pow_mul]
    rw [h1382, map_pow, hρconj, mul_pow,
      ← Units.val_pow_eq_pow_val, hroot691, Units.val_one, one_mul, ← h1382]
  have hq2 : q ^ 2 = ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 1382 := by
    rw [hq, mul_pow, ← Units.val_pow_eq_pow_val, ← pow_mul]
  have hηsqVal :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) =
        ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) := by
    apply mul_right_cancel₀ (pow_ne_zero 1382 hρ0)
    calc
      NumberField.IsCMField.ringOfIntegersComplexConj K
            ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 1382 =
          NumberField.IsCMField.ringOfIntegersComplexConj K
            (((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 1382) := by
              rw [map_mul, hρ1382real]
      _ = NumberField.IsCMField.ringOfIntegersComplexConj K (q ^ 2) := by
        rw [hq2]
      _ = q ^ 2 := by rw [map_pow,
        historicalEquationEightAQuotient_real691 hζ s hs]
      _ = ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 1382 := hq2
  ...
  · calc
      s.omega + s.theta =
          kappa hζ ^ (691 * s.m - 345) * q :=
        historicalEquationEightAQuotient_spec691 hζ s hs
      _ = kappa hζ ^ (691 * s.m - 345) *
          ((η : 𝓞 K) * ρ ^ 691) := by rw [hq]
      _ = (η : 𝓞 K) * kappa hζ ^ (691 * s.m - 345) * ρ ^ 691 := by
        ring
  · simpa only [J] using hJ

/-- Complex conjugation sends any integral 691st root of unity to its
690th power, i.e. its inverse. -/
lemma ringOfIntegersComplexConj_nthRoot691
    (η : Polynomial.nthRootsFinset 691 (1 : 𝓞 K)) :
    NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
      (η : 𝓞 K) ^ 690 := by
  have hηpow : (η : 𝓞 K) ^ 691 = 1 :=
    (Polynomial.mem_nthRootsFinset (by norm_num : 0 < 691) (1 : 𝓞 K)).mp η.prop
  let u : (𝓞 K)ˣ :=
    ⟨(η : 𝓞 K), (η : 𝓞 K) ^ 690,
      by simpa only [← pow_succ'] using hηpow,
      by simpa only [mul_comm, ← pow_succ'] using hηpow⟩
  have hupow : u ^ 691 = 1 := by
    apply Units.ext
...
The generic factor-allocation theorem selects a `691`th root `η` for which

`(ζ - 1)^((2*m-2)*691+1) ∣ ρₐ - η*ρ₋ₐ`.

Multiplying the pair symmetrically by `η^345` and `η^346` turns this into a
literal difference.  Both 691st powers are unchanged; the exponents are the
two half-powers surrounding `346`, the inverse of `2` modulo `691`.  No
class-number hypothesis is used in this step. -/
theorem equationNineA_normalized691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (m : ℕ) (hm : 1 < m) (ρa ρminus ρzero : 𝓞 K) (ε : (𝓞 K)ˣ)
    (hdiff : ρa ^ 691 - ρminus ^ 691 =
      ε * ((hζ.unit'.1 - 1) ^ (2 * m - 1) * ρzero) ^ 691)
    (hminus : ¬ hζ.unit'.1 - 1 ∣ ρminus) :
    ∃ ρa' ρminus' : 𝓞 K,
      ρa' ^ 691 = ρa ^ 691 ∧
      ρminus' ^ 691 = ρminus ^ 691 ∧
      (hζ.unit'.1 - 1) ^ ((2 * m - 2) * 691 + 1) ∣
        ρa' - ρminus' := by
  have hexp : 2 * m - 1 = (2 * m - 2) + 1 := by omega
  have e' : ρa ^ 691 + (-ρminus) ^ 691 =
      ε * ((hζ.unit'.1 - 1) ^ ((2 * m - 2) + 1) * ρzero) ^ 691 := by
    rw [← hexp]
    simpa only [Odd.neg_pow (by norm_num : Odd 691), sub_eq_add_neg] using hdiff
  have hy' : ¬ hζ.unit'.1 - 1 ∣ -ρminus := by
    simpa using hminus
  let η := zeta_sub_one_dvd_root (by norm_num : 691 ≠ 2) hζ e' hy'
  have hηdiv : (hζ.unit'.1 - 1) ^ ((2 * m - 2) * 691 + 1) ∣
      ρa - (η : 𝓞 K) * ρminus := by
    dsimp only [η]
    simpa only [sub_eq_add_neg, neg_mul, mul_comm] using
      (distinguishedFactor_highDivisibility (by norm_num : 691 ≠ 2)
        hζ e' hy')
  have hηpow : (η : 𝓞 K) ^ 691 = 1 :=
    (Polynomial.mem_nthRootsFinset (by norm_num : 0 < 691) (1 : 𝓞 K)).mp η.prop
  let ρa' : 𝓞 K := (η : 𝓞 K) ^ 345 * ρa
  let ρminus' : 𝓞 K := (η : 𝓞 K) ^ 346 * ρminus
  refine ⟨ρa', ρminus', ?_, ?_, ?_⟩
  · dsimp [ρa']
    rw [mul_pow, ← pow_mul]
    rw [show 345 * 691 = 691 * 345 by norm_num, pow_mul, hηpow, one_pow,
      one_mul]
  · dsimp [ρminus']
    rw [mul_pow, ← pow_mul]
    rw [show 346 * 691 = 691 * 346 by norm_num, pow_mul, hηpow, one_pow,
      one_mul]
  · have hrewrite : ρa' - ρminus' =
        (η : 𝓞 K) ^ 345 * (ρa - (η : 𝓞 K) * ρminus) := by
      dsimp [ρa', ρminus']
      rw [pow_succ' (η : 𝓞 K) 345]
      ring
    rw [hrewrite]
    exact dvd_mul_of_dvd_right hηdiv ((η : 𝓞 K) ^ 345)

/-- Conjugation-compatible form of equation (9a).

When the two inputs are conjugate, the symmetric normalization by the
345th and 346th powers of the distinguished 691st root preserves that
relation: conjugation changes the root to its inverse and
`-345 ≡ 346 (mod 691)`.  The normalized inverse generator remains prime to
`ζ - 1`. -/
theorem equationNineA_normalized_conjugate691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (m : ℕ) (hm : 1 < m) (ρa ρminus ρzero : 𝓞 K) (ε : (𝓞 K)ˣ)
    (hdiff : ρa ^ 691 - ρminus ^ 691 =
      ε * ((hζ.unit'.1 - 1) ^ (2 * m - 1) * ρzero) ^ 691)
  ...
  have hηconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
        (η : 𝓞 K) ^ 690 :=
    ringOfIntegersComplexConj_nthRoot691 η
  ...
  · dsimp [ρa', ρminus']
    ...
    rw [show 690 * 345 = 691 * 344 + 346 by norm_num,
      pow_add, pow_mul, hηpow, one_pow, one_mul]
  · dsimp [ρminus']
    intro hram
    rcases hζ.zeta_sub_one_prime'.dvd_mul.mp hram with hroot | hρ
    · exact hζ.zeta_sub_one_prime'.not_unit
        (isUnit_of_dvd_unit hroot (hηunit.pow 346))
    · exact hminus hρ

/-- A `691`th root of a real unit can be adjusted by a power of `ζ`
without changing its `691`th power so that the root itself is real. This is
the real-root normalization used implicitly between Vandiver's equations
(10) and (10b). -/
lemma exists_real_unit_root_691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (a v : (𝓞 K)ˣ)
    (hv : a = v ^ 691) :
    ∃ w : (𝓞 K)ˣ, a = w ^ 691 ∧
  ...
  · dsimp [w]
    ...
    have hzpow : hζ.unit' ^ 691 = 1 := by
      ext
      exact hζ.pow_eq_one
    rw [← pow_mul, show j * 691 = 691 * j by omega, pow_mul, hzpow, one_pow,
      div_one]
  · dsimp [w]
    rw [map_div, map_pow, unitsComplexConj_zeta691 hζ]
...
/-- At exponent `691`, Vandiver's `κ^345` is associated to the rational
prime `691`. -/
lemma kappa_pow_threeHundredFortyFive_associated_691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    Associated (kappa hζ ^ 345) (691 : 𝓞 K) := by
  ...
  have hkappaPow := hkappa.pow_pow (n := 345)
...
/-- A primitive rational second-case solution at exponent `691` gives
Vandiver's equation (6) with `m = 345`. This discharges the first abstract
boundary of `VandiverHistoricalDescent` for the concrete real invariant. -/
theorem secondCaseStartsHistoricalDescent_691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    SecondCaseStartsHistoricalDescent hζ (RealSourceAdmissible hζ) := by
  intro x y z hgcd hz hz0 e
  have hx : x ≠ 0 := by
    intro hx0
    have hyz : y = z := (show Odd 691 by norm_num).pow_injective (by simpa [hx0] using e)
    have h691one : (691 : ℤ) ∣ 1 := by
  ...
  have hy : y ≠ 0 := by
    intro hy0
    have hxz : x = z := (show Odd 691 by norm_num).pow_injective (by simpa [hy0] using e)
    have h691one : (691 : ℤ) ∣ 1 := by
      rw [← hgcd, Finset.dvd_gcd_iff]
      intro w hw
      simp only [Finset.mem_insert, Finset.mem_singleton] at hw
  ...
  have ht0 : t ≠ 0 := by
    ...
    apply hz0
    simp [ht]
  obtain ⟨u, hu⟩ := kappa_pow_threeHundredFortyFive_associated_691 hζ
  have hkappa0 : kappa hζ ^ 345 ≠ 0 :=
    (kappa_pow_threeHundredFortyFive_associated_691 hζ).ne_zero_iff.mpr (by norm_num)
  have huReal :
  ...
  let s : HistoricalState hζ :=
    { omega := x
      ...
      equation := by
        ...
        calc
          (x : 𝓞 K) ^ 691 + (y : 𝓞 K) ^ 691 =
              ((((691 : ℤ) * t : ℤ) : 𝓞 K)) ^ 691 := by exact_mod_cast e
          _ = ((691 : 𝓞 K) * (t : 𝓞 K)) ^ 691 := by norm_num
          _ = (kappa hζ ^ 345 * ((u : 𝓞 K) * (t : 𝓞 K))) ^ 691 := by
            congr 1
            rw [← mul_assoc, hu]
          _ = (kappa hζ ^ 345 * ξ) ^ 691 := rfl }
  refine ⟨s, ?_⟩
  refine ⟨?_, ?_, ?_, ?_⟩
  · dsimp [s]
    simp
...
(10). -/
def equationTenTraceOne691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691) : 𝓞 K :=
  (hζ.unit' : 𝓞 K) + (hζ.unit'⁻¹ : (𝓞 K)ˣ)

/-- The real cyclotomic trace `B = ζ² + ζ⁻²` used in equation
(10). -/
def equationTenTraceTwo691 {ζ : K} (hζ : IsPrimitiveRoot ζ 691) : 𝓞 K :=
  (hζ.unit' : 𝓞 K) ^ 2 + (hζ.unit'⁻¹ : (𝓞 K)ˣ) ^ 2

/-- The cyclotomic unit `ζ⁻¹(1+ζ+ζ²) = A+1`. -/
def equationTenTraceOneUnit691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) : (𝓞 K)ˣ :=
  hζ.unit'⁻¹ *
    (hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 3 691)).unit

/-- The cyclotomic unit `ζ⁻¹(1+ζ)² = A+2`. -/
def equationTenTraceTwoUnit691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) : (𝓞 K)ˣ :=
  hζ.unit'⁻¹ *
    (hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 2 691)).unit ^ 2

omit [IsCyclotomicExtension {691} ℚ K] in
lemma equationTenTraceOneUnit691_val {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    (equationTenTraceOneUnit691 hζ : 𝓞 K) =
      equationTenTraceOne691 hζ + 1 := by
  have huinv : (hζ.unit'⁻¹ : (𝓞 K)ˣ) * (hζ.unit' : 𝓞 K) = 1 := by
    rw [← Units.val_mul]
    simp
  have hgeom :
      (((hζ.unit'_coe.geom_sum_isUnit (by norm_num)
        (by norm_num : Nat.Coprime 3 691)).unit : (𝓞 K)ˣ) : 𝓞 K) =
        1 + (hζ.unit' : 𝓞 K) + (hζ.unit' : 𝓞 K) ^ 2 := by
    rw [(hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 3 691)).unit_spec]
    norm_num [Finset.sum_range_succ]
  simp only [equationTenTraceOneUnit691, Units.val_mul]
  rw [hgeom]
  simp only [equationTenTraceOne691]
  linear_combination (1 + (hζ.unit' : 𝓞 K)) * huinv

omit [IsCyclotomicExtension {691} ℚ K] in
lemma equationTenTraceTwoUnit691_val {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    (equationTenTraceTwoUnit691 hζ : 𝓞 K) =
      equationTenTraceOne691 hζ + 2 := by
  have huinv : (hζ.unit'⁻¹ : (𝓞 K)ˣ) * (hζ.unit' : 𝓞 K) = 1 := by
    rw [← Units.val_mul]
    simp
  have hgeom :
      (((hζ.unit'_coe.geom_sum_isUnit (by norm_num)
        (by norm_num : Nat.Coprime 2 691)).unit : (𝓞 K)ˣ) : 𝓞 K) =
        1 + (hζ.unit' : 𝓞 K) := by
    rw [(hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 2 691)).unit_spec]
    norm_num [Finset.sum_range_succ]
  simp only [equationTenTraceTwoUnit691, Units.val_mul,
...
lemma two_sub_equationTenTraceOne691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    2 - equationTenTraceOne691 hζ = kappa hζ := by
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  simp only [equationTenTraceOne691, kappa]
  linear_combination -huinv

omit [IsCyclotomicExtension {691} ℚ K] in
/-- The trace difference `A-B` is `κ` times the explicit unit `A+1`. -/
lemma equationTenTraceOne_sub_two691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    equationTenTraceOne691 hζ - equationTenTraceTwo691 hζ =
      kappa hζ * (equationTenTraceOneUnit691 hζ : 𝓞 K) := by
  rw [equationTenTraceOneUnit691_val]
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  rw [← two_sub_equationTenTraceOne691 hζ]
  simp only [equationTenTraceOne691, equationTenTraceTwo691]
  linear_combination 2 * huinv

omit [IsCyclotomicExtension {691} ℚ K] in
/-- The second concrete coefficient `2-B` is `κ` times the explicit
unit `A+2`. -/
lemma two_sub_equationTenTraceTwo691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    2 - equationTenTraceTwo691 hζ =
      kappa hζ * (equationTenTraceTwoUnit691 hζ : 𝓞 K) := by
  rw [equationTenTraceTwoUnit691_val]
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  rw [← two_sub_equationTenTraceOne691 hζ]
...
lemma powTwoPrimitiveRoot_unit691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
...
`A+2`. -/
lemma kappa_powTwoPrimitiveRoot691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    kappa (hζ.pow_of_coprime 2 (by norm_num)) =
      kappa hζ * (equationTenTraceTwoUnit691 hζ : 𝓞 K) := by
  rw [← two_sub_equationTenTraceTwo691 hζ]
  simp only [kappa, powTwoPrimitiveRoot_unit691 hζ,
    Units.val_pow_eq_pow_val, equationTenTraceTwo691]
...
lemma equationTenTraceOne_real691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (equationTenTraceOne691 hζ) =
      equationTenTraceOne691 hζ := by
  have hz :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (hζ.unit' : 𝓞 K) =
        (hζ.unit'⁻¹ : (𝓞 K)ˣ) :=
  ...
  have hzinv :
    ...
    rw [map_inv₀,
      Fermat.Irregular.CyclotomicDiscriminantPrime.complexConj_zeta_inv hζ,
      inv_inv]
  simp only [equationTenTraceOne691, map_add, hz, hzinv, add_comm]

/-- The cyclotomic unit `A+2` used to transport the historical state to
the primitive root `ζ²` is real. -/
lemma equationTenTraceTwoUnit_real691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) :
    NumberField.IsCMField.unitsComplexConj K
        (equationTenTraceTwoUnit691 hζ) =
      equationTenTraceTwoUnit691 hζ := by
  apply Units.ext
  change NumberField.IsCMField.ringOfIntegersComplexConj K
      (equationTenTraceTwoUnit691 hζ : 𝓞 K) =
    (equationTenTraceTwoUnit691 hζ : 𝓞 K)
  rw [equationTenTraceTwoUnit691_val, map_add,
    equationTenTraceOne_real691 hζ, map_ofNat]
...
noncomputable def historicalStateAtTwo691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ) :
    HistoricalState (hζ.pow_of_coprime 2 (by norm_num)) :=
  let u := equationTenTraceTwoUnit691 hζ
  { omega := s.omega
    theta := s.theta
    ...
    equation := by
      ...
      have hbase :
        ...
        rw [kappa_powTwoPrimitiveRoot691 hζ, mul_pow]
        simp only [Units.val_pow_eq_pow_val]
        calc
...
`ζ` to `ζ²`. -/
lemma historicalStateAtTwo_admissible691 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    RealSourceAdmissible (hζ.pow_of_coprime 2 (by norm_num))
      (historicalStateAtTwo691 hζ s) := by
  let u := equationTenTraceTwoUnit691 hζ
  have hureal : NumberField.IsCMField.unitsComplexConj K u = u := by
    simpa only [u] using equationTenTraceTwoUnit_real691 hζ
  have huinvpow :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (((u⁻¹) ^ s.m : (𝓞 K)ˣ) : 𝓞 K) =
        (((u⁻¹) ^ s.m : (𝓞 K)ˣ) : 𝓞 K) := by
...
theorem exists_historicalEquationEight_pair_two691
    (hlemma : LemmaOne K 691)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρtwo ρminus : 𝓞 K) (η : (𝓞 K)ˣ),
      ...
      s.omega + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * η * ρtwo ^ 691 ∧
      s.omega + ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ)) * η * ρminus ^ 691 ∧
      NumberField.IsCMField.ringOfIntegersComplexConj K ρtwo = ρminus ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρtwo ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρminus := by
  let hζtwo := hζ.pow_of_coprime 2 (by norm_num)
  let stwo := historicalStateAtTwo691 hζ s
  have hstwo : RealSourceAdmissible hζtwo stwo := by
    simpa only [hζtwo, stwo] using
      historicalStateAtTwo_admissible691 hζ s hs
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one691 hlemma hζtwo stwo hstwo
  have hassoc :
      Associated ((hζ.unit' : 𝓞 K) - 1)
        ((hζtwo.unit' : 𝓞 K) - 1) := by
    simpa only [hζtwo, powTwoPrimitiveRoot_unit691 hζ,
      Units.val_pow_eq_pow_val] using
      hζ.unit'_coe.associated_sub_one_pow_sub_one_of_coprime
        (by norm_num : Nat.Coprime 2 691)
  refine ⟨ρtwo, ρminus, η, hη, ?_, ?_, hconj, ?_, ?_⟩
  · simpa only [stwo, historicalStateAtTwo691, hζtwo,
      powTwoPrimitiveRoot_unit691 hζ, Units.val_pow_eq_pow_val] using htwo
  · simpa only [stwo, historicalStateAtTwo691, hζtwo,
      powTwoPrimitiveRoot_unit691 hζ] using hminus
...
lemma equationTenA_quadraticElimination691
    (ω θ A B Ua Ub Uzero Xa Xb Xzero : 𝓞 K)
    (ha : ω ^ 2 + A * (ω * θ) + θ ^ 2 = Ua * Xa ^ 691)
    (hb : ω ^ 2 + B * (ω * θ) + θ ^ 2 = Ub * Xb ^ 691)
...
equation: the coefficient of `Xb ^ 691` is absorbed into the unit `-Ub`,
while the other two coefficient quotients are the explicit geometric-sum
units above. -/
theorem equationTenB_cyclotomicSimplification691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (ω θ Xa Xb Xzero : 𝓞 K) (Ua Ub Uzero : (𝓞 K)ˣ)
    (ha : ω ^ 2 + equationTenTraceOne691 hζ * (ω * θ) + θ ^ 2 =
      Ua * Xa ^ 691)
    (hb : ω ^ 2 + equationTenTraceTwo691 hζ * (ω * θ) + θ ^ 2 =
      Ub * Xb ^ 691)
    (hzero : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 = Uzero * Xzero) :
    (equationTenTraceTwoUnit691 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 691 +
        (-Ub : (𝓞 K)ˣ) * Xb ^ 691 =
      (equationTenTraceOneUnit691 hζ * Uzero : (𝓞 K)ˣ) * Xzero := by
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit691_mul_sq]
    exact mul_ne_zero (kappaUnit691 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have helim := equationTenA_quadraticElimination691
    ω θ (equationTenTraceOne691 hζ) (equationTenTraceTwo691 hζ)
    Ua Ub Uzero Xa Xb Xzero ha hb hzero
  rw [two_sub_equationTenTraceTwo691,
    two_sub_equationTenTraceOne691,
    equationTenTraceOne_sub_two691] at helim
  apply mul_left_cancel₀ hkappa0
  calc
    kappa hζ *
        ((equationTenTraceTwoUnit691 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 691 +
          (-Ub : (𝓞 K)ˣ) * Xb ^ 691) =
        kappa hζ * (equationTenTraceTwoUnit691 hζ : 𝓞 K) *
            ((Ua : 𝓞 K) * Xa ^ 691) -
          kappa hζ * ((Ub : 𝓞 K) * Xb ^ 691) := by
            simp only [Units.val_mul, Units.val_neg]
            ring
    _ = kappa hζ * (equationTenTraceOneUnit691 hζ : 𝓞 K) *
...
`equationTenB_cyclotomicSimplification691`. -/
theorem equationTenB_commonKappa691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (ω θ Xa Xb Xzero : 𝓞 K) (Ua Ub Uzero : (𝓞 K)ˣ)
    (ha : ω ^ 2 + equationTenTraceOne691 hζ * (ω * θ) + θ ^ 2 =
      kappa hζ * (Ua * Xa ^ 691))
    (hb : ω ^ 2 + equationTenTraceTwo691 hζ * (ω * θ) + θ ^ 2 =
      kappa hζ * (Ub * Xb ^ 691))
    (hzero : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 =
      kappa hζ * (Uzero * Xzero ^ 691)) :
    (equationTenTraceTwoUnit691 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 691 +
        (-Ub : (𝓞 K)ˣ) * Xb ^ 691 =
      (equationTenTraceOneUnit691 hζ * Uzero : (𝓞 K)ˣ) *
        Xzero ^ 691 := by
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit691_mul_sq]
    exact mul_ne_zero (kappaUnit691 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have ha' : ω ^ 2 + equationTenTraceOne691 hζ * (ω * θ) + θ ^ 2 =
      (kappa hζ * Ua) * Xa ^ 691 := by
    simpa only [mul_assoc] using ha
  have hb' : ω ^ 2 + equationTenTraceTwo691 hζ * (ω * θ) + θ ^ 2 =
      (kappa hζ * Ub) * Xb ^ 691 := by
    simpa only [mul_assoc] using hb
  have hzero' : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 =
      (kappa hζ * Uzero) * Xzero ^ 691 := by
    simpa only [mul_assoc] using hzero
  have helim := equationTenA_quadraticElimination691
    ω θ (equationTenTraceOne691 hζ) (equationTenTraceTwo691 hζ)
    (kappa hζ * Ua) (kappa hζ * Ub) (kappa hζ * Uzero)
    Xa Xb (Xzero ^ 691) ha' hb' hzero'
  rw [two_sub_equationTenTraceTwo691,
    two_sub_equationTenTraceOne691,
    equationTenTraceOne_sub_two691] at helim
  ...
  calc
    kappa hζ * kappa hζ *
        ((equationTenTraceTwoUnit691 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 691 +
          (-Ub : (𝓞 K)ˣ) * Xb ^ 691) =
        kappa hζ * (equationTenTraceTwoUnit691 hζ : 𝓞 K) *
            (kappa hζ * ((Ua : 𝓞 K) * Xa ^ 691)) -
          kappa hζ * (kappa hζ * ((Ub : 𝓞 K) * Xb ^ 691)) := by
            simp only [Units.val_mul, Units.val_neg]
            ring
    _ = kappa hζ * (equationTenTraceOneUnit691 hζ : 𝓞 K) *
          (kappa hζ * ((Uzero : 𝓞 K) * Xzero ^ 691)) := by
...
Compared with WeightedReductionData691, this structure asks for the exact
...
unity, every 691st power in Vandiver's equation is unchanged. -/
...
`691`th root to be real, absorbs it into the first summand, divides out the
...
For each of these two ideals, `exists_real_unit_mul_pow_generator691`
...
historical state, verify their displayed 691st-power identities, and carry
...
`equationTenA_quadraticElimination691`.
...
theorem equationsSevenToTenReduction_691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
...
theorem secondCaseExcluded_691_of_historical
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (heliminate : RealPrincipalGeneratorElimination691 hζ)
    (hkummer : KummerUnitPowerConclusion K 691) :
    Fermat.SecondCaseExcluded 691 :=
  secondCaseExcluded_of_historical_descent (by norm_num) hζ
    (RealSourceAdmissible hζ) (secondCaseStartsHistoricalDescent_691 hζ)
    (equationsSevenToTenReduction_691 hζ heliminate) hkummer

/-- Assemble the exact source statement of Vandiver's Lemma 2 with the
directly checked exponent-`691` Bernoulli cube data. No semiprimary deepening
hypothesis is used on this historical route. -/
theorem secondCaseExcluded_691_of_vandiverLemmaTwo
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (heliminate : RealPrincipalGeneratorElimination691 hζ)
