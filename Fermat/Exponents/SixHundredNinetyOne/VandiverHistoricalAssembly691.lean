import Fermat.Exponents.SixHundredNinetyOne.VandiverPreparedPair691
import Fermat.Exponents.SixHundredNinetyOne.VandiverLinearComparison691
import Fermat.Exponents.SixHundredNinetyOne.VandiverEquationEightAFactorization691
import Fermat.Exponents.SixHundredNinetyOne.VandiverLemmaTwo
import Fermat.Exponents.SixHundredNinetyOne.FirstCase
import Fermat.Core.Cases

/-!
# Completion of Vandiver's historical proof at exponent 691
...
isolated `RealPrincipalGeneratorElimination691`, then combines it with the
fully formalized Vandiver Lemma II and Sophie Germain auxiliary prime 11057
to obtain `Fermat.HoldsAt 691`.
-/
...
lemma historicalEquationEightA_quadratic691
    ...
    s.omega ^ 2 + 2 * (s.omega * s.theta) + s.theta ^ 2 =
      kappa hζ *
        ((etaZero ^ 2 : (𝓞 K)ˣ) *
          (kappa hζ ^ (2 * s.m - 1) * rhoZero ^ 2) ^ 691) := by
  ...
  calc
    ...
    _ = ((etaZero : 𝓞 K) *
          kappa hζ ^ (691 * s.m - 345) * rhoZero ^ 691) ^ 2 := hsquare
    _ = kappa hζ *
        ((etaZero ^ 2 : (𝓞 K)ˣ) *
          (kappa hζ ^ (2 * s.m - 1) * rhoZero ^ 2) ^ 691) := by
...
exponent 691.  This is the final concrete input required by the historical
well-founded descent. -/
theorem realPrincipalGeneratorElimination691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691) :
    RealPrincipalGeneratorElimination691 hζ := by
  intro s hs
  obtain ⟨rhoZero, etaZero, jZero, hzero, hgenerator,
      hconjZero, hetaZeroReal⟩ :=
    exists_historicalEquationEightA691 hζ s hs
  obtain ⟨dOne⟩ :=
    exists_preparedEquationEightPair_one691
      hζ s hs rhoZero etaZero hzero
  obtain ⟨dTwo⟩ :=
    exists_preparedEquationEightPair_two691
      hζ s hs rhoZero etaZero hzero
  let D : ℕ := (2 * s.m - 2) * 691
  have hcoefficient :
      ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
        (dOne.coefficient : 𝓞 K) * dOne.rplus ^ 691 -
          (dTwo.coefficient : 𝓞 K) * dTwo.rplus ^ 691 := by
    simpa only [D] using
      historicalEquationEight_one_two_coefficients_close691
        hζ s hs dOne.rplus dTwo.rplus
        dOne.coefficient dTwo.coefficient
        dOne.equation_plus dTwo.equation_plus
  have hpowOne :
      ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
        dOne.rplus ^ 691 - dOne.mu ^ 477481 := by
    have h := dOne.close_plus.trans
      (sub_dvd_pow_sub_pow dOne.rplus (dOne.mu ^ 691) 691)
    simpa only [D, ← pow_mul] using h
  have hpowTwo :
      ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
        dTwo.rplus ^ 691 - dTwo.mu ^ 477481 := by
    have h := dTwo.close_plus.trans
      (sub_dvd_pow_sub_pow dTwo.rplus (dTwo.mu ^ 691) 691)
    simpa only [D, ← pow_mul] using h
  have herrors :
      ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
        (dOne.coefficient : 𝓞 K) *
            (dOne.rplus ^ 691 - dOne.mu ^ 477481) -
          (dTwo.coefficient : 𝓞 K) *
            (dTwo.rplus ^ 691 - dTwo.mu ^ 477481) :=
    dvd_sub
      (dvd_mul_of_dvd_right hpowOne (dOne.coefficient : 𝓞 K))
      (dvd_mul_of_dvd_right hpowTwo (dTwo.coefficient : 𝓞 K))
  have hmuD :
      ((hζ.unit' : 𝓞 K) - 1) ^ D ∣
        (dOne.coefficient : 𝓞 K) * dOne.mu ^ 477481 -
          (dTwo.coefficient : 𝓞 K) * dTwo.mu ^ 477481 := by
    have h := dvd_sub hcoefficient herrors
    convert h using 1
    ring
  have h1382D : 1382 ≤ D := by
    have hm := s.one_lt_m
    dsimp [D]
    omega
  have hmu1382 :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1382 ∣
        (dOne.coefficient : 𝓞 K) * dOne.mu ^ 477481 -
          (dTwo.coefficient : 𝓞 K) * dTwo.mu ^ 477481 :=
    (pow_dvd_pow ((hζ.unit' : 𝓞 K) - 1) h1382D).trans hmuD
  obtain ⟨c, hratio477481⟩ :=
    exists_int_ratio_pow477481_congruent691 hζ
      dOne.coefficient dTwo.coefficient dOne.mu dTwo.mu
      dOne.mu_real dTwo.mu_real dOne.mu_not_ramified hmu1382
  obtain ⟨rationalBase, hnegativeRatio⟩ :=
    exists_int_negative_square_ratio_pow691_congruent691 hζ
      (dOne.coefficient / dTwo.coefficient) c hratio477481
  let minusOne : (𝓞 K)ˣ := -1
  let epsilonOne : (𝓞 K)ˣ :=
    minusOne *
      (equationTenTraceTwoUnit691 hζ * dOne.coefficient ^ 2)
  let epsilonTwo : (𝓞 K)ˣ :=
    equationTenTraceTwoUnit691 hζ * dTwo.coefficient ^ 2
  let epsilonThree : (𝓞 K)ˣ :=
    minusOne * (equationTenTraceOneUnit691 hζ * etaZero ^ 2)
  have hepsilonRatio :
  ...
  have hquadZero :=
    historicalEquationEightA_quadratic691
      hζ s rhoZero etaZero hzero
  have hweightedRaw :=
    equationTenB_commonKappa691 hζ s.omega s.theta
  ...
  have hweighted :
      epsilonOne * (dOne.rplus * dOne.rminus) ^ 691 +
          epsilonTwo * (dTwo.rplus * dTwo.rminus) ^ 691 =
        epsilonThree *
          (kappa hζ ^ (2 * s.m - 1) * rhoZero ^ 2) ^ 691 := by
  ...
  have hrhoZero : rhoZero ≠ 0 := by
    ...
    norm_num
  have hcop :=
    equationEight_generators_products_coprime691 hζ
      dOne.coefficient dOne.coefficient
      dTwo.coefficient dTwo.coefficient
      ((etaZero : 𝓞 K) * kappa hζ ^ (691 * s.m - 345))
  ...
  have hepsilonThreeReal :
    ...
    rw [map_mul, hminusOneReal, map_mul,
      equationTenTraceOneUnit_real691 hζ, hetaZeroReal]
  have hepsilonTwoReal :
      NumberField.IsCMField.unitsComplexConj K epsilonTwo =
        epsilonTwo := by
    dsimp [epsilonTwo]
    rw [map_mul, map_pow,
      equationTenTraceTwoUnit_real691 hζ, dTwo.coefficient_real]
  have hrealEta :
      NumberField.IsCMField.unitsComplexConj K
  ...
  exact ⟨
    { x := dOne.rplus * dOne.rminus
      ...
      factorSupport_strict :=
        historicalEquationEightA_square_support_strict_unconditional691
...
/-- Vandiver's historical proof excludes the second case at exponent 691,
with the cyclotomic field and primitive root chosen canonically. -/
theorem secondCaseExcluded_sixHundredNinetyOne :
...
theorem holdsAt_sixHundredNinetyOne : Fermat.HoldsAt 691 := by
  exact Fermat.holdsAt_of_auxiliaryPrime_of_secondCaseExcluded
    ...
    secondCaseExcluded_sixHundredNinetyOne
