...
/-!
# Vandiver's equation-(8a) factorization at exponent 691
...
noncomputable def historicalEquationEightAComplement691
  ...
  by
    ...
    let root := fun η : nthRootsFinset 691 (1 : 𝓞 K) ↦
      root_div_zeta_sub_one_dvd_gcd
        (K := K) (p := 691) (x := s.omega) (y := s.theta)
        (z := s.xi) (ε := historicalRegularUnit691 hζ s)
    ...
    exact
      ∏ η ∈ Finset.attach (nthRootsFinset 691 (1 : 𝓞 K)) \ {η₀}, root η

set_option maxRecDepth 50000 in
theorem historicalEquationEightA_factorization_and_coprime691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    Ideal.span {s.xi} =
        historicalEquationEightAIdeal691 hζ s *
          historicalEquationEightAComplement691 hζ s ∧
      IsCoprime (historicalEquationEightAIdeal691 hζ s)
        (historicalEquationEightAComplement691 hζ s) := by
  classical
  let e := historicalState_regularEquation691 hζ s
  let hy := historicalState_theta_not_dvd691 hζ s
  ...
  let Z : Ideal (𝓞 K) :=
    z_div_m (K := K) (p := 691)
      ...
      (ε := historicalRegularUnit691 hζ s) (m := 2 * s.m - 1)
      hζ e hy
  let root := fun η : nthRootsFinset 691 (1 : 𝓞 K) ↦
    root_div_zeta_sub_one_dvd_gcd
      (K := K) (p := 691) (x := s.omega) (y := s.theta)
      (z := s.xi) (ε := historicalRegularUnit691 hζ s)
      (m := 2 * s.m - 1) (by norm_num) hζ e hy η
  let c := fun η : nthRootsFinset 691 (1 : 𝓞 K) ↦
  ...
  have hZ : Z = Ideal.span {s.xi} := by
    have hzspec :
      ...
      simpa only [M, Z] using
        (z_div_m_spec (K := K) (p := 691)
          (x := s.omega) (y := s.theta) (z := s.xi)
          (ε := historicalRegularUnit691 hζ s)
          (m := 2 * s.m - 1) hζ e hy)
    rw [hM, one_mul] at hzspec
    exact hzspec.symm
  have hrootpow (η : nthRootsFinset 691 (1 : 𝓞 K)) :
      root η ^ 691 = c η := by
    simpa only [root, c] using
      (root_div_zeta_sub_one_dvd_gcd_spec
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy η)
  have hprodC :
      ∏ η ∈ Finset.attach (nthRootsFinset 691 (1 : 𝓞 K)), c η =
        (Z * P ^ (2 * s.m - 1)) ^ 691 := by
    simpa only [c, Z, P, π] using
      (prod_c (K := K) (p := 691)
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy)
  have hprodRootPow :
      (∏ η ∈ Finset.attach (nthRootsFinset 691 (1 : 𝓞 K)),
          root η) ^ 691 =
        (Z * P ^ (2 * s.m - 1)) ^ 691 := by
    rw [← Finset.prod_pow]
    simpa only [hrootpow] using hprodC
  have hprodRoot :
      ∏ η ∈ Finset.attach (nthRootsFinset 691 (1 : 𝓞 K)), root η =
        Z * P ^ (2 * s.m - 1) :=
    pow_left_injective (M := Ideal (𝓞 K))
      (by norm_num : 691 ≠ 0) hprodRootPow
  have hη₀one :
      η₀ = oneNthRoot (K := K) (p := 691) := by
    simpa only [η₀, e, hy] using
      distinguishedRoot_eq_one_of_real691 hζ e hy hs.1 hs.2.1
  have hrootη₀ :
      root η₀ = historicalOneFactorIdeal691 hζ s := by
    rw [hη₀one]
    rfl
  have hsplit :
  ...
  have hfactor :
      Ideal.span {s.xi} =
        historicalEquationEightAIdeal691 hζ s * Q := by
    apply mul_left_cancel₀ hP0
    symm
    calc
      P ^ (2 * s.m - 1) *
          (historicalEquationEightAIdeal691 hζ s * Q) =
          (P ^ (2 * s.m - 1) *
            historicalEquationEightAIdeal691 hζ s) * Q := by
              rw [mul_assoc]
  ...
  have hcop :
    ...
    have hroots : IsCoprime (root η₀) (root η) :=
      (IsCoprime.pow_left_iff (by norm_num : 0 < 691)).mp
        ((IsCoprime.pow_right_iff (by norm_num : 0 < 691)).mp hc)
    rw [← hspec] at hroots
    exact hroots.of_mul_left_right
  simpa only [historicalEquationEightAComplement691, e, hy, root, η₀, Q] using
    And.intro hfactor hcop

set_option maxRecDepth 50000 in
theorem historicalEquationEightAComplement_ne_top691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAComplement691 hζ s ≠ ⊤ := by
  classical
  ...
  let η₀ := zeta_sub_one_dvd_root
    ...
    (z := s.xi) (ε := historicalRegularUnit691 hζ s)
    (m := 2 * s.m - 1) (by norm_num) hζ e hy
  let ηζ : nthRootsFinset 691 (1 : 𝓞 K) :=
  ...
  have hηζ_ne : ηζ ≠ η₀ := by
    ...
    intro h
    have hv := congrArg
      (fun x : nthRootsFinset 691 (1 : 𝓞 K) ↦ (x : 𝓞 K)) h
    exact hζ.unit'_coe.ne_one (by norm_num) hv
  have hηζmem :
      ηζ ∈
        Finset.attach (nthRootsFinset 691 (1 : 𝓞 K)) \ {η₀} := by
    simp only [Finset.mem_sdiff, Finset.mem_attach,
      Finset.mem_singleton, true_and]
    exact hηζ_ne
  have hrootDvd :
      root ηζ ∣ historicalEquationEightAComplement691 hζ s := by
    simpa only [historicalEquationEightAComplement691, e, hy, root, η₀] using
  ...
  have hfactorIdealTop :
      historicalZetaFactorIdeal691 hζ s = ⊤ := by
    simpa only [root, ηζ, historicalZetaFactorIdeal691] using hrootTop
  have hqspan :
      Ideal.span {historicalZetaFactor691 hζ s} = ⊤ := by
    rw [← historicalZetaFactorIdeal_pow691 hζ s, hfactorIdealTop]
    simp
  have hqunit :
      IsUnit (historicalZetaFactor691 hζ s) :=
    Ideal.span_singleton_eq_top.mp hqspan
  let qUnit : (𝓞 K)ˣ := hqunit.unit
  let A : (𝓞 K)ˣ := -qUnit
  let B : (𝓞 K)ˣ := NumberField.IsCMField.unitsComplexConj K A
  have hqUnitVal :
      (qUnit : 𝓞 K) = historicalZetaFactor691 hζ s :=
    hqunit.unit_spec
  have hAval :
      (A : 𝓞 K) = -historicalZetaFactor691 hζ s := by
    dsimp [A]
  ...
  have hone :
    ...
    have hqmul :
        historicalZetaFactor691 hζ s * π =
  ...
  have hr_ne : (r : 𝓞 K) ≠ 1 := by
    ...
    exact historicalState_omega_add_theta_ne_zero691 hζ s
      ((mul_eq_zero.mp hzero).resolve_left hplusUnit.ne_zero)
  have h691sum : (691 : 𝓞 K) ∣ s.omega + s.theta := by
    have h692 :=
      historicalState_omega_add_theta_highDivisibility691 hζ s hs
    have h690 :
        π ^ 690 ∣ s.omega + s.theta :=
      (pow_dvd_pow π (by norm_num : 690 ≤ 692)).trans
        (by simpa only [π] using h692)
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mp
      (by simpa only [π] using h690)
  have h691prod :
      (691 : 𝓞 K) ∣
        (1 - (r : 𝓞 K)) *
          (1 - (hζ.unit' : 𝓞 K)) := by
    have hwithB :
        (691 : 𝓞 K) ∣
          ((1 - (r : 𝓞 K)) *
            (1 - (hζ.unit' : 𝓞 K))) * (B : 𝓞 K) := by
      rw [hidentity]
      exact dvd_mul_of_dvd_right h691sum _
    exact (B.isUnit.dvd_mul_right).mp hwithB
  have hrpow : (r : 𝓞 K) ^ 691 = 1 := by
    change ((((hζ.unit' : 𝓞 K) ^ j) ^ 2) ^ 691) = 1
    rw [← pow_mul, ← pow_mul]
    rw [show j * (2 * 691) = 691 * (j * 2) by omega, pow_mul,
      hζ.unit'_coe.pow_eq_one, one_pow]
  let ηr : nthRootsFinset 691 (1 : 𝓞 K) :=
    ⟨(r : 𝓞 K), by
      rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 691)]
      exact hrpow⟩
  let ηone : nthRootsFinset 691 (1 : 𝓞 K) :=
  ...
  have hrAssocSub :
    ...
    simpa only [ηr, ηone, oneNthRoot, π] using
      (hζ.unit'_coe.ntRootsFinset_pairwise_associated_sub_one_sub_of_prime
        (by norm_num : Nat.Prime 691) ηr.prop ηone.prop hηr_ne).symm
  have hrAssoc :
    ...
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mpr
      (by
        simpa only [π] using
          (show ((691 : ℕ) : 𝓞 K) ∣
              (1 - (r : 𝓞 K)) * (1 - (hζ.unit' : 𝓞 K)) by
            convert h691prod using 1))
  have hbad : π ^ 690 ∣ π ^ 2 :=
    (hprodAssoc.dvd_iff_dvd_right).mp hπ690prod
  have hle :
      690 ≤ 2 :=
    (pow_dvd_pow_iff
      (hζ.unit'_coe.sub_one_ne_zero (by norm_num))
...
theorem historicalEquationEightA_square_support_strict_unconditional691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s)
    (ρzero : 𝓞 K)
    (hgenerator :
      historicalEquationEightAIdeal691 hζ s =
        Ideal.span {ρzero}) :
    primeIdealFactorSupport691 (ρzero ^ 2) ⊂
      primeIdealFactorSupport691 s.xi := by
  obtain ⟨hfactor, hcop⟩ :=
    historicalEquationEightA_factorization_and_coprime691 hζ s hs
  exact historicalEquationEightA_square_support_strict691
    hζ s ρzero (historicalEquationEightAComplement691 hζ s)
    hgenerator hfactor
    (historicalEquationEightAComplement_ne_top691 hζ s hs) hcop
