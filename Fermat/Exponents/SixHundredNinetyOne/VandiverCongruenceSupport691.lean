...
import Fermat.Exponents.SixHundredNinetyOne.VandiverRelationNormalization

/-!
# Local congruence support for Vandiver's historical descent at 691
...
691st-power depth step twice then turns congruence modulo `(ζ - 1)^2` into
congruence modulo `(ζ - 1)^1382` for 477481st powers.  The final two theorems
...
so `2k` is divisible.  Since the prime above 691 does not divide 2, it
divides `k`. -/
theorem exists_zeta_sub_one_sq_dvd_sub_int_of_real691
  ...
  have hconjk :
    ...
    rw [map_sub,
      ringOfIntegersComplexConj_eq_mod_zeta_sub_one691 hζ k,
      sub_self]
  have hrel :
      k = ((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) *
      ...
      _ = π * (((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) *
        ...
        rw [show NumberField.IsCMField.ringOfIntegersComplexConj K π =
          (((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) * π) by
            simpa only [π] using
              ringOfIntegersComplexConj_zeta_sub_one691 hζ]
        ring
  have huplus :
  ...
  have hπnot2 : ¬ π ∣ (2 : 𝓞 K) := by
    intro htwo
    have : (691 : ℤ) ∣ 2 := (zeta_sub_one_dvd_Int_iff hζ).mp (by
...
/-- If real, nonramified factors `a` and `b` occur in a depth-1382 product
congruence

`U*a^(691²) ≡ V*b^(691²)`,

then `U/V` is congruent to the 477481st power of a rational integer at the
same depth.

The rational residue of `a` is invertible modulo 691.  A Bézout inverse
...
this to depth 1382. -/
theorem exists_int_ratio_pow477481_congruent691
    ...
    (hprod :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1382 ∣
        (U : 𝓞 K) * a ^ 477481 - (V : 𝓞 K) * b ^ 477481) :
    ∃ c : ℤ,
      ((hζ.unit' : 𝓞 K) - 1) ^ 1382 ∣
        (((U / V : (𝓞 K)ˣ) : 𝓞 K) - (c : 𝓞 K) ^ 477481) := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  obtain ⟨ca, hca⟩ :=
    exists_zeta_sub_one_sq_dvd_sub_int_of_real691 hζ a ha_real
  obtain ⟨cb, hcb⟩ :=
    exists_zeta_sub_one_sq_dvd_sub_int_of_real691 hζ b hb_real
  have hπ690 : π ^ 690 ∣ (691 : 𝓞 K) := by
    have hp := (associated_zeta_sub_one_pow_prime hζ).dvd
    norm_num at hp
    simpa only [π] using hp
  have hπ2_691 : π ^ 2 ∣ (691 : 𝓞 K) :=
    (pow_dvd_pow π (by norm_num : 2 ≤ 690)).trans hπ690
  have hca_not : ¬ (691 : ℤ) ∣ ca := by
    intro h691ca
    have hπca : π ∣ (ca : 𝓞 K) := by
      simpa only [π] using
        (zeta_sub_one_dvd_Int_iff hζ).mpr h691ca
    ...
    ring
  have hcop691ca : IsCoprime (691 : ℤ) ca :=
    ((show Prime (691 : ℤ) by norm_num).coprime_iff_not_dvd).mpr hca_not
  obtain ⟨u, v, huv⟩ := hcop691ca.symm
  let c : ℤ := u * cb
  have hmiddle_int : (691 : ℤ) ∣ ca * c - cb := by
    refine ⟨-(v * cb), ?_⟩
    dsimp only [c]
    calc
  ...
  have hmiddle :
    ...
    have hcast :
      ...
      norm_cast
    rw [hcast]
    exact hπ2_691.trans (dvd_mul_right _ _)
  have hacb : π ^ 2 ∣ a * (c : 𝓞 K) - b := by
    ...
    ring
  have hp692 :=
    zeta_sub_one_pow_add_sixHundredNinety_dvd_pow_sub_pow691
      hζ 2 (by norm_num) (a * (c : 𝓞 K)) b hacb
  have hp1382 :=
    zeta_sub_one_pow_add_sixHundredNinety_dvd_pow_sub_pow691
      hζ 692 (by norm_num)
      ((a * (c : 𝓞 K)) ^ 691) (b ^ 691)
      (by simpa only [π] using hp692)
  have hpowers :
      π ^ 1382 ∣
        a ^ 477481 * (c : 𝓞 K) ^ 477481 - b ^ 477481 := by
    simpa only [π, mul_pow, ← pow_mul] using hp1382
  have hcombined :
      π ^ 1382 ∣
        a ^ 477481 *
          ((U : 𝓞 K) - (V : 𝓞 K) * (c : 𝓞 K) ^ 477481) := by
  ...
  have hUV :
    ...
    have htoInteger : hζ.toInteger = (hζ.unit' : 𝓞 K) := by
      ...
      rfl
    have hapow : ¬ π ∣ a ^ 477481 := by
      intro h
      exact haπ (hζ.zeta_sub_one_prime'.dvd_of_dvd_pow h)
    apply hζ.zeta_sub_one_prime'.pow_dvd_of_dvd_mul_left 1382 hapow
  ...
  rw [show (V : 𝓞 K) *
      ((c : 𝓞 K) ^ 477481 * ((V⁻¹ : (𝓞 K)ˣ) : 𝓞 K)) =
        (c : 𝓞 K) ^ 477481 *
          (((V : 𝓞 K) * ((V⁻¹ : (𝓞 K)ˣ) : 𝓞 K))) by ring,
    ← Units.val_mul]
  simp

/-- The negative square of a unit which is a rational 477481st power modulo
`(ζ - 1)^1382` is a rational 691st power modulo `(1 - ζ)^1382`.

Concretely, if `R ≡ c^477481`, then

`-R² ≡ (-c^1382)^691`.

The change from `ζ - 1` to `1 - ζ` is harmless because the exponent 1382 is
even. -/
theorem exists_int_negative_square_ratio_pow691_congruent691
    {ζ : K} (hζ : IsPrimitiveRoot ζ 691)
    (R : (𝓞 K)ˣ) (c : ℤ)
    (h :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1382 ∣
    ...
    ∃ d : ℤ,
      ((1 : 𝓞 K) - hζ.unit') ^ 1382 ∣
        (((-(R ^ 2) : (𝓞 K)ˣ) : 𝓞 K) - (d : 𝓞 K) ^ 691) := by
  refine ⟨-(c ^ 1382), ?_⟩
  have hsquare :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1382 ∣
        (R : 𝓞 K) ^ 2 - ((c : 𝓞 K) ^ 477481) ^ 2 :=
    h.trans (sub_dvd_pow_sub_pow (R : 𝓞 K) ((c : 𝓞 K) ^ 477481) 2)
  have heven :
      ((1 : 𝓞 K) - hζ.unit') ^ 1382 =
        ((hζ.unit' : 𝓞 K) - 1) ^ 1382 := by
  ...
  rw [Odd.neg_pow (by norm_num : Odd 691)]
  norm_num [← pow_mul]
  ring
