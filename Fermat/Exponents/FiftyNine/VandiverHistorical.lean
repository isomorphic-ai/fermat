...
import Fermat.Exponents.FiftyNine.ArithmeticCertificate
import Fermat.Exponents.FiftyNine.SinnottKummer
import FltRegular.NumberTheory.Cyclotomic.MoreLemmas

/-!
# Vandiver's historical descent at exponent 59

This file instantiates the source-faithful historical descent with the
concrete real-subfield invariant at exponent `59`. It first proves the
passage from a primitive rational second-case solution to Vandiver's
equation (6), using

`59 ~ (1 - ζ) ^ 58 ~ ((1 - ζ) * (1 - ζ⁻¹)) ^ 29`.
...
def RealSourceAdmissible {ζ : K} (hζ : IsPrimitiveRoot ζ 59) :
...
inverse. -/
lemma unitsComplexConj_zeta59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59) :
...
explicit unit `-ζ⁻¹`. -/
lemma ringOfIntegersComplexConj_zeta_sub_one59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
  ...
  have hconjζ :
    ...
    congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K) (unitsComplexConj_zeta59 hζ)
  rw [hconjζ]
  have hinv :
...
`ringOfIntegersComplexConj_zeta_sub_one59` keeps the proof entirely at
element level. -/
lemma zeta_sub_one_pow_dvd_conj_of_dvd59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (n : ℕ) (a : 𝓞 K)
    (h : ((hζ.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hζ.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow, ringOfIntegersComplexConj_zeta_sub_one59, mul_pow]
  refine ⟨(((-hζ.unit'⁻¹ : (𝓞 K)ˣ) ^ n : (𝓞 K)ˣ) : 𝓞 K) *
      NumberField.IsCMField.ringOfIntegersComplexConj K b, ?_⟩
  simp only [Units.val_pow_eq_pow_val]
  ring

/-- Every 59th power is fixed by complex conjugation modulo
`(ζ - 1)^59`.  Both powers are compared with the same rational 59th
power; conjugating the first comparison is legitimate by the preceding
uniformizer calculation. -/
lemma zeta_sub_one_pow_fiftyNine_dvd_pow_sub_conj_pow59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 59 ∣
      a ^ 59 -
        NumberField.IsCMField.ringOfIntegersComplexConj K a ^ 59 := by
  obtain ⟨c, hc⟩ := exists_int_pow_congruent_mod_primary hζ a
  have hcconj := zeta_sub_one_pow_dvd_conj_of_dvd59 hζ 59
    (a ^ 59 - (c : 𝓞 K) ^ 59) hc
  have hcconj' : ((hζ.unit' : 𝓞 K) - 1) ^ 59 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a ^ 59 -
        (c : 𝓞 K) ^ 59 := by
    simpa only [map_sub, map_pow, map_intCast] using hcconj
  convert dvd_sub hc hcconj' using 1
  ring

/-- A 59th root of unity which is `1` modulo `(ζ - 1)^2` is literally
`1`.  A nontrivial power `ζ^k - 1` is associated to the single
uniformizer `ζ - 1`, so it cannot contain its square. -/
lemma zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (j : ℕ)
  ...
  · have hknotdvd : ¬ 59 ∣ k := by
      intro h
      obtain ⟨t, ht⟩ := h
      apply hk
      omega
    have hkcoprime : k.Coprime 59 := by
      apply Nat.coprime_comm.mpr
      exact (show Nat.Prime 59 by norm_num).coprime_iff_not_dvd.mpr hknotdvd
...
lemma unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (u : (𝓞 K)ˣ)
    (hdiv : ((hζ.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
...
completely explicit. -/
def kappaUnit59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59) : (𝓞 K)ˣ :=
  -hζ.unit'⁻¹

omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
/-- The literal unit identity relating Vandiver's `κ` to `(ζ - 1)²`. -/
lemma kappa_eq_kappaUnit59_mul_sq {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    kappa hζ = (kappaUnit59 hζ : 𝓞 K) *
      ((hζ.unit' : 𝓞 K) - 1) ^ 2 := by
  simp only [kappa, kappaUnit59, Units.val_neg, neg_mul, pow_two]
...
omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
/-- Power form of `kappa_eq_kappaUnit59_mul_sq`. -/
lemma kappa_pow_eq_kappaUnit59_pow_mul {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (m : ℕ) :
    kappa hζ ^ m = ((kappaUnit59 hζ ^ m : (𝓞 K)ˣ) : 𝓞 K) *
      ((hζ.unit' : 𝓞 K) - 1) ^ (2 * m) := by
  rw [kappa_eq_kappaUnit59_mul_sq, mul_pow, ← Units.val_pow_eq_pow_val,
    ← pow_mul]
...
def historicalRegularUnit59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (s : HistoricalState hζ) : (𝓞 K)ˣ :=
  s.eta * kappaUnit59 hζ ^ (s.m * 59)

omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
/-- Every historical state at exponent `59` is literally an input to the
generic factor-ideal construction, with upstream depth parameter
`2 * m - 1`:

`ω^59 + θ^59 = ε * (((ζ - 1)^(2*m) * ξ)^59)`.
...
without alteration. -/
lemma historicalState_regularEquation59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    s.omega ^ 59 + s.theta ^ 59 = historicalRegularUnit59 hζ s *
      (((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 1) + 1) * s.xi) ^ 59 := by
  have hm : 1 ≤ 2 * s.m := by
    have := s.one_lt_m
    omega
  rw [Nat.sub_add_cancel hm]
  rw [s.equation, kappa_pow_eq_kappaUnit59_pow_mul]
  simp only [historicalRegularUnit59, mul_pow, ← Units.val_pow_eq_pow_val,
    Units.val_mul]
  rw [← pow_mul]
  ac_rfl
...
lemma historicalState_theta_not_dvd59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ s.theta := by
  intro htheta
  have hsum : (hζ.unit' : 𝓞 K) - 1 ∣
      s.omega ^ 59 + s.theta ^ 59 :=
    zeta_sub_one_dvd (p := 59) hζ
      (historicalState_regularEquation59 hζ s)
  have homegaPow : (hζ.unit' : 𝓞 K) - 1 ∣ s.omega ^ 59 := by
    simpa using dvd_sub hsum (dvd_pow (n := 59) htheta (by norm_num))
...
/-- For a factorization with real entries at exponent `59`, the unique
...
therefore gives `ζ^i = ζ⁻ⁱ`.  Since `59` is odd, this forces `i = 0`.
...
theorem distinguishedRoot_eq_one_of_real59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x y z : 𝓞 K} {ε : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 59 + y ^ 59 = ε *
      ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 59)
    (hy : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ y)
    (hxreal : NumberField.IsCMField.ringOfIntegersComplexConj K x = x)
    (hyreal : NumberField.IsCMField.ringOfIntegersComplexConj K y = y) :
    zeta_sub_one_dvd_root (by norm_num : 59 ≠ 2) hζ e hy =
      oneNthRoot (K := K) (p := 59) := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let η0 := zeta_sub_one_dvd_root (by norm_num : 59 ≠ 2) hζ e hy
  let q0 : 𝓞 K :=
    div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ e η0
  have hπ0 : π ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hq0 : π ∣ q0 := by
    simpa only [π, q0, η0] using
      (Ideal.Quotient.eq_zero_iff_dvd
        ((hζ.unit' : 𝓞 K) - 1)
        (div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ e
          (zeta_sub_one_dvd_root
            (by norm_num : 59 ≠ 2) hζ e hy))).mp
        (zeta_sub_one_dvd_root_spec
          (by norm_num : 59 ≠ 2) hζ e hy)
  have hetaPow : (η0 : 𝓞 K) ^ 59 = 1 := by
    exact (Polynomial.mem_nthRootsFinset
      (by norm_num : 0 < 59) (1 : 𝓞 K)).mp η0.prop
  obtain ⟨i, hi, heta⟩ :=
  ...
  · have hiPos : 0 < i := Nat.pos_of_ne_zero hi0
    ...
    have hj : j < 59 := by
      dsimp [j]
      omega
    let ηj : Polynomial.nthRootsFinset 59 (1 : 𝓞 K) :=
      ⟨(hζ.unit' : 𝓞 K) ^ j, by
        rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 59)]
        rw [← pow_mul, show j * 59 = 59 * j by omega, pow_mul,
          hζ.unit'_coe.pow_eq_one, one_pow]⟩
    let qj : 𝓞 K :=
      div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ e ηj
    have hzpowU : hζ.unit' ^ 59 = 1 := by
      apply Units.ext
      apply NumberField.RingOfIntegers.ext
      change ζ ^ 59 = 1
      exact hζ.pow_eq_one
...
it carries far more than the `60` local powers needed for Lemma 1. -/
theorem historicalState_omega_add_theta_highDivisibility59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 60 ∣ s.omega + s.theta := by
  let e := historicalState_regularEquation59 hζ s
  let hy := historicalState_theta_not_dvd59 hζ s
  have hroot := distinguishedRoot_eq_one_of_real59 hζ e hy hs.1 hs.2.1
  have hhigh := distinguishedFactor_highDivisibility
    (by norm_num : 59 ≠ 2) hζ e hy
  rw [hroot] at hhigh
  simp only [oneNthRoot, mul_one] at hhigh
  have hle : 60 ≤ (2 * s.m - 1) * 59 + 1 := by
    have hm := s.one_lt_m
    omega
...
`(ω + θ) / (ζ - 1)`. -/
noncomputable def historicalOneFactor59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (oneNthRoot (K := K) (p := 59))

/-- The allocated ideal root of the distinguished factor at `1`. -/
noncomputable def historicalOneFactorIdeal59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
...
noncomputable def historicalEquationEightAIdeal59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  a_eta_zero_dvd_p_pow (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)

/-- Multiplying the fixed-denominator factor at `1` back by `ζ - 1`
recovers `ω + θ`. -/
lemma historicalOneFactor_mul_zeta_sub_one59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    historicalOneFactor59 hζ s * ((hζ.unit' : 𝓞 K) - 1) =
      s.omega + s.theta := by
  simpa only [historicalOneFactor59, oneNthRoot, mul_one] using
...
/-- The allocated ideal at the root `1` has 59th power generated by its
literal fixed-denominator factor. -/
lemma historicalOneFactorIdeal_pow59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    historicalOneFactorIdeal59 hζ s ^ 59 =
      Ideal.span {historicalOneFactor59 hζ s} := by
  exact (linearFactorQuotient_span_eq_factorRoot_pow
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
    s.coprime_omega_theta
    (oneNthRoot (K := K) (p := 59))).symm

/-- The forced ramified part times the residual equation-(8a) ideal is the
allocated ideal at the root `1`. -/
lemma historicalEquationEightAIdeal_spec59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ^ (2 * s.m - 1) *
        historicalEquationEightAIdeal59 hζ s =
      historicalOneFactorIdeal59 hζ s := by
  have hspec := a_eta_zero_dvd_p_pow_spec
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
  rw [distinguishedRoot_eq_one_of_real59 hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s) hs.1 hs.2.1] at hspec
  simpa only [historicalEquationEightAIdeal59,
    historicalOneFactorIdeal59] using hspec

/-- The real factor `ω + θ` is nonzero in every historical state. -/
lemma historicalState_omega_add_theta_ne_zero59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
  ...
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit59_mul_sq]
    exact mul_ne_zero (kappaUnit59 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have hrhs :
      (s.eta : 𝓞 K) * (kappa hζ ^ s.m * s.xi) ^ 59 ≠ 0 :=
    mul_ne_zero s.eta.isUnit.ne_zero
      (pow_ne_zero 59
        (mul_ne_zero (pow_ne_zero s.m hkappa0) s.xi_ne_zero))
  apply hrhs
  rw [← s.equation, htheta]
  simp only [Odd.neg_pow (by norm_num : Odd 59), add_neg_cancel]

/-- The exact `κ`-power displayed in equation (8a) divides `ω + θ`.
The exponent identity is

`2 * (59*m - 29) = (2*m - 1)*59 + 1`.

Thus this is the element-level counterpart of the forced ramified ideal
factorization above. -/
lemma historicalState_kappaPower_dvd_omega_add_theta59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    kappa hζ ^ (59 * s.m - 29) ∣ s.omega + s.theta := by
  let E : ℕ := 59 * s.m - 29
  let N : ℕ := (2 * s.m - 1) * 59 + 1
  have hexp : 2 * E = N := by
    dsimp [E, N]
    have hm := s.one_lt_m
    omega
  have hroot := distinguishedRoot_eq_one_of_real59 hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s) hs.1 hs.2.1
  have hhigh := distinguishedFactor_highDivisibility
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
...
noncomputable def historicalEquationEightAQuotient59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) : 𝓞 K :=
  (historicalState_kappaPower_dvd_omega_add_theta59 hζ s hs).choose

/-- Definition equation for the integral quotient in (8a). -/
lemma historicalEquationEightAQuotient_spec59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    s.omega + s.theta =
      kappa hζ ^ (59 * s.m - 29) *
        historicalEquationEightAQuotient59 hζ s hs :=
  (historicalState_kappaPower_dvd_omega_add_theta59 hζ s hs).choose_spec

/-- The quotient occurring in equation (8a) is real. -/
lemma historicalEquationEightAQuotient_real59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (historicalEquationEightAQuotient59 hζ s hs) =
      historicalEquationEightAQuotient59 hζ s hs := by
  let E : ℕ := 59 * s.m - 29
  let q : 𝓞 K := historicalEquationEightAQuotient59 hζ s hs
  have hkappa0 : kappa hζ ^ E ≠ 0 := by
    rw [kappa_pow_eq_kappaUnit59_pow_mul]
    exact mul_ne_zero (kappaUnit59 hζ ^ E).isUnit.ne_zero
      (pow_ne_zero (2 * E) (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have hq := historicalEquationEightAQuotient_spec59 hζ s hs
...
lemma historicalEquationEightAQuotient_ne_zero59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAQuotient59 hζ s hs ≠ 0 := by
  intro hq
  apply historicalState_omega_add_theta_ne_zero59 hζ s
  rw [historicalEquationEightAQuotient_spec59 hζ s hs, hq, mul_zero]

/-- Principal-ideal form of the exact ramified exponent in equation (8a). -/
lemma historical_span_kappaPower59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    Ideal.span ({kappa hζ ^ (59 * s.m - 29)} : Set (𝓞 K)) =
  ...
  have hexp : 2 * E = N := by
    ...
    have hm := s.one_lt_m
    omega
  rw [show 59 * s.m - 29 = E by rfl,
    kappa_pow_eq_kappaUnit59_pow_mul, hexp]
  change Ideal.span
      ({((kappaUnit59 hζ ^ E : (𝓞 K)ˣ) : 𝓞 K) *
        ((hζ.unit' : 𝓞 K) - 1) ^ N} : Set (𝓞 K)) =
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ^ N
  rw [← Ideal.span_singleton_mul_span_singleton]
  rw [Ideal.span_singleton_eq_top.mpr
    (kappaUnit59 hζ ^ E).isUnit, ← Ideal.one_eq_top, one_mul]
  exact (Ideal.span_singleton_pow ((hζ.unit' : 𝓞 K) - 1) N).symm

set_option maxRecDepth 3000 in
/-- The residual equation-(8a) ideal has 59th power generated by the
literal real quotient `(ω + θ) / κ^(59*m-29)`. -/
theorem historicalEquationEightAIdeal_pow59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAIdeal59 hζ s ^ 59 =
      Ideal.span {historicalEquationEightAQuotient59 hζ s hs} := by
  let P : Ideal (𝓞 K) :=
    Ideal.span ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K))
  let A : Ideal (𝓞 K) := historicalOneFactorIdeal59 hζ s
  let J : Ideal (𝓞 K) := historicalEquationEightAIdeal59 hζ s
  let qone : 𝓞 K := historicalOneFactor59 hζ s
  let qzero : 𝓞 K := historicalEquationEightAQuotient59 hζ s hs
  let E : ℕ := 59 * s.m - 29
  let d : ℕ := 2 * s.m - 1
  let N : ℕ := d * 59 + 1
  have hPJ : P ^ d * J = A := by
    simpa only [P, d, J, A] using
      historicalEquationEightAIdeal_spec59 hζ s hs
  have hA59 : A ^ 59 = Ideal.span {qone} := by
    simpa only [A, qone] using historicalOneFactorIdeal_pow59 hζ s
  have hqone : qone * ((hζ.unit' : 𝓞 K) - 1) =
      s.omega + s.theta := by
    simpa only [qone] using historicalOneFactor_mul_zeta_sub_one59 hζ s
  have hqzero : s.omega + s.theta = kappa hζ ^ E * qzero := by
    simpa only [E, qzero] using
      historicalEquationEightAQuotient_spec59 hζ s hs
  have hspanK : Ideal.span {kappa hζ ^ E} = P ^ N := by
    simpa only [E, P, N, d] using historical_span_kappaPower59 hζ s
  have hramified : P ^ N * J ^ 59 =
      Ideal.span {s.omega + s.theta} := by
    calc
      P ^ N * J ^ 59 = P * (P ^ d * J) ^ 59 := by
        dsimp only [N]
        rw [mul_pow, ← pow_mul]
        rw [pow_succ']
        ring
      _ = P * A ^ 59 := by rw [hPJ]
      _ = P * Ideal.span {qone} := by rw [hA59]
      _ = Ideal.span
      ...
      _ = Ideal.span {s.omega + s.theta} := by
        congr 2
        rw [mul_comm, hqone]
  have hspanEq : Ideal.span {kappa hζ ^ E} * J ^ 59 =
      Ideal.span {kappa hζ ^ E} * Ideal.span {qzero} := by
    calc
      Ideal.span {kappa hζ ^ E} * J ^ 59 =
          P ^ N * J ^ 59 := by rw [hspanK]
...
noncomputable def historicalZetaFactor59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (zetaNthRoot (K := K) (p := 59) hζ)

/-- The fixed-denominator conjugate linear factor at `ζ⁻¹`. -/
noncomputable def historicalInverseZetaFactor59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : 𝓞 K :=
  div_zeta_sub_one (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (inverseZetaNthRoot (K := K) (p := 59) hζ)

/-- The unit-normalized inverse factor.  Multiplication by `-ζ` changes
the fixed denominator `ζ - 1` to its conjugate normalization. -/
noncomputable def normalizedHistoricalInverseZetaFactor59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : 𝓞 K :=
  ((-hζ.unit' : (𝓞 K)ˣ) : 𝓞 K) *
    historicalInverseZetaFactor59 hζ s

/-- The actual normalized factor product to which Vandiver applies
Lemma 1 is Kummer-primary in every real historical state. -/
theorem historicalConjugateFactorProduct_isKummerPrimary59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    IsKummerPrimary hζ
      (historicalZetaFactor59 hζ s *
        normalizedHistoricalInverseZetaFactor59 hζ s ^ 58) := by
  simpa only [historicalZetaFactor59,
    normalizedHistoricalInverseZetaFactor59,
    historicalInverseZetaFactor59] using
    normalizedConjugateLinearFactor_isKummerPrimary
      (by norm_num : 59 ≠ 2) hζ
      (historicalState_regularEquation59 hζ s)
      (historicalState_theta_not_dvd59 hζ s)
      (historicalState_omega_add_theta_highDivisibility59 hζ s hs)

/-- The allocated ideal root of the linear factor at `ζ`. -/
noncomputable def historicalZetaFactorIdeal59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
...
noncomputable def historicalInverseZetaFactorIdeal59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) : Ideal (𝓞 K) :=
  root_div_zeta_sub_one_dvd_gcd (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
...
set_option maxRecDepth 2000 in
/-- The allocated ideal at `ζ` has 59th power generated by the literal
historical linear factor. -/
theorem historicalZetaFactorIdeal_pow59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    historicalZetaFactorIdeal59 hζ s ^ 59 =
      Ideal.span {historicalZetaFactor59 hζ s} := by
  exact (linearFactorQuotient_span_eq_factorRoot_pow
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
    s.coprime_omega_theta
    (zetaNthRoot (K := K) (p := 59) hζ)).symm

set_option maxRecDepth 2000 in
/-- The allocated inverse ideal has 59th power generated by the
unit-normalized inverse factor. -/
theorem historicalInverseZetaFactorIdeal_pow59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    historicalInverseZetaFactorIdeal59 hζ s ^ 59 =
      Ideal.span {normalizedHistoricalInverseZetaFactor59 hζ s} := by
  calc
    historicalInverseZetaFactorIdeal59 hζ s ^ 59 =
        Ideal.span {historicalInverseZetaFactor59 hζ s} :=
      (linearFactorQuotient_span_eq_factorRoot_pow
        (by norm_num : 59 ≠ 2) hζ
        (historicalState_regularEquation59 hζ s)
        (historicalState_theta_not_dvd59 hζ s)
        s.coprime_omega_theta
        (inverseZetaNthRoot (K := K) (p := 59) hζ)).symm
    _ = Ideal.span {normalizedHistoricalInverseZetaFactor59 hζ s} :=
...
theorem ringOfIntegersComplexConj_historicalZetaFactor59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (historicalZetaFactor59 hζ s) =
      normalizedHistoricalInverseZetaFactor59 hζ s := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  let qplus : 𝓞 K := historicalZetaFactor59 hζ s
  let qminus : 𝓞 K := historicalInverseZetaFactor59 hζ s
  let u : (𝓞 K)ˣ := -hζ.unit'⁻¹
  have hπ0 : π ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hconjζ :
...
uniformizer. -/
theorem historicalZetaFactor_not_ramified59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ historicalZetaFactor59 hζ s := by
  have hlocal := normalizedConjugateLinearFactors
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
    (historicalState_omega_add_theta_highDivisibility59 hζ s hs)
  simpa only [historicalZetaFactor59] using hlocal.1

/-- The normalized historical factor and its conjugate agree modulo
`(ζ - 1)^59`.  This is the local compatibility behind Vandiver's claim
that the unit in equation (8) may be chosen real. -/
theorem historicalZetaFactor_sub_conj_highDivisibility59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ((hζ.unit' : 𝓞 K) - 1) ^ 59 ∣
      historicalZetaFactor59 hζ s -
        NumberField.IsCMField.ringOfIntegersComplexConj K
          (historicalZetaFactor59 hζ s) := by
  have hlocal := normalizedConjugateLinearFactors
    (by norm_num : 59 ≠ 2) hζ
    (historicalState_regularEquation59 hζ s)
    (historicalState_theta_not_dvd59 hζ s)
    (historicalState_omega_add_theta_highDivisibility59 hζ s hs)
  rw [ringOfIntegersComplexConj_historicalZetaFactor59 hζ s hs]
  simpa only [historicalZetaFactor59, historicalInverseZetaFactor59,
    normalizedHistoricalInverseZetaFactor59] using hlocal.2.2
...
theorem exists_historicalEquationSevenA59
    (hlemma : LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal59 hζ s *
          historicalInverseZetaFactorIdeal59 hζ s ^ 58 =
        Ideal.span {r} := by
  exact exists_equationSevenA_generator hlemma hζ
    (historicalZetaFactorIdeal59 hζ s)
    (historicalInverseZetaFactorIdeal59 hζ s)
    (historicalZetaFactor59 hζ s)
    (normalizedHistoricalInverseZetaFactor59 hζ s)
    (historicalZetaFactorIdeal_pow59 hζ s)
    (historicalInverseZetaFactorIdeal_pow59 hζ s)
    (by simpa using
      historicalConjugateFactorProduct_isKummerPrimary59 hζ s hs)
...
`RelevantIdealQuotientsPrincipal` at the irregular prime `59`: a genuine
`59`-torsion minus class may remain.  The definitions and theorems below
...
`I / conj(I)`.  Since `I ^ 59` is already principal, once the symmetric
...
/-- The exact class-group consequence of the exponent-`59`
Sinnott--Kummer theorem used by Vandiver in (7b) and again after (9): a
fractional ideal of the maximal real field whose `59`th power is principal
...
has `59`th power `(a)`, then it has a real generator `ρ`, and `a` differs
from `ρ ^ 59` by a real unit. -/
theorem exists_real_unit_mul_pow_generator59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
    (hpow : I ^ 59 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      I = Ideal.span {ρ} ∧ a = ε * ρ ^ 59 := by
  exact exists_unit_mul_pow_eq_of_ideal_pow_eq_span
    (F := K⁺) (p := 59) (by norm_num)
    (Fermat.FiftyNine.SinnottKummer.not_dvd_classNumber hzeta) I a hpow

set_option maxRecDepth 2000 in
/-- Vandiver's real-ideal step in the relative-norm form naturally
produced by (7b).  If `J ^ 59 = (a)` in the cyclotomic ring, multiplicativity
of the relative ideal norm gives

`Norm(J) ^ 59 = (intNorm(a))`

in the maximal real ring.  The unconditional result `59 ∤ h⁺` then
...
theorem exists_realGenerator_of_relativeNorm59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (J : Ideal (𝓞 K)) (a : 𝓞 K)
    (hpow : J ^ 59 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      Ideal.relNorm (𝓞 K⁺) J = Ideal.span {ρ} ∧
      Algebra.intNorm (𝓞 K⁺) (𝓞 K) a = ε * ρ ^ 59 := by
  apply exists_real_unit_mul_pow_generator59 hzeta
    (Ideal.relNorm (𝓞 K⁺) J) (Algebra.intNorm (𝓞 K⁺) (𝓞 K) a)
  calc
    Ideal.relNorm (𝓞 K⁺) J ^ 59 =
        Ideal.relNorm (𝓞 K⁺) (J ^ 59) := by
      rw [map_pow]
...
and `complexConj`. -/
theorem algebraMap_intNorm_eq_mul_conj59 (a : 𝓞 K) :
...
If `J ^ 59 = (a)`, relative ideal norm and `59 ∤ h⁺` produce a real
...
theorem exists_equationSevenD_of_idealPower59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (J : Ideal (𝓞 K)) (a : 𝓞 K)
    (hpow : J ^ 59 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      ...
      a * NumberField.IsCMField.ringOfIntegersComplexConj K a =
        algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
          algebraMap (𝓞 K⁺) (𝓞 K) ρ ^ 59 := by
  obtain ⟨ρ, ε, hI, ha⟩ :=
    exists_realGenerator_of_relativeNorm59 hzeta J a hpow
  refine ⟨ρ, ε, hI,
    (NumberField.IsCMField.ringOfIntegersComplexConj K).commutes ρ, ?_⟩
  rw [← algebraMap_intNorm_eq_mul_conj59]
...
exponent `59`.  Principality of `I * J^58` and `I * J` gives principality
of `J^57`; the coprime powers `57` and `59` then principalize `J`, and
division principalizes `I`. -/
theorem fractionalIdeal_isPrincipal_of_vandiverSeven59
    {A L : Type*} [CommRing A] [IsDedekindDomain A]
    [Field L] [Algebra A L] [IsFractionRing A L]
    {I J : FractionalIdeal A⁰ L} (hI0 : I ≠ 0) (hJ0 : J ≠ 0)
    (hJ59 : Submodule.IsPrincipal
      ((J ^ 59 : FractionalIdeal A⁰ L) : Submodule A L))
    (hsevenA : Submodule.IsPrincipal
      ((I * J ^ 58 : FractionalIdeal A⁰ L) : Submodule A L))
  ...
  have hJ57 : Submodule.IsPrincipal
      ((J ^ 57 : FractionalIdeal A⁰ L) : Submodule A L) := by
    have hquot := fractionalIdeal_isPrincipal_div hsevenA hsevenD
    have heq : (I * J ^ 58) / (I * J) = J ^ 57 := by
      apply (div_eq_iff (mul_ne_zero hI0 hJ0)).mpr
      rw [show J ^ 58 = J ^ 57 * J by rw [← pow_succ]]
      ac_rfl
    rw [← heq]
    exact hquot
  have hJ : Submodule.IsPrincipal (J : Submodule A L) :=
    fractionalIdeal_isPrincipal_of_coprime_powers (by norm_num) J hJ57 hJ59
...
`J = 𝔦₋ₐ`, then (7a) says `I * J^58` is principal, while (7d)
says `I * J` is principal.  Their quotient makes `J^57` principal.  Since
`J^59` is principal as well, the checked Bézout lemma
`fractionalIdeal_isPrincipal_of_vandiverSeven59` principalizes `I`.
Finally `I^59 = (a)` gives the element equation
`a = η * ρ^59`, which is exactly (8). -/
theorem exists_equationEight_of_sevenASevenD59
    (I J : Ideal (𝓞 K)) (a b r s : 𝓞 K)
    (hI0 : I ≠ 0) (hJ0 : J ≠ 0)
    (hIpow : I ^ 59 = Ideal.span {a})
    (hJpow : J ^ 59 = Ideal.span {b})
    (hsevenA : I * J ^ 58 = Ideal.span {r})
    (hsevenD : I * J = Ideal.span {s}) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      I = Ideal.span {ρ} ∧ a = η * ρ ^ 59 := by
  ...
  have hJF0 : JF ≠ 0 := by
    ...
    rw [FractionalIdeal.coeIdeal_eq_zero] at h
    exact hJ0 h
  have hJ59 : Submodule.IsPrincipal
      ((JF ^ 59 : FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K) := by
  ...
  have h7a : Submodule.IsPrincipal
      ((IF * JF ^ 58 : FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K) := by
    rw [FractionalIdeal.isPrincipal_iff]
    refine ⟨(r : K), ?_⟩
    dsimp [IF, JF]
  ...
  have hIF : Submodule.IsPrincipal (IF : Submodule (𝓞 K) K) :=
    fractionalIdeal_isPrincipal_of_vandiverSeven59 hIF0 hJF0 hJ59 h7a h7d
...
The primary element in Lemma 1 is exactly `a * b^58`: its principal ideal
is the 59th power of `I * J^58`.  `exists_equationSevenA_generator` turns
...
Lemma-1 theorem. -/
theorem exists_equationEight_of_lemmaOneSevenD59
    (hlemma : Fermat.Irregular.VandiverLemmaOne.LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    ...
    (hJpow : J ^ 59 = Ideal.span {b})
    (hprimary :
      Fermat.Irregular.VandiverLemmaOne.IsKummerPrimary hζ (a * b ^ 58))
    (hsevenD : I * J = Ideal.span {s}) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      I = Ideal.span {ρ} ∧ a = η * ρ ^ 59 := by
  obtain ⟨r, hsevenA⟩ :=
  ...
  exact exists_equationEight_of_sevenASevenD59
...
is therefore naturally `u * (a * b^58)`.  Since `u` does not change the
principal ideal, Lemma 1 still principalizes `I * J^58`; the rest is the
same checked Bézout calculation as in
`exists_equationEight_of_lemmaOneSevenD59`. -/
theorem exists_equationEight_of_lemmaOneSevenDUnit59
    (hlemma : Fermat.Irregular.VandiverLemmaOne.LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
...
inverse factor `q₋`.  Comparing 59th powers of integral ideals and using
injectivity of the power map then proves that the product of the two
allocated factor ideals is `(ρ)`. -/
theorem exists_historicalEquationSevenD59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal59 hζ s *
          historicalInverseZetaFactorIdeal59 hζ s =
        Ideal.span {r} := by
  let I : Ideal (𝓞 K) := historicalZetaFactorIdeal59 hζ s
  let J : Ideal (𝓞 K) := historicalInverseZetaFactorIdeal59 hζ s
  let a : 𝓞 K := historicalZetaFactor59 hζ s
  let b : 𝓞 K := normalizedHistoricalInverseZetaFactor59 hζ s
  have hIpow : I ^ 59 = Ideal.span {a} := by
    simpa only [I, a] using historicalZetaFactorIdeal_pow59 hζ s
  have hJpow : J ^ 59 = Ideal.span {b} := by
    simpa only [J, b] using historicalInverseZetaFactorIdeal_pow59 hζ s
  have hbconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K a = b := by
    simpa only [a, b] using
      ringOfIntegersComplexConj_historicalZetaFactor59 hζ s hs
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower59 hζ I a hIpow
  ...
  have hpow : (I * J) ^ 59 = (Ideal.span {ρK}) ^ 59 := by
    calc
      ...
      _ = Ideal.span {(εK : 𝓞 K) * ρK ^ 59} := by rw [hnorm']
      _ = Ideal.span {ρK ^ 59} :=
        Ideal.span_singleton_eq_span_singleton.mpr
          (associated_unit_mul_left _ _ εK.isUnit)
      _ = (Ideal.span {ρK}) ^ 59 :=
        (Ideal.span_singleton_pow ρK 59).symm
  exact ⟨ρK,
    pow_left_injective (M := Ideal (𝓞 K))
      (by norm_num : 59 ≠ 0) hpow⟩
...
theorem historicalFactorIdeals_ne_zero59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    historicalZetaFactorIdeal59 hζ s ≠ 0 ∧
      historicalInverseZetaFactorIdeal59 hζ s ≠ 0 := by
  let a : 𝓞 K := historicalZetaFactor59 hζ s
  let b : 𝓞 K := normalizedHistoricalInverseZetaFactor59 hζ s
  have hprimary :=
    historicalConjugateFactorProduct_isKummerPrimary59 hζ s hs
  have hab0 : a * b ^ 58 ≠ 0 := by
    intro hzero
    apply hprimary.1
    change (hζ.unit' : 𝓞 K) - 1 ∣ a * b ^ 58
    rw [hzero]
    exact dvd_zero _
  have ha0 : a ≠ 0 := left_ne_zero_of_mul hab0
  have hbpow0 : b ^ 58 ≠ 0 := right_ne_zero_of_mul hab0
  have hb0 : b ≠ 0 := by
    intro hzero
    apply hbpow0
    rw [hzero]
  ...
  · intro hzero
    apply hspanA
    rw [← historicalZetaFactorIdeal_pow59 hζ s, hzero]
    norm_num
  · intro hzero
    apply hspanB
    rw [← historicalInverseZetaFactorIdeal_pow59 hζ s, hzero]
    norm_num

set_option maxRecDepth 3000 in
/-- Vandiver's equation (8), constructed from an arbitrary real historical
state at exponent `59`.
...
theorem exists_historicalEquationEight59
    (hlemma : LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal59 hζ s = Ideal.span {ρ} ∧
      historicalZetaFactor59 hζ s = η * ρ ^ 59 := by
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA59 hlemma hζ s hs
  obtain ⟨t, hsevenD⟩ := exists_historicalEquationSevenD59 hζ s hs
  obtain ⟨hI0, hJ0⟩ := historicalFactorIdeals_ne_zero59 hζ s hs
  exact exists_equationEight_of_sevenASevenD59
    (historicalZetaFactorIdeal59 hζ s)
    (historicalInverseZetaFactorIdeal59 hζ s)
    (historicalZetaFactor59 hζ s)
...
The factor at `ζ` agrees with its conjugate modulo `(ζ - 1)^59`, and so
does every 59th power.  After cancelling the generator, which is prime to
`ζ - 1`, the unit's conjugation defect is divisible by `(ζ - 1)^2`.
`unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj59` then removes its possible
cyclotomic torsion component.  This is the compatibility needed to obtain
equation (8) at `a` and `-a` with one common unit. -/
theorem historicalEquationEight_unit_real59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) (ρ : 𝓞 K) (η : (𝓞 K)ˣ)
    (heq : historicalZetaFactor59 hζ s = η * ρ ^ 59) :
    NumberField.IsCMField.unitsComplexConj K η = η := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  have hqnot : ¬ π ∣ historicalZetaFactor59 hζ s := by
    simpa only [π] using historicalZetaFactor_not_ramified59 hζ s hs
  have hρnot : ¬ π ∣ ρ := by
    intro hρ
    apply hqnot
    rw [heq]
    exact dvd_mul_of_dvd_right (dvd_pow (n := 59) hρ (by norm_num)) _
  have hρpowNot : ¬ π ∣ ρ ^ 59 := by
    intro h
    exact hρnot ((hζ.zeta_sub_one_prime'.dvd_pow_iff_dvd
      (by norm_num : 59 ≠ 0)).mp h)
  have hq59 := historicalZetaFactor_sub_conj_highDivisibility59 hζ s hs
  have hq2 : π ^ 2 ∣
      historicalZetaFactor59 hζ s -
        NumberField.IsCMField.ringOfIntegersComplexConj K
          (historicalZetaFactor59 hζ s) := by
    exact (pow_dvd_pow_of_dvd_of_le (dvd_refl π) (by norm_num : 2 ≤ 59)).trans
      (by simpa only [π] using hq59)
  have hρ59 :=
    zeta_sub_one_pow_fiftyNine_dvd_pow_sub_conj_pow59 hζ ρ
  have hρ2 : π ^ 2 ∣
      ρ ^ 59 -
        NumberField.IsCMField.ringOfIntegersComplexConj K ρ ^ 59 := by
    exact (pow_dvd_pow_of_dvd_of_le (dvd_refl π) (by norm_num : 2 ≤ 59)).trans
      (by simpa only [π] using hρ59)
  have herror : π ^ 2 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) *
        (ρ ^ 59 -
          NumberField.IsCMField.ringOfIntegersComplexConj K ρ ^ 59) :=
  ...
  have hunit : π ^ 2 ∣
    ...
    obtain ⟨d₁, hd₁⟩ := hd
    obtain ⟨c, hc⟩ := hmul
    have hcancel : d₁ * ρ ^ 59 = π * c := by
      apply mul_left_cancel₀ hπ0
      calc
        π * (d₁ * ρ ^ 59) = d * ρ ^ 59 := by rw [hd₁]; ring
        _ = π ^ 2 * c := hc
        _ = π * (π * c) := by ring
    have hd₁div : π ∣ d₁ := by
      have hπd₁ : π ∣ d₁ * ρ ^ 59 := ⟨c, hcancel⟩
    ...
    rw [hd₁, hd₂]
    ring
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj59 hζ η
...
theorem exists_historicalEquationEight_realUnit59
    (hlemma : LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal59 hζ s = Ideal.span {ρ} ∧
      historicalZetaFactor59 hζ s = η * ρ ^ 59 ∧
      NumberField.IsCMField.unitsComplexConj K η = η := by
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight59 hlemma hζ s hs
  exact ⟨ρ, η, hI, heq,
    historicalEquationEight_unit_real59 hζ s hs ρ η heq⟩
...
exactly the same `-η`. -/
theorem exists_historicalEquationEight_pair_one59
    (hlemma : LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  have hqmul : historicalZetaFactor59 hζ s *
      ((hζ.unit' : 𝓞 K) - 1) =
  ...
  have hminus :
      s.omega + (hζ.unit'⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit'⁻¹ : (𝓞 K)ˣ)) * η * ρminus ^ 59 := by
    have hc := congrArg
  ...
  have hρnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρ := by
    intro hρ
    apply historicalZetaFactor_not_ramified59 hζ s hs
    rw [heq]
    exact dvd_mul_of_dvd_right (dvd_pow (n := 59) hρ (by norm_num)) _
  have hρminusNot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρminus := by
    intro hρminus
    have hc := zeta_sub_one_pow_dvd_conj_of_dvd59 hζ 1 ρminus
...
lemma equationEight_pair_difference59
  ...
  have hfirst : (1 - (t : 𝓞 K)) * eta * rhoa ^ 59 =
      omega + (t : 𝓞 K) * theta := ha.symm
  have hsecond : (1 - (t : 𝓞 K)) * eta * rhominus ^ 59 =
      -((t : 𝓞 K) * omega + theta) := by
    rw [hcoef]
    calc
      (-(t : 𝓞 K) * (1 - (t⁻¹ : (𝓞 K)ˣ))) * eta * rhominus ^ 59 =
          -(t : 𝓞 K) *
            ((1 - (t⁻¹ : (𝓞 K)ˣ)) * eta * rhominus ^ 59) := by ring
      _ = -(t : 𝓞 K) *
...
`2 * (59*m - 29) - 1 = (2*m - 1) * 59`.

No ideal-class hypothesis occurs here. -/
theorem exists_equationEight_difference59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (a m : ℕ) (ha : a.Coprime 59) (hm : 1 < m)
    (omega theta rhoa rhominus rhozero : 𝓞 K)
    (etaa etazero : (𝓞 K)ˣ)
    (hea : omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * theta =
      (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) * etaa * rhoa ^ 59)
    (heminus : omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * theta =
      (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) * etaa * rhominus ^ 59)
    (hezero : omega + theta =
      etazero * kappa hζ ^ (59 * m - 29) * rhozero ^ 59) :
    ∃ ε : (𝓞 K)ˣ,
      rhoa ^ 59 - rhominus ^ 59 =
        ε * (((hζ.unit' : 𝓞 K) - 1) ^ (2 * m - 1) * rhozero) ^ 59 := by
  let t : (𝓞 K)ˣ := hζ.unit' ^ a
  have htprim : IsPrimitiveRoot (t : 𝓞 K) 59 :=
    hζ.unit'_coe.pow_of_coprime a ha
  let hplus : IsUnit ((1 : 𝓞 K) + t) := by
    simpa [add_comm] using
      htprim.geom_sum_isUnit (by norm_num) (by norm_num : Nat.Coprime 2 59)
  ...
  have helim := equationEight_pair_difference59 t etaa omega theta rhoa rhominus
  ...
  let rightUnit : (𝓞 K)ˣ := uplus * etazero * kappaUnit59 hζ ^ E
  have hpi : (hζ.unit' : 𝓞 K) - 1 ≠ 0 :=
    hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hcancel : (leftUnit : 𝓞 K) * (rhoa ^ 59 - rhominus ^ 59) =
      (rightUnit : 𝓞 K) *
        ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 59 := by
    apply mul_left_cancel₀ hpi
    calc
      ((hζ.unit' : 𝓞 K) - 1) *
          ((leftUnit : 𝓞 K) * (rhoa ^ 59 - rhominus ^ 59)) =
          (1 - (t : 𝓞 K)) * etaa *
            (rhoa ^ 59 - rhominus ^ 59) := by
        dsimp [leftUnit]
        rw [huden]
        ring
      ...
      _ = ((hζ.unit' : 𝓞 K) - 1) *
        ...
        have hezero' : omega + theta =
            etazero * kappa hζ ^ E * rhozero ^ 59 := by
  ...
  refine ⟨ε, ?_⟩
  have hdiff : rhoa ^ 59 - rhominus ^ 59 =
      (ε : 𝓞 K) * ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 59 := by
    calc
      rhoa ^ 59 - rhominus ^ 59 =
          (leftUnit⁻¹ : (𝓞 K)ˣ) *
            ((leftUnit : 𝓞 K) * (rhoa ^ 59 - rhominus ^ 59)) := by
        rw [← mul_assoc, ← Units.val_mul]
        simp
      _ = (leftUnit⁻¹ : (𝓞 K)ˣ) *
          ((rightUnit : 𝓞 K) *
            ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 59) := by rw [hcancel]
      _ = (ε : 𝓞 K) * ((hζ.unit' : 𝓞 K) - 1) ^ N * rhozero ^ 59 := by
        dsimp [ε]
        ring
  rw [hdiff]
  dsimp [N]
...
def conjugateIdeal59 (I : Ideal (𝓞 K)) : Ideal (𝓞 K) :=
...
cyclotomic ring. -/
def extendRealIdeal59 (I : Ideal (𝓞 K⁺)) : Ideal (𝓞 K) :=
  I.map (algebraMap (𝓞 K⁺) (𝓞 K))

/-- An extended real ideal is fixed by CM complex conjugation. -/
@[simp] theorem conjugateIdeal59_extendRealIdeal59
    (I : Ideal (𝓞 K⁺)) :
    conjugateIdeal59 (extendRealIdeal59 I) = extendRealIdeal59 I := by
  rw [conjugateIdeal59, extendRealIdeal59, Ideal.map_map]
  have hmap :
...
theorem extendRealIdeal59_span (ρ : 𝓞 K⁺) :
    extendRealIdeal59 (Ideal.span {ρ}) =
      Ideal.span {algebraMap (𝓞 K⁺) (𝓞 K) ρ} := by
  simp only [extendRealIdeal59, Ideal.map_span, Set.image_singleton]
...
`𝓞 K⁺ → 𝓞 K`. -/
theorem extendRealIdeal59_injective :
    Function.Injective (extendRealIdeal59 (K := K)) := by
  intro I J hIJ
  have hcomap := congrArg
    (Ideal.comap (algebraMap (𝓞 K⁺) (𝓞 K))) hIJ
  simpa only [extendRealIdeal59,
    Ideal.comap_map_eq_self_of_faithfullyFlat] using hcomap

omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
/-- Extension commutes with ideal powers. -/
theorem extendRealIdeal59_pow (I : Ideal (𝓞 K⁺)) (n : ℕ) :
    extendRealIdeal59 (I ^ n) = extendRealIdeal59 I ^ n := by
  exact Ideal.map_pow (algebraMap (𝓞 K⁺) (𝓞 K)) I n

set_option maxRecDepth 2000 in
/-- The complete plus-class-number bridge needed at each of Vandiver's
real ideals in (7b) and (9).  A real ideal with 59th power `(a)` extends to
...
the extension of `I` and verify the 59th-power equality `hpow`; all class
number and generator extraction work is discharged here. -/
theorem exists_extendedRealGenerator59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
    (hpow : I ^ 59 = Ideal.span {a}) :
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      ...
      conjugateIdeal59 (extendRealIdeal59 I) = extendRealIdeal59 I ∧
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (algebraMap (𝓞 K⁺) (𝓞 K) ρ) =
        algebraMap (𝓞 K⁺) (𝓞 K) ρ ∧
      algebraMap (𝓞 K⁺) (𝓞 K) a =
        algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
          algebraMap (𝓞 K⁺) (𝓞 K) ρ ^ 59 := by
  obtain ⟨ρ, ε, hI, ha⟩ := exists_real_unit_mul_pow_generator59 hzeta I a hpow
  refine ⟨ρ, ε, ?_, conjugateIdeal59_extendRealIdeal59 I,
    (NumberField.IsCMField.ringOfIntegersComplexConj K).commutes ρ, ?_⟩
  · rw [hI, extendRealIdeal59_span]
  · rw [ha, map_mul, map_pow]

set_option maxRecDepth 2000 in
/-- Version of `exists_extendedRealGenerator59` whose 59th-power identity
...
to the real ideal, where `59 ∤ h⁺` supplies the generator. -/
theorem exists_extendedRealGenerator59_of_pow_eq_span
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (I : Ideal (𝓞 K⁺)) (a : 𝓞 K⁺)
    (hpow : extendRealIdeal59 I ^ 59 =
    ...
    ∃ (ρ : 𝓞 K⁺) (ε : (𝓞 K⁺)ˣ),
      ...
      algebraMap (𝓞 K⁺) (𝓞 K) a =
        algebraMap (𝓞 K⁺) (𝓞 K) (ε : 𝓞 K⁺) *
          algebraMap (𝓞 K⁺) (𝓞 K) ρ ^ 59 := by
  apply exists_extendedRealGenerator59 hzeta I a
  apply extendRealIdeal59_injective (K := K)
  rw [extendRealIdeal59_pow, extendRealIdeal59_span]
  exact hpow

/-- Complex conjugation preserves whether an integral ideal is zero. -/
@[simp] lemma conjugateIdeal59_eq_zero_iff (I : Ideal (𝓞 K)) :
    conjugateIdeal59 I = 0 ↔ I = 0 := by
...
denominator. -/
def conjugateIdealQuotient59 (A B : Ideal (𝓞 K)) :
    FractionalIdeal (𝓞 K)⁰ K :=
  (conjugateIdeal59 A : FractionalIdeal (𝓞 K)⁰ K) /
    (conjugateIdeal59 B : FractionalIdeal (𝓞 K)⁰ K)

/-- The actual fractional ideal whose principality is requested by
`RelevantIdealQuotientsPrincipal` at exponent `59`. -/
noncomputable def relevantIdealQuotient59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 59 + y ^ 59 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 59)
    (hy : ¬ hζ.unit'.1 - 1 ∣ y)
    (η : Polynomial.nthRootsFinset 59 (1 : 𝓞 K)) :
    FractionalIdeal (𝓞 K)⁰ K :=
  (root_div_zeta_sub_one_dvd_gcd (K := K) (p := 59)
      (x := x) (y := y) (z := z) (ε := ε₀) (m := m)
        (by norm_num) hζ e hy η : FractionalIdeal (𝓞 K)⁰ K) /
    (a_eta_zero_dvd_p_pow (K := K) (p := 59)
      (x := x) (y := y) (z := z) (ε := ε₀) (m := m)
        (by norm_num) hζ e hy : FractionalIdeal (𝓞 K)⁰ K)

/-- The conjugate of the relevant exponent-`59` quotient. -/
noncomputable def conjugateRelevantIdealQuotient59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 59 + y ^ 59 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 59)
...
def RelevantIdealQuotientPlusComponentsPrincipal59 : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 59 + y ^ 59 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 59)
    ...
    Submodule.IsPrincipal
      (((relevantIdealQuotient59 hζ e hy η *
          conjugateRelevantIdealQuotient59 hζ e hy η) :
        FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K)

/-- Every antisymmetric relevant quotient class is principal.  At `59`,
this is exactly the residual minus-component assertion that cannot follow
from `59 ∤ h⁺` alone. -/
def RelevantIdealQuotientMinusComponentsPrincipal59 : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {x y z : 𝓞 K} {ε₀ : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ 59 + y ^ 59 =
      ε₀ * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ 59)
...
lemma relevantIdealQuotient59_conjugate_eq_zero_iff
    (η : Polynomial.nthRootsFinset 59 (1 : 𝓞 K)) :
    ((𝔞 η / 𝔞₀ : FractionalIdeal (𝓞 K)⁰ K) = 0) ↔
      conjugateIdealQuotient59 (𝔞 η) 𝔞₀ = 0 := by
  simp only [conjugateIdealQuotient59, div_eq_mul_inv,
    mul_eq_zero, inv_eq_zero, FractionalIdeal.coeIdeal_eq_zero]
  constructor <;> rintro (h | h)
  · exact Or.inl ((conjugateIdeal59_eq_zero_iff _).mpr h)
  · exact Or.inr ((conjugateIdeal59_eq_zero_iff _).mpr h)
  · exact Or.inl ((conjugateIdeal59_eq_zero_iff _).mp h)
  · exact Or.inr ((conjugateIdeal59_eq_zero_iff _).mp h)
...
The proof uses the unconditional fact that the quotient's `59`th power is
principal and the coprimality of `59` and `2`. -/
lemma relevantIdealQuotient59_isPrincipal_iff_minus
    (η : Polynomial.nthRootsFinset 59 (1 : 𝓞 K))
    (hplus : Submodule.IsPrincipal
      ((((𝔞 η / 𝔞₀ : FractionalIdeal (𝓞 K)⁰ K) *
          conjugateIdealQuotient59 (𝔞 η) 𝔞₀) :
        FractionalIdeal (𝓞 K)⁰ K) : Submodule (𝓞 K) K)) :
    Submodule.IsPrincipal
  ...
  let J : FractionalIdeal (𝓞 K)⁰ K :=
    conjugateIdealQuotient59 (𝔞 η) 𝔞₀
  have hzero : I = 0 ↔ J = 0 :=
    relevantIdealQuotient59_conjugate_eq_zero_iff hζ e hy η
  ...
  · have hJ0 : J ≠ 0 := fun h ↦ hI0 (hzero.mpr h)
    apply fractionalIdeal_isPrincipal_iff_minus_of_plus (p := 59)
      (by norm_num) hI0 hJ0
    · exact relevantIdealQuotient_pow_isPrincipal
        (K := K) (p := 59) (by norm_num) hζ e hy η
    · exact hplus
...
theorem relevantIdealQuotientsPrincipal_iff_minus_of_plus59
    (hplus : RelevantIdealQuotientPlusComponentsPrincipal59 (K := K)) :
    RelevantIdealQuotientsPrincipal (K := K) (p := 59) (by norm_num) ↔
      RelevantIdealQuotientMinusComponentsPrincipal59 (K := K) := by
  constructor
  · intro hprincipal ζ hζ x y z ε₀ m e hy η
    have hI := hprincipal hζ e hy η
    have hiff := relevantIdealQuotient59_isPrincipal_iff_minus hζ e hy η
      (hplus hζ e hy η)
    simpa only [relevantIdealQuotient59,
      conjugateRelevantIdealQuotient59] using hiff.mp hI
  · intro hminus ζ hζ x y z ε₀ m e hy η
    have hiff := relevantIdealQuotient59_isPrincipal_iff_minus hζ e hy η
      (hplus hζ e hy η)
    apply hiff.mpr
    simpa only [relevantIdealQuotient59,
      conjugateRelevantIdealQuotient59] using hminus hζ e hy η
...
lemma ringOfIntegersComplexConj_eq_mod_zeta_sub_one59
...
lemma conjugateIdeal59_span (a : 𝓞 K) :
    conjugateIdeal59 (Ideal.span {a}) =
      Ideal.span {ringOfIntegersComplexConj K a} := by
  simp only [conjugateIdeal59, Ideal.map_span, Set.image_singleton]
  rfl
...
because `2 ∉ (ζ - 1)`. -/
lemma conjugation_eq_zeta_pow_of_stable_principal59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K)
    (ha : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a)
    (hstable : conjugateIdeal59 (Ideal.span {a}) = Ideal.span {a}) :
    ∃ j : ℕ, ringOfIntegersComplexConj K a =
      (hζ.unit' ^ j : (𝓞 K)ˣ) * a := by
  have hassoc : Associated (ringOfIntegersComplexConj K a) a := by
    rw [← Ideal.span_singleton_eq_span_singleton]
    rw [← conjugateIdeal59_span]
    exact hstable
  obtain ⟨u, hu⟩ := hassoc
  let v : (𝓞 K)ˣ := u⁻¹
  have ha0 : a ≠ 0 := by
    ...
    have hvQ : algebraMap (𝓞 K) Q (v : 𝓞 K) = 1 := by
      ...
      calc
        ...
        _ = algebraMap (𝓞 K) Q a :=
          ringOfIntegersComplexConj_eq_mod_zeta_sub_one59 hζ a
...
quotient of a stable principal ideal a pure `59`th root of unity.
...
lemma conjugation_sq_eq_zeta_pow_of_stable_principal59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K)
    (ha0 : a ≠ 0)
    (hstable : conjugateIdeal59 (Ideal.span {a}) = Ideal.span {a}) :
...
conjugation exponent required by `ConjugationPowerReductionData59`. -/
lemma exists_conjugation_power_generator59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (I : Ideal (𝓞 K))
    (hprincipal : Submodule.IsPrincipal
      (I : Submodule (𝓞 K) (𝓞 K)))
    (hprime : ¬ Ideal.span
      ({(hζ.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ∣ I)
    (hstable : conjugateIdeal59 I = I) :
    ∃ (a : 𝓞 K) (j : ℕ),
      I = Ideal.span {a} ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a ∧
      ringOfIntegersComplexConj K a =
  ...
  have ha : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ a := by
    ...
    exact ha
  have hstableA : conjugateIdeal59 (Ideal.span {a}) =
      Ideal.span {a} := by
    rw [← haI]
    exact hstable
  obtain ⟨j, hj⟩ := conjugation_eq_zeta_pow_of_stable_principal59
    hζ a ha hstableA
  exact ⟨a, j, haI, ha, hj⟩

end StablePrincipalGenerator

/-- The inverse of 2 modulo 59, used to make a generator real. -/
def realGeneratorHalfExponent59 : ℕ := 30

theorem two_mul_realGeneratorHalfExponent59_mod :
    2 * realGeneratorHalfExponent59 % 59 = 1 := by
  decide

/-- The explicit adjustment of a principal generator by the half-power of
its cyclotomic conjugation quotient. -/
def realAdjustedGenerator59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (a : 𝓞 K) (j : ℕ) : 𝓞 K :=
  (hζ.unit' ^ (realGeneratorHalfExponent59 * j) : (𝓞 K)ˣ) * a

omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
lemma realAdjustedGenerator59_associated {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K) (j : ℕ) :
    Associated (realAdjustedGenerator59 hζ a j) a := by
  let v : (𝓞 K)ˣ := hζ.unit' ^ (realGeneratorHalfExponent59 * j)
  refine ⟨v⁻¹, ?_⟩
...
omit [NumberField K] [IsCyclotomicExtension {59} ℚ K] in
lemma realAdjustedGenerator59_pow_fiftyNine {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K) (j : ℕ) :
    realAdjustedGenerator59 hζ a j ^ 59 = a ^ 59 := by
  rw [realAdjustedGenerator59, mul_pow]
  have hzpow : hζ.unit' ^ 59 = 1 := by
    ext
    exact hζ.pow_eq_one
  have hvpow :
      (hζ.unit' ^ (realGeneratorHalfExponent59 * j)) ^ 59 = 1 := by
    rw [← pow_mul]
    rw [show (realGeneratorHalfExponent59 * j) * 59 =
      59 * (realGeneratorHalfExponent59 * j) by omega]
    rw [pow_mul, hzpow, one_pow]
  rw [← Units.val_pow_eq_pow_val, hvpow]
  simp

lemma realAdjustedGenerator59_real
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (a : 𝓞 K) (j : ℕ)
    (ha : NumberField.IsCMField.ringOfIntegersComplexConj K a =
      (hζ.unit' ^ j : (𝓞 K)ˣ) * a) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
      (realAdjustedGenerator59 hζ a j) =
        realAdjustedGenerator59 hζ a j := by
  let k := realGeneratorHalfExponent59 * j
  let v : (𝓞 K)ˣ := hζ.unit' ^ k
  have hzpow : hζ.unit' ^ 59 = 1 := by
    ext
    exact hζ.pow_eq_one
  have hv_sq : v ^ 2 = hζ.unit' ^ j := by
    dsimp [v, k, realGeneratorHalfExponent59]
    rw [← pow_mul]
    have hexp : (30 * j) * 2 = j + 59 * j := by omega
  ...
  have hvconj : NumberField.IsCMField.ringOfIntegersComplexConj K (v : 𝓞 K) =
    ...
    have hvconjU :
      ...
      dsimp [v]
      rw [map_pow, unitsComplexConj_zeta59 hζ, inv_pow]
...
ζ ^ j, multiplying the generator by ζ ^ (30 * j) makes it real.

This is the explicit odd-order normalization in the real-generator step
behind Vandiver's equations (7b)--(10a): 30 is the inverse of 2 modulo
59, so

conj (ζ ^ (30*j) * a) = ζ ^ (j-30*j) * a = ζ ^ (30*j) * a.

The resulting generator is associated to the original one, hence generates
the same principal ideal. What remains in
RealPrincipalGeneratorElimination59 is to prove that the conjugation
...
lemma exists_real_associated_generator_of_conj_eq_zeta_pow
  ...
  exact ⟨realAdjustedGenerator59 hζ a j,
    realAdjustedGenerator59_associated hζ a j,
    realAdjustedGenerator59_real hζ a j ha⟩

set_option maxRecDepth 3000 in
/-- The real-generator conclusion used immediately after Vandiver's
equation (9).

Suppose `J ^ 59 = (q)`, the element `q` is fixed by conjugation, and `J` is
...
59th-power map on the unique-factorization monoid of integral ideals shows
that `J²` is principal.  Since `J^59` is principal and
`gcd(2,59)=1`, `J` itself is principal.  Its conjugation stability follows
from the reality of `q`; the explicit `ζ^(30*j)` normalization then
chooses a real generator `μ`.  Finally

`q = η * μ^59`,

which is the element equation written after (9) in the 1929 paper. -/
theorem exists_realEquationNineGenerator59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (J : Ideal (𝓞 K)) (q : 𝓞 K)
    (hprime : ¬ Ideal.span
      ({(hzeta.unit' : 𝓞 K) - 1} : Set (𝓞 K)) ∣ J)
    (hqreal : NumberField.IsCMField.ringOfIntegersComplexConj K q = q)
    (hpow : J ^ 59 = Ideal.span {q}) :
    ∃ (μ : 𝓞 K) (η : (𝓞 K)ˣ),
      J = Ideal.span {μ} ∧
      NumberField.IsCMField.ringOfIntegersComplexConj K μ = μ ∧
      q = η * μ ^ 59 := by
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower59 hzeta J q hpow
  let ρK : 𝓞 K := algebraMap (𝓞 K⁺) (𝓞 K) ρ
  ...
  have hassoc : Associated (q ^ 2) (ρK ^ 59) := by
    ...
    rw [hq_sq]
    calc
      ((εK : 𝓞 K) * ρK ^ 59) * (εK⁻¹ : (𝓞 K)ˣ) =
          ρK ^ 59 * ((εK : 𝓞 K) * (εK⁻¹ : (𝓞 K)ˣ)) := by
        ac_rfl
      _ = ρK ^ 59 := by rw [← Units.val_mul]; simp
  have hspan : Ideal.span {q ^ 2} = Ideal.span {ρK ^ 59} :=
    Ideal.span_singleton_eq_span_singleton.mpr hassoc
  have hpoweq : (J ^ 2) ^ 59 = (Ideal.span {ρK}) ^ 59 := by
    calc
      (J ^ 2) ^ 59 = (J ^ 59) ^ 2 := by
        rw [← pow_mul, ← pow_mul]
      _ = (Ideal.span {q}) ^ 2 := by rw [hpow]
      _ = Ideal.span {q ^ 2} := Ideal.span_singleton_pow q 2
      _ = Ideal.span {ρK ^ 59} := hspan
      _ = (Ideal.span {ρK}) ^ 59 := (Ideal.span_singleton_pow ρK 59).symm
  have hJ2eq : J ^ 2 = Ideal.span {ρK} :=
    pow_left_injective (M := Ideal (𝓞 K)) (by norm_num : 59 ≠ 0) hpoweq
  have hJ2 : Submodule.IsPrincipal (J ^ 2 : Ideal (𝓞 K)) := by
    rw [hJ2eq]
    infer_instance
  have hJ59 : Submodule.IsPrincipal (J ^ 59 : Ideal (𝓞 K)) := by
    rw [hpow]
    infer_instance
  have hJprincipal : Submodule.IsPrincipal (J : Ideal (𝓞 K)) :=
    ideal_isPrincipal_of_coprime_powers (L := K) (by norm_num) J hJ2 hJ59
  have hstable : conjugateIdeal59 J = J := by
    apply pow_left_injective (M := Ideal (𝓞 K)) (by norm_num : 59 ≠ 0)
    calc
      conjugateIdeal59 J ^ 59 = conjugateIdeal59 (J ^ 59) := by
        exact (Ideal.map_pow
          (NumberField.IsCMField.ringOfIntegersComplexConj K).toRingHom J 59).symm
      _ = conjugateIdeal59 (Ideal.span {q}) := by rw [hpow]
      _ = Ideal.span
          {NumberField.IsCMField.ringOfIntegersComplexConj K q} :=
        conjugateIdeal59_span q
      _ = Ideal.span {q} := by rw [hqreal]
      _ = J ^ 59 := hpow.symm
  obtain ⟨a, j, hJa, -, hconj⟩ := exists_conjugation_power_generator59
  ...
  have hassoc_q : Associated (μ ^ 59) q := by
    rw [← Ideal.span_singleton_eq_span_singleton,
...
If `J ^ 59 = (q)` and `q` is real, relative norm makes `J ^ 2`
principal.  The coprime exponents `2` and `59` then principalize `J`, and
...
theorem ideal_isPrincipal_and_stable_of_real_pow59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (J : Ideal (𝓞 K)) (q : 𝓞 K)
    (hqreal : NumberField.IsCMField.ringOfIntegersComplexConj K q = q)
    (hpow : J ^ 59 = Ideal.span {q}) :
    Submodule.IsPrincipal (J : Ideal (𝓞 K)) ∧
      conjugateIdeal59 J = J := by
  obtain ⟨ρ, ε, -, -, hnorm⟩ :=
    exists_equationSevenD_of_idealPower59 hzeta J q hpow
  let ρK : 𝓞 K := algebraMap (𝓞 K⁺) (𝓞 K) ρ
  let εK : (𝓞 K)ˣ := Units.map
...
theorem exists_squaredConjugationGenerator_of_real_pow59
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
  ...
  obtain ⟨hprincipal, hstable⟩ :=
    ideal_isPrincipal_and_stable_of_real_pow59 hzeta J q hqreal hpow
  ...
  have hρ0 : ρ ≠ 0 := by
    ...
    calc
      Ideal.span {q} = J ^ 59 := hpow.symm
      _ = (Ideal.span {ρ}) ^ 59 := by rw [hJρ]
      _ = 0 := by
        rw [hρ]
        norm_num
        rw [Ideal.zero_eq_bot]
  have hstableρ : conjugateIdeal59 (Ideal.span {ρ}) =
      Ideal.span {ρ} := by
    rw [← hJρ]
    exact hstable
  obtain ⟨j, hj⟩ :=
    conjugation_sq_eq_zeta_pow_of_stable_principal59 hzeta ρ hρ0 hstableρ
  have hassoc_q : Associated (ρ ^ 59) q := by
    rw [← Ideal.span_singleton_eq_span_singleton,
...
elimination uses. -/
theorem exists_historicalEquationEightA59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρzero : 𝓞 K) (ηzero : (𝓞 K)ˣ) (jzero : ℕ),
      s.omega + s.theta =
        ηzero * kappa hζ ^ (59 * s.m - 29) * ρzero ^ 59 ∧
      historicalEquationEightAIdeal59 hζ s = Ideal.span {ρzero} ∧
  ...
  obtain ⟨ρ, η, j, hJ, hq, hρconj⟩ :=
    exists_squaredConjugationGenerator_of_real_pow59 hζ J q
      (by simpa only [q] using
        historicalEquationEightAQuotient_ne_zero59 hζ s hs)
      (by simpa only [q] using
        historicalEquationEightAQuotient_real59 hζ s hs)
      (by simpa only [J, q] using
        historicalEquationEightAIdeal_pow59 hζ s hs)
  have hq0 : q ≠ 0 := by
    simpa only [q] using historicalEquationEightAQuotient_ne_zero59 hζ s hs
  have hρ0 : ρ ≠ 0 := by
    intro hρ
    apply hq0
    rw [hq, hρ]
    norm_num
  have hzeta59 : hζ.unit' ^ 59 = 1 := by
    apply Units.ext
    apply NumberField.RingOfIntegers.ext
    change ζ ^ 59 = 1
    exact hζ.pow_eq_one
  have hroot59 : (hζ.unit' ^ j) ^ 59 = 1 := by
    rw [← pow_mul]
    rw [show j * 59 = 59 * j by omega, pow_mul, hzeta59, one_pow]
  have hρ118real :
      NumberField.IsCMField.ringOfIntegersComplexConj K (ρ ^ 118) =
        ρ ^ 118 := by
    have h118 : ρ ^ 118 = (ρ ^ 2) ^ 59 := by
      rw [← pow_mul]
    rw [h118, map_pow, hρconj, mul_pow,
      ← Units.val_pow_eq_pow_val, hroot59, Units.val_one, one_mul, ← h118]
  have hq2 : q ^ 2 = ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 118 := by
    rw [hq, mul_pow, ← Units.val_pow_eq_pow_val, ← pow_mul]
  have hηsqVal :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) =
        ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) := by
    apply mul_right_cancel₀ (pow_ne_zero 118 hρ0)
    calc
      NumberField.IsCMField.ringOfIntegersComplexConj K
            ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 118 =
          NumberField.IsCMField.ringOfIntegersComplexConj K
            (((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 118) := by
              rw [map_mul, hρ118real]
      _ = NumberField.IsCMField.ringOfIntegersComplexConj K (q ^ 2) := by
        rw [hq2]
      _ = q ^ 2 := by rw [map_pow,
        historicalEquationEightAQuotient_real59 hζ s hs]
      _ = ((η ^ 2 : (𝓞 K)ˣ) : 𝓞 K) * ρ ^ 118 := hq2
  ...
  · calc
      s.omega + s.theta =
          kappa hζ ^ (59 * s.m - 29) * q :=
        historicalEquationEightAQuotient_spec59 hζ s hs
      _ = kappa hζ ^ (59 * s.m - 29) *
          ((η : 𝓞 K) * ρ ^ 59) := by rw [hq]
      _ = (η : 𝓞 K) * kappa hζ ^ (59 * s.m - 29) * ρ ^ 59 := by
        ring
  · simpa only [J] using hJ

/-- Complex conjugation sends any integral 59th root of unity to its
58th power, i.e. its inverse. -/
lemma ringOfIntegersComplexConj_nthRoot59
    (η : Polynomial.nthRootsFinset 59 (1 : 𝓞 K)) :
    NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
      (η : 𝓞 K) ^ 58 := by
  have hηpow : (η : 𝓞 K) ^ 59 = 1 :=
    (Polynomial.mem_nthRootsFinset (by norm_num : 0 < 59) (1 : 𝓞 K)).mp η.prop
  let u : (𝓞 K)ˣ :=
    ⟨(η : 𝓞 K), (η : 𝓞 K) ^ 58,
...
The generic factor-allocation theorem selects a `59`th root `η` for which

`(ζ - 1)^((2*m-2)*59+1) ∣ ρₐ - η*ρ₋ₐ`.

Multiplying the pair symmetrically by `η^29` and `η^30` turns this into a
literal difference.  Both 59th powers are unchanged; the exponents are the
two half-powers surrounding `30`, the inverse of `2` modulo `59`.  No
class-number hypothesis is used in this step. -/
theorem equationNineA_normalized59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (m : ℕ) (hm : 1 < m) (ρa ρminus ρzero : 𝓞 K) (ε : (𝓞 K)ˣ)
    (hdiff : ρa ^ 59 - ρminus ^ 59 =
      ε * ((hζ.unit'.1 - 1) ^ (2 * m - 1) * ρzero) ^ 59)
    (hminus : ¬ hζ.unit'.1 - 1 ∣ ρminus) :
    ∃ ρa' ρminus' : 𝓞 K,
      ρa' ^ 59 = ρa ^ 59 ∧
      ρminus' ^ 59 = ρminus ^ 59 ∧
      (hζ.unit'.1 - 1) ^ ((2 * m - 2) * 59 + 1) ∣
        ρa' - ρminus' := by
  have hexp : 2 * m - 1 = (2 * m - 2) + 1 := by omega
  have e' : ρa ^ 59 + (-ρminus) ^ 59 =
      ε * ((hζ.unit'.1 - 1) ^ ((2 * m - 2) + 1) * ρzero) ^ 59 := by
    rw [← hexp]
    simpa only [Odd.neg_pow (by norm_num : Odd 59), sub_eq_add_neg] using hdiff
  ...
  have hηpow : (η : 𝓞 K) ^ 59 = 1 :=
    (Polynomial.mem_nthRootsFinset (by norm_num : 0 < 59) (1 : 𝓞 K)).mp η.prop
  let ρa' : 𝓞 K := (η : 𝓞 K) ^ 29 * ρa
  let ρminus' : 𝓞 K := (η : 𝓞 K) ^ 30 * ρminus
  refine ⟨ρa', ρminus', ?_, ?_, ?_⟩
  · dsimp [ρa']
    rw [mul_pow, ← pow_mul]
    rw [show 29 * 59 = 59 * 29 by norm_num, pow_mul, hηpow, one_pow,
      one_mul]
  · dsimp [ρminus']
    rw [mul_pow, ← pow_mul]
    rw [show 30 * 59 = 59 * 30 by norm_num, pow_mul, hηpow, one_pow,
      one_mul]
  · have hrewrite : ρa' - ρminus' =
        (η : 𝓞 K) ^ 29 * (ρa - (η : 𝓞 K) * ρminus) := by
      dsimp [ρa', ρminus']
      rw [pow_succ' (η : 𝓞 K) 29]
      ring
    rw [hrewrite]
    exact dvd_mul_of_dvd_right hηdiv ((η : 𝓞 K) ^ 29)

/-- Conjugation-compatible form of equation (9a).

When the two inputs are conjugate, the symmetric normalization by the
18th and 19th powers of the distinguished 59th root preserves that
relation: conjugation changes the root to its inverse and
`-29 ≡ 30 (mod 59)`.  The normalized inverse generator remains prime to
`ζ - 1`. -/
theorem equationNineA_normalized_conjugate59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (m : ℕ) (hm : 1 < m) (ρa ρminus ρzero : 𝓞 K) (ε : (𝓞 K)ˣ)
    (hdiff : ρa ^ 59 - ρminus ^ 59 =
      ε * ((hζ.unit'.1 - 1) ^ (2 * m - 1) * ρzero) ^ 59)
  ...
  have hηconj :
      NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
        (η : 𝓞 K) ^ 58 :=
    ringOfIntegersComplexConj_nthRoot59 η
  ...
  · dsimp [ρa', ρminus']
    ...
    rw [show 58 * 29 = 59 * 28 + 30 by norm_num,
      pow_add, pow_mul, hηpow, one_pow, one_mul]
  · dsimp [ρminus']
    intro hram
    rcases hζ.zeta_sub_one_prime'.dvd_mul.mp hram with hroot | hρ
    · exact hζ.zeta_sub_one_prime'.not_unit
        (isUnit_of_dvd_unit hroot (hηunit.pow 30))
    · exact hminus hρ

/-- A `59`th root of a real unit can be adjusted by a power of `ζ`
without changing its `59`th power so that the root itself is real. This is
the real-root normalization used implicitly between Vandiver's equations
(10) and (10b). -/
lemma exists_real_unit_root_59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (a v : (𝓞 K)ˣ)
    (hv : a = v ^ 59) :
    ∃ w : (𝓞 K)ˣ, a = w ^ 59 ∧
  ...
  · dsimp [w]
    ...
    have hzpow : hζ.unit' ^ 59 = 1 := by
      ext
      exact hζ.pow_eq_one
    rw [← pow_mul, show j * 59 = 59 * j by omega, pow_mul, hzpow, one_pow,
      div_one]
  · dsimp [w]
    rw [map_div, map_pow, unitsComplexConj_zeta59 hζ]
...
/-- At exponent `59`, Vandiver's `κ^29` is associated to the rational
prime `59`. -/
lemma kappa_pow_twentyNine_associated_59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    Associated (kappa hζ ^ 29) (59 : 𝓞 K) := by
  ...
  have hkappaPow := hkappa.pow_pow (n := 29)
...
/-- A primitive rational second-case solution at exponent `59` gives
Vandiver's equation (6) with `m = 29`. This discharges the first abstract
boundary of `VandiverHistoricalDescent` for the concrete real invariant. -/
theorem secondCaseStartsHistoricalDescent_59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    SecondCaseStartsHistoricalDescent hζ (RealSourceAdmissible hζ) := by
  intro x y z hgcd hz hz0 e
  have hx : x ≠ 0 := by
    intro hx0
    have hyz : y = z := (show Odd 59 by norm_num).pow_injective (by simpa [hx0] using e)
    have h59one : (59 : ℤ) ∣ 1 := by
  ...
  have hy : y ≠ 0 := by
    intro hy0
    have hxz : x = z := (show Odd 59 by norm_num).pow_injective (by simpa [hy0] using e)
    have h59one : (59 : ℤ) ∣ 1 := by
      rw [← hgcd, Finset.dvd_gcd_iff]
      intro w hw
      simp only [Finset.mem_insert, Finset.mem_singleton] at hw
  ...
  have ht0 : t ≠ 0 := by
    ...
    apply hz0
    simp [ht]
  obtain ⟨u, hu⟩ := kappa_pow_twentyNine_associated_59 hζ
  have hkappa0 : kappa hζ ^ 29 ≠ 0 :=
    (kappa_pow_twentyNine_associated_59 hζ).ne_zero_iff.mpr (by norm_num)
  have huReal :
  ...
  let s : HistoricalState hζ :=
    { omega := x
      ...
      equation := by
        ...
        calc
          (x : 𝓞 K) ^ 59 + (y : 𝓞 K) ^ 59 =
              ((((59 : ℤ) * t : ℤ) : 𝓞 K)) ^ 59 := by exact_mod_cast e
          _ = ((59 : 𝓞 K) * (t : 𝓞 K)) ^ 59 := by norm_num
          _ = (kappa hζ ^ 29 * ((u : 𝓞 K) * (t : 𝓞 K))) ^ 59 := by
            congr 1
            rw [← mul_assoc, hu]
          _ = (kappa hζ ^ 29 * ξ) ^ 59 := rfl }
  refine ⟨s, ?_⟩
  refine ⟨?_, ?_, ?_, ?_⟩
  · dsimp [s]
    simp
...
(10). -/
def equationTenTraceOne59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59) : 𝓞 K :=
  (hζ.unit' : 𝓞 K) + (hζ.unit'⁻¹ : (𝓞 K)ˣ)

/-- The real cyclotomic trace `B = ζ² + ζ⁻²` used in equation
(10). -/
def equationTenTraceTwo59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59) : 𝓞 K :=
  (hζ.unit' : 𝓞 K) ^ 2 + (hζ.unit'⁻¹ : (𝓞 K)ˣ) ^ 2

/-- The cyclotomic unit `ζ⁻¹(1+ζ+ζ²) = A+1`. -/
def equationTenTraceOneUnit59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) : (𝓞 K)ˣ :=
  hζ.unit'⁻¹ *
    (hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 3 59)).unit

/-- The cyclotomic unit `ζ⁻¹(1+ζ)² = A+2`. -/
def equationTenTraceTwoUnit59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) : (𝓞 K)ˣ :=
  hζ.unit'⁻¹ *
    (hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 2 59)).unit ^ 2

omit [IsCyclotomicExtension {59} ℚ K] in
lemma equationTenTraceOneUnit59_val {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    (equationTenTraceOneUnit59 hζ : 𝓞 K) =
      equationTenTraceOne59 hζ + 1 := by
  have huinv : (hζ.unit'⁻¹ : (𝓞 K)ˣ) * (hζ.unit' : 𝓞 K) = 1 := by
    rw [← Units.val_mul]
    simp
  have hgeom :
      (((hζ.unit'_coe.geom_sum_isUnit (by norm_num)
        (by norm_num : Nat.Coprime 3 59)).unit : (𝓞 K)ˣ) : 𝓞 K) =
        1 + (hζ.unit' : 𝓞 K) + (hζ.unit' : 𝓞 K) ^ 2 := by
    rw [(hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 3 59)).unit_spec]
    norm_num [Finset.sum_range_succ]
  simp only [equationTenTraceOneUnit59, Units.val_mul]
  rw [hgeom]
  simp only [equationTenTraceOne59]
  linear_combination (1 + (hζ.unit' : 𝓞 K)) * huinv

omit [IsCyclotomicExtension {59} ℚ K] in
lemma equationTenTraceTwoUnit59_val {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    (equationTenTraceTwoUnit59 hζ : 𝓞 K) =
      equationTenTraceOne59 hζ + 2 := by
  have huinv : (hζ.unit'⁻¹ : (𝓞 K)ˣ) * (hζ.unit' : 𝓞 K) = 1 := by
    rw [← Units.val_mul]
    simp
  have hgeom :
      (((hζ.unit'_coe.geom_sum_isUnit (by norm_num)
        (by norm_num : Nat.Coprime 2 59)).unit : (𝓞 K)ˣ) : 𝓞 K) =
        1 + (hζ.unit' : 𝓞 K) := by
    rw [(hζ.unit'_coe.geom_sum_isUnit (by norm_num)
      (by norm_num : Nat.Coprime 2 59)).unit_spec]
    norm_num [Finset.sum_range_succ]
  simp only [equationTenTraceTwoUnit59, Units.val_mul,
...
lemma two_sub_equationTenTraceOne59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    2 - equationTenTraceOne59 hζ = kappa hζ := by
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  simp only [equationTenTraceOne59, kappa]
  linear_combination -huinv

omit [IsCyclotomicExtension {59} ℚ K] in
/-- The trace difference `A-B` is `κ` times the explicit unit `A+1`. -/
lemma equationTenTraceOne_sub_two59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    equationTenTraceOne59 hζ - equationTenTraceTwo59 hζ =
      kappa hζ * (equationTenTraceOneUnit59 hζ : 𝓞 K) := by
  rw [equationTenTraceOneUnit59_val]
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  rw [← two_sub_equationTenTraceOne59 hζ]
  simp only [equationTenTraceOne59, equationTenTraceTwo59]
  linear_combination 2 * huinv

omit [IsCyclotomicExtension {59} ℚ K] in
/-- The second concrete coefficient `2-B` is `κ` times the explicit
unit `A+2`. -/
lemma two_sub_equationTenTraceTwo59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    2 - equationTenTraceTwo59 hζ =
      kappa hζ * (equationTenTraceTwoUnit59 hζ : 𝓞 K) := by
  rw [equationTenTraceTwoUnit59_val]
  have huinv : (hζ.unit' : 𝓞 K) * (hζ.unit'⁻¹ : (𝓞 K)ˣ) = 1 := by
    rw [← Units.val_mul]
    simp
  rw [← two_sub_equationTenTraceOne59 hζ]
...
lemma powTwoPrimitiveRoot_unit59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
...
`A+2`. -/
lemma kappa_powTwoPrimitiveRoot59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    kappa (hζ.pow_of_coprime 2 (by norm_num)) =
      kappa hζ * (equationTenTraceTwoUnit59 hζ : 𝓞 K) := by
  rw [← two_sub_equationTenTraceTwo59 hζ]
  simp only [kappa, powTwoPrimitiveRoot_unit59 hζ,
    Units.val_pow_eq_pow_val, equationTenTraceTwo59]
...
lemma equationTenTraceOne_real59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    NumberField.IsCMField.ringOfIntegersComplexConj K
        (equationTenTraceOne59 hζ) =
      equationTenTraceOne59 hζ := by
  have hz :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (hζ.unit' : 𝓞 K) =
        (hζ.unit'⁻¹ : (𝓞 K)ˣ) :=
  ...
  have hzinv :
    ...
    rw [map_inv₀,
      Fermat.Irregular.CyclotomicDiscriminantPrime.complexConj_zeta_inv hζ,
      inv_inv]
  simp only [equationTenTraceOne59, map_add, hz, hzinv, add_comm]

/-- The cyclotomic unit `A+2` used to transport the historical state to
the primitive root `ζ²` is real. -/
lemma equationTenTraceTwoUnit_real59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) :
    NumberField.IsCMField.unitsComplexConj K
        (equationTenTraceTwoUnit59 hζ) =
      equationTenTraceTwoUnit59 hζ := by
  apply Units.ext
  change NumberField.IsCMField.ringOfIntegersComplexConj K
      (equationTenTraceTwoUnit59 hζ : 𝓞 K) =
    (equationTenTraceTwoUnit59 hζ : 𝓞 K)
  rw [equationTenTraceTwoUnit59_val, map_add,
    equationTenTraceOne_real59 hζ, map_ofNat]
...
noncomputable def historicalStateAtTwo59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) :
    HistoricalState (hζ.pow_of_coprime 2 (by norm_num)) :=
  let u := equationTenTraceTwoUnit59 hζ
  { omega := s.omega
    theta := s.theta
    ...
    equation := by
      ...
      have hbase :
        ...
        rw [kappa_powTwoPrimitiveRoot59 hζ, mul_pow]
        simp only [Units.val_pow_eq_pow_val]
        calc
...
`ζ` to `ζ²`. -/
lemma historicalStateAtTwo_admissible59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    RealSourceAdmissible (hζ.pow_of_coprime 2 (by norm_num))
      (historicalStateAtTwo59 hζ s) := by
  let u := equationTenTraceTwoUnit59 hζ
  have hureal : NumberField.IsCMField.unitsComplexConj K u = u := by
    simpa only [u] using equationTenTraceTwoUnit_real59 hζ
  have huinvpow :
      NumberField.IsCMField.ringOfIntegersComplexConj K
          (((u⁻¹) ^ s.m : (𝓞 K)ˣ) : 𝓞 K) =
        (((u⁻¹) ^ s.m : (𝓞 K)ˣ) : 𝓞 K) := by
...
theorem exists_historicalEquationEight_pair_two59
    (hlemma : LemmaOne K 59)
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρtwo ρminus : 𝓞 K) (η : (𝓞 K)ˣ),
      ...
      s.omega + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * η * ρtwo ^ 59 ∧
      s.omega + ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ)) * η * ρminus ^ 59 ∧
      NumberField.IsCMField.ringOfIntegersComplexConj K ρtwo = ρminus ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρtwo ∧
      ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρminus := by
  let hζtwo := hζ.pow_of_coprime 2 (by norm_num)
  let stwo := historicalStateAtTwo59 hζ s
  have hstwo : RealSourceAdmissible hζtwo stwo := by
    simpa only [hζtwo, stwo] using
      historicalStateAtTwo_admissible59 hζ s hs
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one59 hlemma hζtwo stwo hstwo
  have hassoc :
      Associated ((hζ.unit' : 𝓞 K) - 1)
        ((hζtwo.unit' : 𝓞 K) - 1) := by
    simpa only [hζtwo, powTwoPrimitiveRoot_unit59 hζ,
  ...
  · simpa only [stwo, historicalStateAtTwo59, hζtwo,
      powTwoPrimitiveRoot_unit59 hζ, Units.val_pow_eq_pow_val] using htwo
  · simpa only [stwo, historicalStateAtTwo59, hζtwo,
      powTwoPrimitiveRoot_unit59 hζ] using hminus
...
lemma equationTenA_quadraticElimination59
    (ω θ A B Ua Ub Uzero Xa Xb Xzero : 𝓞 K)
    (ha : ω ^ 2 + A * (ω * θ) + θ ^ 2 = Ua * Xa ^ 59)
    (hb : ω ^ 2 + B * (ω * θ) + θ ^ 2 = Ub * Xb ^ 59)
    (hzero : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 = Uzero * Xzero) :
    (2 - B) * (Ua * Xa ^ 59) - (2 - A) * (Ub * Xb ^ 59) =
      (A - B) * (Uzero * Xzero) := by
  rw [← ha, ← hb, ← hzero]
  ring
...
equation: the coefficient of `Xb ^ 59` is absorbed into the unit `-Ub`,
while the other two coefficient quotients are the explicit geometric-sum
units above. -/
theorem equationTenB_cyclotomicSimplification59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (ω θ Xa Xb Xzero : 𝓞 K) (Ua Ub Uzero : (𝓞 K)ˣ)
    (ha : ω ^ 2 + equationTenTraceOne59 hζ * (ω * θ) + θ ^ 2 =
      Ua * Xa ^ 59)
    (hb : ω ^ 2 + equationTenTraceTwo59 hζ * (ω * θ) + θ ^ 2 =
      Ub * Xb ^ 59)
    (hzero : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 = Uzero * Xzero) :
    (equationTenTraceTwoUnit59 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 59 +
        (-Ub : (𝓞 K)ˣ) * Xb ^ 59 =
      (equationTenTraceOneUnit59 hζ * Uzero : (𝓞 K)ˣ) * Xzero := by
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit59_mul_sq]
    exact mul_ne_zero (kappaUnit59 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have helim := equationTenA_quadraticElimination59
    ω θ (equationTenTraceOne59 hζ) (equationTenTraceTwo59 hζ)
    Ua Ub Uzero Xa Xb Xzero ha hb hzero
  rw [two_sub_equationTenTraceTwo59,
    two_sub_equationTenTraceOne59,
    equationTenTraceOne_sub_two59] at helim
  apply mul_left_cancel₀ hkappa0
  calc
    kappa hζ *
        ((equationTenTraceTwoUnit59 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 59 +
          (-Ub : (𝓞 K)ˣ) * Xb ^ 59) =
        kappa hζ * (equationTenTraceTwoUnit59 hζ : 𝓞 K) *
            ((Ua : 𝓞 K) * Xa ^ 59) -
          kappa hζ * ((Ub : 𝓞 K) * Xb ^ 59) := by
            simp only [Units.val_mul, Units.val_neg]
            ring
    _ = kappa hζ * (equationTenTraceOneUnit59 hζ : 𝓞 K) *
...
`equationTenB_cyclotomicSimplification59`. -/
theorem equationTenB_commonKappa59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (ω θ Xa Xb Xzero : 𝓞 K) (Ua Ub Uzero : (𝓞 K)ˣ)
    (ha : ω ^ 2 + equationTenTraceOne59 hζ * (ω * θ) + θ ^ 2 =
      kappa hζ * (Ua * Xa ^ 59))
    (hb : ω ^ 2 + equationTenTraceTwo59 hζ * (ω * θ) + θ ^ 2 =
      kappa hζ * (Ub * Xb ^ 59))
    (hzero : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 =
      kappa hζ * (Uzero * Xzero ^ 59)) :
    (equationTenTraceTwoUnit59 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 59 +
        (-Ub : (𝓞 K)ˣ) * Xb ^ 59 =
      (equationTenTraceOneUnit59 hζ * Uzero : (𝓞 K)ˣ) *
        Xzero ^ 59 := by
  have hkappa0 : kappa hζ ≠ 0 := by
    rw [kappa_eq_kappaUnit59_mul_sq]
    exact mul_ne_zero (kappaUnit59 hζ).isUnit.ne_zero
      (pow_ne_zero 2 (sub_ne_zero.mpr
        (hζ.unit'_coe.ne_one (by norm_num))))
  have ha' : ω ^ 2 + equationTenTraceOne59 hζ * (ω * θ) + θ ^ 2 =
      (kappa hζ * Ua) * Xa ^ 59 := by
    simpa only [mul_assoc] using ha
  have hb' : ω ^ 2 + equationTenTraceTwo59 hζ * (ω * θ) + θ ^ 2 =
      (kappa hζ * Ub) * Xb ^ 59 := by
    simpa only [mul_assoc] using hb
  have hzero' : ω ^ 2 + 2 * (ω * θ) + θ ^ 2 =
      (kappa hζ * Uzero) * Xzero ^ 59 := by
    simpa only [mul_assoc] using hzero
  have helim := equationTenA_quadraticElimination59
    ω θ (equationTenTraceOne59 hζ) (equationTenTraceTwo59 hζ)
    (kappa hζ * Ua) (kappa hζ * Ub) (kappa hζ * Uzero)
    Xa Xb (Xzero ^ 59) ha' hb' hzero'
  rw [two_sub_equationTenTraceTwo59,
    two_sub_equationTenTraceOne59,
    equationTenTraceOne_sub_two59] at helim
  ...
  calc
    kappa hζ * kappa hζ *
        ((equationTenTraceTwoUnit59 hζ * Ua : (𝓞 K)ˣ) * Xa ^ 59 +
          (-Ub : (𝓞 K)ˣ) * Xb ^ 59) =
        kappa hζ * (equationTenTraceTwoUnit59 hζ : 𝓞 K) *
            (kappa hζ * ((Ua : 𝓞 K) * Xa ^ 59)) -
          kappa hζ * (kappa hζ * ((Ub : 𝓞 K) * Xb ^ 59)) := by
            simp only [Units.val_mul, Units.val_neg]
            ring
    _ = kappa hζ * (equationTenTraceOneUnit59 hζ : 𝓞 K) *
          (kappa hζ * ((Uzero : 𝓞 K) * Xzero ^ 59)) := by
...
principal ideal `(x)`. -/
def primeIdealFactorSupport59 (x : 𝓞 K) : Finset (Ideal (𝓞 K)) :=
...
structure WeightedReductionData59 {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
  ...
  highCongruence :
    ((1 : 𝓞 K) - hζ.unit') ^ 118 ∣
      (((epsilon₁ / epsilon₂ : (𝓞 K)ˣ) : 𝓞 K) -
        (rationalBase : 𝓞 K) ^ 59)
  weightedEquation :
    epsilon₁ * x ^ 59 + epsilon₂ * y ^ 59 =
      epsilon₃ * (kappa hζ ^ (2 * s.m - 1) * z) ^ 59
  z_ne_zero : z ≠ 0
  ...
  real_eta : NumberField.IsCMField.unitsComplexConj K (epsilon₃ / epsilon₂) =
    epsilon₃ / epsilon₂
  factorSupport_strict : primeIdealFactorSupport59 z ⊂ primeIdealFactorSupport59 s.xi

/-- The output of Vandiver's elimination before its three principal
generators have been made literally real.

Compared with WeightedReductionData59, this structure asks for the exact
cyclotomic conjugation quotients of x, y, and z. The theorem
weightedReductionData_of_conjugationPowers59 below performs the explicit
ζ^(30*j) adjustment and proves that it preserves the weighted equation,
coprimality, nonvanishing, and the strict prime-support descent. -/
structure ConjugationPowerReductionData59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ) where
  x : 𝓞 K
  y : 𝓞 K
  z : 𝓞 K
  epsilon₁ : (𝓞 K)ˣ
...
their conjugation quotients. Since those multipliers are 59th roots of
unity, every 59th power in Vandiver's equation is unchanged. -/
noncomputable def weightedReductionData_of_conjugationPowers59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) {s : HistoricalState hζ}
    (d : ConjugationPowerReductionData59 hζ s) :
    WeightedReductionData59 hζ s where
  x := realAdjustedGenerator59 hζ d.x d.conjugationExponent_x
  y := realAdjustedGenerator59 hζ d.y d.conjugationExponent_y
  z := realAdjustedGenerator59 hζ d.z d.conjugationExponent_z
  epsilon₁ := d.epsilon₁
  epsilon₂ := d.epsilon₂
  epsilon₃ := d.epsilon₃
  ...
  weightedEquation := by
    simpa only [mul_pow, realAdjustedGenerator59_pow_fiftyNine] using
      d.weightedEquation
  z_ne_zero := by
    dsimp [realAdjustedGenerator59]
    exact mul_ne_zero
      (hζ.unit' ^ (realGeneratorHalfExponent59 *
        d.conjugationExponent_z)).isUnit.ne_zero d.z_ne_zero
  coprime_xy :=
    (isCoprime_mul_unit_left_left
      (hζ.unit' ^ (realGeneratorHalfExponent59 * d.conjugationExponent_x)).isUnit
      d.x
      (realAdjustedGenerator59 hζ d.y d.conjugationExponent_y)).mpr
      ((isCoprime_mul_unit_left_right
        (hζ.unit' ^ (realGeneratorHalfExponent59 * d.conjugationExponent_y)).isUnit
        d.x d.y).mpr d.coprime_xy)
  coprime_yz :=
    (isCoprime_mul_unit_left_left
      (hζ.unit' ^ (realGeneratorHalfExponent59 * d.conjugationExponent_y)).isUnit
      d.y
      (realAdjustedGenerator59 hζ d.z d.conjugationExponent_z)).mpr
      ((isCoprime_mul_unit_left_right
        (hζ.unit' ^ (realGeneratorHalfExponent59 * d.conjugationExponent_z)).isUnit
  ...
  real_x :=
    realAdjustedGenerator59_real hζ d.x d.conjugationExponent_x d.conjugation_x
  real_y :=
    realAdjustedGenerator59_real hζ d.y d.conjugationExponent_y d.conjugation_y
  real_z :=
    realAdjustedGenerator59_real hζ d.z d.conjugationExponent_z d.conjugation_z
  real_eta := d.real_eta
  factorSupport_strict := by
    have hsupp :
        primeIdealFactorSupport59
            (realAdjustedGenerator59 hζ d.z d.conjugationExponent_z) =
          primeIdealFactorSupport59 d.z := by
      unfold primeIdealFactorSupport59
      rw [Ideal.span_singleton_eq_span_singleton.mpr
        (realAdjustedGenerator59_associated hζ d.z d.conjugationExponent_z)]
    rw [hsupp]
    exact d.factorSupport_strict

private noncomputable def adjustedRoot59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a v : (𝓞 K)ˣ)
    (hv : a = v ^ 59) : (𝓞 K)ˣ :=
  (exists_real_unit_root_59 hζ a v hv).choose

private lemma adjustedRoot59_pow {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a v : (𝓞 K)ˣ)
    (hv : a = v ^ 59) :
    a = adjustedRoot59 hζ a v hv ^ 59 :=
  (exists_real_unit_root_59 hζ a v hv).choose_spec.1

private lemma adjustedRoot59_real {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) (a v : (𝓞 K)ˣ)
    (hv : a = v ^ 59) :
    NumberField.IsCMField.unitsComplexConj K (adjustedRoot59 hζ a v hv) =
      adjustedRoot59 hζ a v hv :=
  (exists_real_unit_root_59 hζ a v hv).choose_spec.2

private noncomputable def weightedNextState59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) {s : HistoricalState hζ}
    (d : WeightedReductionData59 hζ s)
    (v : (𝓞 K)ˣ) (hv : d.epsilon₁ / d.epsilon₂ = v ^ 59) :
    HistoricalState hζ :=
  let w := adjustedRoot59 hζ (d.epsilon₁ / d.epsilon₂) v hv
  { omega := w * d.x
    theta := d.y
    xi := d.z
    ...
    equation := by
      rw [mul_pow, ← Units.val_pow_eq_pow_val,
        ← adjustedRoot59_pow hζ (d.epsilon₁ / d.epsilon₂) v hv,
...
private lemma weightedNextState59_admissible {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) {s : HistoricalState hζ}
    (d : WeightedReductionData59 hζ s)
    (v : (𝓞 K)ˣ) (hv : d.epsilon₁ / d.epsilon₂ = v ^ 59) :
    RealSourceAdmissible hζ (weightedNextState59 hζ d v hv) := by
  let w := adjustedRoot59 hζ (d.epsilon₁ / d.epsilon₂) v hv
  have hwUnits : NumberField.IsCMField.unitsComplexConj K w = w :=
    adjustedRoot59_real hζ (d.epsilon₁ / d.epsilon₂) v hv
...
`59`th root to be real, absorbs it into the first summand, divides out the
second coefficient unit, and verifies every invariant of equation (10b). -/
noncomputable def equationSevenToTenData_of_weighted59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) {s : HistoricalState hζ}
    (d : WeightedReductionData59 hζ s) :
  ...
  rationalBase := d.rationalBase
  highCongruence := d.highCongruence
  nextState := weightedNextState59 hζ d
  next_admissible := weightedNextState59_admissible hζ d
...
For each of these two ideals, `exists_real_unit_mul_pow_generator59`
...
historical state, verify their displayed 59th-power identities, and carry
...
`equationTenA_quadraticElimination59`.
...
def RealPrincipalGeneratorElimination59 {ζ : K}
    (hζ : IsPrimitiveRoot ζ 59) : Prop :=
  ∀ s : HistoricalState hζ, RealSourceAdmissible hζ s →
    Nonempty (ConjugationPowerReductionData59 hζ s)
...
theorem equationsSevenToTenReduction_59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (heliminate : RealPrincipalGeneratorElimination59 hζ) :
    EquationsSevenToTenReduction hζ (RealSourceAdmissible hζ) := by
  intro s hs
  exact (heliminate s hs).map fun d ↦
    equationSevenToTenData_of_weighted59 hζ
      (weightedReductionData_of_conjugationPowers59 hζ d)

/-- The exponent-`59` historical second case, conditional only on the exact
...
theorem secondCaseExcluded_59_of_historical
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (heliminate : RealPrincipalGeneratorElimination59 hζ)
    (hkummer : KummerUnitPowerConclusion K 59) :
    Fermat.SecondCaseExcluded 59 :=
  secondCaseExcluded_of_historical_descent (by norm_num) hζ
    (RealSourceAdmissible hζ) (secondCaseStartsHistoricalDescent_59 hζ)
    (equationsSevenToTenReduction_59 hζ heliminate) hkummer

/-- Assemble the exact source statement of Vandiver's Lemma 2 with the
directly checked exponent-`59` Bernoulli cube data. No semiprimary deepening
hypothesis is used on this historical route. -/
theorem secondCaseExcluded_59_of_vandiverLemmaTwo
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (heliminate : RealPrincipalGeneratorElimination59 hζ)
    (hLemmaTwo : Fermat.Irregular.VandiverUnitLemma.VandiverLemmaTwo K 59) :
    Fermat.SecondCaseExcluded 59 :=
  secondCaseExcluded_59_of_historical hζ heliminate
