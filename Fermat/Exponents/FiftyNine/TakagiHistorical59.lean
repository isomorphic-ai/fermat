import Fermat.Descent.Irregular.TakagiReflection59
import Fermat.Exponents.FiftyNine.VandiverHistorical

/-!
# Unconditional Takagi--Furtwängler step in Vandiver's historical proof

The universal predicate `LemmaOne K 59` is stronger than the instance
...
`TakagiReflection59` therefore proves the required historical equation
...
namespace Fermat.FiftyNine.TakagiHistorical59
...
theorem exists_historicalEquationSevenA59_unconditional
    ...
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal59 hζ s *
        ...
        Ideal.span {r} := by
  exact exists_conjugatePairEquationSevenAGenerator59 hζ
    (historicalZetaFactorIdeal59 hζ s)
...
historical results. -/
theorem exists_historicalEquationEight59_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
  ...
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA59_unconditional hζ s hs
  obtain ⟨t, hsevenD⟩ :=
    exists_historicalEquationSevenD59 hζ s hs
  obtain ⟨hI0, hJ0⟩ :=
    historicalFactorIdeals_ne_zero59 hζ s hs
...
conjugation. -/
theorem exists_historicalEquationEight_realUnit59_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal59 hζ s = Ideal.span {ρ} ∧
  ...
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight59_unconditional hζ s hs
  exact ⟨ρ, η, hI, heq,
...
theorem exists_historicalEquationEight_pair_one59_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρ, η₀, -, heq, hη₀real⟩ :=
    exists_historicalEquationEight_realUnit59_unconditional hζ s hs
  let η : (𝓞 K)ˣ := -η₀
  let ρminus : 𝓞 K :=
...
theorem exists_historicalEquationEight_pair_two59_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one59_unconditional
      hζtwo stwo hstwo
  have hassoc :
...
end Fermat.FiftyNine.TakagiHistorical59
