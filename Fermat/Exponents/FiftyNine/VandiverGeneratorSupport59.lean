import Fermat.Exponents.FiftyNine.VandiverHistoricalSupport

/-!
# Generator coprimality and support in Vandiver's exponent-59 reduction
...
differences of distinct 59th roots are associated to the standard
...
lemma isCoprime_linearRoot_theta59
    {ω θ t c r : R}
    (hωθ : IsCoprime ω θ)
    (heq : ω + t * θ = c * r ^ 59) :
    IsCoprime r θ := by
  ...
  rw [heq] at hlinear
  exact (IsCoprime.pow_left_iff (by norm_num : 0 < 59)).mp
    hlinear.of_mul_left_right

/-- Two distinct linear equations whose coefficients and root difference are
all associated to the same nonzero uniformizer have coprime 59th-power
generators.
...
theorem coprime_generators_of_distinct_linearEquations59
    [IsDomain R]
    {ω θ π t₁ t₂ c₁ c₂ r₁ r₂ : R}
    (hπ0 : π ≠ 0)
    (hωθ : IsCoprime ω θ)
  ...
  have hdiff :
      (v : R) * θ =
        (u₁ : R) * r₁ ^ 59 - (u₂ : R) * r₂ ^ 59 := by
    apply mul_left_cancel₀ hπ0
    calc
      π * ((v : R) * θ) = (t₁ - t₂) * θ := by rw [← hv]; ring
      _ = (ω + t₁ * θ) - (ω + t₂ * θ) := by ring
      _ = (π * (u₁ : R) * r₁ ^ 59) -
          (π * (u₂ : R) * r₂ ^ 59) := by rw [heq₁', heq₂']
      _ = π * ((u₁ : R) * r₁ ^ 59 -
          (u₂ : R) * r₂ ^ 59) := by ring
  have hr₁θ : IsCoprime r₁ θ :=
    isCoprime_linearRoot_theta59 hωθ heq₁
  have hcop :
      IsCoprime r₁
        ((u₁ : R) * r₁ ^ 59 - (u₂ : R) * r₂ ^ 59) := by
    rw [← hdiff]
    exact (isCoprime_mul_unit_left_right v.isUnit r₁ θ).mpr hr₁θ
  have hcop' : IsCoprime r₁ ((u₂ : R) * r₂ ^ 59) := by
    rw [show (u₁ : R) * r₁ ^ 59 =
      r₁ * ((u₁ : R) * r₁ ^ 58) by ring] at hcop
    exact IsCoprime.mul_sub_left_right_iff.mp hcop
  have hpow : IsCoprime r₁ (r₂ ^ 59) :=
    (isCoprime_mul_unit_left_right u₂.isUnit r₁ (r₂ ^ 59)).mp hcop'
  exact (IsCoprime.pow_right_iff (by norm_num : 0 < 59)).mp hpow
...
theorem isCoprime_prime_of_not_dvd_dimensionOne59
...
argument. -/
theorem coprime_generator_zero_of_linearEquations59
    [IsDomain R] [Ring.DimensionLEOne R]
    {ω θ π t c r d r₀ : R}
    (hπ : Prime π)
    (hrπ : ¬ π ∣ r)
    ...
    IsCoprime r r₀ := by
  obtain ⟨u, hu⟩ := hc.symm
  have heq' : ω + t * θ = π * (u : R) * r ^ 59 := by
    calc
      ω + t * θ = c * r ^ 59 := heq
      _ = π * (u : R) * r ^ 59 := by rw [hu]
  have hπr : IsCoprime π r :=
    isCoprime_prime_of_not_dvd_dimensionOne59 hπ hrπ
  obtain ⟨v, hv⟩ := ht.symm
  ...
  have hrewrite :
      ω + θ =
        r * (π * (u : R) * r ^ 58) - (t - 1) * θ := by
    calc
      ω + θ = (ω + t * θ) - (t - 1) * θ := by ring
      _ = (π * (u : R) * r ^ 59) - (t - 1) * θ := by rw [heq']
      _ = r * (π * (u : R) * r ^ 58) - (t - 1) * θ := by ring
  have hrsum : IsCoprime r (ω + θ) := by
    rw [hrewrite]
    exact IsCoprime.mul_sub_left_right_iff.mpr hrprod
  rw [hzero] at hrsum
  exact (IsCoprime.pow_right_iff (by norm_num : 0 < 59)).mp
...
`ConjugationPowerReductionData59` with

`x = r₁*r₋₁`, `y = r₂*r₋₂`, and `z = r₀^2`. -/
theorem products_and_square_coprimality59
    [IsDomain R]
...
element-level form of pairwise association for the finite set of 59th roots
of unity. -/
lemma associated_zetaPowers_sub59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {a b : ℕ} (ha : a < 59) (hb : b < 59) (hab : a ≠ b) :
    Associated
      ((hζ.unit' : 𝓞 K) ^ a - (hζ.unit' : 𝓞 K) ^ b)
      ((hζ.unit' : 𝓞 K) - 1) := by
  have hroot (j : ℕ) :
      (hζ.unit' : 𝓞 K) ^ j ∈
        Polynomial.nthRootsFinset 59 (1 : 𝓞 K) := by
    rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 59)]
    rw [← pow_mul, Nat.mul_comm j 59, pow_mul,
  ...
  exact
    (hζ.unit'_coe.ntRootsFinset_pairwise_associated_sub_one_sub_of_prime
      (by norm_num : Nat.Prime 59) (hroot a) (hroot b) hne).symm

/-- The coefficient `(1-ζ^a)ε` in equation (8) is associated to the
standard uniformizer whenever `0 < a < 59`. -/
lemma associated_one_sub_zetaPow_mul_unit59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (a : ℕ) (ha0 : a ≠ 0) (ha59 : a < 59)
    (ε : (𝓞 K)ˣ) :
    Associated
  ...
  have hsub :
    ...
    simpa only [pow_zero] using
      associated_zetaPowers_sub59 hζ ha59 (by norm_num) ha0
  have honeSub :
...
`ζ^58`. -/
lemma zetaUnit_inv_eq_pow_fiftyEight59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) :
    hζ.unit'⁻¹ = hζ.unit' ^ 58 := by
  symm
...
/-- Likewise, the inverse of `ζ²` is `ζ^57`. -/
lemma zetaUnit_sq_inv_eq_pow_thirtyFive59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59) :
    (hζ.unit' ^ 2)⁻¹ = hζ.unit' ^ 57 := by
  symm
  apply eq_inv_of_mul_eq_one_left
  rw [← pow_add]
  norm_num
...
The four roots are represented by the standard exponents `1, 58, 2, 57`.
...
`ConjugationPowerReductionData59`. -/
theorem equationEight_generators_products_coprime_powForms59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    ...
    (heq₁ :
      ω + (hζ.unit' : 𝓞 K) * θ =
        (1 - (hζ.unit' : 𝓞 K)) * ε₁ * r₁ ^ 59)
    (heqminus₁ :
      ω + (hζ.unit' : 𝓞 K) ^ 58 * θ =
        (1 - (hζ.unit' : 𝓞 K) ^ 58) * εminus₁ *
          rminus₁ ^ 59)
    (heq₂ :
      ω + (hζ.unit' : 𝓞 K) ^ 2 * θ =
        (1 - (hζ.unit' : 𝓞 K) ^ 2) * ε₂ * r₂ ^ 59)
    (heqminus₂ :
      ω + (hζ.unit' : 𝓞 K) ^ 57 * θ =
        (1 - (hζ.unit' : 𝓞 K) ^ 57) * εminus₂ *
          rminus₂ ^ 59)
    (hzero : ω + θ = d * r₀ ^ 59)
  ...
  have hc₁ :
    ...
    simpa only [π, pow_one] using
      associated_one_sub_zetaPow_mul_unit59 hζ 1
        (by norm_num) (by norm_num) ε₁
  have hcminus₁ :
      Associated
        ((1 - (hζ.unit' : 𝓞 K) ^ 58) * (εminus₁ : 𝓞 K)) π := by
    simpa only [π] using
      associated_one_sub_zetaPow_mul_unit59 hζ 58
        (by norm_num) (by norm_num) εminus₁
  have hc₂ :
      Associated
        ((1 - (hζ.unit' : 𝓞 K) ^ 2) * (ε₂ : 𝓞 K)) π := by
    simpa only [π] using
      associated_one_sub_zetaPow_mul_unit59 hζ 2
        (by norm_num) (by norm_num) ε₂
  have hcminus₂ :
      Associated
        ((1 - (hζ.unit' : 𝓞 K) ^ 57) * (εminus₂ : 𝓞 K)) π := by
    simpa only [π] using
      associated_one_sub_zetaPow_mul_unit59 hζ 57
        (by norm_num) (by norm_num) εminus₂
  have ht₁₂ :
      Associated
        ((hζ.unit' : 𝓞 K) - (hζ.unit' : 𝓞 K) ^ 2) π := by
    simpa only [π, pow_one] using
      associated_zetaPowers_sub59 hζ
        (a := 1) (b := 2) (by norm_num) (by norm_num) (by norm_num)
  have ht₁minus₂ :
      Associated
        ((hζ.unit' : 𝓞 K) - (hζ.unit' : 𝓞 K) ^ 57) π := by
    simpa only [π, pow_one] using
      associated_zetaPowers_sub59 hζ
        (a := 1) (b := 57) (by norm_num) (by norm_num) (by norm_num)
  have htminus₁₂ :
      Associated
        ((hζ.unit' : 𝓞 K) ^ 58 - (hζ.unit' : 𝓞 K) ^ 2) π := by
    simpa only [π] using
      associated_zetaPowers_sub59 hζ
        (a := 58) (b := 2) (by norm_num) (by norm_num) (by norm_num)
  have htminus₁minus₂ :
      Associated
        ((hζ.unit' : 𝓞 K) ^ 58 - (hζ.unit' : 𝓞 K) ^ 57) π := by
    simpa only [π] using
      associated_zetaPowers_sub59 hζ
        (a := 58) (b := 57) (by norm_num) (by norm_num) (by norm_num)
  have ht₁zero :
      Associated ((hζ.unit' : 𝓞 K) - 1) π :=
    Associated.refl π
  have htminus₁zero :
      Associated ((hζ.unit' : 𝓞 K) ^ 58 - 1) π := by
    simpa only [π, pow_zero] using
      associated_zetaPowers_sub59 hζ
        (a := 58) (b := 0) (by norm_num) (by norm_num) (by norm_num)
  have ht₂zero :
  ...
  have htminus₂zero :
      Associated ((hζ.unit' : 𝓞 K) ^ 57 - 1) π := by
    simpa only [π, pow_zero] using
      associated_zetaPowers_sub59 hζ
        (a := 57) (b := 0) (by norm_num) (by norm_num) (by norm_num)
  have h₁₂ : IsCoprime r₁ r₂ :=
    coprime_generators_of_distinct_linearEquations59 hπ0 hωθ
      hc₁ hc₂ ht₁₂ heq₁ heq₂
  ...
  have h₁zero : IsCoprime r₁ r₀ :=
    coprime_generator_zero_of_linearEquations59
  ...
  have hminus₂zero : IsCoprime rminus₂ r₀ :=
    coprime_generator_zero_of_linearEquations59
      hζ.zeta_sub_one_prime' hrminus₂π hωθ hcminus₂
        htminus₂zero heqminus₂ hzero
  exact products_and_square_coprimality59 hr₀
...
`equationEight_generators_products_coprime_powForms59`. -/
theorem equationEight_generators_products_coprime59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    {ω θ r₁ rminus₁ r₂ rminus₂ r₀ : 𝓞 K}
    (ε₁ εminus₁ ε₂ εminus₂ : (𝓞 K)ˣ) (d : 𝓞 K)
    (hr₀ : r₀ ≠ 0)
  ...
  have heqminus₁' :
    ...
    rw [← Units.val_pow_eq_pow_val,
      ← zetaUnit_inv_eq_pow_fiftyEight59 hζ]
    exact heqminus₁
  have heq₂' :
      ω + (hζ.unit' : 𝓞 K) ^ 2 * θ =
        (1 - (hζ.unit' : 𝓞 K) ^ 2) * ε₂ * r₂ ^ 59 := by
  ...
  have heqminus₂' :
      ω + (hζ.unit' : 𝓞 K) ^ 57 * θ =
        (1 - (hζ.unit' : 𝓞 K) ^ 57) * εminus₂ *
          rminus₂ ^ 59 := by
    rw [← Units.val_pow_eq_pow_val,
      ← zetaUnit_sq_inv_eq_pow_thirtyFive59 hζ]
    exact heqminus₂
  exact equationEight_generators_products_coprime_powForms59
...
theorem primeIdealFactorSupport59_eq_of_associated
    {x y : 𝓞 K} (hxy : Associated x y) :
    primeIdealFactorSupport59 x = primeIdealFactorSupport59 y := by
...
theorem primeIdealFactorSupport59_unit_mul_pow
    (u : (𝓞 K)ˣ) (x : 𝓞 K) {n : ℕ} (hn : n ≠ 0) :
    primeIdealFactorSupport59 ((u : 𝓞 K) * x ^ n) =
      primeIdealFactorSupport59 x := by
  calc
    primeIdealFactorSupport59 ((u : 𝓞 K) * x ^ n) =
        primeIdealFactorSupport59 (x ^ n) :=
      primeIdealFactorSupport59_eq_of_associated
        (associated_unit_mul_left (x ^ n) (u : 𝓞 K) u.isUnit)
    _ = primeIdealFactorSupport59 x :=
      primeIdealFactorSupport59_pow x hn

/-- In particular, the zero-root normalization used after equation (10)
does not alter the support of the equation-(8a) generator. -/
theorem primeIdealFactorSupport59_unit_mul_square
    (u : (𝓞 K)ˣ) (r₀ : 𝓞 K) :
    primeIdealFactorSupport59 ((u : 𝓞 K) * r₀ ^ 2) =
      primeIdealFactorSupport59 r₀ :=
  primeIdealFactorSupport59_unit_mul_pow u r₀ (by norm_num)

/-- Strict support descent is unchanged by the unit-times-square
normalization of the equation-(8a) generator. -/
theorem primeIdealFactorSupport59_unit_mul_square_ssubset_iff
    (u : (𝓞 K)ˣ) (r₀ ξ : 𝓞 K) :
    primeIdealFactorSupport59 ((u : 𝓞 K) * r₀ ^ 2) ⊂
        primeIdealFactorSupport59 ξ ↔
      primeIdealFactorSupport59 r₀ ⊂ primeIdealFactorSupport59 ξ := by
  rw [primeIdealFactorSupport59_unit_mul_square]
