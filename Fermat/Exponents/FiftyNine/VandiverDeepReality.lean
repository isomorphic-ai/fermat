...
import Fermat.Exponents.FiftyNine.VandiverRelationNormalization

/-!
# Deep Vandiver units are real at exponent 59
...
that the depth-118 local hypothesis itself forces an ambient cyclotomic unit
to be real.
...
namespace Fermat.FiftyNine.VandiverDeepReality
...
inverse. -/
theorem unitsComplexConj_zeta59 {zeta : K}
...
theorem ringOfIntegersComplexConj_zeta_sub_one59 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) :
...
conjugation. -/
theorem zeta_sub_one_pow_dvd_conj_of_dvd59 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) (n : ℕ) (a : 𝓞 K)
    (h : ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow,
    ringOfIntegersComplexConj_zeta_sub_one59, mul_pow]
...
/-- A square of a 59th root of unity which is `1` modulo
`(zeta - 1)^2` is literally `1`. -/
theorem zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd59 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) (j : ℕ)
  ...
  · have hknotdvd : ¬59 ∣ k := by
    ...
    have hkcoprime : k.Coprime 59 := by
      ...
      exact (show Nat.Prime 59 by norm_num).coprime_iff_not_dvd.mpr
        hknotdvd
...
theorem unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj59 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) (u : (𝓞 K)ˣ)
    (hdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
  ...
  have hroot :=
    zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd59 hzeta j hrootdiv
  rw [hroot] at hj
  have hucu : u = cu := by
    calc
      u = (u * cu⁻¹) * cu := by simp
...
/-- The depth-118 hypothesis forces a unit to be real. -/
theorem isVandiverDeep_unitsComplexConj_eq {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 59) hzeta u) :
    NumberField.IsCMField.unitsComplexConj K u = u := by
  obtain ⟨c, hc⟩ := hdeep
  have hsign : ((hzeta.unit' : 𝓞 K) - 1) ^ 118 ∣
      (u : 𝓞 K) - (c : 𝓞 K) ^ 59 := by
    have hbase : (hzeta.unit' : 𝓞 K) - 1 ∣
        (1 : 𝓞 K) - hzeta.unit' := by
      refine ⟨-1, ?_⟩
      ring
    exact (pow_dvd_pow_of_dvd hbase 118).trans hc
  have hconj := zeta_sub_one_pow_dvd_conj_of_dvd59
    hzeta 118 ((u : 𝓞 K) - (c : 𝓞 K) ^ 59) hsign
  have hconj' : ((hzeta.unit' : 𝓞 K) - 1) ^ 118 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (u : 𝓞 K) -
        (c : 𝓞 K) ^ 59 := by
    simpa only [map_sub, map_pow, map_intCast] using hconj
  have hdefect118 : ((hzeta.unit' : 𝓞 K) - 1) ^ 118 ∣
      (u : 𝓞 K) -
  ...
  have hdefect2 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
    ...
    (pow_dvd_pow ((hzeta.unit' : 𝓞 K) - 1) (by omega)).trans
      hdefect118
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj59
    hzeta u hdefect2

/-- A deeply congruent ambient unit, packaged in the real-unit subgroup. -/
def deepRealUnit59 {zeta : K} (hzeta : IsPrimitiveRoot zeta 59)
    (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 59) hzeta u) :
...
@[simp]
theorem deepRealUnit59_coe {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 59) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 59) hzeta u) :
...
end Fermat.FiftyNine.VandiverDeepReality
