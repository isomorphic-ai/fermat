...
theorem actualTameLedger827_ne_zero
  ...
  exact
    ledger_ne_zero_of_sum_ne_zero
      (actualTameLedger827 (K := K) omega chi lift)
      (actualTameLedger827_sum_ne_zero
        (K := K) omega chi heven hne lift selected)
...
theorem wild_ne_zero_of_actualTameLedgerReciprocity827
    ...
    wildReading ≠ 0 := by
  exact
    distinguished_ne_zero_of_add_ledger_sum_eq_zero
        (actualTameLedger827 (K := K) omega chi lift) wildReading
        (actualTameLedger827_sum_ne_zero
          (K := K) omega chi heven hne lift selected)
        reciprocity
