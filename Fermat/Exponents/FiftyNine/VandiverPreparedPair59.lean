import Fermat.Exponents.FiftyNine.TakagiHistorical59
import Fermat.Exponents.FiftyNine.VandiverPostNineGenerator59
import Fermat.Exponents.FiftyNine.VandiverGeneratorSupport59

/-!
# Prepared conjugate equation-(8) pairs at exponent 59
...
structure PreparedEquationEightPair59
  ...
  equation_plus :
    s.omega + (t : 𝓞 K) * s.theta =
      (1 - (t : 𝓞 K)) * coefficient * rplus ^ 59
  equation_minus :
    s.omega + (t⁻¹ : (𝓞 K)ˣ) * s.theta =
      (1 - (t⁻¹ : (𝓞 K)ˣ)) * coefficient * rminus ^ 59
  conjugate :
  ...
  mu_not_ramified :
    ¬ (hζ.unit' : 𝓞 K) - 1 ∣ mu
  close_plus :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 2) * 59) ∣
      rplus - mu ^ 59
  close_minus :
    ((hζ.unit' : 𝓞 K) - 1) ^ ((2 * s.m - 2) * 59) ∣
      rminus - mu ^ 59

namespace PreparedEquationEightPair59

variable {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
  (s : HistoricalState hζ) (t : (𝓞 K)ˣ)
  (d : PreparedEquationEightPair59 hζ s t)
...
lemma quadraticEquation :
  ...
  calc
    ...
    _ = ((1 - (t : 𝓞 K)) * d.coefficient * d.rplus ^ 59) *
...
lemma quadraticEquation_one
    (d : PreparedEquationEightPair59 hζ s hζ.unit') :
    s.omega ^ 2 +
        equationTenTraceOne59 hζ * (s.omega * s.theta) +
        s.theta ^ 2 =
      kappa hζ *
        ((d.coefficient ^ 2 : (𝓞 K)ˣ) *
          (d.rplus * d.rminus) ^ 59) := by
  simpa only [equationTenTraceOne59, kappa] using
    d.quadraticEquation hζ s hζ.unit'

/-- Quadratic equation (10) for the prepared pair at `2,-2`. -/
lemma quadraticEquation_two
    (d : PreparedEquationEightPair59 hζ s (hζ.unit' ^ 2)) :
    s.omega ^ 2 +
        equationTenTraceTwo59 hζ * (s.omega * s.theta) +
        s.theta ^ 2 =
      kappa hζ *
        ((equationTenTraceTwoUnit59 hζ * d.coefficient ^ 2 :
            (𝓞 K)ˣ) *
          (d.rplus * d.rminus) ^ 59) := by
  have htrace :
      (((hζ.unit' ^ 2 : (𝓞 K)ˣ) : 𝓞 K) +
          (((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) : 𝓞 K)) =
        equationTenTraceTwo59 hζ := by
    simp only [equationTenTraceTwo59, Units.val_pow_eq_pow_val]
    congr 1
  have hden :
    ...
    calc
      ...
      _ = _ := kappa_powTwoPrimitiveRoot59 hζ
...
end PreparedEquationEightPair59

namespace PostEquationNineNormalizedData59

/-- Since both Vandiver's fixed-denominator quotient and its chosen
generator are real, the residual unit in `q = η μ^59` is real as well. -/
lemma generator_unit_real
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (d : PostEquationNineNormalizedData59 hζ)
    (μ : 𝓞 K) (η : (𝓞 K)ˣ)
    (hμreal :
      NumberField.IsCMField.ringOfIntegersComplexConj K μ = μ)
    (hμnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ μ)
    (hq : quotient hζ d = η * μ ^ 59) :
  ...
  change
    NumberField.IsCMField.ringOfIntegersComplexConj K (η : 𝓞 K) =
      (η : 𝓞 K)
  apply mul_right_cancel₀ (pow_ne_zero 59 hμ0)
  have hqreal := d.quotient_real hζ
  calc
    (NumberField.IsCMField.unitsComplexConj K η : 𝓞 K) * μ ^ 59 =
        NumberField.IsCMField.ringOfIntegersComplexConj K
          ((η : 𝓞 K) * μ ^ 59) := by
...
end PostEquationNineNormalizedData59
...
unit. -/
theorem exists_preparedEquationEightPair59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    ...
    (hplus :
      s.omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) * eta * rhoPlus ^ 59)
    (hminus :
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
    ...
    Nonempty
      (PreparedEquationEightPair59 hζ s (hζ.unit' ^ a)) := by
  classical
  obtain ⟨epsilon, hdifference⟩ :=
    exists_equationEight_difference59 hζ a s.m ha s.one_lt_m
      s.omega s.theta rhoPlus rhoMinus rhoZero eta etaZero
      hplus hminus hzero
  obtain ⟨nplus, nminus, hnplus, hnminus, hclose,
      hnconj, hnminusNot⟩ :=
    equationNineA_normalized_conjugate59 hζ s.m s.one_lt_m
      rhoPlus rhoMinus rhoZero epsilon hdifference hconj hminusNot
  have hnpair : IsCoprime nplus nminus := by
    have hpowers : IsCoprime (nplus ^ 59) (nminus ^ 59) := by
      rw [hnplus, hnminus]
      exact hpair.pow_left.pow_right
    exact
      (IsCoprime.pow_left_iff (by norm_num : 0 < 59)).mp
        ((IsCoprime.pow_right_iff (by norm_num : 0 < 59)).mp hpowers)
  let D : ℕ := (2 * s.m - 2) * 59
  let post : PostEquationNineNormalizedData59 hζ :=
    { m := s.m
      one_lt_m := s.one_lt_m
      rplus := nplus
      rminus := nminus
  ...
  let coefficient : (𝓞 K)ˣ := eta * delta ^ 59
  ...
  have hdeltaCancel :
      (delta : 𝓞 K) ^ 59 *
          ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 59 = 1 := by
    rw [← mul_pow, ← Units.val_mul]
    simp
  have hequationPlus :
      s.omega + (hζ.unit' ^ a : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) *
          coefficient * rplus ^ 59 := by
    have hscaled :
        (coefficient : 𝓞 K) * rplus ^ 59 =
          (eta : 𝓞 K) * rhoPlus ^ 59 := by
      dsimp [coefficient, rplus]
      simp only [mul_pow]
      rw [hnplus]
      calc
        ((eta : 𝓞 K) * (delta : 𝓞 K) ^ 59) *
            (((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 59 *
              rhoPlus ^ 59) =
          (eta : 𝓞 K) *
            (((delta : 𝓞 K) ^ 59 *
              ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 59) *
                rhoPlus ^ 59) := by ac_rfl
        _ = (eta : 𝓞 K) * rhoPlus ^ 59 := by
    ...
    calc
      ...
      _ = (1 - (hζ.unit' ^ a : (𝓞 K)ˣ)) *
          ((coefficient : 𝓞 K) * rplus ^ 59) := by
        rw [hscaled]
      _ = _ := by rw [mul_assoc]
  have hequationMinus :
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
        (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) *
          coefficient * rminus ^ 59 := by
    have hscaled :
        (coefficient : 𝓞 K) * rminus ^ 59 =
          (eta : 𝓞 K) * rhoMinus ^ 59 := by
      ...
      calc
        ((eta : 𝓞 K) * (delta : 𝓞 K) ^ 59) *
          ...
          (eta : 𝓞 K) *
            (((delta : 𝓞 K) ^ 59 *
              ((delta⁻¹ : (𝓞 K)ˣ) : 𝓞 K) ^ 59) *
                rhoMinus ^ 59) := by ac_rfl
        _ = (eta : 𝓞 K) * rhoMinus ^ 59 := by
          rw [hdeltaCancel, one_mul]
    calc
      s.omega + ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ) * s.theta =
          (1 - ((hζ.unit' ^ a)⁻¹ : (𝓞 K)ˣ)) *
  ...
  have hrplusNot :
    ...
    have hn : (hζ.unit' : 𝓞 K) - 1 ∣ nplus := by
      ...
      simp
    have hc := zeta_sub_one_pow_dvd_conj_of_dvd59 hζ 1 nplus
...
theorem equationEight_pair_one_generators_coprime59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (s : HistoricalState hζ)
    (rhoPlus rhoMinus : 𝓞 K) (eta : (𝓞 K)ˣ)
    (hplus :
      s.omega + (hζ.unit' : 𝓞 K) * s.theta =
        (1 - (hζ.unit' : 𝓞 K)) * eta * rhoPlus ^ 59)
    (hminus :
  ...
  have hcMinus :
    ...
    rw [zetaUnit_inv_eq_pow_fiftyEight59 hζ]
  ...
  exact coprime_generators_of_distinct_linearEquations59
...
theorem equationEight_pair_two_generators_coprime59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (s : HistoricalState hζ)
    (rhoPlus rhoMinus : 𝓞 K) (eta : (𝓞 K)ˣ)
    (hplus :
      s.omega + (hζ.unit' ^ 2 : (𝓞 K)ˣ) * s.theta =
        (1 - (hζ.unit' ^ 2 : (𝓞 K)ˣ)) * eta * rhoPlus ^ 59)
    (hminus :
      s.omega + ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ) * s.theta =
  ...
  have hcMinus :
      Associated
        ((1 - ((hζ.unit' ^ 2)⁻¹ : (𝓞 K)ˣ)) *
          (eta : 𝓞 K)) pi := by
    rw [zetaUnit_sq_inv_eq_pow_thirtyFive59 hζ]
    simpa only [pi, Units.val_pow_eq_pow_val] using
...
theorem exists_preparedEquationEightPair_one59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    ...
    (hzero :
      s.omega + s.theta =
        etaZero * kappa hζ ^ (59 * s.m - 29) * rhoZero ^ 59) :
    Nonempty (PreparedEquationEightPair59 hζ s hζ.unit') := by
  obtain ⟨rhoPlus, rhoMinus, eta, hetaReal, hplus, hminus,
      hconj, -, hminusNot⟩ :=
    _root_.Fermat.FiftyNine.TakagiHistorical59.exists_historicalEquationEight_pair_one59_unconditional
      hζ s hs
  have hpair :=
    equationEight_pair_one_generators_coprime59
      hζ s rhoPlus rhoMinus eta hplus hminus
  simpa only [pow_one] using
    exists_preparedEquationEightPair59 hζ s 1
...
Takagi--Furtwängler input already discharged. -/
theorem exists_preparedEquationEightPair_two59
    {ζ : K} (hζ : IsPrimitiveRoot ζ 59)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s)
    (rhoZero : 𝓞 K) (etaZero : (𝓞 K)ˣ)
    (hzero :
      s.omega + s.theta =
        etaZero * kappa hζ ^ (59 * s.m - 29) * rhoZero ^ 59) :
    Nonempty (PreparedEquationEightPair59 hζ s (hζ.unit' ^ 2)) := by
  obtain ⟨rhoPlus, rhoMinus, eta, hetaReal, hplus, hminus,
      hconj, -, hminusNot⟩ :=
    _root_.Fermat.FiftyNine.TakagiHistorical59.exists_historicalEquationEight_pair_two59_unconditional
      hζ s hs
  have hpair :=
    equationEight_pair_two_generators_coprime59
      hζ s rhoPlus rhoMinus eta hplus hminus
  exact exists_preparedEquationEightPair59 hζ s 2
    (by norm_num) rhoPlus rhoMinus rhoZero eta etaZero
    hetaReal
