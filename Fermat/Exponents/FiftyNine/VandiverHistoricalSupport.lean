...
/-!
# Prime-support bookkeeping for Vandiver's historical descent at 59
...
lemma primeIdealFactorSupport59_pow (a : 𝓞 K) {n : ℕ} (hn : n ≠ 0) :
    primeIdealFactorSupport59 (a ^ n) =
      primeIdealFactorSupport59 a := by
...
lemma primeIdealFactorSupport59_subset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q) :
    primeIdealFactorSupport59 a ⊆ primeIdealFactorSupport59 b := by
  ...
  have hP' :
      P ∈ UniqueFactorizationMonoid.normalizedFactors
        (Ideal.span {a}) := by
    simpa only [primeIdealFactorSupport59, Multiset.mem_toFinset] using hP
  have hprime :
...
lemma primeIdealFactorSupport59_ssubset_of_span_factor_of_witness
  ...
  have hsubset :=
    primeIdealFactorSupport59_subset_of_span_factor hb hfactor
  ...
  · change P ∈
    ...
    exact Or.inr hPQ
  · simpa only [primeIdealFactorSupport59, Multiset.mem_toFinset] using hPa
...
`(a)`. -/
lemma primeIdealFactorSupport59_ssubset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q)
    (hQ : Q ≠ ⊤)
  ...
  apply primeIdealFactorSupport59_ssubset_of_span_factor_of_witness
    hb hfactor hPQ
  intro hPa
...
and gives the strict inclusion required by `ConjugationPowerReductionData59`.
-/
theorem historicalEquationEightA_square_support_strict59
    ...
    (hQ : Q ≠ ⊤)
    (hcop :
      IsCoprime (historicalEquationEightAIdeal59 hζ s) Q) :
    primeIdealFactorSupport59 (ρzero ^ 2) ⊂
      primeIdealFactorSupport59 s.xi := by
  rw [primeIdealFactorSupport59_pow ρzero (by norm_num : 2 ≠ 0)]
  apply primeIdealFactorSupport59_ssubset_of_span_factor
    s.xi_ne_zero
