...
import Fermat.Exponents.FiveHundredEightySeven.VandiverDeepPolynomial
import Fermat.Exponents.FiveHundredEightySeven.VandiverNormalizedRelationDerivative

/-!
# Vandiver's positive-relation derivative congruence at 587

This file completes the polynomial-remainder calculation for a relation
with natural exponents in the 292 actual diagonal units at exponent `587`.

A deep relation first gives the integer polynomial

`A = P - C - 587² H`

from `VandiverDeepPolynomial`, with `A(zeta) = 0` and `587² ∣ A(1)`.
After substituting `W = exp V`, this becomes

`P(exp V) = C + 587² H(exp V) + A(exp V)`.
...
orders.  The `587² H(exp V)` term has valuation at least two at every order.
Moreover, `P(1) ≡ 1 mod 587²`, so the source series has a nonzero
...
with `relationDerivative587`.  The endpoint is the exact
`PositiveRelationDerivativeCongruences587` premise consumed by
`VandiverLemmaTwoAssembly`.
-/

open scoped BigOperators NumberField

namespace Fermat.FiveHundredEightySeven.VandiverPositiveRelationDerivative
...
open Fermat.FiveHundredEightySeven.VandiverDeepPolynomial
...
source's positive polynomial in the exact form `P - C - 587² H`. -/
theorem exists_vanishingPositiveRelationPolynomial587
    ...
    ∃ H : Polynomial ℤ,
      let P := positiveRelationPolynomial587 b
      let C : ℤ := c ^ (587 * (586 * t))
      let A := vanishingRelationPolynomial587 P 1 H C
      Polynomial.aeval zeta A = 0 ∧
        (587 : ℤ) ^ 2 ∣ A.eval 1 := by
  have hrelZ : u ^ t =
      ∏ i, diagonalVandiverUnit587 hzeta i ^ (b i : ℤ) := by
    simpa only [zpow_natCast] using hrel
  obtain ⟨H, hzero, hsquare⟩ :=
    exists_vanishingRelationPolynomial587 hzeta u c t
      (fun i ↦ (b i : ℤ)) hdeep hrelZ
  refine ⟨H, ?_, ?_⟩
  · simpa [positiveRelationPolynomial587] using hzero
  · simpa [positiveRelationPolynomial587] using hsquare

/-- Exponential substitution turns `A = P - C - 587² H` into the additive
source decomposition used in the derivative recursion. -/
theorem polynomialExp_vanishingRelationPolynomial587
    (P H : Polynomial ℤ) (C : ℤ) :
    polynomialExp P =
      PowerSeries.C (C : ℚ) +
        PowerSeries.C ((587 : ℤ) ^ 2 : ℚ) * polynomialExp H +
        polynomialExp (vanishingRelationPolynomial587 P 1 H C) := by
  rw [vanishingRelationPolynomial587]
...
/-- Every derivative of `587² * H(exp V)` has `587`-adic valuation at
least two. -/
theorem scaledPolynomialExp587sq_derivative_hasPadicValAtLeast_two
    (P : Polynomial ℤ) (N : ℕ) :
    HasPadicValAtLeast 587 2
      (formalDerivativeAtZero N
        (PowerSeries.C ((587 : ℤ) ^ 2 : ℚ) * polynomialExp P)) := by
  rw [formalDerivativeAtZero_C_mul,
    formalDerivativeAtZero_polynomialExp_eq_intCast]
  have hp2 := HasPadicValAtLeast.primePow (p := 587) 2
  have hmoment :=
    HasPadicValAtLeast.intCast (p := 587) (polynomialMomentInt P N)
  convert hp2.mul hmoment using 1

/-- Since `P(1) ≡ 1 mod 587²`, the positive relation polynomial is not
divisible by `587` at `1`. -/
theorem fiveHundredEightySeven_not_dvd_eval_one_positiveRelationPolynomial587
    (b : SourceIndex 587 → ℕ) :
    ¬(587 : ℤ) ∣
      (positiveRelationPolynomial587 b).eval 1 := by
  have hmod :=
    eval_one_positiveRelationPolynomial587_mod_sq b
  have hzero :
      ((((positiveRelationPolynomial587 b).eval 1 - 1 : ℤ)) :
        ZMod (587 ^ 2)) = 0 := by
    push_cast
    rw [hmod]
    ring
  have hsquare :
      (((587 ^ 2 : ℕ) : ℤ) ∣
        (positiveRelationPolynomial587 b).eval 1 - 1) :=
    (ZMod.intCast_zmod_eq_zero_iff_dvd _ _).mp hzero
  have hFiveHundredEightySeven :
      (587 : ℤ) ∣
        (positiveRelationPolynomial587 b).eval 1 - 1 :=
    (by norm_num : (587 : ℤ) ∣ ((587 ^ 2 : ℕ) : ℤ)).trans hsquare
  intro heval
  have hone : (587 : ℤ) ∣ 1 := by
    convert dvd_sub heval hFiveHundredEightySeven using 1
    ring
  norm_num at hone

theorem constantCoeff_positiveRelationPolynomial587_ne_zero
    (b : SourceIndex 587 → ℕ) :
    PowerSeries.constantCoeff
      (polynomialExp (positiveRelationPolynomial587 b)) ≠ 0 := by
  rw [constantCoeff_polynomialExp]
  exact_mod_cast
    (show (positiveRelationPolynomial587 b).eval 1 ≠ 0 from
      fun hzero ↦
        fiveHundredEightySeven_not_dvd_eval_one_positiveRelationPolynomial587 b
          (hzero ▸ dvd_zero 587))

theorem constantCoeff_positiveRelationPolynomial587_padicVal_eq_zero
    (b : SourceIndex 587 → ℕ) :
    padicValRat 587
      (PowerSeries.constantCoeff
        (polynomialExp (positiveRelationPolynomial587 b))) = 0 := by
...
`N = (2 * sourceNumber k) * 587`.  Thus `587 ∣ N`, while the certified
source range gives `586 ∤ N`, exactly the hypotheses of the generic
remainder theorem. -/
theorem positiveRelation_logarithmicDerivative_hasPadicValAtLeast_two
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 587)
    (b : SourceIndex 587 → ℕ) (H : Polynomial ℤ) (C : ℤ)
    (hzero : Polynomial.aeval zeta
      (vanishingRelationPolynomial587
        (positiveRelationPolynomial587 b) 1 H C) = 0)
    (hsquare : (587 : ℤ) ^ 2 ∣
      (vanishingRelationPolynomial587
        (positiveRelationPolynomial587 b) 1 H C).eval 1) :
    ∀ k : SourceIndex 587,
      HasPadicValAtLeast 587 2
        (formalDerivativeAtZero (sourceDerivativeOrder587 k)
          (logarithmicDerivative
            (polynomialExp (positiveRelationPolynomial587 b)))) := by
  let P := positiveRelationPolynomial587 b
  let A := vanishingRelationPolynomial587 P 1 H C
  have hzeroA : Polynomial.aeval zeta A = 0 := by
    simpa only [A, P] using hzero
  have hsquareA : (587 : ℤ) ^ 2 ∣ A.eval 1 := by
    simpa only [A, P] using hsquare
  obtain ⟨V, b₁, hAseries⟩ :=
    exists_polynomialExp_eq_additiveRemainder
      (p := 587) (by norm_num) hzeta A hzeroA hsquareA
  have hsource :
      polynomialExp P =
        PowerSeries.C (C : ℚ) +
          PowerSeries.C ((587 : ℤ) ^ 2 : ℚ) * polynomialExp H +
          additiveRemainder 587 V b₁ := by
    calc
      polynomialExp P =
          PowerSeries.C (C : ℚ) +
            PowerSeries.C ((587 : ℤ) ^ 2 : ℚ) * polynomialExp H +
            polynomialExp A := by
              simpa only [A] using
                polynomialExp_vanishingRelationPolynomial587 P H C
      _ = _ := by rw [hAseries]
  intro k
  let N := derivativeBernoulliIndex587 k
  have hNpos : 0 < N := by
    simp [N, derivativeBernoulliIndex587, sourceNumber]
  have hNgt : 1 < N := by
    simp only [N, derivativeBernoulliIndex587, sourceNumber]
    omega
  have hNdiv : 587 ∣ N := by
    refine ⟨2 * sourceNumber k, ?_⟩
    simp only [N, derivativeBernoulliIndex587]
    ring
  have hNnot : ¬(587 - 1) ∣ N := by
    simpa only [show 587 - 1 = 586 by norm_num, N] using
  ...
  have hlog :
    ...
    apply logarithmicDerivative_formalDerivative_hasPadicValAtLeast_two
      (p := 587) (N := N) (by norm_num) hNpos
        (polynomialExp P)
    · simpa only [P] using
        constantCoeff_positiveRelationPolynomial587_ne_zero b
    · simpa only [P] using
        constantCoeff_positiveRelationPolynomial587_padicVal_eq_zero b
    · intro s hs hsN
      ...
      have hscaled :=
        scaledPolynomialExp587sq_derivative_hasPadicValAtLeast_two H s
      have hremainder :=
        additiveRemainder_derivative_hasPadicValAtLeast_one
          (p := 587) (by norm_num) s V b₁
    ...
    · rw [hsource, formalDerivativeAtZero_add,
      ...
      rw [hconstant, zero_add]
      exact
        (scaledPolynomialExp587sq_derivative_hasPadicValAtLeast_two H N).add
          (additiveRemainder_derivative_hasPadicValAtLeast_two
            (by norm_num) hNdiv hNgt hNnot V b₁)
  simpa only [N, sourceDerivativeOrder587] using hlog

/-- The derivative of the logarithm of the positive relation polynomial is
exactly the existing exponent-weighted `relationDerivative587`. -/
theorem relationDerivative587_natCast_eq
    (b : SourceIndex 587 → ℕ) (k : SourceIndex 587) :
    relationDerivative587 (fun i ↦ (b i : ℤ)) k =
      formalDerivativeAtZero (sourceDerivativeOrder587 k)
        (logarithmicDerivative
          (polynomialExp (positiveRelationPolynomial587 b))) := by
  rw [show polynomialExp (positiveRelationPolynomial587 b) =
      polynomialExp587 (positiveRelationPolynomial587 b) by rfl]
  rw [formalDerivativeAtZero_positiveRelationPolynomial587]
  rw [relationDerivative587]
  apply Finset.sum_congr rfl
  intro i hi
  norm_cast
...
/-- Every deep positive relation in the 292 actual diagonal units has all
of Vandiver's selected relation derivatives divisible by `587²`.

This is the exact final premise expected by
`VandiverNormalizedRelationDerivative` and `VandiverLemmaTwoAssembly`. -/
theorem positiveRelationDerivativeCongruences587
    {zeta : K} (hzeta : IsPrimitiveRoot zeta 587) :
    PositiveRelationDerivativeCongruences587 hzeta := by
  intro v t b hvdeep hrel k
  obtain ⟨c, hdeep⟩ := hvdeep
  obtain ⟨H, hzero, hsquare⟩ :=
    exists_vanishingPositiveRelationPolynomial587
      hzeta v c t b hdeep hrel
  rw [relationDerivative587_natCast_eq]
  exact
    positiveRelation_logarithmicDerivative_hasPadicValAtLeast_two
      hzeta b H (c ^ (587 * (586 * t))) hzero hsquare k

end

end Fermat.FiveHundredEightySeven.VandiverPositiveRelationDerivative
