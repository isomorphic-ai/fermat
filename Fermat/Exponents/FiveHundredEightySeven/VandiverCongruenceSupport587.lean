import Fermat.Exponents.FiveHundredEightySeven.VandiverHistorical
import Fermat.Exponents.FiveHundredEightySeven.VandiverRelationNormalization

/-!
# Local congruence support for Vandiver's historical descent at 587
...
587th-power depth step twice then turns congruence modulo `(ζ - 1)^2` into
congruence modulo `(ζ - 1)^1174` for 344569th powers.  The final two theorems
...
so `2k` is divisible.  Since the prime above 587 does not divide 2, it
divides `k`. -/
theorem exists_zeta_sub_one_sq_dvd_sub_int_of_real587
  ...
  have hconjk :
    ...
    rw [map_sub,
      ringOfIntegersComplexConj_eq_mod_zeta_sub_one587 hζ k,
      sub_self]
  have hrel :
      k = ((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) *
      ...
      _ = π * (((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) *
        ...
        rw [show NumberField.IsCMField.ringOfIntegersComplexConj K π =
          (((-hζ.unit'⁻¹ : (𝓞 K)ˣ) : 𝓞 K) * π) by
            simpa only [π] using
              ringOfIntegersComplexConj_zeta_sub_one587 hζ]
        ring
  have huplus :
  ...
  have hπnot2 : ¬ π ∣ (2 : 𝓞 K) := by
    intro htwo
    have : (587 : ℤ) ∣ 2 := (zeta_sub_one_dvd_Int_iff hζ).mp (by
...
/-- If real, nonramified factors `a` and `b` occur in a depth-1174 product
congruence

`U*a^(587²) ≡ V*b^(587²)`,

then `U/V` is congruent to the 344569th power of a rational integer at the
same depth.

The rational residue of `a` is invertible modulo 587.  A Bézout inverse
...
this to depth 1174. -/
theorem exists_int_ratio_pow344569_congruent587
    ...
    (hprod :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1174 ∣
        (U : 𝓞 K) * a ^ 344569 - (V : 𝓞 K) * b ^ 344569) :
    ∃ c : ℤ,
      ((hζ.unit' : 𝓞 K) - 1) ^ 1174 ∣
        (((U / V : (𝓞 K)ˣ) : 𝓞 K) - (c : 𝓞 K) ^ 344569) := by
  let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
  obtain ⟨ca, hca⟩ :=
    exists_zeta_sub_one_sq_dvd_sub_int_of_real587 hζ a ha_real
  obtain ⟨cb, hcb⟩ :=
    exists_zeta_sub_one_sq_dvd_sub_int_of_real587 hζ b hb_real
  have hπ586 : π ^ 586 ∣ (587 : 𝓞 K) := by
    have hp := (associated_zeta_sub_one_pow_prime hζ).dvd
    norm_num at hp
    simpa only [π] using hp
  have hπ2_587 : π ^ 2 ∣ (587 : 𝓞 K) :=
    (pow_dvd_pow π (by norm_num : 2 ≤ 586)).trans hπ586
  have hca_not : ¬ (587 : ℤ) ∣ ca := by
    intro h587ca
    have hπca : π ∣ (ca : 𝓞 K) := by
      simpa only [π] using
        (zeta_sub_one_dvd_Int_iff hζ).mpr h587ca
    ...
    ring
  have hcop587ca : IsCoprime (587 : ℤ) ca :=
    ((show Prime (587 : ℤ) by norm_num).coprime_iff_not_dvd).mpr hca_not
  obtain ⟨u, v, huv⟩ := hcop587ca.symm
  let c : ℤ := u * cb
  have hmiddle_int : (587 : ℤ) ∣ ca * c - cb := by
    refine ⟨-(v * cb), ?_⟩
    dsimp only [c]
    calc
  ...
  have hmiddle :
    ...
    have hcast :
      ...
      norm_cast
    rw [hcast]
    exact hπ2_587.trans (dvd_mul_right _ _)
  have hacb : π ^ 2 ∣ a * (c : 𝓞 K) - b := by
    ...
    ring
  have hp588 :=
    zeta_sub_one_pow_add_fiveHundredEightySix_dvd_pow_sub_pow587
      hζ 2 (by norm_num) (a * (c : 𝓞 K)) b hacb
  have hp1174 :=
    zeta_sub_one_pow_add_fiveHundredEightySix_dvd_pow_sub_pow587
      hζ 588 (by norm_num)
      ((a * (c : 𝓞 K)) ^ 587) (b ^ 587)
      (by simpa only [π] using hp588)
  have hpowers :
      π ^ 1174 ∣
        a ^ 344569 * (c : 𝓞 K) ^ 344569 - b ^ 344569 := by
    simpa only [π, mul_pow, ← pow_mul] using hp1174
  have hcombined :
      π ^ 1174 ∣
        a ^ 344569 *
          ((U : 𝓞 K) - (V : 𝓞 K) * (c : 𝓞 K) ^ 344569) := by
  ...
  have hUV :
    ...
    have htoInteger : hζ.toInteger = (hζ.unit' : 𝓞 K) := by
      ...
      rfl
    have hapow : ¬ π ∣ a ^ 344569 := by
      intro h
      exact haπ (hζ.zeta_sub_one_prime'.dvd_of_dvd_pow h)
    apply hζ.zeta_sub_one_prime'.pow_dvd_of_dvd_mul_left 1174 hapow
  ...
  rw [show (V : 𝓞 K) *
      ((c : 𝓞 K) ^ 344569 * ((V⁻¹ : (𝓞 K)ˣ) : 𝓞 K)) =
        (c : 𝓞 K) ^ 344569 *
          (((V : 𝓞 K) * ((V⁻¹ : (𝓞 K)ˣ) : 𝓞 K))) by ring,
    ← Units.val_mul]
  simp

/-- The negative square of a unit which is a rational 344569th power modulo
`(ζ - 1)^1174` is a rational 587th power modulo `(1 - ζ)^1174`.

Concretely, if `R ≡ c^344569`, then

`-R² ≡ (-c^1174)^587`.

The change from `ζ - 1` to `1 - ζ` is harmless because the exponent 1174 is
even. -/
theorem exists_int_negative_square_ratio_pow587_congruent587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    (R : (𝓞 K)ˣ) (c : ℤ)
    (h :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1174 ∣
    ...
    ∃ d : ℤ,
      ((1 : 𝓞 K) - hζ.unit') ^ 1174 ∣
        (((-(R ^ 2) : (𝓞 K)ˣ) : 𝓞 K) - (d : 𝓞 K) ^ 587) := by
  refine ⟨-(c ^ 1174), ?_⟩
  have hsquare :
      ((hζ.unit' : 𝓞 K) - 1) ^ 1174 ∣
        (R : 𝓞 K) ^ 2 - ((c : 𝓞 K) ^ 344569) ^ 2 :=
    h.trans (sub_dvd_pow_sub_pow (R : 𝓞 K) ((c : 𝓞 K) ^ 344569) 2)
  have heven :
      ((1 : 𝓞 K) - hζ.unit') ^ 1174 =
        ((hζ.unit' : 𝓞 K) - 1) ^ 1174 := by
  ...
  rw [Odd.neg_pow (by norm_num : Odd 587)]
  norm_num [← pow_mul]
  ring
