...
/-!
# Prime-support bookkeeping for Vandiver's historical descent at 587
...
lemma primeIdealFactorSupport587_pow (a : 𝓞 K) {n : ℕ} (hn : n ≠ 0) :
    primeIdealFactorSupport587 (a ^ n) =
      primeIdealFactorSupport587 a := by
...
lemma primeIdealFactorSupport587_subset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q) :
    primeIdealFactorSupport587 a ⊆ primeIdealFactorSupport587 b := by
  ...
  have hP' :
      P ∈ UniqueFactorizationMonoid.normalizedFactors
        (Ideal.span {a}) := by
    simpa only [primeIdealFactorSupport587, Multiset.mem_toFinset] using hP
  have hprime :
...
lemma primeIdealFactorSupport587_ssubset_of_span_factor_of_witness
  ...
  have hsubset :=
    primeIdealFactorSupport587_subset_of_span_factor hb hfactor
  ...
  · change P ∈
    ...
    exact Or.inr hPQ
  · simpa only [primeIdealFactorSupport587, Multiset.mem_toFinset] using hPa
...
`(a)`. -/
lemma primeIdealFactorSupport587_ssubset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q)
    (hQ : Q ≠ ⊤)
  ...
  apply primeIdealFactorSupport587_ssubset_of_span_factor_of_witness
    hb hfactor hPQ
  intro hPa
...
and gives the strict inclusion required by `ConjugationPowerReductionData587`.
-/
theorem historicalEquationEightA_square_support_strict587
    ...
    (hQ : Q ≠ ⊤)
    (hcop :
      IsCoprime (historicalEquationEightAIdeal587 hζ s) Q) :
    primeIdealFactorSupport587 (ρzero ^ 2) ⊂
      primeIdealFactorSupport587 s.xi := by
  rw [primeIdealFactorSupport587_pow ρzero (by norm_num : 2 ≠ 0)]
  apply primeIdealFactorSupport587_ssubset_of_span_factor
    s.xi_ne_zero
