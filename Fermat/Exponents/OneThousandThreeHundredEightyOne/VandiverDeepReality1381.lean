...
import Fermat.Exponents.OneThousandThreeHundredEightyOne.VandiverDeepPolynomial
import Fermat.Exponents.OneThousandThreeHundredEightyOne.VandiverRelationNormalization1381

/-!
# Deep Vandiver units are real at exponent 1381
...
that the depth-2762 local hypothesis itself forces an ambient cyclotomic unit
to be real.
...
namespace Fermat.OneThousandThreeHundredEightyOne.VandiverDeepReality
...
inverse. -/
theorem unitsComplexConj_zeta1381 {zeta : K}
  ...
  exact
    Fermat.Irregular.CyclotomicDiscriminantPrime.complexConj_zeta_inv hzeta

/-- Conjugation changes `zeta - 1` by the explicit unit `-zeta⁻¹`. -/
theorem ringOfIntegersComplexConj_zeta_sub_one1381 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) :
  ...
  have hconjzeta :
    ...
    congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K)
      (unitsComplexConj_zeta1381 hzeta)
  rw [hconjzeta]
  have hinv :
...
conjugation. -/
theorem zeta_sub_one_pow_dvd_conj_of_dvd1381 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) (n : ℕ) (a : 𝓞 K)
    (h : ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow,
    ringOfIntegersComplexConj_zeta_sub_one1381, mul_pow]
...
/-- A square of a 1381st root of unity which is `1` modulo
`(zeta - 1)^2` is literally `1`. -/
theorem zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd1381 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) (j : ℕ)
  ...
  · have hknotdvd : ¬1381 ∣ k := by
      intro h
      obtain ⟨t, ht⟩ := h
      apply hk
      omega
    have hkcoprime : k.Coprime 1381 := by
      apply Nat.coprime_comm.mpr
      exact (show Nat.Prime 1381 by norm_num).coprime_iff_not_dvd.mpr
        hknotdvd
...
theorem unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj1381 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) (u : (𝓞 K)ˣ)
    (hdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
  ...
  have hassoc : Associated
      (((((hzeta.unit' ^ j) ^ 2 : (𝓞 K)ˣ) : 𝓞 K)) - 1)
      ((u : 𝓞 K) - (cu : 𝓞 K)) :=
    ⟨cu, by simp only [hfactor]⟩
  have hrootdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (((((hzeta.unit' ^ j) ^ 2 : (𝓞 K)ˣ) : 𝓞 K)) - 1) :=
    hassoc.dvd_iff_dvd_right.mpr (hcu ▸ hdiv)
  have hroot :=
    zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd1381 hzeta j hrootdiv
  rw [hroot] at hj
  have hucu : u = cu := by
    calc
      u = (u * cu⁻¹) * cu := by simp
...
/-- The depth-2762 hypothesis forces a unit to be real. -/
theorem isVandiverDeep_unitsComplexConj_eq {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 1381) hzeta u) :
    NumberField.IsCMField.unitsComplexConj K u = u := by
  obtain ⟨c, hc⟩ := hdeep
  have hsign : ((hzeta.unit' : 𝓞 K) - 1) ^ 2762 ∣
      (u : 𝓞 K) - (c : 𝓞 K) ^ 1381 := by
    have hbase : (hzeta.unit' : 𝓞 K) - 1 ∣
        (1 : 𝓞 K) - hzeta.unit' := by
      refine ⟨-1, ?_⟩
      ring
    exact (pow_dvd_pow_of_dvd hbase 2762).trans hc
  have hconj := zeta_sub_one_pow_dvd_conj_of_dvd1381
    hzeta 2762 ((u : 𝓞 K) - (c : 𝓞 K) ^ 1381) hsign
  have hconj' : ((hzeta.unit' : 𝓞 K) - 1) ^ 2762 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (u : 𝓞 K) -
        (c : 𝓞 K) ^ 1381 := by
    simpa only [map_sub, map_pow, map_intCast] using hconj
  have hdefect2762 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2762 ∣
      (u : 𝓞 K) -
  ...
  have hdefect2 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
    ...
    (pow_dvd_pow ((hzeta.unit' : 𝓞 K) - 1) (by omega)).trans
      hdefect2762
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj1381
    hzeta u hdefect2

/-- A deeply congruent ambient unit, packaged in the real-unit subgroup. -/
def deepRealUnit1381 {zeta : K} (hzeta : IsPrimitiveRoot zeta 1381)
    (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 1381) hzeta u) :
...
@[simp]
theorem deepRealUnit1381_coe {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 1381) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 1381) hzeta u) :
...
end Fermat.OneThousandThreeHundredEightyOne.VandiverDeepReality
