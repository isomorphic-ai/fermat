...
import Fermat.Exponents.OneThousandThreeHundredEightyOne.CircularUnitMatrix

/-!
# Natural cyclic-correlation evaluator at exponent 1381

The 690 phase and inverse representatives are evaluated by the generic
tail-recursive natural-number loop. The structural theorem below transports
the result back to the abstract correlation over `ZMod 1381`; concrete
...
theorem natCorrelation_cast (d : Cyc) :
  ...
  simpa only [natCorrelation, arrayFunction, symbolPhase,
    ...
    cast_evaluate_eq_cyclicCorrelation
      1381 690 symbolPhaseData correlationInverseData d
...
theorem phaseCorrelation_of_nat {d : Cyc}
  ...
  · rw [if_pos hd] at h ⊢
    simpa only [Nat.cast_one] using
      congrArg (fun n : ℕ ↦ (n : ZMod 1381)) h
  · rw [if_neg hd] at h ⊢
