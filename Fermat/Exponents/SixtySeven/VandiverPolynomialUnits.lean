import Fermat.Exponents.SixtySeven.VandiverDiagonalDerivative67
import Fermat.Exponents.SixtySeven.VandiverDiagonalUnits67

/-!
# Vandiver's integral unit polynomials at exponent 67
...
`epsilon_s(W) = W^(33*s) * (1 + W^s + ... + W^(1341*s))`.
...
namespace Fermat.SixtySeven.VandiverPolynomialUnits
...
local instance : Fact (Nat.Prime 67) := ⟨by decide⟩

set_option maxRecDepth 100000

/-- Formal substitution of `exp V` into an integer polynomial. -/
def polynomialExp67 (P : Polynomial ℤ) : PowerSeries ℚ :=
  Polynomial.eval₂ (algebraMap ℤ (PowerSeries ℚ))
    (PowerSeries.exp ℚ) P

/-- Vandiver's basic integral polynomial
`W^(33*s) * (1 + W^s + ... + W^(1341*s))`. -/
def basicVandiverPolynomial67 (s : ℕ) : Polynomial ℤ :=
  Polynomial.X ^ (33 * s) *
    ∑ j ∈ Finset.range 1342, Polynomial.X ^ (s * j)
...
theorem polynomialExp67_basicVandiverPolynomial67 (s : ℕ) :
    polynomialExp67 (basicVandiverPolynomial67 s) =
      normalizedGeomExp 1342 33 s := by
  rw [polynomialExp67, basicVandiverPolynomial67, Polynomial.eval₂_mul,
...
basic Vandiver unit. -/
theorem eval₂_basicVandiverPolynomial67 {zeta : K}
    ...
    Polynomial.eval₂ (Int.castRingHom (𝓞 K)) hzeta.toInteger
      ...
      (basicVandiverUnit67 hzeta j : 𝓞 K) := by
  rw [basicVandiverUnit67,
    Fermat.Irregular.CircularUnitFamily.normalizedCircularUnit_val]
  rw [conjugate_toInteger (hzeta := hzeta) j]
  simp only [basicVandiverPolynomial67, Polynomial.eval₂_mul,
    Polynomial.eval₂_pow, Polynomial.eval₂_X,
    Polynomial.eval₂_finsetSum]
...
diagonal unit. -/
def diagonalVandiverPolynomial67
    (i : SourceIndex 67) : Polynomial ℤ :=
  ∏ j : VandiverFactorIndex67,
    basicVandiverPolynomial67 (conjugateExponent67 j) ^
...
theorem polynomialExp67_diagonalVandiverPolynomial67
    (i : SourceIndex 67) :
    polynomialExp67 (diagonalVandiverPolynomial67 i) =
      Fermat.SixtySeven.VandiverDiagonalDerivative.integralDiagonalSeries67 i := by
  rw [diagonalVandiverPolynomial67, polynomialExp67,
    Polynomial.eval₂_finsetProd]
  simp_rw [Polynomial.eval₂_pow]
  rw [Fermat.SixtySeven.VandiverDiagonalDerivative.integralDiagonalSeries67,
    integralDiagonalExp]
  norm_num
  ...
  rw [dif_pos hj]
  simp only [integralDiagonalFactor, diagonalWeight67,
    conjugateExponent67, sourceNumber]
  change
    (polynomialExp67 (basicVandiverPolynomial67 (1342 ^ j))) ^
          integralDiagonalWeight 67 1342 (i.val + 1) j =
      normalizedGeomExp 1342 33 (1342 ^ j) ^
          integralDiagonalWeight 67 1342 (i.val + 1) j
  rw [polynomialExp67_basicVandiverPolynomial67]

variable [NumberField K] [IsCyclotomicExtension {67} ℚ K]
  [NumberField.IsCMField K]

/-- Evaluation at the chosen primitive root gives the literal ambient
diagonal unit constructed in `VandiverDiagonalUnits`. -/
theorem eval₂_diagonalVandiverPolynomial67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (i : SourceIndex 67) :
    Polynomial.eval₂ (Int.castRingHom (𝓞 K)) hzeta.toInteger
        (diagonalVandiverPolynomial67 i) =
      (diagonalVandiverUnit67 hzeta i : 𝓞 K) := by
  rw [diagonalVandiverPolynomial67, Polynomial.eval₂_finsetProd,
    diagonalVandiverUnit67]
  simp only [Units.coe_prod, Units.val_pow_eq_pow_val]
  apply Finset.prod_congr rfl
  intro j hj
  rw [Polynomial.eval₂_pow]
  congr 1
  exact eval₂_basicVandiverPolynomial67 hzeta j
...
factor `66 = 67 - 1` is exactly Vandiver's factor in equation (3b). -/
def positiveRelationPolynomial67
    (b : SourceIndex 67 → ℕ) : Polynomial ℤ :=
  ∏ i, diagonalVandiverPolynomial67 i ^ (66 * b i)

/-- Exponential substitution commutes with the positive relation product. -/
theorem polynomialExp67_positiveRelationPolynomial67
    (b : SourceIndex 67 → ℕ) :
    polynomialExp67 (positiveRelationPolynomial67 b) =
      ∏ i,
        Fermat.SixtySeven.VandiverDiagonalDerivative.integralDiagonalSeries67 i ^
          (66 * b i) := by
  rw [positiveRelationPolynomial67, polynomialExp67,
    Polynomial.eval₂_finsetProd]
  apply Finset.prod_congr rfl
  intro i hi
  rw [Polynomial.eval₂_pow]
  congr 1
  exact polynomialExp67_diagonalVandiverPolynomial67 i

/-- Every diagonal series has nonzero constant coefficient. -/
theorem constantCoeff_integralDiagonalSeries67_ne_zero
    (i : SourceIndex 67) :
...
theorem logarithmicDerivative_positiveRelationPolynomial67
    (b : SourceIndex 67 → ℕ) :
    logarithmicDerivative
        (polynomialExp67 (positiveRelationPolynomial67 b)) =
      ∑ i, PowerSeries.C ((66 * b i : ℕ) : ℚ) *
        logarithmicDerivative
          (Fermat.SixtySeven.VandiverDiagonalDerivative.integralDiagonalSeries67 i) := by
  rw [polynomialExp67_positiveRelationPolynomial67 b]
  rw [logarithmicDerivative_prod]
  · apply Finset.sum_congr rfl
    intro i hi
    rw [logarithmicDerivative_pow]
    exact constantCoeff_integralDiagonalSeries67_ne_zero i
  · intro i hi
    apply constantCoeff_pow_ne_zero
    exact constantCoeff_integralDiagonalSeries67_ne_zero i

/-- High derivatives of the preceding identity, in the exact normalization
used by `relationDerivative67`. -/
theorem formalDerivativeAtZero_positiveRelationPolynomial67
    (b : SourceIndex 67 → ℕ) (k : SourceIndex 67) :
    formalDerivativeAtZero
      (Fermat.SixtySeven.VandiverDiagonalDerivative.sourceDerivativeOrder67 k)
      (logarithmicDerivative
        (polynomialExp67 (positiveRelationPolynomial67 b))) =
      ∑ i, ((66 * b i : ℕ) : ℚ) *
        formalDerivativeAtZero
          ...
          (logarithmicDerivative
            (Fermat.SixtySeven.VandiverDiagonalDerivative.integralDiagonalSeries67 i)) := by
  rw [logarithmicDerivative_positiveRelationPolynomial67 b,
...
theorem relationDerivative67_eq_positive_sub_negative
    (a : SourceIndex 67 → ℤ) (k : SourceIndex 67) :
    Fermat.SixtySeven.VandiverDiagonalDerivative.relationDerivative67 a k =
      formalDerivativeAtZero
        (Fermat.SixtySeven.VandiverDiagonalDerivative.sourceDerivativeOrder67 k)
        (logarithmicDerivative
          (polynomialExp67
            (positiveRelationPolynomial67 (fun i ↦ (a i).toNat)))) -
      formalDerivativeAtZero
        (Fermat.SixtySeven.VandiverDiagonalDerivative.sourceDerivativeOrder67 k)
        (logarithmicDerivative
          (polynomialExp67
            (positiveRelationPolynomial67
              (fun i ↦ (-a i).toNat)))) := by
  rw [formalDerivativeAtZero_positiveRelationPolynomial67,
    formalDerivativeAtZero_positiveRelationPolynomial67]
  rw [Fermat.SixtySeven.VandiverDiagonalDerivative.relationDerivative67,
...
`positiveRelationPolynomial67`. -/
def positiveRelationUnit67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67)
    (b : SourceIndex 67 → ℕ) : (𝓞 K)ˣ :=
...
theorem eval₂_positiveRelationPolynomial67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67)
    (b : SourceIndex 67 → ℕ) :
    Polynomial.eval₂ (Int.castRingHom (𝓞 K)) hzeta.toInteger
        (positiveRelationPolynomial67 b) =
      (positiveRelationUnit67 hzeta b ^ 66 : 𝓞 K) := by
  rw [positiveRelationPolynomial67, Polynomial.eval₂_finsetProd,
    positiveRelationUnit67, Units.coe_prod]
  simp_rw [Units.val_pow_eq_pow_val]
  rw [← Finset.prod_pow Finset.univ 66
    (fun i ↦ (diagonalVandiverUnit67 hzeta i : 𝓞 K) ^ b i)]
  apply Finset.prod_congr rfl
  intro i hi
  rw [Polynomial.eval₂_pow, eval₂_diagonalVandiverPolynomial67,
    ← pow_mul]
  congr 1
  omega
...
theorem eval₂_positive_relation_of_relation {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67)
    (u : (𝓞 K)ˣ) (t : ℕ) (a : SourceIndex 67 → ℤ)
    (hrel : u ^ t =
    ...
    Polynomial.eval₂ (Int.castRingHom (𝓞 K)) hzeta.toInteger
      ...
      (u : 𝓞 K) ^ (66 * t) *
        Polynomial.eval₂ (Int.castRingHom (𝓞 K)) hzeta.toInteger
          (positiveRelationPolynomial67 (fun i ↦ (-a i).toNat)) := by
  rw [eval₂_positiveRelationPolynomial67,
    eval₂_positiveRelationPolynomial67]
  have hpos := positive_relation_of_zpow_relation
    (E := diagonalVandiverUnit67 hzeta) t a hrel
  have hpow := congrArg (fun x : (𝓞 K)ˣ ↦ x ^ 66) hpos
  have hval := congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K) hpow
  simpa only [positiveRelationUnit67, Units.val_pow_eq_pow_val,
    Units.val_mul, mul_pow, ← pow_mul, Nat.mul_comm] using hval

end

end Fermat.SixtySeven.VandiverPolynomialUnits
