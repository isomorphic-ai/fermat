...
/-!
# Prime-support bookkeeping for Vandiver's historical descent at 67
...
lemma primeIdealFactorSupport67_pow (a : 𝓞 K) {n : ℕ} (hn : n ≠ 0) :
    primeIdealFactorSupport67 (a ^ n) =
      primeIdealFactorSupport67 a := by
...
lemma primeIdealFactorSupport67_subset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q) :
    primeIdealFactorSupport67 a ⊆ primeIdealFactorSupport67 b := by
  ...
  have hP' :
      P ∈ UniqueFactorizationMonoid.normalizedFactors
        (Ideal.span {a}) := by
    simpa only [primeIdealFactorSupport67, Multiset.mem_toFinset] using hP
  have hprime :
...
lemma primeIdealFactorSupport67_ssubset_of_span_factor_of_witness
  ...
  have hsubset :=
    primeIdealFactorSupport67_subset_of_span_factor hb hfactor
  ...
  · change P ∈
    ...
    exact Or.inr hPQ
  · simpa only [primeIdealFactorSupport67, Multiset.mem_toFinset] using hPa
...
`(a)`. -/
lemma primeIdealFactorSupport67_ssubset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q)
    (hQ : Q ≠ ⊤)
  ...
  apply primeIdealFactorSupport67_ssubset_of_span_factor_of_witness
    hb hfactor hPQ
  intro hPa
...
and gives the strict inclusion required by `ConjugationPowerReductionData67`.
-/
theorem historicalEquationEightA_square_support_strict67
    ...
    (hQ : Q ≠ ⊤)
    (hcop :
      IsCoprime (historicalEquationEightAIdeal67 hζ s) Q) :
    primeIdealFactorSupport67 (ρzero ^ 2) ⊂
      primeIdealFactorSupport67 s.xi := by
  rw [primeIdealFactorSupport67_pow ρzero (by norm_num : 2 ≠ 0)]
  apply primeIdealFactorSupport67_ssubset_of_span_factor
    s.xi_ne_zero
