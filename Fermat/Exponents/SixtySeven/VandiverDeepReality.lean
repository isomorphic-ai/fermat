...
import Fermat.Exponents.SixtySeven.VandiverRelationNormalization

/-!
# Deep Vandiver units are real at exponent 67
...
that the depth-134 local hypothesis itself forces an ambient cyclotomic unit
to be real.
...
namespace Fermat.SixtySeven.VandiverDeepReality
...
inverse. -/
theorem unitsComplexConj_zeta67 {zeta : K}
...
theorem ringOfIntegersComplexConj_zeta_sub_one67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) :
...
conjugation. -/
theorem zeta_sub_one_pow_dvd_conj_of_dvd67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (n : ℕ) (a : 𝓞 K)
    (h : ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow,
    ringOfIntegersComplexConj_zeta_sub_one67, mul_pow]
...
/-- A square of a 67th root of unity which is `1` modulo
`(zeta - 1)^2` is literally `1`. -/
theorem zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (j : ℕ)
  ...
  · have hknotdvd : ¬67 ∣ k := by
    ...
    have hkcoprime : k.Coprime 67 := by
      ...
      exact (show Nat.Prime 67 by norm_num).coprime_iff_not_dvd.mpr
        hknotdvd
...
theorem unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj67 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (u : (𝓞 K)ˣ)
    (hdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
...
/-- The depth-134 hypothesis forces a unit to be real. -/
theorem isVandiverDeep_unitsComplexConj_eq {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 67) hzeta u) :
    NumberField.IsCMField.unitsComplexConj K u = u := by
  obtain ⟨c, hc⟩ := hdeep
  have hsign : ((hzeta.unit' : 𝓞 K) - 1) ^ 134 ∣
      (u : 𝓞 K) - (c : 𝓞 K) ^ 67 := by
    have hbase : (hzeta.unit' : 𝓞 K) - 1 ∣
        (1 : 𝓞 K) - hzeta.unit' := by
      refine ⟨-1, ?_⟩
      ring
    exact (pow_dvd_pow_of_dvd hbase 134).trans hc
  have hconj := zeta_sub_one_pow_dvd_conj_of_dvd67
    hzeta 134 ((u : 𝓞 K) - (c : 𝓞 K) ^ 67) hsign
  have hconj' : ((hzeta.unit' : 𝓞 K) - 1) ^ 134 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (u : 𝓞 K) -
        (c : 𝓞 K) ^ 67 := by
    simpa only [map_sub, map_pow, map_intCast] using hconj
  have hdefect134 : ((hzeta.unit' : 𝓞 K) - 1) ^ 134 ∣
      (u : 𝓞 K) -
  ...
  have hdefect2 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
    ...
    (pow_dvd_pow ((hzeta.unit' : 𝓞 K) - 1) (by omega)).trans
      hdefect134
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj67
    hzeta u hdefect2

/-- A deeply congruent ambient unit, packaged in the real-unit subgroup. -/
def deepRealUnit67 {zeta : K} (hzeta : IsPrimitiveRoot zeta 67)
    (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 67) hzeta u) :
...
@[simp]
theorem deepRealUnit67_coe {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 67) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 67) hzeta u) :
...
end Fermat.SixtySeven.VandiverDeepReality
