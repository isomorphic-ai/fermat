import Fermat.Descent.Irregular.TakagiReflection67
import Fermat.Exponents.SixtySeven.VandiverHistorical

/-!
# Unconditional Takagi--Furtwängler step in Vandiver's historical proof

The universal predicate `LemmaOne K 67` is stronger than the instance
...
`TakagiReflection67` therefore proves the required historical equation
...
namespace Fermat.SixtySeven.TakagiHistorical67

noncomputable section

open Fermat.Irregular.VandiverHistoricalDescent
open Fermat.Irregular.VandiverLemmaOne
open Fermat.Irregular.TakagiReflection67
open Fermat.SixtySeven.VandiverHistorical
...
theorem exists_historicalEquationSevenA67_unconditional
    ...
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal67 hζ s *
        ...
        Ideal.span {r} := by
  exact exists_conjugatePairEquationSevenAGenerator67 hζ
    (historicalZetaFactorIdeal67 hζ s)
...
historical results. -/
theorem exists_historicalEquationEight67_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 67) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
  ...
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA67_unconditional hζ s hs
  obtain ⟨t, hsevenD⟩ :=
    exists_historicalEquationSevenD67 hζ s hs
  obtain ⟨hI0, hJ0⟩ :=
    historicalFactorIdeals_ne_zero67 hζ s hs
...
conjugation. -/
theorem exists_historicalEquationEight_realUnit67_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 67) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal67 hζ s = Ideal.span {ρ} ∧
  ...
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight67_unconditional hζ s hs
  exact ⟨ρ, η, hI, heq,
...
theorem exists_historicalEquationEight_pair_one67_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 67) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρ, η₀, -, heq, hη₀real⟩ :=
    exists_historicalEquationEight_realUnit67_unconditional hζ s hs
  let η : (𝓞 K)ˣ := -η₀
  let ρminus : 𝓞 K :=
...
theorem exists_historicalEquationEight_pair_two67_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 67) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one67_unconditional
      hζtwo stwo hstwo
  have hassoc :
...
end Fermat.SixtySeven.TakagiHistorical67
