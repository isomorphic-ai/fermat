import Fermat.Exponents.Five.Conservation.PowerSplitting
...
Dirichlet entry-arithmetic formalization in
`Fermat/Five/InitialArithmetic.lean`.  This module does not import that file
or cite any declaration from the earlier exponent-five route.
...
theorem split_odd_core {q r z : ℕ} (hq : 0 < q) (hr : 0 < r)
  ...
  have hpowInt : ((25 * r : ℕ) : ℤ) * K = ((5 * z : ℕ) : ℤ) ^ 5 := by
    exact_mod_cast hpow
  obtain ⟨a, b, ha, hb⟩ := Reconstruction.exists_two_pow_eq_of_mul_eq_pow
