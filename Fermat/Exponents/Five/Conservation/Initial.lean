import Fermat.Exponents.Five.Conservation.Coordinates
import Fermat.Exponents.Five.Conservation.DescentCore
import Fermat.Exponents.Five.Conservation.PowerExtraction

/-!
# Entry data for Dirichlet's exponent-five descents

This arithmetic is reconstructed, with credit, from the repository's earlier
Dirichlet initial-state formalization in `Fermat/Five/Initial.lean`.  This
module does not import that file or cite any declaration from the earlier
exponent-five route.
...
namespace Fermat.Five.Conservation.Reconstruction.Dirichlet
...
theorem OddCore.exists_oddState {q r z : ℕ} (d : OddCore q r z) :
  ...
  obtain ⟨t, s, htPos, hsPos, hts, htOdd, hsOdd, htFive, hcoord⟩ :=
    Reconstruction.PowerExtraction.exists_odd_coordinates_nat
  ...
  have hrelation : 16 * r ^ 2 = s * F t s := by
    ...
    calc
      ...
      _ = 5 * s * Reconstruction.PowerExtraction.quarticNat t s := hcoord
      _ = 5 * (s * F t s) := by
        simp only [F, Reconstruction.PowerExtraction.quarticNat]
        ring
...
theorem EvenCore.exists_evenState {q r z : ℕ} (d : EvenCore q r z) :
  ...
  obtain ⟨t, s, htPos, hsPos, hts, htOdd, hsEven, htFive, hcoord⟩ :=
    Reconstruction.PowerExtraction.exists_oppositeParity_coordinates_nat
