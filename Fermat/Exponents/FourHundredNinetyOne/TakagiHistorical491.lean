import Fermat.Descent.Irregular.TakagiReflection491
import Fermat.Exponents.FourHundredNinetyOne.VandiverHistorical
...
The universal predicate `LemmaOne K 491` is stronger than the instance
...
`TakagiReflection491` therefore proves the required historical equation
...
namespace Fermat.FourHundredNinetyOne.TakagiHistorical491

noncomputable section

open Fermat.Irregular.VandiverHistoricalDescent
open Fermat.Irregular.VandiverLemmaOne
open Fermat.Irregular.TakagiReflection491
open Fermat.FourHundredNinetyOne.VandiverHistorical
...
theorem exists_historicalEquationSevenA491_unconditional
    ...
    ∃ r : 𝓞 K,
      historicalZetaFactorIdeal491 hζ s *
        ...
        Ideal.span {r} := by
  exact exists_conjugatePairEquationSevenAGenerator491 hζ
    (historicalZetaFactorIdeal491 hζ s)
...
historical results. -/
theorem exists_historicalEquationEight491_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
  ...
  obtain ⟨r, hsevenA⟩ :=
    exists_historicalEquationSevenA491_unconditional hζ s hs
  obtain ⟨t, hsevenD⟩ :=
    exists_historicalEquationSevenD491 hζ s hs
  obtain ⟨hI0, hJ0⟩ :=
    historicalFactorIdeals_ne_zero491 hζ s hs
...
conjugation. -/
theorem exists_historicalEquationEight_realUnit491_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
    ∃ (ρ : 𝓞 K) (η : (𝓞 K)ˣ),
      historicalZetaFactorIdeal491 hζ s = Ideal.span {ρ} ∧
  ...
  obtain ⟨ρ, η, hI, heq⟩ :=
    exists_historicalEquationEight491_unconditional hζ s hs
  exact ⟨ρ, η, hI, heq,
...
theorem exists_historicalEquationEight_pair_one491_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρ, η₀, -, heq, hη₀real⟩ :=
    exists_historicalEquationEight_realUnit491_unconditional hζ s hs
  let η : (𝓞 K)ˣ := -η₀
  let ρminus : 𝓞 K :=
  ...
  have hρnot : ¬ (hζ.unit' : 𝓞 K) - 1 ∣ ρ := by
    ...
    exact dvd_mul_of_dvd_right
      (dvd_pow (n := 491) hρ (by norm_num)) _
...
theorem exists_historicalEquationEight_pair_two491_unconditional
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491) (s : HistoricalState hζ)
    (hs : RealSourceAdmissible hζ s) :
  ...
  obtain ⟨ρtwo, ρminus, η, hη, htwo, hminus, hconj,
      hρtwo, hρminus⟩ :=
    exists_historicalEquationEight_pair_one491_unconditional
      hζtwo stwo hstwo
  have hassoc :
...
end Fermat.FourHundredNinetyOne.TakagiHistorical491
