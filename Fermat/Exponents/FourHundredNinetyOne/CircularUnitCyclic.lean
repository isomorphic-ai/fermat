...
theory to the real residue cycle of order `245`. It preserves the original
public API used by the exponent-491 circular-unit certificate.
-/
...
theorem sum_cyc_eq_zero_add_coord (F : Cyc → ZMod 491) :
  ...
  simpa only [coord] using
    (CyclicDifferenceMatrix.sum_cyc_eq_zero_add_coord (n := 244) F)
