...
/-!
# Prime-support bookkeeping for Vandiver's historical descent at 491
...
lemma primeIdealFactorSupport491_pow (a : 𝓞 K) {n : ℕ} (hn : n ≠ 0) :
    primeIdealFactorSupport491 (a ^ n) =
      primeIdealFactorSupport491 a := by
...
lemma primeIdealFactorSupport491_subset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q) :
    primeIdealFactorSupport491 a ⊆ primeIdealFactorSupport491 b := by
  ...
  have hP' :
      P ∈ UniqueFactorizationMonoid.normalizedFactors
        (Ideal.span {a}) := by
    simpa only [primeIdealFactorSupport491, Multiset.mem_toFinset] using hP
  have hprime :
...
lemma primeIdealFactorSupport491_ssubset_of_span_factor_of_witness
  ...
  have hsubset :=
    primeIdealFactorSupport491_subset_of_span_factor hb hfactor
  ...
  · change P ∈
    ...
    exact Or.inr hPQ
  · simpa only [primeIdealFactorSupport491, Multiset.mem_toFinset] using hPa
...
`(a)`. -/
lemma primeIdealFactorSupport491_ssubset_of_span_factor
    {a b : 𝓞 K} {Q : Ideal (𝓞 K)}
    (hb : b ≠ 0)
    (hfactor : Ideal.span {b} = Ideal.span {a} * Q)
    (hQ : Q ≠ ⊤)
  ...
  apply primeIdealFactorSupport491_ssubset_of_span_factor_of_witness
    hb hfactor hPQ
  intro hPa
...
and gives the strict inclusion required by `ConjugationPowerReductionData491`.
-/
theorem historicalEquationEightA_square_support_strict491
    ...
    (hQ : Q ≠ ⊤)
    (hcop :
      IsCoprime (historicalEquationEightAIdeal491 hζ s) Q) :
    primeIdealFactorSupport491 (ρzero ^ 2) ⊂
      primeIdealFactorSupport491 s.xi := by
  rw [primeIdealFactorSupport491_pow ρzero (by norm_num : 2 ≠ 0)]
  apply primeIdealFactorSupport491_ssubset_of_span_factor
    s.xi_ne_zero
