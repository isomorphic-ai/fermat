import Fermat.Exponents.FourHundredNinetyOne.VandiverGeneratorSupport491

/-!
# Vandiver's equation-(8a) factorization at exponent 491
...
noncomputable def historicalEquationEightAComplement491
  ...
  by
    ...
    let root := fun η : nthRootsFinset 491 (1 : 𝓞 K) ↦
      root_div_zeta_sub_one_dvd_gcd
        (K := K) (p := 491) (x := s.omega) (y := s.theta)
        (z := s.xi) (ε := historicalRegularUnit491 hζ s)
    ...
    exact
      ∏ η ∈ Finset.attach (nthRootsFinset 491 (1 : 𝓞 K)) \ {η₀}, root η

set_option maxRecDepth 50000 in
theorem historicalEquationEightA_factorization_and_coprime491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    Ideal.span {s.xi} =
        historicalEquationEightAIdeal491 hζ s *
          historicalEquationEightAComplement491 hζ s ∧
      IsCoprime (historicalEquationEightAIdeal491 hζ s)
        (historicalEquationEightAComplement491 hζ s) := by
  classical
  let e := historicalState_regularEquation491 hζ s
  let hy := historicalState_theta_not_dvd491 hζ s
  ...
  let Z : Ideal (𝓞 K) :=
    z_div_m (K := K) (p := 491)
      ...
      (ε := historicalRegularUnit491 hζ s) (m := 2 * s.m - 1)
      hζ e hy
  let root := fun η : nthRootsFinset 491 (1 : 𝓞 K) ↦
    root_div_zeta_sub_one_dvd_gcd
      (K := K) (p := 491) (x := s.omega) (y := s.theta)
      (z := s.xi) (ε := historicalRegularUnit491 hζ s)
      (m := 2 * s.m - 1) (by norm_num) hζ e hy η
  let c := fun η : nthRootsFinset 491 (1 : 𝓞 K) ↦
  ...
  have hZ : Z = Ideal.span {s.xi} := by
    have hzspec :
      ...
      simpa only [M, Z] using
        (z_div_m_spec (K := K) (p := 491)
          (x := s.omega) (y := s.theta) (z := s.xi)
          (ε := historicalRegularUnit491 hζ s)
          (m := 2 * s.m - 1) hζ e hy)
    rw [hM, one_mul] at hzspec
    exact hzspec.symm
  have hrootpow (η : nthRootsFinset 491 (1 : 𝓞 K)) :
      root η ^ 491 = c η := by
    simpa only [root, c] using
      (root_div_zeta_sub_one_dvd_gcd_spec
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy η)
  have hprodC :
      ∏ η ∈ Finset.attach (nthRootsFinset 491 (1 : 𝓞 K)), c η =
        (Z * P ^ (2 * s.m - 1)) ^ 491 := by
    simpa only [c, Z, P, π] using
      (prod_c (K := K) (p := 491)
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy)
  have hprodRootPow :
      (∏ η ∈ Finset.attach (nthRootsFinset 491 (1 : 𝓞 K)),
          root η) ^ 491 =
        (Z * P ^ (2 * s.m - 1)) ^ 491 := by
    rw [← Finset.prod_pow]
    simpa only [hrootpow] using hprodC
  have hprodRoot :
      ∏ η ∈ Finset.attach (nthRootsFinset 491 (1 : 𝓞 K)), root η =
        Z * P ^ (2 * s.m - 1) :=
    pow_left_injective (M := Ideal (𝓞 K))
      (by norm_num : 491 ≠ 0) hprodRootPow
  have hη₀one :
      η₀ = oneNthRoot (K := K) (p := 491) := by
    simpa only [η₀, e, hy] using
      distinguishedRoot_eq_one_of_real491 hζ e hy hs.1 hs.2.1
  have hrootη₀ :
      root η₀ = historicalOneFactorIdeal491 hζ s := by
    rw [hη₀one]
    rfl
  have hsplit :
  ...
  have hfactor :
      Ideal.span {s.xi} =
        historicalEquationEightAIdeal491 hζ s * Q := by
    apply mul_left_cancel₀ hP0
    symm
    calc
      P ^ (2 * s.m - 1) *
          (historicalEquationEightAIdeal491 hζ s * Q) =
          (P ^ (2 * s.m - 1) *
            historicalEquationEightAIdeal491 hζ s) * Q := by
              rw [mul_assoc]
  ...
  have hcop :
    ...
    have hroots : IsCoprime (root η₀) (root η) :=
      (IsCoprime.pow_left_iff (by norm_num : 0 < 491)).mp
        ((IsCoprime.pow_right_iff (by norm_num : 0 < 491)).mp hc)
    rw [← hspec] at hroots
    exact hroots.of_mul_left_right
  simpa only [historicalEquationEightAComplement491, e, hy, root, η₀, Q] using
    And.intro hfactor hcop

set_option maxRecDepth 50000 in
theorem historicalEquationEightAComplement_ne_top491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAComplement491 hζ s ≠ ⊤ := by
  classical
  ...
  let η₀ := zeta_sub_one_dvd_root
    ...
    (z := s.xi) (ε := historicalRegularUnit491 hζ s)
    (m := 2 * s.m - 1) (by norm_num) hζ e hy
  let ηζ : nthRootsFinset 491 (1 : 𝓞 K) :=
  ...
  have hηζ_ne : ηζ ≠ η₀ := by
    ...
    intro h
    have hv := congrArg
      (fun x : nthRootsFinset 491 (1 : 𝓞 K) ↦ (x : 𝓞 K)) h
    exact hζ.unit'_coe.ne_one (by norm_num) hv
  have hηζmem :
      ηζ ∈
        Finset.attach (nthRootsFinset 491 (1 : 𝓞 K)) \ {η₀} := by
    simp only [Finset.mem_sdiff, Finset.mem_attach,
      Finset.mem_singleton, true_and]
    exact hηζ_ne
  have hrootDvd :
      root ηζ ∣ historicalEquationEightAComplement491 hζ s := by
    simpa only [historicalEquationEightAComplement491, e, hy, root, η₀] using
  ...
  have hfactorIdealTop :
      historicalZetaFactorIdeal491 hζ s = ⊤ := by
    simpa only [root, ηζ, historicalZetaFactorIdeal491] using hrootTop
  have hqspan :
      Ideal.span {historicalZetaFactor491 hζ s} = ⊤ := by
    rw [← historicalZetaFactorIdeal_pow491 hζ s, hfactorIdealTop]
    simp
  have hqunit :
      IsUnit (historicalZetaFactor491 hζ s) :=
    Ideal.span_singleton_eq_top.mp hqspan
  let qUnit : (𝓞 K)ˣ := hqunit.unit
  let A : (𝓞 K)ˣ := -qUnit
  let B : (𝓞 K)ˣ := NumberField.IsCMField.unitsComplexConj K A
  have hqUnitVal :
      (qUnit : 𝓞 K) = historicalZetaFactor491 hζ s :=
    hqunit.unit_spec
  have hAval :
      (A : 𝓞 K) = -historicalZetaFactor491 hζ s := by
    dsimp [A]
  ...
  have hone :
    ...
    have hqmul :
        historicalZetaFactor491 hζ s * π =
  ...
  have hr_ne : (r : 𝓞 K) ≠ 1 := by
    ...
    exact historicalState_omega_add_theta_ne_zero491 hζ s
      ((mul_eq_zero.mp hzero).resolve_left hplusUnit.ne_zero)
  have h491sum : (491 : 𝓞 K) ∣ s.omega + s.theta := by
    have h492 :=
      historicalState_omega_add_theta_highDivisibility491 hζ s hs
    have h490 :
        π ^ 490 ∣ s.omega + s.theta :=
      (pow_dvd_pow π (by norm_num : 490 ≤ 492)).trans
        (by simpa only [π] using h492)
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mp
      (by simpa only [π] using h490)
  have h491prod :
      (491 : 𝓞 K) ∣
        (1 - (r : 𝓞 K)) *
          (1 - (hζ.unit' : 𝓞 K)) := by
    have hwithB :
        (491 : 𝓞 K) ∣
          ((1 - (r : 𝓞 K)) *
            (1 - (hζ.unit' : 𝓞 K))) * (B : 𝓞 K) := by
      rw [hidentity]
      exact dvd_mul_of_dvd_right h491sum _
    exact (B.isUnit.dvd_mul_right).mp hwithB
  have hrpow : (r : 𝓞 K) ^ 491 = 1 := by
    change ((((hζ.unit' : 𝓞 K) ^ j) ^ 2) ^ 491) = 1
    rw [← pow_mul, ← pow_mul]
    rw [show j * (2 * 491) = 491 * (j * 2) by omega, pow_mul,
      hζ.unit'_coe.pow_eq_one, one_pow]
  let ηr : nthRootsFinset 491 (1 : 𝓞 K) :=
    ⟨(r : 𝓞 K), by
      rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 491)]
      exact hrpow⟩
  let ηone : nthRootsFinset 491 (1 : 𝓞 K) :=
  ...
  have hrAssocSub :
    ...
    simpa only [ηr, ηone, oneNthRoot, π] using
      (hζ.unit'_coe.ntRootsFinset_pairwise_associated_sub_one_sub_of_prime
        (by norm_num : Nat.Prime 491) ηr.prop ηone.prop hηr_ne).symm
  have hrAssoc :
    ...
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mpr
      (by
        simpa only [π] using
          (show ((491 : ℕ) : 𝓞 K) ∣
              (1 - (r : 𝓞 K)) * (1 - (hζ.unit' : 𝓞 K)) by
            convert h491prod using 1))
  have hbad : π ^ 490 ∣ π ^ 2 :=
    (hprodAssoc.dvd_iff_dvd_right).mp hπ490prod
  have hle :
      490 ≤ 2 :=
    (pow_dvd_pow_iff
      (hζ.unit'_coe.sub_one_ne_zero (by norm_num))
...
theorem historicalEquationEightA_square_support_strict_unconditional491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s)
    (ρzero : 𝓞 K)
    (hgenerator :
      historicalEquationEightAIdeal491 hζ s =
        Ideal.span {ρzero}) :
    primeIdealFactorSupport491 (ρzero ^ 2) ⊂
      primeIdealFactorSupport491 s.xi := by
  obtain ⟨hfactor, hcop⟩ :=
    historicalEquationEightA_factorization_and_coprime491 hζ s hs
  exact historicalEquationEightA_square_support_strict491
    hζ s ρzero (historicalEquationEightAComplement491 hζ s)
    hgenerator hfactor
    (historicalEquationEightAComplement_ne_top491 hζ s hs) hcop
