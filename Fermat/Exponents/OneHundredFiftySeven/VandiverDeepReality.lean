...
import Fermat.Exponents.OneHundredFiftySeven.VandiverRelationNormalization

/-!
# Deep Vandiver units are real at exponent 157
...
that the depth-314 local hypothesis itself forces an ambient cyclotomic unit
to be real.
...
namespace Fermat.OneHundredFiftySeven.VandiverDeepReality
...
inverse. -/
theorem unitsComplexConj_zeta157 {zeta : K}
...
theorem ringOfIntegersComplexConj_zeta_sub_one157 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) :
  ...
  have hconjzeta :
    ...
    congrArg ((↑) : (𝓞 K)ˣ → 𝓞 K)
      (unitsComplexConj_zeta157 hzeta)
  rw [hconjzeta]
  have hinv :
...
conjugation. -/
theorem zeta_sub_one_pow_dvd_conj_of_dvd157 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) (n : ℕ) (a : 𝓞 K)
    (h : ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣ a) :
    ((hzeta.unit' : 𝓞 K) - 1) ^ n ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K a := by
  obtain ⟨b, rfl⟩ := h
  rw [map_mul, map_pow,
    ringOfIntegersComplexConj_zeta_sub_one157, mul_pow]
...
/-- A square of a 157th root of unity which is `1` modulo
`(zeta - 1)^2` is literally `1`. -/
theorem zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd157 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) (j : ℕ)
  ...
  · have hknotdvd : ¬157 ∣ k := by
    ...
    have hkcoprime : k.Coprime 157 := by
      ...
      exact (show Nat.Prime 157 by norm_num).coprime_iff_not_dvd.mpr
        hknotdvd
...
theorem unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj157 {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) (u : (𝓞 K)ˣ)
    (hdiv : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
      (u : 𝓞 K) -
  ...
  have hroot :=
    zeta_pow_sq_eq_one_of_zeta_sub_one_sq_dvd157 hzeta j hrootdiv
  rw [hroot] at hj
  have hucu : u = cu := by
    calc
      u = (u * cu⁻¹) * cu := by simp
...
/-- The depth-314 hypothesis forces a unit to be real. -/
theorem isVandiverDeep_unitsComplexConj_eq {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 157) hzeta u) :
    NumberField.IsCMField.unitsComplexConj K u = u := by
  obtain ⟨c, hc⟩ := hdeep
  have hsign : ((hzeta.unit' : 𝓞 K) - 1) ^ 314 ∣
      (u : 𝓞 K) - (c : 𝓞 K) ^ 157 := by
    have hbase : (hzeta.unit' : 𝓞 K) - 1 ∣
        (1 : 𝓞 K) - hzeta.unit' := by
      refine ⟨-1, ?_⟩
      ring
    exact (pow_dvd_pow_of_dvd hbase 314).trans hc
  have hconj := zeta_sub_one_pow_dvd_conj_of_dvd157
    hzeta 314 ((u : 𝓞 K) - (c : 𝓞 K) ^ 157) hsign
  have hconj' : ((hzeta.unit' : 𝓞 K) - 1) ^ 314 ∣
      NumberField.IsCMField.ringOfIntegersComplexConj K (u : 𝓞 K) -
        (c : 𝓞 K) ^ 157 := by
    simpa only [map_sub, map_pow, map_intCast] using hconj
  have hdefect134 : ((hzeta.unit' : 𝓞 K) - 1) ^ 314 ∣
      (u : 𝓞 K) -
  ...
  have hdefect2 : ((hzeta.unit' : 𝓞 K) - 1) ^ 2 ∣
    ...
    (pow_dvd_pow ((hzeta.unit' : 𝓞 K) - 1) (by omega)).trans
      hdefect134
  exact unit_fixed_of_zeta_sub_one_sq_dvd_sub_conj157
    hzeta u hdefect2

/-- A deeply congruent ambient unit, packaged in the real-unit subgroup. -/
def deepRealUnit157 {zeta : K} (hzeta : IsPrimitiveRoot zeta 157)
    (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 157) hzeta u) :
...
@[simp]
theorem deepRealUnit157_coe {zeta : K}
    (hzeta : IsPrimitiveRoot zeta 157) (u : (𝓞 K)ˣ)
    (hdeep : IsVandiverDeep (K := K) (p := 157) hzeta u) :
...
end Fermat.OneHundredFiftySeven.VandiverDeepReality
