import Fermat.Exponents.OneHundredFiftySeven.VandiverGeneratorSupport157

/-!
# Vandiver's equation-(8a) factorization at exponent 157
...
noncomputable def historicalEquationEightAComplement157
  ...
  by
    ...
    let root := fun η : nthRootsFinset 157 (1 : 𝓞 K) ↦
      root_div_zeta_sub_one_dvd_gcd
        (K := K) (p := 157) (x := s.omega) (y := s.theta)
        (z := s.xi) (ε := historicalRegularUnit157 hζ s)
    ...
    exact
      ∏ η ∈ Finset.attach (nthRootsFinset 157 (1 : 𝓞 K)) \ {η₀}, root η

set_option maxRecDepth 4000 in
theorem historicalEquationEightA_factorization_and_coprime157
    {ζ : K} (hζ : IsPrimitiveRoot ζ 157)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    Ideal.span {s.xi} =
        historicalEquationEightAIdeal157 hζ s *
          historicalEquationEightAComplement157 hζ s ∧
      IsCoprime (historicalEquationEightAIdeal157 hζ s)
        (historicalEquationEightAComplement157 hζ s) := by
  classical
  let e := historicalState_regularEquation157 hζ s
  let hy := historicalState_theta_not_dvd157 hζ s
  ...
  let Z : Ideal (𝓞 K) :=
    z_div_m (K := K) (p := 157)
      ...
      (ε := historicalRegularUnit157 hζ s) (m := 2 * s.m - 1)
      hζ e hy
  let root := fun η : nthRootsFinset 157 (1 : 𝓞 K) ↦
    root_div_zeta_sub_one_dvd_gcd
      (K := K) (p := 157) (x := s.omega) (y := s.theta)
      (z := s.xi) (ε := historicalRegularUnit157 hζ s)
      (m := 2 * s.m - 1) (by norm_num) hζ e hy η
  let c := fun η : nthRootsFinset 157 (1 : 𝓞 K) ↦
  ...
  have hZ : Z = Ideal.span {s.xi} := by
    have hzspec :
      ...
      simpa only [M, Z] using
        (z_div_m_spec (K := K) (p := 157)
          (x := s.omega) (y := s.theta) (z := s.xi)
          (ε := historicalRegularUnit157 hζ s)
          (m := 2 * s.m - 1) hζ e hy)
    rw [hM, one_mul] at hzspec
    exact hzspec.symm
  have hrootpow (η : nthRootsFinset 157 (1 : 𝓞 K)) :
      root η ^ 157 = c η := by
    simpa only [root, c] using
      (root_div_zeta_sub_one_dvd_gcd_spec
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy η)
  have hprodC :
      ∏ η ∈ Finset.attach (nthRootsFinset 157 (1 : 𝓞 K)), c η =
        (Z * P ^ (2 * s.m - 1)) ^ 157 := by
    simpa only [c, Z, P, π] using
      (prod_c (K := K) (p := 157)
        ...
        (m := 2 * s.m - 1) (by norm_num) hζ e hy)
  have hprodRootPow :
      (∏ η ∈ Finset.attach (nthRootsFinset 157 (1 : 𝓞 K)),
          root η) ^ 157 =
        (Z * P ^ (2 * s.m - 1)) ^ 157 := by
    rw [← Finset.prod_pow]
    simpa only [hrootpow] using hprodC
  have hprodRoot :
      ∏ η ∈ Finset.attach (nthRootsFinset 157 (1 : 𝓞 K)), root η =
        Z * P ^ (2 * s.m - 1) :=
    pow_left_injective (M := Ideal (𝓞 K))
      (by norm_num : 157 ≠ 0) hprodRootPow
  have hη₀one :
      η₀ = oneNthRoot (K := K) (p := 157) := by
    simpa only [η₀, e, hy] using
      distinguishedRoot_eq_one_of_real157 hζ e hy hs.1 hs.2.1
  have hrootη₀ :
      root η₀ = historicalOneFactorIdeal157 hζ s := by
    rw [hη₀one]
    rfl
  have hsplit :
  ...
  have hfactor :
      Ideal.span {s.xi} =
        historicalEquationEightAIdeal157 hζ s * Q := by
  ...
  have hcop :
    ...
    have hroots : IsCoprime (root η₀) (root η) :=
      (IsCoprime.pow_left_iff (by norm_num : 0 < 157)).mp
        ((IsCoprime.pow_right_iff (by norm_num : 0 < 157)).mp hc)
    rw [← hspec] at hroots
    exact hroots.of_mul_left_right
  simpa only [historicalEquationEightAComplement157, e, hy, root, η₀, Q] using
    And.intro hfactor hcop

set_option maxRecDepth 4000 in
theorem historicalEquationEightAComplement_ne_top157
    {ζ : K} (hζ : IsPrimitiveRoot ζ 157)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s) :
    historicalEquationEightAComplement157 hζ s ≠ ⊤ := by
  classical
  ...
  let η₀ := zeta_sub_one_dvd_root
    ...
    (z := s.xi) (ε := historicalRegularUnit157 hζ s)
    (m := 2 * s.m - 1) (by norm_num) hζ e hy
  let ηζ : nthRootsFinset 157 (1 : 𝓞 K) :=
  ...
  have hηζ_ne : ηζ ≠ η₀ := by
    ...
    intro h
    have hv := congrArg
      (fun x : nthRootsFinset 157 (1 : 𝓞 K) ↦ (x : 𝓞 K)) h
    exact hζ.unit'_coe.ne_one (by norm_num) hv
  have hηζmem :
      ηζ ∈
        Finset.attach (nthRootsFinset 157 (1 : 𝓞 K)) \ {η₀} := by
    simp only [Finset.mem_sdiff, Finset.mem_attach,
      Finset.mem_singleton, true_and]
    exact hηζ_ne
  have hrootDvd :
      root ηζ ∣ historicalEquationEightAComplement157 hζ s := by
    simpa only [historicalEquationEightAComplement157, e, hy, root, η₀] using
  ...
  have hfactorIdealTop :
      historicalZetaFactorIdeal157 hζ s = ⊤ := by
    simpa only [root, ηζ, historicalZetaFactorIdeal157] using hrootTop
  have hqspan :
      Ideal.span {historicalZetaFactor157 hζ s} = ⊤ := by
    rw [← historicalZetaFactorIdeal_pow157 hζ s, hfactorIdealTop]
    simp
  have hqunit :
      IsUnit (historicalZetaFactor157 hζ s) :=
    Ideal.span_singleton_eq_top.mp hqspan
  let qUnit : (𝓞 K)ˣ := hqunit.unit
  let A : (𝓞 K)ˣ := -qUnit
  let B : (𝓞 K)ˣ := NumberField.IsCMField.unitsComplexConj K A
  have hqUnitVal :
      (qUnit : 𝓞 K) = historicalZetaFactor157 hζ s :=
    hqunit.unit_spec
  have hAval :
      (A : 𝓞 K) = -historicalZetaFactor157 hζ s := by
    dsimp [A]
  ...
  have hone :
    ...
    have hqmul :
        historicalZetaFactor157 hζ s * π =
  ...
  have hr_ne : (r : 𝓞 K) ≠ 1 := by
    ...
    exact historicalState_omega_add_theta_ne_zero157 hζ s
      ((mul_eq_zero.mp hzero).resolve_left hplusUnit.ne_zero)
  have h157sum : (157 : 𝓞 K) ∣ s.omega + s.theta := by
    have h158 :=
      historicalState_omega_add_theta_highDivisibility157 hζ s hs
    have h156 :
        π ^ 156 ∣ s.omega + s.theta :=
      (pow_dvd_pow π (by norm_num : 156 ≤ 158)).trans
        (by simpa only [π] using h158)
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mp
      (by simpa only [π] using h156)
  have h157prod :
      (157 : 𝓞 K) ∣
        (1 - (r : 𝓞 K)) *
          (1 - (hζ.unit' : 𝓞 K)) := by
    have hwithB :
        (157 : 𝓞 K) ∣
          ((1 - (r : 𝓞 K)) *
            (1 - (hζ.unit' : 𝓞 K))) * (B : 𝓞 K) := by
      rw [hidentity]
      exact dvd_mul_of_dvd_right h157sum _
    exact (B.isUnit.dvd_mul_right).mp hwithB
  have hrpow : (r : 𝓞 K) ^ 157 = 1 := by
    change ((((hζ.unit' : 𝓞 K) ^ j) ^ 2) ^ 157) = 1
    rw [← pow_mul, ← pow_mul]
    rw [show j * (2 * 157) = 157 * (j * 2) by omega, pow_mul,
      hζ.unit'_coe.pow_eq_one, one_pow]
  let ηr : nthRootsFinset 157 (1 : 𝓞 K) :=
    ⟨(r : 𝓞 K), by
      rw [Polynomial.mem_nthRootsFinset (by norm_num : 0 < 157)]
      exact hrpow⟩
  let ηone : nthRootsFinset 157 (1 : 𝓞 K) :=
  ...
  have hrAssocSub :
    ...
    simpa only [ηr, ηone, oneNthRoot, π] using
      (hζ.unit'_coe.ntRootsFinset_pairwise_associated_sub_one_sub_of_prime
        (by norm_num : Nat.Prime 157) ηr.prop ηone.prop hηr_ne).symm
  have hrAssoc :
    ...
    exact ((associated_zeta_sub_one_pow_prime hζ).dvd_iff_dvd_left).mpr
      (by
        simpa only [π] using
          (show ((157 : ℕ) : 𝓞 K) ∣
              (1 - (r : 𝓞 K)) * (1 - (hζ.unit' : 𝓞 K)) by
            convert h157prod using 1))
  have hbad : π ^ 156 ∣ π ^ 2 :=
    (hprodAssoc.dvd_iff_dvd_right).mp hπ156prod
  have hle :
      156 ≤ 2 :=
    (pow_dvd_pow_iff
      (hζ.unit'_coe.sub_one_ne_zero (by norm_num))
...
theorem historicalEquationEightA_square_support_strict_unconditional157
    {ζ : K} (hζ : IsPrimitiveRoot ζ 157)
    (s : HistoricalState hζ) (hs : RealSourceAdmissible hζ s)
    (ρzero : 𝓞 K)
    (hgenerator :
  ...
  obtain ⟨hfactor, hcop⟩ :=
    historicalEquationEightA_factorization_and_coprime157 hζ s hs
  exact historicalEquationEightA_square_support_strict157
    hζ s ρzero (historicalEquationEightAComplement157 hζ s)
    hgenerator hfactor
    (historicalEquationEightAComplement_ne_top157 hζ s hs) hcop
