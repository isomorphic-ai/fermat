...
/-! ## Source factor identities independent of regularity -/

/-- The entire excess power of the ramified prime is carried by the
distinguished linear factor selected by
`zeta_sub_one_dvd_root`.

This is the generic divisibility engine behind Vandiver's equation (9a).
It uses only the pairwise factor allocation already proved in the upstream
induction step; no class-number or regularity hypothesis enters. -/
theorem distinguishedFactor_highDivisibility
    (hp : p ≠ 2) {ζ : K} (hζ : IsPrimitiveRoot ζ p)
    {x y z : 𝓞 K} {ε : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ p + y ^ p =
      ε * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ p)
    (hy : ¬ hζ.unit'.1 - 1 ∣ y) :
    (hζ.unit'.1 - 1) ^ (m * p + 1) ∣
      x + y * (zeta_sub_one_dvd_root hp hζ e hy : 𝓞 K) := by
  rw [← Ideal.mem_span_singleton, ← Ideal.dvd_span_singleton,
    ← Ideal.span_singleton_pow]
  obtain ⟨Q, hQ⟩ := p_pow_dvd_c_eta_zero hp hζ e hy
  refine ⟨gcd (Ideal.span {x}) (Ideal.span {y}) * Q, ?_⟩
  rw [← m_mul_c_mul_p hp hζ e hy]
  rw [pow_add, pow_one, hQ]
  ac_rfl

/-- Fixed-denominator ideal form of the linear-factor decomposition.
When `x` and `y` are coprime, the principal ideal generated by
`(x + y*η)/(ζ - 1)` is exactly the `p`th power of its allocated factor
ideal.  Replacing the denominator by the associated `1 - η` gives the
ideal identity displayed in Vandiver's equation (9). -/
theorem linearFactorQuotient_span_eq_factorRoot_pow
    (hp : p ≠ 2) {ζ : K} (hζ : IsPrimitiveRoot ζ p)
    {x y z : 𝓞 K} {ε : (𝓞 K)ˣ} {m : ℕ}
    (e : x ^ p + y ^ p =
      ε * ((hζ.unit'.1 - 1) ^ (m + 1) * z) ^ p)
    ...
    (hxy : IsCoprime x y)
    (η : nthRootsFinset p (1 : 𝓞 K)) :
    Ideal.span {div_zeta_sub_one hp hζ e η} =
      root_div_zeta_sub_one_dvd_gcd hp hζ e hy η ^ p := by
  have hgcd : gcd (Ideal.span {x}) (Ideal.span {y}) = 1 := by
    rw [← Ideal.isCoprime_iff_gcd, Ideal.isCoprime_span_singleton_iff]
    exact hxy
  rw [← div_zeta_sub_one_dvd_gcd_spec hp hζ e hy η, hgcd, one_mul]
  exact (root_div_zeta_sub_one_dvd_gcd_spec hp hζ e hy η).symm
