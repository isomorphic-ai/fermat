...
/-!
# The Takagi--Furtwängler boundary for Vandiver's Lemma 1 at exponent 587
...
At `p = 587` we construct the Kummer extension attached to a hypothetical
nonprincipal ideal root.  The radicand is not a `587`th power, so
`X ^ 587 - a` is irreducible; its adjoining-root field is a cyclic Galois
extension of degree `587`, with Galois group explicitly equivalent to
`ZMod 587`.  A different-ideal calculation also proves unramifiedness at
every prime containing neither `587` nor `a`.
...
unramified Kummer extension to `587`-torsion in the real class group.
...
Sinnott--Kummer's proved nondivisibility `587 ∤ h⁺` then proves
`LemmaOne K 587`.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiFurtwangler587
...
/-! ## The canonical degree-587 Kummer extension -/
...
/-- The canonical extension obtained by adjoining a `587`th root of `a`. -/
abbrev KummerExtension587 (K : Type*) [Field K] (a : 𝓞 K) :=
  AdjoinRoot (X ^ 587 - C (a : K))

omit [NumberField K] [IsCyclotomicExtension {587} ℚ K] in
/-- The distinguished root in `KummerExtension587 K a` has `587`th power
equal to `a`. -/
theorem kummerExtension587_root_pow (a : 𝓞 K) :
    (root (X ^ 587 - C (a : K)) : KummerExtension587 K a) ^ 587 =
      algebraMap K (KummerExtension587 K a) (a : K) := by
  simpa only [KummerExtension587, AdjoinRoot.algebraMap_eq] using
    (root_X_pow_sub_C_pow 587 (a : K))

omit [NumberField K] [IsCyclotomicExtension {587} ℚ K] in
/-- An irreducible Kummer polynomial at `587` produces an extension of
degree exactly `587`. -/
theorem kummerExtension587_finrank
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    Module.finrank K (KummerExtension587 K a) = 587 := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  have hroots : (primitiveRoots 587 K).Nonempty :=
    ⟨ζ, (mem_primitiveRoots (by norm_num : 0 < 587)).2 hζ⟩
  letI : IsSplittingField K (KummerExtension587 K a)
      (X ^ 587 - C (a : K)) :=
    isSplittingField_AdjoinRoot_X_pow_sub_C hroots hirr
  exact finrank_of_isSplittingField_X_pow_sub_C hroots hirr _

omit [NumberField K] [IsCyclotomicExtension {587} ℚ K] in
/-- The canonical irreducible Kummer extension at `587` is Galois. -/
theorem kummerExtension587_isGalois
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    IsGalois K (KummerExtension587 K a) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  have hroots : (primitiveRoots 587 K).Nonempty :=
...
`587` is cyclic. -/
theorem kummerExtension587_isCyclic
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    IsCyclic ((KummerExtension587 K a) ≃ₐ[K] (KummerExtension587 K a)) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  have hroots : (primitiveRoots 587 K).Nonempty :=
...
noncomputable def kummerExtension587_autEquivZmod
    ...
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    ((KummerExtension587 K a) ≃ₐ[K] (KummerExtension587 K a)) ≃*
      Multiplicative (ZMod 587) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  have hroots : (primitiveRoots 587 K).Nonempty :=
...
theorem kummerExtension587_numberField
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    letI : Module.Finite K (KummerExtension587 K a) :=
      (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0)).finite_adjoinRoot
    NumberField (KummerExtension587 K a) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  letI : Module.Finite K (KummerExtension587 K a) :=
    (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0)).finite_adjoinRoot
  exact NumberField.of_module_finite K (KummerExtension587 K a)

/-! ## The integral Kummer root and its ideal -/

omit [IsCyclotomicExtension {587} ℚ K] in
/-- The distinguished Kummer root, regarded as an algebraic integer. -/
noncomputable def kummerRootInteger587
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    ...
    letI : Module.Finite K (KummerExtension587 K a) :=
      (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0)).finite_adjoinRoot
    letI : NumberField (KummerExtension587 K a) :=
      NumberField.of_module_finite K (KummerExtension587 K a)
    𝓞 (KummerExtension587 K a) := by
  letI := Fact.mk hirr
  let L := KummerExtension587 K a
  ...
  let α : L := root (X ^ 587 - C (a : K))
  have hαintZ : IsIntegral ℤ α := by
    apply IsIntegral.of_pow (by norm_num : 0 < 587)
    rw [show α ^ 587 = algebraMap K L (a : K) by
      exact kummerExtension587_root_pow a]
...
theorem kummerRootInteger587_pow
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    ...
    letI : NumberField (KummerExtension587 K a) :=
      NumberField.of_module_finite K (KummerExtension587 K a)
    kummerRootInteger587 hirr ^ 587 =
      algebraMap (𝓞 K) (𝓞 (KummerExtension587 K a)) a := by
  letI := Fact.mk hirr
  let L := KummerExtension587 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
...
/-- If `(a) = I ^ 587`, the principal ideal of the integral Kummer root is
exactly the extension of `I` to the Kummer field.

Both ideals have the same `587`th power; injectivity of powers in the
...
theorem span_kummerRootInteger587_eq_map_of_idealPower
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    ...
    let hirr :=
      irreducible_kummerPolynomial_of_nonprincipal_idealRoot
        (by norm_num : Nat.Prime 587) hpow hnonprincipal
    letI := Fact.mk hirr
    letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension587 K a) := inferInstance
    letI : Module.Finite K (KummerExtension587 K a) :=
    ...
    letI : NumberField (KummerExtension587 K a) :=
      NumberField.of_module_finite K (KummerExtension587 K a)
    Ideal.span {kummerRootInteger587 hirr} =
      I.map (algebraMap (𝓞 K) (𝓞 (KummerExtension587 K a))) := by
  let hirr :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 587) hpow hnonprincipal
  letI := Fact.mk hirr
  ...
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  apply pow_left_injective (M := Ideal (𝓞 L)) (by norm_num : 587 ≠ 0)
  calc
    Ideal.span {kummerRootInteger587 hirr} ^ 587 =
        Ideal.span {kummerRootInteger587 hirr ^ 587} :=
      Ideal.span_singleton_pow _ _
    _ = Ideal.span {algebraMap (𝓞 K) (𝓞 L) a} := by
      rw [kummerRootInteger587_pow hirr]
    _ = (Ideal.span {a}).map (algebraMap (𝓞 K) (𝓞 L)) := by
      rw [Ideal.map_span]
      simp
    _ = (I ^ 587).map (algebraMap (𝓞 K) (𝓞 L)) := by rw [hpow]
    _ = (I.map (algebraMap (𝓞 K) (𝓞 L))) ^ 587 :=
      Ideal.map_pow _ _ _

/-! ## Local rescaling away from `587` -/
...
`(√[587]{a}) = I O_L` into the corresponding semilocal localization shows
that the Kummer root and `t` differ by a unit. -/
theorem exists_local_unit_rescaling587
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 587 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    let hirr :=
    ...
    letI : NumberField (KummerExtension587 K a) :=
      NumberField.of_module_finite K (KummerExtension587 K a)
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension587 K a))),
      Q.asIdeal ≠ ⊥ →
      let P := Q.asIdeal.under (𝓞 K)
      let Aₚ := Localization.AtPrime P
      let Bₚ := Localization
        (Algebra.algebraMapSubmonoid
          (𝓞 (KummerExtension587 K a)) P.primeCompl)
      ∃ u : Bₚˣ,
        algebraMap (𝓞 (KummerExtension587 K a)) Bₚ
            (kummerRootInteger587 hirr) =
          algebraMap Aₚ Bₚ
  ...
  change ∀ (Q : PrimeSpectrum (𝓞 L)), Q.asIdeal ≠ ⊥ →
    ...
    ∃ u : Bₚˣ,
      algebraMap (𝓞 L) Bₚ (kummerRootInteger587 hirr) =
        algebraMap Aₚ Bₚ
          (Submodule.IsPrincipal.generator
  ...
  have hα :
    ...
    calc
      Ideal.span
          {algebraMap (𝓞 L) Bₚ (kummerRootInteger587 hirr)} =
          (Ideal.span {kummerRootInteger587 hirr}).map
            (algebraMap (𝓞 L) Bₚ) := by
              rw [Ideal.map_span]
              simp
      _ = (I.map (algebraMap (𝓞 K) (𝓞 L))).map
            (algebraMap (𝓞 L) Bₚ) := by
              rw [span_kummerRootInteger587_eq_map_of_idealPower
                hpow hnonprincipal]
...
`√[587]{a} * d = c * b`.  Thus `b` is a base-field rescaling of the
Kummer root, remains a primitive element of the extension, and is a unit
at `Q`. -/
theorem exists_global_tame_rescaling587
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 587 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    let hirr :=
    ...
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension587 K a))),
      Q.asIdeal ≠ ⊥ →
      ∃ b : 𝓞 (KummerExtension587 K a),
        b ∉ Q.asIdeal ∧
        ∃ c d : 𝓞 K, c ≠ 0 ∧ d ≠ 0 ∧
          kummerRootInteger587 hirr *
              algebraMap (𝓞 K)
                (𝓞 (KummerExtension587 K a)) d =
            algebraMap (𝓞 K)
                (𝓞 (KummerExtension587 K a)) c * b := by
  let hirr :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 587) hpow hnonprincipal
  letI := Fact.mk hirr
  ...
  let t : Aₚ := Submodule.IsPrincipal.generator J
  obtain ⟨u, hαu⟩ :=
    exists_local_unit_rescaling587 hpow hnonprincipal Q hQ0
  obtain ⟨c, dc, hct⟩ :=
...
`b ^ 587 ∈ K`.  Its minimal polynomial therefore has the form
`X ^ 587 - k`, whose derivative is `587 * b ^ 586`.  If `Q` does not lie
above `587`, this derivative is a unit at `Q`; the different criterion
then proves unramifiedness. -/
theorem kummerExtension587_unramifiedAt_of_idealPower_awayFrom587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 587 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    ...
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension587 K a))),
      Q.asIdeal ≠ ⊥ →
      algebraMap (𝓞 K) (𝓞 (KummerExtension587 K a))
          (587 : 𝓞 K) ∉ Q.asIdeal →
  ...
  letI : NumberField L := NumberField.of_module_finite K L
  change ∀ (Q : PrimeSpectrum (𝓞 L)), Q.asIdeal ≠ ⊥ →
    algebraMap (𝓞 K) (𝓞 L) (587 : 𝓞 K) ∉ Q.asIdeal →
      Algebra.IsUnramifiedAt (𝓞 K) Q.asIdeal
  intro Q hQ0 h587
  obtain ⟨b, hbQ, c, d, hc0, hd0, hrel⟩ :=
    exists_global_tame_rescaling587 hpow hnonprincipal Q hQ0
  let α : L := root (X ^ 587 - C (a : K))
  ...
  have hrelL :
    ...
    have hαOcoe :
        algebraMap (𝓞 L) L (kummerRootInteger587 hirr) = α := rfl
  ...
  have hαgen : Algebra.adjoin K {α} = ⊤ := by
    simpa only [α, L, KummerExtension587] using
  ...
  have hgenIF : IntermediateField.adjoin K {β} = ⊤ := by
    ...
    exact hgen
  let k : K := ((d : K) / (c : K)) ^ 587 * (a : K)
  have hβpow : β ^ 587 = algebraMap K L k := by
    calc
      β ^ 587 =
          (algebraMap K L ((d : K) / (c : K)) * α) ^ 587 := by
            rw [hβeq]
      _ = algebraMap K L ((d : K) / (c : K)) ^ 587 * α ^ 587 := by
        rw [mul_pow]
      _ = algebraMap K L ((d : K) / (c : K)) ^ 587 *
          algebraMap K L (a : K) := by
        rw [show α ^ 587 = algebraMap K L (a : K) by
          exact kummerExtension587_root_pow a]
      _ = algebraMap K L k := by simp [k]
  have hfin : Module.finrank K L = 587 :=
    kummerExtension587_finrank hζ hirr
  have hβpow' :
      β ^ Module.finrank K L = algebraMap K L k := by
    simpa only [hfin] using hβpow
  have hirrβ :
      Irreducible (X ^ 587 - C k) := by
    simpa only [hfin] using
      (irreducible_X_pow_sub_C_of_root_adjoin_eq_top hβpow' hgenIF)
  have hβroot : aeval β (X ^ 587 - C k) = 0 := by
    simp [hβpow]
  have hminpolyK : minpoly K β = X ^ 587 - C k := by
    symm
    simpa using minpoly.eq_of_irreducible hirrβ hβroot
  have hkintL : IsIntegral ℤ (algebraMap K L k) := by
    rw [← hβpow]
    exact (NumberField.RingOfIntegers.isIntegral_coe b).pow 587
  have hkint : IsIntegral ℤ k :=
    (isIntegral_algebraMap_iff (algebraMap K L).injective).mp hkintL
  let kO : 𝓞 K := ⟨k, hkint⟩
  let g : (𝓞 K)[X] := X ^ 587 - C kO
  have hminpoly :
      minpoly (𝓞 K) b = g := by
  ...
  have hderivQ' : (587 : 𝓞 L) * b ^ 586 ∈ Q.asIdeal := by
    rw [← hcoef]
    exact hderivQ''
  rcases Q.isPrime.mem_or_mem hderivQ' with h | h
  · exact h587 (by simpa only [map_ofNat] using h)
  · exact hbQ (Q.isPrime.mem_of_pow_mem 586 h)

/-! ## Wild rescaling above `587` -/
...
Writing `π = ζ - 1`, substitute `πX + c` into `X^587 - a`.  Every
coefficient is divisible by `π^587`: the constant coefficient by the
primary congruence, and each mixed coefficient by
`587 ∣ choose 587 i` together with `π^586 ∣ 587`.  Dividing by `π^587`
therefore leaves a monic polynomial of degree exactly `587`. -/
theorem exists_primary_shift_polynomial587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K}
    (hprimary : IsKummerPrimary hζ a) :
    ∃ (c : ℤ) (g : (𝓞 K)[X]),
      g.Monic ∧ g.natDegree = 587 ∧
      let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
      let cA : 𝓞 K := c
      let f : (𝓞 K)[X] := X ^ 587 - C a
      let F : (𝓞 K)[X] :=
  ...
  have hdiv : C (π ^ 587) ∣ F := by
    ...
    · subst i
      ...
      ring
    by_cases hi587 : i < 587
    · have hchoose : 587 ∣ Nat.choose 587 i :=
        Nat.Prime.dvd_choose_self (by norm_num) hi0 hi587
      obtain ⟨m, hm⟩ := hchoose
      have hπ586 : π ^ 586 ∣ (587 : 𝓞 K) := by
        simpa only [π, Nat.reduceSub, Nat.cast_ofNat] using
          (associated_zeta_sub_one_pow_prime hζ).dvd
      obtain ⟨u, hu⟩ := hπ586
      refine ⟨cA ^ (587 - i) * (m : 𝓞 K) * u * π ^ (i - 1), ?_⟩
      have hcast : (Nat.choose 587 i : 𝓞 K) =
          (587 : 𝓞 K) * (m : 𝓞 K) := by
        rw [hm]
        norm_num
      have hpi : π ^ i = π ^ (i - 1) * π := by
        ...
        congr 1
        omega
      have hpi587 : π ^ 587 = π ^ 586 * π := by
        simpa using pow_succ π 586
      rw [coeff_C, if_neg hi0, sub_zero, hcast, hu, hpi, hpi587]
      ring
    · have hi : 587 ≤ i := Nat.le_of_not_gt hi587
      by_cases heq : i = 587
      · subst i
        norm_num
      · have hlt : 587 < i := lt_of_le_of_ne hi (Ne.symm heq)
        rw [Nat.choose_eq_zero_of_lt hlt]
        simp [coeff_C, hi0]
  rcases hdiv with ⟨g, hg⟩
  have hπ0 : π ≠ 0 := by
    exact hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hπ5870 : π ^ 587 ≠ 0 := pow_ne_zero 587 hπ0
  have hF587 : F.coeff 587 = π ^ 587 := by
    dsimp only [F, f, cA]
    rw [comp_C_mul_X_coeff, taylor_apply]
    simp only [sub_comp, pow_comp, X_comp, C_comp]
    rw [coeff_sub, coeff_X_add_C_pow]
    norm_num
  have hg587 : g.coeff 587 = 1 := by
    have h := congrArg (fun q : (𝓞 K)[X] => q.coeff 587) hg
    change F.coeff 587 = (C (π ^ 587) * g).coeff 587 at h
    rw [hF587, coeff_C_mul] at h
    have h' : π ^ 587 * 1 = π ^ 587 * g.coeff 587 := by
      simpa using h
    exact (mul_left_cancel₀ hπ5870 h').symm
  have hgdeg : g.natDegree ≤ 587 := by
    ...
    change F.coeff N = (C (π ^ 587) * g).coeff N at h
    rw [hF0, coeff_C_mul] at h
    exact (mul_eq_zero.mp h.symm).resolve_left hπ5870
  have hgeq : g.natDegree = 587 :=
    le_antisymm hgdeg
      (le_natDegree_of_ne_zero (by simpa only [hg587] using
        (one_ne_zero : (1 : 𝓞 K) ≠ 0)))
  refine ⟨c, g, monic_of_natDegree_le_of_coeff_eq_one 587 hgdeg hg587,
    hgeq, ?_⟩
  simpa only [π, cA, f, F] using hg

set_option maxRecDepth 50000 in
/-- A Kummer-primary radicand eliminates the remaining wild
ramification above `587`.

For an upper prime `Q` above `587`, cyclotomic ramification identifies its
lower prime with `(ζ - 1)`.  The primary congruence gives the integral
generator `β = (√[587]{a} - c)/(ζ - 1)` and the monic polynomial produced
...
theorem kummerExtension587_unramifiedAt_of_primary_above587
    {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ a)
    (hpow : I ^ 587 = Ideal.span {a})
  ...
  letI : Q.asIdeal.LiesOver P := Ideal.LiesOver.mk rfl
  have hP0 : P ≠ ⊥ := mt Ideal.eq_bot_of_comap_eq_bot hQ0
  have h587P : (587 : 𝓞 K) ∈ P := by
    exact h587Q
  have hPover :
      Ideal.span {(587 : ℤ)} = P.under ℤ := by
    refine Ideal.IsMaximal.eq_of_le
      (Int.ideal_span_isMaximal_of_prime 587) Ideal.IsPrime.ne_top' ?_
    rw [Ideal.span_singleton_le_iff_mem, Ideal.mem_comap]
    have hmap587 :
        algebraMap ℤ (𝓞 K) (587 : ℤ) = (587 : 𝓞 K) := by
      norm_num
    rw [hmap587]
    exact h587P
  letI : P.LiesOver (Ideal.span {(587 : ℤ)}) :=
  ...
  obtain ⟨c, g, hgmonic, hgdegree, hFg⟩ :=
    exists_primary_shift_polynomial587 hζ hprimary
  let cA : 𝓞 K := c
  let f : (𝓞 K)[X] := X ^ 587 - C a
  let F : (𝓞 K)[X] :=
    (taylor cA f).comp (C π * X)
  ...
  have hEvalG : aeval β g = 0 := by
    ...
    change aeval β F = aeval β (C (π ^ 587) * g) at h
    rw [hEvalF] at h
    simp only [map_mul, aeval_C] at h
    have hπpowL0 : algebraMap (𝓞 K) L (π ^ 587) ≠ 0 := by
      rw [map_pow]
      exact pow_ne_zero 587 hπL0
  ...
  have hgKdegree : gK.natDegree = 587 := by
  ...
  have hEvalDerivF :
      aeval β (derivative F) =
        algebraMap (𝓞 K) L π *
          algebraMap (𝓞 K) L (587 : 𝓞 K) * α ^ 586 := by
    dsimp only [F]
    rw [derivative_comp]
  ...
  have hαOQ : αO ∉ Q.asIdeal := by
    ...
    apply haP
    have hpowmem :=
      Q.asIdeal.pow_mem_of_mem hαQ 587 (by norm_num)
    change kummerRootInteger587 hirr ^ 587 ∈ Q.asIdeal at hpowmem
    rw [kummerRootInteger587_pow hirr] at hpowmem
    exact hpowmem
  have huQ :
  ...
  have hderivNot :
    ...
    · exact huQ huMem
    · exact hαOQ (Q.isPrime.mem_of_pow_mem 586 hαpow)
  have hgenO :
...
contains neither `587` nor the radicand.
...
calculation below. -/
def KummerExtension587UnramifiedAwayFrom587AndRadicand
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension587 K a) := inferInstance
  letI : Module.Finite K (KummerExtension587 K a) :=
  ...
  letI : NumberField (KummerExtension587 K a) :=
    NumberField.of_module_finite K (KummerExtension587 K a)
  ∀ Q : PrimeSpectrum (𝓞 (KummerExtension587 K a)),
    algebraMap (𝓞 K) (𝓞 (KummerExtension587 K a)) (587 : 𝓞 K) ∉ Q.asIdeal →
    algebraMap (𝓞 K) (𝓞 (KummerExtension587 K a)) a ∉ Q.asIdeal →
      Algebra.IsUnramifiedAt (𝓞 K) Q.asIdeal

omit [IsCyclotomicExtension {587} ℚ K] in
/-- Ramification in `K(√[587]{a})/K` can occur only above `587` or at a
prime containing `a`.

Let `α = √[587]{a}`.  The element `α` is an algebraic integer, generates
the extension, and has minimal polynomial `X ^ 587 - a` over `𝓞 K`.
Consequently `587 * α ^ 586` belongs to the relative different.  If an
upper prime contains neither `587` nor `a = α ^ 587`, it cannot contain
this derivative, hence cannot divide the different and is unramified. -/
theorem kummerExtension587_unramifiedAwayFrom587AndRadicand
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    KummerExtension587UnramifiedAwayFrom587AndRadicand hirr := by
  letI := Fact.mk hirr
  let L := KummerExtension587 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  ...
  have hαOcoe : algebraMap (𝓞 L) L αO = α := rfl
  have hαOpow :
      αO ^ 587 = algebraMap (𝓞 K) (𝓞 L) a := by
    apply NumberField.RingOfIntegers.ext
    exact kummerExtension587_root_pow a
  have hgen : Algebra.adjoin K {algebraMap (𝓞 L) L αO} = ⊤ := by
    simpa only [hαOcoe, α, L, KummerExtension587] using
      (AdjoinRoot.adjoinRoot_eq_top (R := K)
        (f := X ^ 587 - C (a : K)))
  have hminpolyK :
      minpoly K α = X ^ 587 - C (a : K) := by
    change minpoly K (root (X ^ 587 - C (a : K))) =
      X ^ 587 - C (a : K)
    rw [← AdjoinRoot.powerBasis_gen
      (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0)).ne_zero]
    exact AdjoinRoot.minpoly_powerBasis_gen_of_monic
      (monic_X_pow_sub_C (a : K) (by norm_num : 587 ≠ 0))
  have hminpoly :
      minpoly (𝓞 K) αO = g := by
    apply Polynomial.map_injective (algebraMap (𝓞 K) K)
  ...
  have hderivQ : aeval αO (derivative (minpoly (𝓞 K) αO)) ∈ Q.asIdeal :=
    (Ideal.dvd_iff_le.mp hQdiff) hderiv
  have hcoef :
      algebraMap (𝓞 K) (𝓞 L) (586 : 𝓞 K) + 1 = (587 : 𝓞 L) := by
    rw [map_ofNat]
    norm_num
  have hderivQ' : (587 : 𝓞 L) * αO ^ 586 ∈ Q.asIdeal := by
    simpa [hminpoly, g, hcoef] using hderivQ
  have hαO : αO ∉ Q.asIdeal := by
    intro hα
    apply ha
    have hp := Q.asIdeal.pow_mem_of_mem hα 587 (by norm_num)
    rwa [hαOpow] at hp
  rcases Q.isPrime.mem_or_mem hderivQ' with h | h
  · exact h587 (by simpa only [map_ofNat] using h)
  · exact hαO (Q.isPrime.mem_of_pow_mem 586 h)
...
def KummerExtension587Unramified
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
  ...
  letI : NumberField (KummerExtension587 K a) :=
    NumberField.of_module_finite K (KummerExtension587 K a)
  IsUnramifiedAtFinitePlaces K (KummerExtension587 K a)

/-- The restriction of finite-prime unramifiedness to the only primes not
already covered by
`kummerExtension587_unramifiedAwayFrom587AndRadicand`: primes containing
`587` or the radicand. -/
def KummerExtension587UnramifiedAtPotentiallyRamifiedPrimes
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension587 K a) := AdjoinRoot.instField
...
omit [IsCyclotomicExtension {587} ℚ K] in
/-- For the canonical degree-`587` Kummer extension, proving
unramifiedness at every finite prime is equivalent to treating only the
primes containing `587` or `a`.  The complementary primes are discharged
unconditionally by the different-ideal calculation above. -/
theorem kummerExtension587_unramified_iff_atPotentiallyRamifiedPrimes
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 587 - C (a : K))) :
    KummerExtension587Unramified hirr ↔
      KummerExtension587UnramifiedAtPotentiallyRamifiedPrimes hirr := by
  letI := Fact.mk hirr
  let L := KummerExtension587 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  ...
  · intro hbad Q hQ
    by_cases h587 :
        algebraMap (𝓞 K) (𝓞 L) (587 : 𝓞 K) ∈ Q.asIdeal
    · exact hbad Q hQ (Or.inl h587)
    by_cases ha : algebraMap (𝓞 K) (𝓞 L) a ∈ Q.asIdeal
    · exact hbad Q hQ (Or.inr ha)
    exact kummerExtension587_unramifiedAwayFrom587AndRadicand hirr Q h587 ha

/-- The exact real-class-group output of the Takagi--Furtwängler step at
exponent `587`.

If a Kummer-primary integer generates the `587`th power of a nonprincipal
ideal, global reciprocity constructs nontrivial `587`-torsion in the class
...
`lemmaOne_of_primaryNonprincipalProducesRealTorsion587`.  The definitions
below factor it through the actual Kummer extension, separating local
ramification from global reflection. -/
def PrimaryNonprincipalProducesRealTorsion587 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {587} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
      ...
      I ^ 587 = Ideal.span {a} →
        ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
          HasNonprincipalIdealWithPrincipalPower
            (NumberField.maximalRealSubfield K) 587

/-- The local Kummer ramification input.

Away from `587`, the ideal identity `(a) = I ^ 587` makes every valuation of
`a` a multiple of `587`.  At the unique prime above `587`, the Kummer-primary
congruence removes the remaining possible wild ramification.  Together
these assertions say that the concrete extension `K(√[587]{a})/K` is
unramified at every finite prime. -/
def PrimaryIdealRootGivesUnramifiedKummer587 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {587} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
    ∀ (_hprimary : IsKummerPrimary hζ a)
      (hpow : I ^ 587 = Ideal.span {a})
      (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))),
      KummerExtension587Unramified
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
          (by norm_num) hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- The complete local Takagi--Furtwängler ramification theorem at `587`.

At primes above `587` this is
`kummerExtension587_unramifiedAt_of_primary_above587`; at every other
prime it is
`kummerExtension587_unramifiedAt_of_idealPower_awayFrom587`. -/
theorem primaryIdealRootGivesUnramifiedKummer587 :
    PrimaryIdealRootGivesUnramifiedKummer587 K := by
  ...
  by_cases h587 :
      algebraMap (𝓞 K) (𝓞 L) (587 : 𝓞 K) ∈ Q.asIdeal
  · exact kummerExtension587_unramifiedAt_of_primary_above587
      hζ hprimary hpow hnonprincipal Q hQ0 h587
  · exact kummerExtension587_unramifiedAt_of_idealPower_awayFrom587
      hζ hpow hnonprincipal Q hQ0 h587

/-- The remaining local ramification statement after the checked
different-ideal calculation.

For primes containing `a`, one must use `(a) = I ^ 587` to rescale the
Kummer root by a local generator of `I`.  For primes containing `587`, one
must use the primary congruence to eliminate wild ramification. -/
def PrimaryIdealRootPotentialRamificationResolved587 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {587} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
    ∀ (_hprimary : IsKummerPrimary hζ a)
      (hpow : I ^ 587 = Ideal.span {a})
      (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))),
      KummerExtension587UnramifiedAtPotentiallyRamifiedPrimes
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
          (by norm_num) hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- The local Kummer boundary is equivalent to resolving only primes
containing `587` or the radicand. -/
theorem primaryIdealRootGivesUnramifiedKummer587_iff_potentialRamificationResolved :
    PrimaryIdealRootGivesUnramifiedKummer587 K ↔
      PrimaryIdealRootPotentialRamificationResolved587 K := by
  constructor
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact
      (kummerExtension587_unramified_iff_atPotentiallyRamifiedPrimes
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
...
theorems above prove that it is Galois, cyclic, and has degree `587`.
Global reciprocity/reflection sends a primary unramified extension of this
form to nontrivial `587`-torsion in the real class group.  This is the
remaining Hilbert-class-field/Artin-reciprocity theorem absent from
Mathlib. -/
def PrimaryUnramifiedKummerReflection587 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {587} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
    {a : 𝓞 K},
    IsKummerPrimary hζ a →
...
theorem primaryNonprincipalProducesRealTorsion587_of_kummerTheory
    (hunramified : PrimaryIdealRootGivesUnramifiedKummer587 K)
    (hreflection : PrimaryUnramifiedKummerReflection587 K) :
    PrimaryNonprincipalProducesRealTorsion587 K := by
  intro ζ hζ a I hprimary hpow hnonprincipal
  let hirr : Irreducible (X ^ 587 - C (a : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num) hpow hnonprincipal
...
a primary nonprincipal ideal root forces `587 ∣ h⁺`. -/
theorem primaryNonprincipalProducesRealTorsion587_iff_forces_dvd_classNumber :
    PrimaryNonprincipalProducesRealTorsion587 K ↔
      ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
        {a : 𝓞 K} {I : Ideal (𝓞 K)},
        IsKummerPrimary hζ a →
          I ^ 587 = Ideal.span {a} →
            ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
              587 ∣ NumberField.classNumber
                (NumberField.maximalRealSubfield K) := by
  simp only [PrimaryNonprincipalProducesRealTorsion587]
  constructor
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := NumberField.maximalRealSubfield K) (p := 587) (by norm_num)).mp
        (h hζ hprimary hpow hnonprincipal)
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := NumberField.maximalRealSubfield K) (p := 587) (by norm_num)).mpr
        (h hζ hprimary hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- Once the Takagi--Furtwängler real-torsion construction is available,
the unconditional Sinnott--Kummer computation `587 ∤ h⁺` proves
Vandiver's Lemma 1 at exponent `587`. -/
theorem lemmaOne_of_primaryNonprincipalProducesRealTorsion587
    (hTF : PrimaryNonprincipalProducesRealTorsion587 K) :
    LemmaOne K 587 := by
  have hTF' : ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 587)
      {a : 𝓞 K} {I : Ideal (𝓞 K)},
      IsKummerPrimary hζ a →
        I ^ 587 = Ideal.span {a} →
          ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
    ...
    (primaryNonprincipalProducesRealTorsion587_iff_forces_dvd_classNumber
      (K := K)).mp hTF
  unfold LemmaOne
  intro ζ hζ a I hprimary hpow
  by_contra hnonprincipal
  have hdvd : 587 ∣ NumberField.classNumber
      (NumberField.maximalRealSubfield K) :=
    hTF' hζ hprimary hpow hnonprincipal
  exact Fermat.FiveHundredEightySeven.SinnottKummer.not_dvd_classNumber hζ hdvd

end

end Fermat.Irregular.TakagiFurtwangler587
