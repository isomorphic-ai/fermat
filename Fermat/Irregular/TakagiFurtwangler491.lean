...
/-!
# The Takagi--Furtwängler boundary for Vandiver's Lemma 1 at exponent 491
...
At `p = 491` we construct the Kummer extension attached to a hypothetical
nonprincipal ideal root.  The radicand is not a `491`th power, so
`X ^ 491 - a` is irreducible; its adjoining-root field is a cyclic Galois
extension of degree `491`, with Galois group explicitly equivalent to
`ZMod 491`.  A different-ideal calculation also proves unramifiedness at
every prime containing neither `491` nor `a`.
...
unramified Kummer extension to `491`-torsion in the real class group.
...
Sinnott--Kummer's proved nondivisibility `491 ∤ h⁺` then proves
`LemmaOne K 491`.
-/

open scoped NumberField

namespace Fermat.Irregular.TakagiFurtwangler491
...
/-! ## The canonical degree-491 Kummer extension -/
...
/-- The canonical extension obtained by adjoining a `491`th root of `a`. -/
abbrev KummerExtension491 (K : Type*) [Field K] (a : 𝓞 K) :=
  AdjoinRoot (X ^ 491 - C (a : K))

omit [NumberField K] [IsCyclotomicExtension {491} ℚ K] in
/-- The distinguished root in `KummerExtension491 K a` has `491`th power
equal to `a`. -/
theorem kummerExtension491_root_pow (a : 𝓞 K) :
    (root (X ^ 491 - C (a : K)) : KummerExtension491 K a) ^ 491 =
      algebraMap K (KummerExtension491 K a) (a : K) := by
  simpa only [KummerExtension491, AdjoinRoot.algebraMap_eq] using
    (root_X_pow_sub_C_pow 491 (a : K))

omit [NumberField K] [IsCyclotomicExtension {491} ℚ K] in
/-- An irreducible Kummer polynomial at `491` produces an extension of
degree exactly `491`. -/
theorem kummerExtension491_finrank
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    Module.finrank K (KummerExtension491 K a) = 491 := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  have hroots : (primitiveRoots 491 K).Nonempty :=
    ⟨ζ, (mem_primitiveRoots (by norm_num : 0 < 491)).2 hζ⟩
  letI : IsSplittingField K (KummerExtension491 K a)
      (X ^ 491 - C (a : K)) :=
    isSplittingField_AdjoinRoot_X_pow_sub_C hroots hirr
  exact finrank_of_isSplittingField_X_pow_sub_C hroots hirr _

omit [NumberField K] [IsCyclotomicExtension {491} ℚ K] in
/-- The canonical irreducible Kummer extension at `491` is Galois. -/
theorem kummerExtension491_isGalois
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    IsGalois K (KummerExtension491 K a) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  have hroots : (primitiveRoots 491 K).Nonempty :=
...
`491` is cyclic. -/
theorem kummerExtension491_isCyclic
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    IsCyclic ((KummerExtension491 K a) ≃ₐ[K] (KummerExtension491 K a)) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  have hroots : (primitiveRoots 491 K).Nonempty :=
...
noncomputable def kummerExtension491_autEquivZmod
    ...
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    ((KummerExtension491 K a) ≃ₐ[K] (KummerExtension491 K a)) ≃*
      Multiplicative (ZMod 491) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  have hroots : (primitiveRoots 491 K).Nonempty :=
...
theorem kummerExtension491_numberField
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    letI : Module.Finite K (KummerExtension491 K a) :=
      (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0)).finite_adjoinRoot
    NumberField (KummerExtension491 K a) := by
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  letI : Module.Finite K (KummerExtension491 K a) :=
    (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0)).finite_adjoinRoot
  exact NumberField.of_module_finite K (KummerExtension491 K a)

/-! ## The integral Kummer root and its ideal -/

omit [IsCyclotomicExtension {491} ℚ K] in
/-- The distinguished Kummer root, regarded as an algebraic integer. -/
noncomputable def kummerRootInteger491
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    ...
    letI : Module.Finite K (KummerExtension491 K a) :=
      (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0)).finite_adjoinRoot
    letI : NumberField (KummerExtension491 K a) :=
      NumberField.of_module_finite K (KummerExtension491 K a)
    𝓞 (KummerExtension491 K a) := by
  letI := Fact.mk hirr
  let L := KummerExtension491 K a
  ...
  let α : L := root (X ^ 491 - C (a : K))
  have hαintZ : IsIntegral ℤ α := by
    apply IsIntegral.of_pow (by norm_num : 0 < 491)
    rw [show α ^ 491 = algebraMap K L (a : K) by
      exact kummerExtension491_root_pow a]
...
theorem kummerRootInteger491_pow
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    ...
    letI : NumberField (KummerExtension491 K a) :=
      NumberField.of_module_finite K (KummerExtension491 K a)
    kummerRootInteger491 hirr ^ 491 =
      algebraMap (𝓞 K) (𝓞 (KummerExtension491 K a)) a := by
  letI := Fact.mk hirr
  let L := KummerExtension491 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
...
/-- If `(a) = I ^ 491`, the principal ideal of the integral Kummer root is
exactly the extension of `I` to the Kummer field.

Both ideals have the same `491`th power; injectivity of powers in the
...
theorem span_kummerRootInteger491_eq_map_of_idealPower
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    ...
    let hirr :=
      irreducible_kummerPolynomial_of_nonprincipal_idealRoot
        (by norm_num : Nat.Prime 491) hpow hnonprincipal
    letI := Fact.mk hirr
    letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
    letI : Algebra K (KummerExtension491 K a) := inferInstance
    letI : Module.Finite K (KummerExtension491 K a) :=
    ...
    letI : NumberField (KummerExtension491 K a) :=
      NumberField.of_module_finite K (KummerExtension491 K a)
    Ideal.span {kummerRootInteger491 hirr} =
      I.map (algebraMap (𝓞 K) (𝓞 (KummerExtension491 K a))) := by
  let hirr :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 491) hpow hnonprincipal
  letI := Fact.mk hirr
  ...
  letI : Module.Finite K L :=
    (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0)).finite_adjoinRoot
  letI : NumberField L := NumberField.of_module_finite K L
  apply pow_left_injective (M := Ideal (𝓞 L)) (by norm_num : 491 ≠ 0)
  calc
    Ideal.span {kummerRootInteger491 hirr} ^ 491 =
        Ideal.span {kummerRootInteger491 hirr ^ 491} :=
      Ideal.span_singleton_pow _ _
    _ = Ideal.span {algebraMap (𝓞 K) (𝓞 L) a} := by
      rw [kummerRootInteger491_pow hirr]
    _ = (Ideal.span {a}).map (algebraMap (𝓞 K) (𝓞 L)) := by
      rw [Ideal.map_span]
      simp
    _ = (I ^ 491).map (algebraMap (𝓞 K) (𝓞 L)) := by rw [hpow]
    _ = (I.map (algebraMap (𝓞 K) (𝓞 L))) ^ 491 :=
      Ideal.map_pow _ _ _

/-! ## Local rescaling away from `491` -/
...
`(√[491]{a}) = I O_L` into the corresponding semilocal localization shows
that the Kummer root and `t` differ by a unit. -/
theorem exists_local_unit_rescaling491
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 491 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    let hirr :=
    ...
    letI : NumberField (KummerExtension491 K a) :=
      NumberField.of_module_finite K (KummerExtension491 K a)
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension491 K a))),
      Q.asIdeal ≠ ⊥ →
      let P := Q.asIdeal.under (𝓞 K)
      let Aₚ := Localization.AtPrime P
      let Bₚ := Localization
        (Algebra.algebraMapSubmonoid
          (𝓞 (KummerExtension491 K a)) P.primeCompl)
      ∃ u : Bₚˣ,
        algebraMap (𝓞 (KummerExtension491 K a)) Bₚ
            (kummerRootInteger491 hirr) =
          algebraMap Aₚ Bₚ
  ...
  change ∀ (Q : PrimeSpectrum (𝓞 L)), Q.asIdeal ≠ ⊥ →
    ...
    ∃ u : Bₚˣ,
      algebraMap (𝓞 L) Bₚ (kummerRootInteger491 hirr) =
        algebraMap Aₚ Bₚ
          (Submodule.IsPrincipal.generator
  ...
  have hα :
    ...
    calc
      Ideal.span
          {algebraMap (𝓞 L) Bₚ (kummerRootInteger491 hirr)} =
          (Ideal.span {kummerRootInteger491 hirr}).map
            (algebraMap (𝓞 L) Bₚ) := by
              rw [Ideal.map_span]
              simp
      _ = (I.map (algebraMap (𝓞 K) (𝓞 L))).map
            (algebraMap (𝓞 L) Bₚ) := by
              rw [span_kummerRootInteger491_eq_map_of_idealPower
                hpow hnonprincipal]
...
`√[491]{a} * d = c * b`.  Thus `b` is a base-field rescaling of the
Kummer root, remains a primitive element of the extension, and is a unit
at `Q`. -/
theorem exists_global_tame_rescaling491
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 491 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    let hirr :=
    ...
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension491 K a))),
      Q.asIdeal ≠ ⊥ →
      ∃ b : 𝓞 (KummerExtension491 K a),
        b ∉ Q.asIdeal ∧
        ∃ c d : 𝓞 K, c ≠ 0 ∧ d ≠ 0 ∧
          kummerRootInteger491 hirr *
              algebraMap (𝓞 K)
                (𝓞 (KummerExtension491 K a)) d =
            algebraMap (𝓞 K)
                (𝓞 (KummerExtension491 K a)) c * b := by
  let hirr :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num : Nat.Prime 491) hpow hnonprincipal
  letI := Fact.mk hirr
  ...
  let t : Aₚ := Submodule.IsPrincipal.generator J
  obtain ⟨u, hαu⟩ :=
    exists_local_unit_rescaling491 hpow hnonprincipal Q hQ0
  obtain ⟨c, dc, hct⟩ :=
...
`b ^ 491 ∈ K`.  Its minimal polynomial therefore has the form
`X ^ 491 - k`, whose derivative is `491 * b ^ 490`.  If `Q` does not lie
above `491`, this derivative is a unit at `Q`; the different criterion
then proves unramifiedness. -/
theorem kummerExtension491_unramifiedAt_of_idealPower_awayFrom491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hpow : I ^ 491 = Ideal.span {a})
    (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))) :
    ...
    ∀ (Q : PrimeSpectrum (𝓞 (KummerExtension491 K a))),
      Q.asIdeal ≠ ⊥ →
      algebraMap (𝓞 K) (𝓞 (KummerExtension491 K a))
          (491 : 𝓞 K) ∉ Q.asIdeal →
  ...
  letI : NumberField L := NumberField.of_module_finite K L
  change ∀ (Q : PrimeSpectrum (𝓞 L)), Q.asIdeal ≠ ⊥ →
    algebraMap (𝓞 K) (𝓞 L) (491 : 𝓞 K) ∉ Q.asIdeal →
      Algebra.IsUnramifiedAt (𝓞 K) Q.asIdeal
  intro Q hQ0 h491
  obtain ⟨b, hbQ, c, d, hc0, hd0, hrel⟩ :=
    exists_global_tame_rescaling491 hpow hnonprincipal Q hQ0
  let α : L := root (X ^ 491 - C (a : K))
  ...
  have hrelL :
    ...
    have hαOcoe :
        algebraMap (𝓞 L) L (kummerRootInteger491 hirr) = α := rfl
  ...
  have hαgen : Algebra.adjoin K {α} = ⊤ := by
    simpa only [α, L, KummerExtension491] using
  ...
  have hgenIF : IntermediateField.adjoin K {β} = ⊤ := by
    ...
    exact hgen
  let k : K := ((d : K) / (c : K)) ^ 491 * (a : K)
  have hβpow : β ^ 491 = algebraMap K L k := by
    calc
      β ^ 491 =
          (algebraMap K L ((d : K) / (c : K)) * α) ^ 491 := by
            rw [hβeq]
      _ = algebraMap K L ((d : K) / (c : K)) ^ 491 * α ^ 491 := by
        rw [mul_pow]
      _ = algebraMap K L ((d : K) / (c : K)) ^ 491 *
          algebraMap K L (a : K) := by
        rw [show α ^ 491 = algebraMap K L (a : K) by
          exact kummerExtension491_root_pow a]
      _ = algebraMap K L k := by simp [k]
  have hfin : Module.finrank K L = 491 :=
    kummerExtension491_finrank hζ hirr
  have hβpow' :
      β ^ Module.finrank K L = algebraMap K L k := by
    simpa only [hfin] using hβpow
  have hirrβ :
      Irreducible (X ^ 491 - C k) := by
    simpa only [hfin] using
      (irreducible_X_pow_sub_C_of_root_adjoin_eq_top hβpow' hgenIF)
  have hβroot : aeval β (X ^ 491 - C k) = 0 := by
    simp [hβpow]
  have hminpolyK : minpoly K β = X ^ 491 - C k := by
    symm
    simpa using minpoly.eq_of_irreducible hirrβ hβroot
  have hkintL : IsIntegral ℤ (algebraMap K L k) := by
    rw [← hβpow]
    exact (NumberField.RingOfIntegers.isIntegral_coe b).pow 491
  have hkint : IsIntegral ℤ k :=
    (isIntegral_algebraMap_iff (algebraMap K L).injective).mp hkintL
  let kO : 𝓞 K := ⟨k, hkint⟩
  let g : (𝓞 K)[X] := X ^ 491 - C kO
  have hminpoly :
      minpoly (𝓞 K) b = g := by
  ...
  have hderivQ' : (491 : 𝓞 L) * b ^ 490 ∈ Q.asIdeal := by
    rw [← hcoef]
    exact hderivQ''
  rcases Q.isPrime.mem_or_mem hderivQ' with h | h
  · exact h491 (by simpa only [map_ofNat] using h)
  · exact hbQ (Q.isPrime.mem_of_pow_mem 490 h)

/-! ## Wild rescaling above `491` -/
...
Writing `π = ζ - 1`, substitute `πX + c` into `X^491 - a`.  Every
coefficient is divisible by `π^491`: the constant coefficient by the
primary congruence, and each mixed coefficient by
`491 ∣ choose 491 i` together with `π^490 ∣ 491`.  Dividing by `π^491`
therefore leaves a monic polynomial of degree exactly `491`. -/
theorem exists_primary_shift_polynomial491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K}
    (hprimary : IsKummerPrimary hζ a) :
    ∃ (c : ℤ) (g : (𝓞 K)[X]),
      g.Monic ∧ g.natDegree = 491 ∧
      let π : 𝓞 K := (hζ.unit' : 𝓞 K) - 1
      let cA : 𝓞 K := c
      let f : (𝓞 K)[X] := X ^ 491 - C a
      let F : (𝓞 K)[X] :=
  ...
  have hdiv : C (π ^ 491) ∣ F := by
    ...
    · subst i
      ...
      ring
    by_cases hi491 : i < 491
    · have hchoose : 491 ∣ Nat.choose 491 i :=
        Nat.Prime.dvd_choose_self (by norm_num) hi0 hi491
      obtain ⟨m, hm⟩ := hchoose
      have hπ490 : π ^ 490 ∣ (491 : 𝓞 K) := by
        simpa only [π, Nat.reduceSub, Nat.cast_ofNat] using
          (associated_zeta_sub_one_pow_prime hζ).dvd
      obtain ⟨u, hu⟩ := hπ490
      refine ⟨cA ^ (491 - i) * (m : 𝓞 K) * u * π ^ (i - 1), ?_⟩
      have hcast : (Nat.choose 491 i : 𝓞 K) =
          (491 : 𝓞 K) * (m : 𝓞 K) := by
        rw [hm]
        norm_num
      have hpi : π ^ i = π ^ (i - 1) * π := by
        ...
        congr 1
        omega
      have hpi491 : π ^ 491 = π ^ 490 * π := by
        simpa using pow_succ π 490
      rw [coeff_C, if_neg hi0, sub_zero, hcast, hu, hpi, hpi491]
      ring
    · have hi : 491 ≤ i := Nat.le_of_not_gt hi491
      by_cases heq : i = 491
      · subst i
        norm_num
      · have hlt : 491 < i := lt_of_le_of_ne hi (Ne.symm heq)
        rw [Nat.choose_eq_zero_of_lt hlt]
        simp [coeff_C, hi0]
  rcases hdiv with ⟨g, hg⟩
  have hπ0 : π ≠ 0 := by
    exact hζ.unit'_coe.sub_one_ne_zero (by norm_num)
  have hπ4910 : π ^ 491 ≠ 0 := pow_ne_zero 491 hπ0
  have hF491 : F.coeff 491 = π ^ 491 := by
    dsimp only [F, f, cA]
    rw [comp_C_mul_X_coeff, taylor_apply]
    simp only [sub_comp, pow_comp, X_comp, C_comp]
    rw [coeff_sub, coeff_X_add_C_pow]
    norm_num
  have hg491 : g.coeff 491 = 1 := by
    have h := congrArg (fun q : (𝓞 K)[X] => q.coeff 491) hg
    change F.coeff 491 = (C (π ^ 491) * g).coeff 491 at h
    rw [hF491, coeff_C_mul] at h
    have h' : π ^ 491 * 1 = π ^ 491 * g.coeff 491 := by
      simpa using h
    exact (mul_left_cancel₀ hπ4910 h').symm
  have hgdeg : g.natDegree ≤ 491 := by
    ...
    change F.coeff N = (C (π ^ 491) * g).coeff N at h
    rw [hF0, coeff_C_mul] at h
    exact (mul_eq_zero.mp h.symm).resolve_left hπ4910
  have hgeq : g.natDegree = 491 :=
    le_antisymm hgdeg
      (le_natDegree_of_ne_zero (by simpa only [hg491] using
        (one_ne_zero : (1 : 𝓞 K) ≠ 0)))
  refine ⟨c, g, monic_of_natDegree_le_of_coeff_eq_one 491 hgdeg hg491,
    hgeq, ?_⟩
  simpa only [π, cA, f, F] using hg

set_option maxRecDepth 50000 in
/-- A Kummer-primary radicand eliminates the remaining wild
ramification above `491`.

For an upper prime `Q` above `491`, cyclotomic ramification identifies its
lower prime with `(ζ - 1)`.  The primary congruence gives the integral
generator `β = (√[491]{a} - c)/(ζ - 1)` and the monic polynomial produced
...
theorem kummerExtension491_unramifiedAt_of_primary_above491
    {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K} {I : Ideal (𝓞 K)}
    (hprimary : IsKummerPrimary hζ a)
    (hpow : I ^ 491 = Ideal.span {a})
  ...
  letI : Q.asIdeal.LiesOver P := Ideal.LiesOver.mk rfl
  have hP0 : P ≠ ⊥ := mt Ideal.eq_bot_of_comap_eq_bot hQ0
  have h491P : (491 : 𝓞 K) ∈ P := by
    exact h491Q
  have hPover :
      Ideal.span {(491 : ℤ)} = P.under ℤ := by
    refine Ideal.IsMaximal.eq_of_le
      (Int.ideal_span_isMaximal_of_prime 491) Ideal.IsPrime.ne_top' ?_
    rw [Ideal.span_singleton_le_iff_mem, Ideal.mem_comap]
    have hmap491 :
        algebraMap ℤ (𝓞 K) (491 : ℤ) = (491 : 𝓞 K) := by
      norm_num
    rw [hmap491]
    exact h491P
  letI : P.LiesOver (Ideal.span {(491 : ℤ)}) :=
  ...
  obtain ⟨c, g, hgmonic, hgdegree, hFg⟩ :=
    exists_primary_shift_polynomial491 hζ hprimary
  let cA : 𝓞 K := c
  let f : (𝓞 K)[X] := X ^ 491 - C a
  let F : (𝓞 K)[X] :=
    (taylor cA f).comp (C π * X)
  ...
  have hEvalG : aeval β g = 0 := by
    ...
    change aeval β F = aeval β (C (π ^ 491) * g) at h
    rw [hEvalF] at h
    simp only [map_mul, aeval_C] at h
    have hπpowL0 : algebraMap (𝓞 K) L (π ^ 491) ≠ 0 := by
      rw [map_pow]
      exact pow_ne_zero 491 hπL0
  ...
  have hgKdegree : gK.natDegree = 491 := by
  ...
  have hEvalDerivF :
      aeval β (derivative F) =
        algebraMap (𝓞 K) L π *
          algebraMap (𝓞 K) L (491 : 𝓞 K) * α ^ 490 := by
    dsimp only [F]
    rw [derivative_comp]
  ...
  have hαOQ : αO ∉ Q.asIdeal := by
    ...
    apply haP
    have hpowmem :=
      Q.asIdeal.pow_mem_of_mem hαQ 491 (by norm_num)
    change kummerRootInteger491 hirr ^ 491 ∈ Q.asIdeal at hpowmem
    rw [kummerRootInteger491_pow hirr] at hpowmem
    exact hpowmem
  have huQ :
  ...
  have hderivNot :
    ...
    · exact huQ huMem
    · exact hαOQ (Q.isPrime.mem_of_pow_mem 490 hαpow)
  have hgenO :
...
contains neither `491` nor the radicand.
...
calculation below. -/
def KummerExtension491UnramifiedAwayFrom491AndRadicand
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  letI : Algebra K (KummerExtension491 K a) := inferInstance
  letI : Module.Finite K (KummerExtension491 K a) :=
  ...
  letI : NumberField (KummerExtension491 K a) :=
    NumberField.of_module_finite K (KummerExtension491 K a)
  ∀ Q : PrimeSpectrum (𝓞 (KummerExtension491 K a)),
    algebraMap (𝓞 K) (𝓞 (KummerExtension491 K a)) (491 : 𝓞 K) ∉ Q.asIdeal →
    algebraMap (𝓞 K) (𝓞 (KummerExtension491 K a)) a ∉ Q.asIdeal →
      Algebra.IsUnramifiedAt (𝓞 K) Q.asIdeal

omit [IsCyclotomicExtension {491} ℚ K] in
/-- Ramification in `K(√[491]{a})/K` can occur only above `491` or at a
prime containing `a`.

Let `α = √[491]{a}`.  The element `α` is an algebraic integer, generates
the extension, and has minimal polynomial `X ^ 491 - a` over `𝓞 K`.
Consequently `491 * α ^ 490` belongs to the relative different.  If an
upper prime contains neither `491` nor `a = α ^ 491`, it cannot contain
this derivative, hence cannot divide the different and is unramified. -/
theorem kummerExtension491_unramifiedAwayFrom491AndRadicand
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    KummerExtension491UnramifiedAwayFrom491AndRadicand hirr := by
  letI := Fact.mk hirr
  let L := KummerExtension491 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  ...
  have hαOcoe : algebraMap (𝓞 L) L αO = α := rfl
  have hαOpow :
      αO ^ 491 = algebraMap (𝓞 K) (𝓞 L) a := by
    apply NumberField.RingOfIntegers.ext
    exact kummerExtension491_root_pow a
  have hgen : Algebra.adjoin K {algebraMap (𝓞 L) L αO} = ⊤ := by
    simpa only [hαOcoe, α, L, KummerExtension491] using
      (AdjoinRoot.adjoinRoot_eq_top (R := K)
        (f := X ^ 491 - C (a : K)))
  have hminpolyK :
      minpoly K α = X ^ 491 - C (a : K) := by
    change minpoly K (root (X ^ 491 - C (a : K))) =
      X ^ 491 - C (a : K)
    rw [← AdjoinRoot.powerBasis_gen
      (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0)).ne_zero]
    exact AdjoinRoot.minpoly_powerBasis_gen_of_monic
      (monic_X_pow_sub_C (a : K) (by norm_num : 491 ≠ 0))
  have hminpoly :
      minpoly (𝓞 K) αO = g := by
    apply Polynomial.map_injective (algebraMap (𝓞 K) K)
  ...
  have hderivQ : aeval αO (derivative (minpoly (𝓞 K) αO)) ∈ Q.asIdeal :=
    (Ideal.dvd_iff_le.mp hQdiff) hderiv
  have hcoef :
      algebraMap (𝓞 K) (𝓞 L) (490 : 𝓞 K) + 1 = (491 : 𝓞 L) := by
    rw [map_ofNat]
    norm_num
  have hderivQ' : (491 : 𝓞 L) * αO ^ 490 ∈ Q.asIdeal := by
    simpa [hminpoly, g, hcoef] using hderivQ
  have hαO : αO ∉ Q.asIdeal := by
    intro hα
    apply ha
    have hp := Q.asIdeal.pow_mem_of_mem hα 491 (by norm_num)
    rwa [hαOpow] at hp
  rcases Q.isPrime.mem_or_mem hderivQ' with h | h
  · exact h491 (by simpa only [map_ofNat] using h)
  · exact hαO (Q.isPrime.mem_of_pow_mem 490 h)
...
def KummerExtension491Unramified
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
  ...
  letI : NumberField (KummerExtension491 K a) :=
    NumberField.of_module_finite K (KummerExtension491 K a)
  IsUnramifiedAtFinitePlaces K (KummerExtension491 K a)

/-- The restriction of finite-prime unramifiedness to the only primes not
already covered by
`kummerExtension491_unramifiedAwayFrom491AndRadicand`: primes containing
`491` or the radicand. -/
def KummerExtension491UnramifiedAtPotentiallyRamifiedPrimes
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) : Prop :=
  letI := Fact.mk hirr
  letI : Field (KummerExtension491 K a) := AdjoinRoot.instField
...
omit [IsCyclotomicExtension {491} ℚ K] in
/-- For the canonical degree-`491` Kummer extension, proving
unramifiedness at every finite prime is equivalent to treating only the
primes containing `491` or `a`.  The complementary primes are discharged
unconditionally by the different-ideal calculation above. -/
theorem kummerExtension491_unramified_iff_atPotentiallyRamifiedPrimes
    {a : 𝓞 K}
    (hirr : Irreducible (X ^ 491 - C (a : K))) :
    KummerExtension491Unramified hirr ↔
      KummerExtension491UnramifiedAtPotentiallyRamifiedPrimes hirr := by
  letI := Fact.mk hirr
  let L := KummerExtension491 K a
  letI : Field L := AdjoinRoot.instField
  letI : Algebra K L := inferInstance
  ...
  · intro hbad Q hQ
    by_cases h491 :
        algebraMap (𝓞 K) (𝓞 L) (491 : 𝓞 K) ∈ Q.asIdeal
    · exact hbad Q hQ (Or.inl h491)
    by_cases ha : algebraMap (𝓞 K) (𝓞 L) a ∈ Q.asIdeal
    · exact hbad Q hQ (Or.inr ha)
    exact kummerExtension491_unramifiedAwayFrom491AndRadicand hirr Q h491 ha

/-- The exact real-class-group output of the Takagi--Furtwängler step at
exponent `491`.

If a Kummer-primary integer generates the `491`th power of a nonprincipal
ideal, global reciprocity constructs nontrivial `491`-torsion in the class
...
`lemmaOne_of_primaryNonprincipalProducesRealTorsion491`.  The definitions
below factor it through the actual Kummer extension, separating local
ramification from global reflection. -/
def PrimaryNonprincipalProducesRealTorsion491 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {491} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
      ...
      I ^ 491 = Ideal.span {a} →
        ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
          HasNonprincipalIdealWithPrincipalPower
            (NumberField.maximalRealSubfield K) 491

/-- The local Kummer ramification input.

Away from `491`, the ideal identity `(a) = I ^ 491` makes every valuation of
`a` a multiple of `491`.  At the unique prime above `491`, the Kummer-primary
congruence removes the remaining possible wild ramification.  Together
these assertions say that the concrete extension `K(√[491]{a})/K` is
unramified at every finite prime. -/
def PrimaryIdealRootGivesUnramifiedKummer491 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {491} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
    ∀ (_hprimary : IsKummerPrimary hζ a)
      (hpow : I ^ 491 = Ideal.span {a})
      (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))),
      KummerExtension491Unramified
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
          (by norm_num) hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- The complete local Takagi--Furtwängler ramification theorem at `491`.

At primes above `491` this is
`kummerExtension491_unramifiedAt_of_primary_above491`; at every other
prime it is
`kummerExtension491_unramifiedAt_of_idealPower_awayFrom491`. -/
theorem primaryIdealRootGivesUnramifiedKummer491 :
    PrimaryIdealRootGivesUnramifiedKummer491 K := by
  ...
  by_cases h491 :
      algebraMap (𝓞 K) (𝓞 L) (491 : 𝓞 K) ∈ Q.asIdeal
  · exact kummerExtension491_unramifiedAt_of_primary_above491
      hζ hprimary hpow hnonprincipal Q hQ0 h491
  · exact kummerExtension491_unramifiedAt_of_idealPower_awayFrom491
      hζ hpow hnonprincipal Q hQ0 h491

/-- The remaining local ramification statement after the checked
different-ideal calculation.

For primes containing `a`, one must use `(a) = I ^ 491` to rescale the
Kummer root by a local generator of `I`.  For primes containing `491`, one
must use the primary congruence to eliminate wild ramification. -/
def PrimaryIdealRootPotentialRamificationResolved491 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {491} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K} {I : Ideal (𝓞 K)},
    ∀ (_hprimary : IsKummerPrimary hζ a)
      (hpow : I ^ 491 = Ideal.span {a})
      (hnonprincipal : ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K))),
      KummerExtension491UnramifiedAtPotentiallyRamifiedPrimes
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
          (by norm_num) hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- The local Kummer boundary is equivalent to resolving only primes
containing `491` or the radicand. -/
theorem primaryIdealRootGivesUnramifiedKummer491_iff_potentialRamificationResolved :
    PrimaryIdealRootGivesUnramifiedKummer491 K ↔
      PrimaryIdealRootPotentialRamificationResolved491 K := by
  constructor
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact
      (kummerExtension491_unramified_iff_atPotentiallyRamifiedPrimes
        (irreducible_kummerPolynomial_of_nonprincipal_idealRoot
...
theorems above prove that it is Galois, cyclic, and has degree `491`.
Global reciprocity/reflection sends a primary unramified extension of this
form to nontrivial `491`-torsion in the real class group.  This is the
remaining Hilbert-class-field/Artin-reciprocity theorem absent from
Mathlib. -/
def PrimaryUnramifiedKummerReflection491 (K : Type*)
    [Field K] [NumberField K] [IsCyclotomicExtension {491} ℚ K] : Prop :=
  ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
    {a : 𝓞 K},
    IsKummerPrimary hζ a →
...
theorem primaryNonprincipalProducesRealTorsion491_of_kummerTheory
    (hunramified : PrimaryIdealRootGivesUnramifiedKummer491 K)
    (hreflection : PrimaryUnramifiedKummerReflection491 K) :
    PrimaryNonprincipalProducesRealTorsion491 K := by
  intro ζ hζ a I hprimary hpow hnonprincipal
  let hirr : Irreducible (X ^ 491 - C (a : K)) :=
    irreducible_kummerPolynomial_of_nonprincipal_idealRoot
      (by norm_num) hpow hnonprincipal
...
a primary nonprincipal ideal root forces `491 ∣ h⁺`. -/
theorem primaryNonprincipalProducesRealTorsion491_iff_forces_dvd_classNumber :
    PrimaryNonprincipalProducesRealTorsion491 K ↔
      ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
        {a : 𝓞 K} {I : Ideal (𝓞 K)},
        IsKummerPrimary hζ a →
          I ^ 491 = Ideal.span {a} →
            ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
              491 ∣ NumberField.classNumber
                (NumberField.maximalRealSubfield K) := by
  simp only [PrimaryNonprincipalProducesRealTorsion491]
  constructor
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := NumberField.maximalRealSubfield K) (p := 491) (by norm_num)).mp
        (h hζ hprimary hpow hnonprincipal)
  · intro h ζ hζ a I hprimary hpow hnonprincipal
    exact (hasNonprincipalIdealWithPrincipalPower_iff_dvd_classNumber
      (F := NumberField.maximalRealSubfield K) (p := 491) (by norm_num)).mpr
        (h hζ hprimary hpow hnonprincipal)

set_option maxRecDepth 50000 in
/-- Once the Takagi--Furtwängler real-torsion construction is available,
the unconditional Sinnott--Kummer computation `491 ∤ h⁺` proves
Vandiver's Lemma 1 at exponent `491`. -/
theorem lemmaOne_of_primaryNonprincipalProducesRealTorsion491
    (hTF : PrimaryNonprincipalProducesRealTorsion491 K) :
    LemmaOne K 491 := by
  have hTF' : ∀ {ζ : K} (hζ : IsPrimitiveRoot ζ 491)
      {a : 𝓞 K} {I : Ideal (𝓞 K)},
      IsKummerPrimary hζ a →
        I ^ 491 = Ideal.span {a} →
          ¬ Submodule.IsPrincipal (I : Ideal (𝓞 K)) →
    ...
    (primaryNonprincipalProducesRealTorsion491_iff_forces_dvd_classNumber
      (K := K)).mp hTF
  unfold LemmaOne
  intro ζ hζ a I hprimary hpow
  by_contra hnonprincipal
  have hdvd : 491 ∣ NumberField.classNumber
      (NumberField.maximalRealSubfield K) :=
    hTF' hζ hprimary hpow hnonprincipal
  exact Fermat.FourHundredNinetyOne.SinnottKummer.not_dvd_classNumber hζ hdvd

end

end Fermat.Irregular.TakagiFurtwangler491
