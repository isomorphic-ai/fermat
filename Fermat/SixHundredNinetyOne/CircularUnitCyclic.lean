...
Let `Cyc = ZMod 345`. A cyclic correlation inverse

`∑ u, f u * g (u + d) = δ(d,0)`

induces an inverse for the `344 × 344` reduced difference matrices
...
theorem coord_injective : Function.Injective coord := by
  ...
  exact (ZMod.finEquiv 345).injective hij

/-- Every enumerated coordinate is nonzero. -/
theorem coord_ne_zero (i : Fin 344) : coord i ≠ 0 := by
  intro hi
  have : i.succ = (0 : Fin 345) :=
    (ZMod.finEquiv 345).injective (by
      change ZMod.finEquiv 345 i.succ = 0 at hi
...
theorem sum_shifted_product (f g : Cyc → ZMod 691) (x z : Cyc) :
  ...
  let H : Cyc → ZMod 691 := fun u ↦ f u * g (u + (z - x))
  calc
    (∑ v : Cyc, f (x + v) * g (v + z)) =
        ∑ v : Cyc, H (x + v) := by
...
theorem differenceMatrix_mul_eq_one
  ...
  ext i k
  rw [Matrix.mul_apply]
  let F : Cyc → ZMod 691 := fun v ↦
    (f (coord i + v) - f (coord i)) *
      (g (v + coord k) - g v)
  have hFzero : F 0 = 0 := by
    simp [F]
  change (∑ j : Fin 344, F (coord j)) =
    (1 : Matrix (Fin 344) (Fin 344) (ZMod 691)) i k
